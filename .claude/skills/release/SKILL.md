...
# main 側にしか無いコミット（§1.0 の判定と同じ。マージコミット 1 件だけなら健全）
...
git merge --abort
[ "$(git rev-parse develop)" = "$before_head" ] || { echo "ERROR: dry-run 後に develop が動いています。git merge --abort を再実行してください" >&2; exit 1; }
