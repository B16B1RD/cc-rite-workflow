...
| 連結時にシーン / BGM の尺、シーンのフレームレートを数値で取得できない | エラー終了（`N/A` を文字列比較で素通りさせない） |
| 連結時のクロスフェードがシーンのフレーム周期以下 | エラー終了（フレーム規模の欠落は出力尺の許容差に隠れるため入口で弾く） |
| 連結時に BGM が総尺より短い | エラー終了（末尾が無音の完成尺を黙って出さないため。`-shortest` を発行していないので映像は削られない） |
| 連結後の**映像ストリーム尺**が宣言より短い | エラー終了 + 成果物を削除（ffprobe で実測照合。コンテナ尺だと BGM が映像の切り捨てを覆い隠すため映像ストリームを見る。許容差は出力自身のフレーム周期 2 枚分） |
