...
| レンダー時の ffmpeg の起動失敗 / 途中終了 / 非ゼロ終了 | エラー終了（exit code、signal 終了なら signal 名を表示） |
| レンダー出力のフレーム数が宣言尺と不一致 | エラー終了（ffprobe で実測して照合。ffmpeg は入力が尽きても exit 0 で短い mp4 を残せるため） |
| 連結時の `-t` が数値でない / 0 以下 / 先頭ドット表記 | エラー終了（渡された値を表示。ffmpeg のフィルタ解析エラーに原因が埋もれるのを防ぐ） |
| 連結時にシーン / BGM の尺、シーンのフレームレートを数値で取得できない（ffprobe の非ゼロ終了を含む） | エラー終了（`N/A` を文字列比較で素通りさせない） |
| 連結時にシーンのフレームレートが揃っていない | エラー終了（xfade は同一 fps を要求する。ffmpeg のフィルタ構成エラーに原因が埋もれるのを防ぐ） |
| 連結時のクロスフェードがシーンのフレーム周期以下 | エラー終了（フレーム規模の欠落は出力側の許容差に隠れるため入口で弾く） |
| 連結時に BGM が総尺より短い | エラー終了（末尾が無音の完成尺を黙って出さないため。`-shortest` を発行していないので映像は削られない） |
| 連結時にシーン尺がクロスフェードに対して不足（端は `x` 超、中間は `2x` 超） | エラー終了（負ないし重複した xfade offset で素材を捨てた mp4 が exit 0 で残るのを防ぐ） |
| 連結時の ffmpeg が非ゼロ終了 | エラー終了（exit code を表示） |
| 連結出力のフレーム数が期待値と不一致 | エラー終了（`nb_read_packets` で実測照合。秒尺ではなくフレーム数なのは容器に依存しないため。許容差は 2 枚、両側で見る） |

連結は**一時パスへ書いて照合を通ったものだけを出力パスへ `mv` する**。失敗経路（ffmpeg の異常終了・中断・照合不合格）で出力パスに触れないため、壊れた成果物が残らず、既存の正常な成果物も破壊されない。
