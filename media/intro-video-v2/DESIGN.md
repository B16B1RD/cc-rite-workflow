...
| 連結時に BGM の尺を数値で取得できない | エラー終了（`N/A` を文字列比較で素通りさせない） |
| 連結時に BGM が総尺より短い | エラー終了（末尾が無音の完成尺を黙って出さないため。`-shortest` を発行していないので映像は削られない） |
| 連結後の出力尺が宣言より短い | エラー終了（ffprobe で実測照合。許容差は出力自身のフレーム周期 2 枚分。素材が捨てられた mp4 が exit 0 で残るのを防ぐ） |
| 連結時にシーン尺がクロスフェードに対して不足（端は `x` 超、中間は `2x` 超、単一シーンは `0` 超が必要） | エラー終了（負ないし重複した xfade offset で素材を捨てた mp4 が exit 0 で残るのを防ぐ） |
