...
markdown 中心で、全 PR の lint 経路に Chrome 実行を持ち込むのは の Non-goal
...
検査は**契約違反と環境不足**に絞る（ §4.5 のエラー表）。下表は実際に検査を書いた
...
- レンダー出力 mp4 のフレーム数が宣言尺と一致するか。実測照合は の T-01（手動テスト）が担う（連結側 `assemble.sh` は出力を実測照合する）
