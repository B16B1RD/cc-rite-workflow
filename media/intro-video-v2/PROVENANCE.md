...
- スパイク版を本パイプラインとして整備した。スパイク版からの差分は次の 3 系統:
...
- 絵コンテを「回るたび、賢くなる」（複利の物語）へ作り直し、シーンを 7 本に
  組み替えた。同じシーン mp4 群からフルカットと SNS カットの 2 本を作る構成になっている。
- 英語シーン（`scenes-en/`）と画面上のバージョン表記（M1 / M7）を追加した。
  同作業は両 README の Demo を本ディレクトリのフルカットへ差し替えるところまでを範囲に
  含む（差し替えの手順は下記「README への公開手順」）。README 掲載までを 1 本の Issue に
  含めたのは、README 埋め込みを Non-goal とし、動画の差し替えを
...
5 を運用として明記するのは、URL 差し替えを follow-up に回した作業がそのまま忘れられ、
Demo が旧版のまま公開され続けた実績があるためである（README 埋め込み自体を
Non-goal と決めた側で、follow-up に回したのは動画差し替えである）。
