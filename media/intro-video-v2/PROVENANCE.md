...
- でスパイク版を本パイプラインとして整備した。スパイク版からの差分は次の 3 系統:
...
5 を運用として明記するのは、URL 差し替えを follow-up に回した がそのまま忘れられ、
Demo が旧版のまま公開され続けた実績があるためである（ は README 埋め込み自体を
Non-goal と決めた側で、follow-up に回したのは ）。
