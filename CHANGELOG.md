...
- **A Ready PR's board column is no longer left behind after compaction** — the post-compaction hook decided draft state with `gh pr view --json isDraft --jq '.isDraft // null'`, and `//` treats a left-hand `false` as absent, so a Ready PR always yielded `null`, the `= "false"` comparison never held, and the board silently stayed in the `in_progress` column while `gh` itself reported success. The expression is now `.isDraft`, which already returns `null` for a missing key. The test mock ignored `--jq` and returned a fixed string, so the expression never ran; mocks now evaluate fixtures through real `jq` and abort rather than falling back to raw JSON. A `gh pr view` failure is pinned to skip the Status reconciliation entirely instead of only emitting a WARNING.
