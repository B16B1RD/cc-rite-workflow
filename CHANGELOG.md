...
- **Review-Fix Cycle Overhaul (Fail-Fast Response + Separate Issue user confirmation + configuration layer)** — **BREAKING CHANGE** (finalizes the #502 rollout, which began with #507 principles doc layer, #508/#504 reviewer exit layer, and #509 Fact-Check layer). Wires the `fix` response layer and configuration layer to the previously-merged principle/exit/fact-check layers.
  ...
  - `plugins/rite/templates/config/rite-config.yml` adds new keys: `review.observed_likelihood_gate` (enabled / security_exception / hypothetical_exception_reviewers / minimum), `review.fail_fast_first` (enabled / allow_skill_exceptions / wiki_query_required), `review.separate_issue_creation` (require_user_confirmation / report_pre_existing_issues), `fix.fail_fast_response`, and `fix.severity_gating.enabled: false` (deprecated shim). All keys are opt-out with the new default behavior.
  - `plugins/rite/i18n/{ja,en}/pr.yml` adds four user-facing messages (`review_fail_fast_first_warning`, `review_observed_likelihood_demotion_notice`, `review_separate_issue_user_confirmation_question`, `fix_fail_fast_response_checklist_prompt`) with ja/en parity.
  - **Migration guide**: Existing users with `fix.severity_gating.enabled: true` in `rite-config.yml` will find the key silently pinned to `false`. Non-convergence is now resolved via the `retry in current PR / create separate issue / withdraw` `AskUserQuestion` at fix.md Phase 4.3.3. Users who relied on automatic severity-based deferral should adopt `"batched"` or `"scope_lock"` strategy. To opt out of Fail-Fast First recommendation enforcement, set `review.fail_fast_first.enabled: false`. To opt out of user confirmation for separate-issue creation, set `review.separate_issue_creation.require_user_confirmation: false` (not recommended — it defeats the anti-escape-hatch safeguard). (#506)
