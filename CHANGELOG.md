...
- **The fix-plan check no longer overwrites the input plan with its own record** — the helper that inspects a batch fix plan writes its record to `.rite/state/fix-plan-{session}.json`; when the plan itself was placed under that name, `check` replaced the plan with the record and the following `verify` stopped on a missing `review_context`. The helper now resolves the input path and the record path before reading and exits with an error when they name the same object (absolute, relative, or through a file or directory symlink), so the input survives. The procedure names the input `fix-plan-input-{session}.json`, separate from the record. The guard sits ahead of the read, so a recovery run that passes an already-overwritten input stops with the same message.
- **Fix verification no longer rejects sandbox-mask entries as unplanned paths** — the verification helper confirms that no path outside the plan changed by taking the union of the worktree diff and the untracked-file listing. The Bash tool's sandbox lays a character device over files such as `.bashrc` at the worktree root and leaves a zero-byte, non-writable stub behind, so that enumeration carried entries that are not real changes and the fix loop stopped with `unplanned changed path` without a single edit having been made. The untracked enumeration now applies the same mechanism-based exclusion as `git-status-filtered.sh` (character device by `stat`; zero-byte regular file with every write bit cleared by `lstat`; an entry whose stat fails is kept), and each excluded stub is named in a stderr warning. No filename allowlist is used, a normal untracked file of the same name is still rejected, and a symlink to a stub is not excluded.
- **A triage entry in the Decision Log and a record marker no longer stop the loop as a specification change** — the stagnation diagnosis and the fix-plan check compared the related Issue body against the previous observation byte for byte, so rite appending its own out-of-scope triage result to the Issue's `## 9. Decision Log` section, or writing a `<!-- rite:nbr:comment-id:… -->` line at the end of the body, halted the run with `Issue specification changed within run` although the specification had not changed. One normalization function now backs all five comparison paths (stagnation observation, re-observation, replan, the fix-plan gate, and the fix-scope check). Only triage-format lines inside section 9 and the marker line are excluded; the body stored in the observation stays verbatim, and a Goal or acceptance-criteria change, a triage-format line outside the section, and a free-form Decision Log line still stop the run.
- **Cleanup from the main checkout reclaims a worktree the flow state does not record** — running `/rite:cleanup` from the main checkout classified an Issue with no recorded worktree as having none, so a worktree that was still registered was left in place and the following branch deletion was held up by it. Only when the classification comes out as none does the detection now build a candidate path from the configured worktree base and the Issue number and return it as reclaimable when it matches a registered worktree exactly after normalization; a failure to read the worktree list is reported as unknown rather than none. A worktree found this way has no saved state, so cleanup stops unless the claim and the head branch match and asks when there are uncommitted changes (a failed check counts as dirty). Classification from a record or from the current directory is unchanged.
- **Release notes come from the `main` SHA fixed by the promotion, and an empty Release is never published** — the release procedure switched the working tree to `main` and read `CHANGELOG.md` from there; a partial switch left the working tree on the old `main`, the extraction of a section that does not exist returned empty, and a Release with no body was published. The notes are now read from the SHA resolved after fetching `origin/main`, the Release targets that same SHA, and the working-tree switch is gone from the procedure. A failed fetch, a failed SHA resolution, a failed read, a missing version heading (including a near-miss heading), or a body of whitespace only stops before the Release is created.
- **The published tag is verified to point at the commit the notes came from before the Release is created** — GitHub ignores `--target` when a tag of that name already exists, so a tag created in error made the notes source and the published tag point at different commits. The existing tag is now read from the remote, preferring the peeled commit of an annotated tag, and a mismatch with the resolved SHA stops before the Release is created; an absent or matching tag continues and reports its state. A mismatch never deletes the tag automatically. The post-release check compares the tag after fetching tags from the remote, since a freshly created tag is not yet present locally.
- **Two verification paths of the fix-scope helper are pinned by tests** — an untracked entry whose `stat` fails stays a change rather than being excluded, pinned with a dangling symlink, and two stubs share one warning line that names the count and each path once. A fixture that exercises the sibling-ref-only case of the tag check now confirms up front that the ref is returned by the remote-listing pattern, so the case cannot decay into the absent-tag input and lose its discriminating power in silence.
- **Workflow callers reach the documented recovery when host isolation rejects a shell block after worktree entry** — each lifecycle skill that enters a session worktree now points at the shared recovery procedure, so a rejected compound shell is rewritten as a scratch script and rerun in place instead of being left without a next step.
- **A stopped review run can be parked and restored without renewing its budget** — switching Issues no longer discards a breaker-stopped run; restoration keeps the counters and history, and a divergence stop permits one validated retry per run.
- **Review recovery detects a HEAD change and can abandon an unfinished cycle that has no saved evidence** — the run's counters and history survive Issue switches. Resume tables distinguish collecting from completed when HEAD changed, and after abandon the lost-repair path no longer offers a cycle that no longer exists.
- **Ready and merge compare saved review evidence to the target PR head, not the local checkout** — an unresolved PR head refuses progression.
- **Review-clock instructions name shared Bash recipes and their required substitutions** — they no longer read as CLI subcommands that do not exist.
- **Fix-plan verification treats a successful check as exit 0 and shows assertions for expected nonzero exits** — a failed check reports the actual exit status and the saved evidence.
- **A batch run that has no lint command configured or detected skips lint explicitly** — it reports the reason and how to configure the command. A configured lint failure remains a failure, and standalone lint still offers the existing choices.
- **Acceptance review recognises supported Japanese headings and handwritten checkbox criteria** — those entries are verified instead of being treated as an absent acceptance section.
- **Commit checks fire before an ordinary git commit can move HEAD under an incomplete review or unverified fix** — supported normal commit workflows remain; this is not a universal shell security boundary.
- **A pinned replan can correct its verification commands in the same run** — the amendment is audited, stale approval and verification are invalidated, and run history and limits are kept.
- **After a HEAD change, iterate still invokes the review step when the cycle is already done or completed** — it no longer skips that invoke on those statuses.

## [0.16.0] - 2026-09-17

### Added

- **`/rite:pr-review` verifies every acceptance criterion of the related Issue at the current HEAD** — a dedicated acceptance reviewer runs on every cycle for a PR whose Issue carries acceptance criteria, and its per-criterion verdict table is stored in the review-result JSON. An unmet criterion becomes a blocking finding with a measured anchor and returns to the fix loop; a cycle with zero blocking findings but a criterion still unverified stops with `[review:error]` instead of reporting mergeable. The consequence-class downgrade helper treats a row marked unmet as an exclusion discriminator, so a documentation-synchronisation criterion classified as class B is no longer dropped from the loop when class A is empty, and a malformed verdict table stops with `acceptance_criteria_invalid` rather than silently skipping rows. The single output table carrying the unverified-stop row is now the only one the loop reads, the lightweight lane instruction for small Issues is not injected into the acceptance reviewer, and the scope-out triage runs on the unverified stop as well as on mergeable — it is skipped only when the outcome is fix-needed.
- **Projects Status is decided by `role`, not by a column-name string** — `rite-config.yml` maps each of `todo` / `in_progress` / `in_review` / `done` / `cancelled` to the board's own display name, and the shared resolver `hooks/scripts/lib/projects-status-config.sh` translates in both directions. Every writer (Issue creation, the reached-state gate in `/rite:open`, the `/rite:lint` drift check, the Stop-hook watchdog, the post-compaction reconciler, parent-Issue propagation) now compares roles, so boards labelled `To-Do`, `In progress`, or in Japanese are read correctly; a column with no role is left untouched and reported in a WARNING naming it, and a board without a cancelled column no longer counts a cancelled Issue as drift. `/rite:setup` creates Status options only on a board it created itself and otherwise verifies an exact name match without mutating an existing board. The parent-Status marker is emitted through the shared emitter's wire format, so an operator-controlled column name containing `;` cannot forge a field.
- **The review–fix loop confirms its inputs, plans fixes in one pass, and detects stagnation** — `/rite:pr-review` fixes the reviewer roster and the target commit at the start and advances only once every reviewer's result has been collected and its file written, so an interrupted run resumes the same review instead of re-running or double-counting it. `/rite:fix` builds one batch plan from all saved review results and the current Issue specification before editing, reuses a related test result only when content, command, and relevant environment match, and runs a whole-scope verification after the batch. `/rite:iterate` distinguishes stagnation from slow progress by actual working time plus a root cause that reproduces after a verified fix, then moves to a strategy review, saves the alternative and its verification, and — when it still does not converge — stops while preserving the results, the history, and the batch cursor.
- **The release skill checks promotion history before it creates anything** — `/release` verifies at the start that `main` is contained in `develop` (either as an ancestor or as the second parent of a merge commit) and stops with a recovery procedure before filing an Issue, bumping versions, or opening a preparation PR, so a `develop` that was once squashed or rebased into `main` no longer fails a later promotion gate after the work is done. The recovery procedure verifies that `main`'s tree matches a commit on `develop` before merging through a PR, aborts its dry-run merge and refuses to continue if the abort fails or `MERGE_HEAD` remains, reports an `already-contained` `develop` as such instead of diagnosing it as a failed abort, routes the guidance by whether the remote `develop` contains `main` rather than by the presence of local-only commits, and emits a pre-recovery tree only on the path where the pre-check returned `ok`, so the post-recovery tree check can never compare a value with itself.
- **`scripts/rite-dev` can launch Google Antigravity CLI (`agy`)** — the host list accepts `agy`, symlinks `plugins/rite/skills/*` into `.agents/skills/`, verifies each link, and starts the CLI with the working tree's plugin skills loaded; a conflicting path or a link pointing elsewhere is diagnosed instead of overwritten, and the repository-local `.agents/` is gitignored.

### Fixed

- **Issues that rite files automatically declare their complexity in the body** — the scope-out finding, the repeated-finding split, the merge-time remaining-task, and the non-blocking sweep templates all begin with `**Type**` / `**Complexity**`, taking the same value that is written to the Projects field. `/rite:open` reads complexity only from the body declaration, so such an Issue stopped every `/rite:batch-run` just before the implementation plan. Static checks now fail when a declaration is missing, disagrees with the Projects value, or is reordered on any of the four paths.
- **`/rite:batch-run` resumes a stopped Issue at the stage its flow-state phase names** — an argument-less resume called `/rite:open` again for every stopped Issue, and the open resume table does not cover phases after PR creation, so the run returned without a completion notice and stopped on the same Issue. Resume now reads the phase once per run and routes review and fix to `/rite:iterate`, the ready phases to `/rite:ready`, and the cleanup phases to `/rite:cleanup`; a read failure, a missing PR number, or an unknown phase stops and points at `/rite:recover` instead of guessing. `/rite:iterate` also refuses to write its run-start pin when the PR-number placeholder was never substituted — a literal `review-run-since-{pr_number}.txt` was written and read back consistently, so nothing errored — and the reply-only run-close record now matches its `review-defer` implementation.
- **Worktree exit is confirmed against the host's own working directory, and a guard rejection after entry has a defined fallback** — with a host-native entry tool, `/rite:cleanup` passed its exit check merely by `cd`-ing to the main checkout inside the script, while the host session stayed in the previous Issue's worktree, so the next `/rite:open` in the same session had its native entry refused and `/rite:batch-run` stopped. The exit check now runs in an independent shell with no `cd`, `git -C`, or workdir argument; when native entry is refused, `/rite:open` and `/rite:recover` inspect the working directory the same way and, only if it is a registered session worktree of the previous Issue, exit natively (keeping the worktree), confirm the main root, and re-enter exactly once. `git-worktree-patterns.md` owns the procedure and adds the case where entry succeeded but the host's isolation guard rejects subsequent compound shell blocks: the rejected block is written to a scratch script and run as a single command, except when the block itself targets a path outside the worktree via `cd`, `git -C`, or a workdir argument, in which case it stops rather than trying another form. Verifying a leftover stub lock is documented as a step to run outside the sandbox, where a masked lock is not a regular file and `find -type f` finds nothing.
- **A sandbox-created empty file no longer blocks worktree reap, and pasted recovery commands survive apostrophes** — sandboxed Bash leaves zero-byte, unwritable files at a session worktree's root; the shared dirty check counted them as uncommitted changes, so a merged worktree and its branch were skipped at every session start, and an identically shaped empty `.git/config.lock` made every `git config` update fail. Those stubs are now excluded, the leftover lock is reported with its path and a manual removal step rather than deleted, and reap declines a worktree whose claim check failed — showing the error — instead of treating the failure as "no claim" and reclaiming it. Every copy-and-run recovery command emitted by the hooks (worktree removal, branch checkout, staged-raw restore) builds its embedded values with `printf %q`, so a path or branch name containing an apostrophe or a space no longer splits into extra arguments under `rm -rf`.
- **A failed Wiki ingest leaves neither a staged raw source nor an unexplained git error** — when the commit failed, the raw sources staged on the wiki branch were carried back by the checkout to the original branch, and the next ingest stopped at the tracked-raw invariant with exit 3. The automatic restore now unstages the raw entries only when a checkout to the wiki branch actually happened, does so before popping the stash so a user's own pre-staged work keeps its index state, and the manual procedure includes the unstage and pops the stash only when one exists. A read-only `.git/worktrees/wiki-worktree/` under a sandbox is detected by a write probe before anything is staged and stops with `reason=sandbox-mask` and a one-time "re-run this block outside the sandbox" instruction, which `/rite:wiki-init` also emits for its migration commit as a non-blocking WARNING; failure to resolve the git directory outside a repository is reported as itself rather than misclassified as a sandbox mask.
- **The rejection ledger is written, read, and diagnosed consistently** — the non-blocking sweep records rejected candidates as a rejection ledger in the related Issue's record comment. The record helper returned `skipped` when there were zero non-measured findings and no existing comment, even though the body carried a ledger, so the sweep completed with the ledger stored nowhere; it now counts the body's ledger entries and omits the post only when there are zero entries, no ledger, and no existing comment, and the sweep succeeds only on `created` or `updated`, stopping with `[fix:error]` otherwise. The ledger's heading, section boundary, and count tests in `nb-sweep-ledger.sh` use the same expression as the record helper (trailing `CR` stripped, `index()` prefix match), so a CRLF body is read as the same range by both and macOS `awk` cannot match a different Japanese heading by locale collation; the sweep's collect step and the follow-up filing normalise `\r\n` to `\n` before splitting out the ledger. Diagnostics name the tool that actually failed — `jq:` for the body predicate, `awk:` for the ledger count — instead of prefixing everything with `gh:` and pointing at GitHub authentication.
- **`/rite:cleanup` files each remaining finding once, and only once** — the non-blocking sweep can file an Issue during `/rite:iterate`, and the post-merge follow-up could not recognise it, so the same finding was filed twice. Follow-up filing now reads the rejection ledger from the related Issue's record comment and drops findings already marked `issued` in the review-result JSON the sweep read, while findings from earlier cycles are still transferred; if every remainder is already filed, no follow-up Issue is created. The exclusion key for a re-verified finding is the source JSON's file name plus `#` plus the number, so the same finding number appearing in several cycles is no longer ambiguous, and the same-second collision suffix `~{4 lowercase hex digits}` is accepted by both the follow-up helper and the re-verification projection. Findings identical in everything but their source are collapsed to one entry. When neither the ledger nor the latest review-result JSON can be read, only the already-filed exclusion is suspended — the re-verification exclusion still applies — and a WARNING plus a marker says so; the duplicate WARNING covers only a prior-cycle finding at the same `file:line` as a filed one, and a re-report on a shifted line cannot be distinguished from the ledger.
- **Decision Log numbering counts only the Decision Log** — a scope-out decision is recorded as `D-NN` in the related Issue's Decision Log section. Numbering scanned the whole body, so a follow-up Issue quoting a finding that contained `D-04` made the second record jump to `D-05` or `D-10` and look like a gap; it now counts inside the section only, bounded exactly as the insertion logic bounds it, and the extraction reads `D-NN` out of its leading boundary character so a run like `9D-02` no longer contributes its digits. An Issue with neither a Decision Log section nor a work-memory comment gets a new section and `D-01` instead of being handed to a human with "please append this manually"; the work-memory fallback and its `wm_sync_failure` reason are gone.
- **A Ready PR's board column is no longer left behind after compaction** — the post-compaction hook decided draft state with `gh pr view --json isDraft --jq '.isDraft // null'`, and `//` treats a left-hand `false` as absent, so a Ready PR always yielded `null`, the `= "false"` comparison never held, and the board silently stayed in the `in_progress` column while `gh` itself reported success. The expression is now `.isDraft`, which already returns `null` for a missing key. The test mock ignored `--jq` and returned a fixed string, so the expression never ran; mocks now evaluate fixtures through real `jq` and abort rather than falling back to raw JSON. A `gh pr view` failure already left the reconciliation unreached; the test now pins that the reconcile helper is never invoked, not merely that the WARNING text appears.
...
- **Distributed skills, hooks, and references no longer cite development-repository numbering** — acceptance-criterion identifiers such as `AC-3` carried no meaning at an install destination; they were removed from the skill bodies and their references and from the hook shell comments, and the constraint each one stood for is now written as a present-tense clause. Decision Log identifiers (`D-01` and the like) are deliberately kept. Cross-file references around `nb-sweep.md` now name the file and the real section or item they point at, so a bare number cannot be read as a heading of `fix/SKILL.md` or of the review skill, which both use the same numbering.
- **The hook test suite is hermetic and runs in parallel** — the variables that must be cleared before a hook test runs (`RITE_HOST`, the per-host session IDs, `RITE_STATE_ROOT`) live in one list in `_hermetic-env.sh` that `run-tests.sh`, `_test-helpers.sh`, and the tests that load neither read; failing to load it stops the test regardless of `set -e`. Running a test file on its own inside a Claude Code session no longer inherits the host's session environment and fails. The runner gained `--jobs N` (four by default, refilling free slots), reporting an invalid value or a missing result with a non-zero exit, which brings the measured macOS job down to about nine minutes, with `timeout-minutes` set to twice that measurement (18); heavy Git fixtures are built from an independent seed copy with Git's automatic maintenance disabled so a lock file cannot be copied mid-write, and `main` pushes plus the required-tool check moved into `ci.yml` with the duplicate workflow deleted. A number of pins that passed while their subject was broken were tightened: `grep -q` early exits that produced `pipefail` false failures were replaced with full-input reads, mechanical-rail exclusions now match the pre-replacement line exactly, ledger extraction failures are no longer indistinguishable from an empty ledger, and the release skill's recovery bash blocks are extracted and executed against a real repository.
- **Upgrade notice: Projects Status configuration now uses `role` entries** — a legacy `github.projects.fields.status.options` list still works and is resolved as the five standard English names (`Todo` / `In Progress` / `In Review` / `Done` / `Cancelled`), so a standard board needs no action. Existing projects should run `/rite:setup --upgrade` once after updating the plugin; it performs the role migration even when `schema_version` is already 2. The migration does not guess custom or Japanese column names — it substitutes the five standard English roles regardless of what the legacy list contained — so anyone whose board uses custom column names must check the `role` / `name` mapping written into `rite-config.yml` against their actual columns. An explicit `role` configuration is preserved unchanged, and an invalid one stops before any config is written. `--upgrade` never adds a column to the GitHub Project itself: if there is no `Cancelled` column, either add one to the board or use an explicit `role` configuration that omits `cancelled`.
...
- **`/rite:issue-cancel` cancels an Issue you decided not to do** — closes it with `--reason "not planned"`, moves the board Status to the terminal `Cancelled`, and records the mandatory reason in the closing comment. An Issue already carrying a PR, a branch, a session worktree, or work memory is torn down through the existing cleanup helpers: the PR is closed unmerged, then the worktree, branch, PR-specific state, and local work memory are removed. The order PR close → Status → Issue close is load-bearing: a failed PR close stops the whole flow rather than advancing the board. Cancelling a child Issue never propagates `Done` to its parent. The skill runs only on an explicit human invocation.
- **Multi-host runtime contract** — `host-runtime-contract.md` defines, inside the distribution, the eight common operations (command and file operations, working-directory selection, skill loading and continuation, delegation, questions, hooks, session identity, approval and permissions) and the per-host execution route table for Claude Code, Codex CLI, and Grok Build; `host-workflow-operations.md` gives the concrete call procedures for skills, tasks, independent reviewers, and questions. `session-identity.sh` resolves the current host's real session ID and stops instead of degrading to shared state when it is missing or ambiguous; `host-runtime.sh` exposes init, pre/post-operation guards, checkpoint, and next-step reads as explicit commands; `reviewer-completion-check.sh` lets result merging start only when every selected reviewer's run ID and completed output are present.
- **Worktree entry, resume, and exit without `EnterWorktree`** — hosts without the native tool select an available operation or an explicit `cd` per shell from one shared contract. Working directory, branch, saved state, and claim are cross-checked before every edit and commit, and `/rite:cleanup` deletes the target worktree only after confirming the exit to the main checkout. `/rite:open`, `/rite:recover`, `/rite:iterate`, `/rite:pr-review`, and `/rite:fix` share the same entry path.
- **Three-host end-to-end verification tooling** — `tests/runtime-e2e/prepare.sh` builds an offline fixture repository and records the source version, `results.py` aggregates per-host evidence for the same commit and install path (stub success and real-run success are never conflated), and the launcher failure-propagation tests plus the tooling contract suite run in CI.
- **`/rite:pr-review` feeds CI results to reviewers and the report** — `pr-checks-classify.sh` (shared with `/rite:merge`) classifies the target commit's checks and lists jobs; each reviewer prompt, the consolidated report, and the E2E exit line show them. A failing job is reported only with evidence tying it to the change, and a pending CI never blocks the review.
- **Issue and PR bodies lead with a summary and collapse the details** — the templates open with the problem, the change, and what to look at, and fold the implementation contract and validation into `<details>`. Diagrams prefer an attached SVG (Mermaid only when the GitHub CLI cannot attach), a diagram-less body keeps its reason in an HTML comment, a PR re-uses the Issue's diagram and redraws only when the structure changed, decomposed parent Issues can carry an SVG, and a PR without an Issue can still draw one.
- **`/rite:fix` fixes only verified CRITICAL/HIGH findings** — the helper decides fatality and atomically updates the review JSON; every other finding is transferred to the non-blocking record with its severity intact and excluded from later resolution checks. After mergeable, the NB sweep routes each finding mechanically to `issued` (verified MEDIUM → Issue) or `recorded`, and no longer edits, commits, or pushes code.
- **`/rite:fix` completion reports are matched against the diff** — the finding ID → change location table is mandatory, `fix-report-diff-gate.sh` verifies each row exists in the cycle's `git diff -U0`, and findings absent from the diff remain "unaddressed" so the next verification does not assume them resolved.
- **Number-reference detection is one grammar with three modes** — `number-reference-check.sh --all | --diff | --stdin --label` is the single definition; `/rite:lint` blocks on it, `/rite:pr-review` scans each cycle's PR diff into a blocking class-A finding whose description leads the Verification section, and `/rite:fix` self-checks before committing (with intent-to-add so new files are rewritten in the same cycle). Distribution files, docs, Wiki pages, and both CHANGELOGs were stripped of Issue/PR numbers, the CI full scan is blocking, and the deletion-damage scanner recognises the residue forms left by number removal.
- **The Bash guard rejects direct `gh issue create`** — the PreToolUse guard allows only the approved Issue-creation helpers and every non-create `gh issue` operation; a failed evaluation of that pattern is also a rejection, with recovery steps in the spec.
- **`/rite:issue-create` refuses an Issue whose Target Files hold no real path** — measurement-, decision-, or investigation-only input is offered `docs/designs/{slug}.md` as the deliverable before filing; the real-path check and the question order are the same on the direct and exploration-summary paths, and parent specifications are exempt.
- **Canonical confirmation gates are enumerated** — `autonomous-execution.md` lists the places rite stops on purpose (plan approval, Issue state, dirty conflicts, forced fetches, destructive operations); any other transient failure is retried once automatically and then routed to `/rite:recover`. The `[lint:error]` forced-continue prompt is gone.
- **Action-required WARNINGs are copied into completion reports** — bash output is LLM-facing diagnostics, so `/rite:open`, `/rite:iterate`, and `/rite:batch-run` add a `要対応:` block listing each line that needs a human action (omitted when empty); the "stderr is always visible" premise was corrected.
- **Shared principles** — `prefer_partial_edit` (edit only the affected span when the result is unchanged) in `coding-principles.md`; a one-line nudge to batch independent tool calls in one response in the reviewer base and `autonomous-execution.md`; a test-file discipline common to every Complexity in `/rite:issue-implement` (one behaviour per test, sized like its neighbours, no scratch left, extend existing suites first); a two-line note on the long-form generation steps of `/rite:pr-review`, `/rite:issue-create`, and `/rite:wiki-ingest` that reasoning decides structure and output writes the body once; a rule against mannered prose in generated text; and a `/rite:wiki-ingest` instruction with an example to avoid verbatim transcription of Raw Sources.
- **`SessionStart` reclaims stale run-queues of other sessions** — `run-queue-reap.sh` runs at session start so a batch left by a dead session no longer blocks.

### Fixed

- **`/rite:batch-run` no longer stops silently after a sub-skill finishes** — the Stop hook's run-queue watchdog reads the session's own queue when the handoff is empty and re-drives it (stopping after three no-progress rounds); the compaction recovery message carries the batch frame instead of pointing to `/rite:recover`. The batch also halts when the circuit breaker trips.
- **`/rite:iterate` stops after the circuit breaker fires** — divergence or cycle-cap trips no longer run an extra review; the stop reason and counter reset are recorded together and an explicit rerun restarts from full scope. A non-fatal transfer now passes through the NB sweep, a reply-only exit reason survives the sweep, and fix save records are excluded from the convergence trend.
- **`/rite:pr-review` output contracts** — Decision Log numbering is fixed at base 10; a malformed verified anchor and an undetermined verification each stop before classification with an error; the verification gate on review results is enforced; zero findings still print the five-column header; reviewers are launched in one batch with timing recorded for all; and untracked files are separated from the review diff and the commit guard.
...
- **Independent reviewers receive the body by absolute path** — the handoff is one contract across `host-workflow-operations.md`, the placeholder table, the design record, and the entry skills, and its qualifiers are pinned by tests.
- **`Cancelled` is a terminal Projects Status end to end** — `/rite:setup` provisions the option as a union with existing ones, the status helper refuses to overwrite `Cancelled` with `Done`, reconciliation branches on `stateReason` (a DUPLICATE close becomes `Cancelled`), a parent whose children include a `Cancelled` one is not auto-closed to `Done`, and the `/rite:issue-close` completion report no longer fixes `Done` after `skipped_terminal_conflict`. `/rite:issue-close` scopes its PR search by Issue number.
- **`/rite:cleanup`** — follow-up Issues are filed from the non-verified findings of every cycle without omission, the follow-up report and failure counts are correct, work memory is read from the `state_root` absolute path, and worktree exit, branch deletion, and state removal live in one helper.
- **`/rite:wiki-ingest` and Wiki writes** — commit failures are explained per cause and the `git add` guidance no longer depends on an anchor, the quoted-heredoc index writer matches the single-heredoc and six-read layout, every Wiki write path runs the number-reference check before committing, `/rite:wiki-lint` reports staleness as informational instead of blocking, and catalog parse warnings and missing Raw Sources were cleaned up.
- **Hooks** — the post-compaction recovery text is injected from `SessionStart` (`source=compact`) since PreCompact/PostCompact stdout never reaches the model; the plugin-conflict warning at session start is decided by the real load path; work memory is always written under the state root; a sandbox-blocked worktree removal leaves no half-deleted residue; state writes use a safe temporary file; and the session-start test sandbox copies `run-queue-reap.sh`.
- **`/rite:issue-implement` stops loudly on an empty commit** — a branch with no diff no longer produces a commit.
- **`/rite:fix` treats documentation claims literally** — the Simplification-First generalisation is not applied to prose findings; the recommended action is checked against the implementation as a candidate.
- **Marketplace plugin markdown no longer links into development-repo `docs/`** — six relative links from `plugins/rite` to `docs/` 404 at install destinations that do not ship that tree. The links now stay inside the plugin (or drop to a one-line pointer), `/rite:lint` Phase 3.5 runs `distribution-docs-link-check.sh` so the hop cannot return, and the detector has helper-level tests.
- **Unresolved-warning transcription is deterministic**, residual contract drift between skills and helpers is removed, and reviewer agents declare `effort` explicitly with `model: inherit`.
- **Work memory** — a digit-only SHA no longer causes duplicate updates, parse failures print their cause, and the replica-absent stop check and session-ID description are complete.

### Changed

- **Review and fix procedures are loaded only when needed** — `/rite:iterate` and `/rite:fix` read their sub-procedures (including the work memory update extracted from `/rite:fix`) at the step that uses them, reducing the per-invocation context.
- **`/rite:pr-review` reviewers are launched in a single batch** with spawn timing recorded for every reviewer.
- **Tests and lint** — the fixture rewrite helper is shared, failure detail dumps go to stdout, the path exclusion list has one definition, and the pre-existing warnings of the full-scan lint are cleared. Review-loop documentation describes withdrawn decisions as implemented.

## [0.14.0] - 2026-08-30

### Fixed

- **`/rite:merge` waits for pending CI checks instead of stopping immediately** — when `statusCheckRollup` is pending and `--force-ci` is not set, merge polls every 15 seconds for up to 540 seconds, then rejoins the existing healthy / unhealthy / unknown classification. A timeout still fail-closes with `[merge:not-ready]` and the unfinished check names.
...
- **`post-tool-wm-sync.sh` stays silent about a missing work memory replica during the replica init window** — while `/rite:open` has recorded `phase=init` but has not yet created the replica, the hook suppresses the "replica not found, run `/rite:open`" systemMessage. The `wm_replica=absent` negative-cache record and the `last_synced_phase` advance are unchanged, and the suppression leaves a debug-log trace.
- **`wm_comment_id` / `wm_replica` are scoped to the Issue they belong to** — `flow-state.sh` preserves both keys only when the Issue being written matches the stored one. `/rite:batch-run` no longer PATCHes the previous Issue's replica comment, and a `wm_replica: "absent"` negative cache no longer silences replica sync for every remaining phase of that Issue.
- **`.rite/release-promotions/` ignores itself** — `release-promotion-verify.sh` places a `*`-only `.gitignore` in the attestation directory right after creating it, so attestation files no longer accumulate as untracked entries in `git status`. A failure to place it is a WARNING and leaves the exit code, head OID, and attestation intact.
- **`/rite:open` verifies the Projects Status → In Progress transition** — the new read-only `projects-status-gate.sh` reads the Issue's actual Projects Status and emits `[CONTEXT] PROJECTS_STATUS_INVARIANT=ok|missing|skipped|unknown`. `watchdog-status-mismatch.sh` now applies two rules (Todo residue / In Progress residue), records `expected_status` in each mismatch, and makes the `--reconcile` target rule-dependent.
- **`/rite:open` records the detected parent Issue number in flow-state** — `--parent-issue` is passed to `flow-state.sh set` when a parent is detected, so `/rite:issue-implement` updates the parent task list instead of skipping with `PARENT_ISSUE=none`. A standalone Issue omits the flag rather than writing `0`.
- **`issue-complexity-lane.sh` accepts a table-form Section 0 Meta** — `| **Complexity** | S |` is read as a third accepted notation, so `/rite:open` no longer stops at the `complexity_absent` fail-loud for Issues written that way.
- **`/rite:cleanup` re-verifies non-blocking findings against the merged HEAD before filing a follow-up Issue** — `cleanup-follow-up-issue.sh` gained `--exclude-ids <csv>`; findings already resolved by later fix cycles are excluded, and an all-resolved set skips the filing with `skipped; reason=all_resolved`. An unknown id is a WARNING and the known ones are still excluded.
- **Auto-filed follow-up and `pr-create` Issues carry Section 0 Meta** — their bodies open with `**Type**` / `**Complexity**`, so `/rite:open` does not stop with `complexity_absent` on them.
...
- **`gitignore-health-check.sh` compares the nested `.rite/.gitignore` at the `state_root`** — running `/rite:lint` from a linked worktree no longer reports a false DRIFT for a nested gitignore that is present in the main checkout.
- **The NB sweep rejection ledger count is extracted with the validator's expression** — `fix/SKILL.md` reads `non_blocking_count` with the same expression `review-nonblocking-record.sh` re-checks it with, so a ledger-bearing NB sweep no longer always ends in `[fix:error]` with `reason=nb_sweep_ledger_count_unreadable`.
- **A non-numeric `nb_sweep_fixed` emits a WARNING** — `/rite:fix --nb-sweep` writes the WARNING and the received value to stderr while the done-file stays a single `done` line, so no false pass is created.
- **The NB sweep re-entry guard is a persistent file** — `/rite:iterate` records it in `.rite/state/nb-sweep-done-{pr}.txt` instead of a conversation marker, so a completed sweep no longer returns to a full review. `pr-cycle-cleanup.sh` reclaims orphaned files.
- **`/rite:ready`'s reviewed-head gate accepts the commit an NB sweep pushed** — `fix/SKILL.md` writes the post-push HEAD SHA on the done-file's second line, and `ready-reviewed-head-gate.sh` matches the review-results JSON first (`via=json`) and the second line only on mismatch (`via=sweep`). Any other unreviewed commit, and a malformed second line, are still fail-loud.
- **`guardrail_audit_log[]` keys are enumerated and validated on write** — `pr-review/SKILL.md` lists the seven keys explicitly and `review-result-save.sh` rejects a missing or extra key with `guardrail_audit_log_keys_violation`. An empty array is accepted.
- **The `nb-sweep` contract test pins that the producer and validator count expressions match** — `nb-sweep-contract.test.sh` extracts the `body_count=` assignment from both `fix/SKILL.md` and `review-nonblocking-record.sh`, asserts a single assignment on each side, and fails loudly when either right-hand side cannot be extracted rather than passing on two empty strings.
- **The gitignore health-check test pins nested drift seen from a worktree cwd** — `gitignore-health-check-multi-session.test.sh` asserts that skipping the nested comparison when `state_root` differs from the repository root hides drift in a worktree cwd but not in the main cwd.
- **The NB sweep re-entry guard test pins the one-line `done` of the outer branch** — `nb-sweep-reentry-guard.test.sh` mutates the outer and inner branches independently so a first-else regression no longer stays green.

### Changed

- **Runtime state under the repository root moved into `.rite/`** — session-id, work-memory, plugin-root, flow-debug.log, initialized-version, and settings-hooks-cleaned are written under `.rite/`. `session-start.sh` relocates existing files once, and readers fall back to the old path when the new one is absent.
- **`.rite/` carries a nested self-gitignore** — `session-start.sh` and `flow-state.sh` place a three-line `.gitignore` (`*` / `!wiki/` / `!wiki/**`) so runtime state is ignored without touching the consumer's root `.gitignore`.
- **`/rite:setup` generates the nested `.rite/.gitignore` instead of listing runtime state in the root `.gitignore`** — `--upgrade` performs the same generation and the old-path relocation idempotently, and the `/rite:lint` health-check verifies the same three-line definition without rewriting any file.
- **The legacy `.rite-flow-state` write paths are settled** — no path creates the file anew; only `flow-state.sh migrate` and the resolver-failure fallbacks in `issue-comment-wm-sync.sh` / `cleanup-work-memory.sh` update an existing file in place. README's uninstall steps now match the current implementation.
...
- **The hook registration tables in `setup/SKILL.md` name the replica sync path** — `post-tool-wm-sync.sh`'s Purpose covers both Path A and Path B in both tables.
- **`docs/SPEC.md`'s hook registration description matches `hooks.json`** — the PreToolUse row drops the `Block tool usage after compact` entry removed in v0.7 and names `pre-tool-bash-guard.sh` / `pre-tool-edit-guard.sh`; the PostToolUse row names `post-tool-wm-sync.sh`'s Path B and `scripts/bang-backtick-edit-hook.sh`; and `issue-comment-wm-sync.sh` is described as a helper called from Path B and several skills rather than a registered PostToolUse hook.

## [0.13.2] - 2026-08-28

### Added

- **`/rite:iterate` sweeps remaining non-blocking findings after `[review:mergeable]`** — the post-mergeable sweep in `iterate` invokes `/rite:fix --nb-sweep`, which consumes each finding as a fix, a rejection with a mandatory judgement statement, or a new Issue, and finalizes with `[fix:sweep-done]`. `nb-sweep-collect.sh` / `nb-sweep-ledger.sh` back the collection and the rejection ledger, and the completion notice requires zero remaining findings after overlay.
- **`/rite:open` runs a multi-perspective self-review of the implementation plan before approval** — `open` runs the self-review once for Complexity S and above, applying the four perspectives in `open/references/plan-self-review.md` to the plan and folding the findings back into it before the approval gate. A failed review is reported as a WARNING rather than blocking.
- **Findings that delete or weaken existing text are excluded from consequence-class demotion** — the classification map takes an optional `exclusion` key, and `review-class-demotion-gate.sh` keeps an excluded class B finding blocking even in a cycle with zero class A findings. Wording polish on newly added text is demoted as before.

### Fixed

- **A `{rejected_ledger}` retrieval failure in `/rite:pr-review` no longer degrades into an empty ledger** — a failing `gh api`, extract, or `mktemp` now yields `REJECTED_LEDGER=failed` with a stderr WARNING and a one-line note in the placeholder. A successful retrieval that finds no matching comment stays `empty`, and the review itself does not stop.
- **The `nb-sweep` contract test pins the ledger retrieval failure path** — T-07 asserts `REJECTED_LEDGER=failed`, the stderr WARNING, and the placeholder note, so a regression back to a silent empty ledger no longer passes green.

### Changed

- **The `exclusion` judgement statement in `_reviewer-base.md` is addressed to the consolidation side** — reviewers leave anchored material under `内容`, and the consolidation side of `pr-review` writes the `exclusion` judgement into the classification map.

## [0.13.1] - 2026-08-27

### Added

- **Non-measured review findings are recorded on the related Issue instead of the PR** — `review-nonblocking-record.sh` posts the cycle-time record as a related-Issue comment, resolving the Issue through a closing keyword or the `issue-{N}` branch name and failing loud with `related_issue_unresolved` when neither resolves. The update-in-place comment id is persisted in the related Issue body, and `severity-levels.md` / `assessment-rules.md` describe the same channel. Record contract D-01 is unchanged; only the posting channel moves.
- **`/rite:cleanup` opens a `follow-up` Issue for non-measured findings left on a merged PR** — `cleanup-follow-up-issue.sh` creates one Issue per PR, none when the finding count is zero, and stays idempotent across re-runs through a machine marker in the Issue body. The Issue is registered to Projects as `Todo` and opened before the review JSON is moved to `archive/`.

### Fixed

- **`/rite:fix` no longer posts the `## レビュー指摘対応完了` report or `nit、認知済` replies to the PR** — PR thread replies are limited to human-authored findings and the fix details are recorded in the fix commit message instead. `nit-noted` is surfaced as a display and a count only.
- **The cleanup follow-up reader anchors on the first body line and scopes recency to the follow-up side** — existing-Issue detection compares the first line's HTML comment for equality, so a complete marker on the second body line still yields `created`. The `{review_cleanup_check}` recency selection applies to the follow-up side only, while the state deletion side keeps its presence check.
- **`/release` Phase 1.3 no longer misclassifies an empty PR body as a `gh` error** — the branch is rebuilt around the `gh pr view` exit status, so a successful call returning an empty body is treated as "no closing keyword".
- **`lint/SKILL.md` and `docs/SPEC.md` describe the actual number-free scan surface** — both still listed CHANGELOG after it was removed from `number-reference-check.sh`'s `DEFAULT_TARGETS`.

### Changed

- **`/release` Phase 1.3 resolves PR numbers to Issue numbers** — the trailing number of each squash-merge subject is resolved through `gh pr view` and printed as a `PR #N → Issue #M` table. A number that cannot be resolved to a PR is taken as an Issue number, and a missing closing keyword is reported as a resolution failure rather than falling back to the PR number.
- **CHANGELOG number references are canonical and outside the number-free scan surface** — `number-reference-check.sh` no longer scans `CHANGELOG.md` / `CHANGELOG.ja.md`, and the header note in both files documents the policy.
...
- **`/rite:iterate` mergeable completion notices carry a mandatory unresolved non-blocking count** — the notice splits into zero / non-zero / retrieval-failure forms with the count read mechanically from the latest review JSON, so remaining items no longer depend on the model volunteering them. `stop-loop-continuation.sh` sends the turn back once when the field is missing or cannot be inspected. The `[review:mergeable]` sentinel is unchanged.
- **`/rite:ready` refuses to promote when HEAD differs from the last reviewed commit** — `ready-reviewed-head-gate.sh` compares the latest review JSON's `commit_sha` against `git rev-parse HEAD`. A mismatch, an absent JSON (including archived), or a `rev-parse` failure stops fail-loud without marking the PR ready and points to `/rite:iterate {pr}`.
- **Out-of-scope triage must post a handoff comment on the receiving Issue** — when `pr-review` routes a finding to an existing Issue, the Decision Log entry alone is not sufficient; the handoff comment is a required action. A CLOSED receiving Issue gets no comment and the triage returns to reselection.

### Fixed

- **`stop-loop-continuation.sh` no longer re-requests a completion notice it already saw** — the FINALIZE path now inspects the latest assistant output for the `## /rite:iterate 完了` / `## /rite:iterate 中断` markers and sends the turn back only when the notice is absent or uninspectable. A mergeable notice missing the unresolved non-blocking field is still sent back.
- **`pr-cycle-cleanup.sh` keeps review JSON for PRs that are still open** — the orphan reap gained a GitHub PR state gate, so only MERGED / CLOSED PRs have their review JSON moved to `archive/`; OPEN (including draft) and undeterminable states are retained.
- **`/rite:cleanup` reaps flow-state and run-queue entries left by other sessions** — `flow-state.sh reap-issue --issue N` deactivates every flow-state for the Issue, deactivates run-queues with no remaining work, and collects the corresponding and orphaned `.flow-state.lock` files. Failures continue with a WARNING naming the path, and batch run-queues that still hold other Issues are left untouched.
- **`PHASE_7_ASKUSER_INVOKED` is emitted only after the confirmation completes** — in interactive out-of-scope triage the marker now carries `mode` / `choice` / `reason` and is emitted after the user answers, closing the path that reached the Decision Log before confirmation. Automatic Decision Log entries for E2E and batch modes are unchanged.

## [0.12.3] - 2026-08-23

### Fixed

- **`post-tool-wm-sync` reduces GitHub API round-trips from 9 to 2 per phase transition** — `issue-comment-wm-sync.sh` now exposes `fetch` / `patch`; `update` is a fetch → transform → patch wrapper. Replica-absent first detect (`no_comment`) is fail-loud via PostToolUse `systemMessage`; subsequent `wm_replica: "absent"` is a negative cache (no gh, empty stdout). Unresolved `branch.base` skips `update-progress` with `systemMessage` instead of a `develop` fallback.
- **`issue-comment-wm-sync.sh` update mode keeps `COMMENT_ID` outside a subshell** — `do_fetch` stdout is read via a temp file so PATCH still succeeds when flow-state is absent. T-11 restores `TMPDIR`; the hook success path emits `status=success round_trips=2`.

## [0.12.2] - 2026-08-21

### Changed

- **Japanese prose replaces 「正本」 by usage** — `cleanup`'s WM source selection uses 「採用元 / 採用する」, `pr-review`'s retained-flag rationale and intro-video provenance use 「判断基準 / 従う」, and the video location uses 「原本」. Runtime values (`WM_SOURCE`, sentinels, WARNING text) are unchanged.
- **awk extraction anchors are annotated at the source comment** — `iterate` / `cleanup` SKILL.md and `issue-comment-wm-sync.sh` now carry a one-line note immediately above each comment that tests use as an awk extraction start, so the coupling is visible at the edit site. Extraction method, fail-loud hard exit, and test-side awk patterns are unchanged.

## [0.12.1] - 2026-08-15

### Fixed

- **`/release` Phase 2.5 merges the prep PR with `--squash`** — `release-promotion-verify.sh` requires every develop commit since the last release to be a squash commit (the SHA itself is a merged PR's `merge_commit_sha`). Phase 2.5 now uses `gh pr merge {PREP_PR_NUMBER} --squash` so the next develop→main promotion can pass that invariant. Branch deletion is left to `/rite:cleanup`.
...
- **SKILL description diet: outcome + verification + mechanical steps, with rationale exiled to bundled references** — prose (`bytes_prose`) is compressed and moved to per-skill `references/`, while mechanical rails (fenced bash, branch tables, sentinels) stay byte-identical. The method is recorded in `plugins/rite/references/skill-diet-method.md` (line-drawing checklist, pin-transfer procedure, measurement template) and piloted on `open` (~−36%). Wave 1 applied it to `iterate` (−33.3%) / `fix` (−32.3%) / `pr-review` (−25.5%). Wave 2 covered the remaining skills: the large four (`setup` / `cleanup` / `wiki-lint` / `wiki-ingest`), the medium four (`pr-create` / `lint` / `issue-implement` / `issue-close`), and the remaining 19 small skills. CLAUDE.md's skill line-count principle is rewritten around the diet generation: `bytes_prose` is the primary metric, and the 4,000-line cap is no longer treated as a writing budget.
- **README Demo points at the v0.11.0 English/Japanese v2 intro videos** (~55s full cuts from `media/intro-video-v2`). The retired HyperFrames `PROVENANCE.md` opening claim is updated so it no longer says the README still references that cut.
- **Autonomous-execution preamble forbids stopping for a context-budget reason and limits reports to outcome / next action.** Resume after exhaustion stays with the existing compact/recovery path.

### Fixed

- **`iterate` promotes a missing review-result JSON at the cycle boundary to a repair gate** — if the previous cycle's result is still in context it is saved immediately through the existing helper; otherwise the cycle counter is held (`INC=held`) and the review is re-run. LOST annotation and trend calculation stay unchanged.
- **`/rite:cleanup` / `/rite:issue-close` still sync a parent Issue's Projects Status to Done when the parent is already CLOSED** — close-idempotency skip and board sync are separated, so an already-closed parent with all children closed still reaches Done.
- **Reviewer spawn-spread is measured from the orchestrator's spawn timestamp**, not from the time the report is written, so a long parallel review is no longer misclassified as serialized. Reviewers in the same message share one value.
- **`/rite:batch-run --merge` completion text no longer says "excluding failed" when `failed=[]`.** A non-empty failed list is still listed on its own line.
- **The project-local `/release` skill can complete a develop→main promotion safely** — the prep PR now runs `/rite:iterate`; promotion commits must already have arrived via merged PRs (`release-promotion-verify.sh`); the verified head SHA is pinned with `--match-head-commit`; Phase 2.5 stages only the seven release files (sandbox-safe); with `multi_session.enabled` the prep branch is placed in the issue session worktree; Phase 3 exits that worktree (`action: keep`) before touching `main`.

## [0.10.0] - 2026-08-12

### Added

- **XS/S lanes make per-phase ceremony cost proportional to the Issue's declared Complexity** — every phase previously ran at full weight regardless of change size: a one-line config change still spawned a full reviewer set with the full verification mandate attached. The Issue's declared Complexity (never auto-inferred, always present in the Issue's Section 0 Meta) now selects a binary lane — XS/S run `light`, while M/L/XL and every fail-safe path run `full` (identical to prior behavior; a missing or unreadable Complexity always falls back to `full` with a WARNING, since the safe side is never to reduce ceremony). `light` changes exactly two things in `pr-review`: `complexity_max=3` is passed into the reviewers Phase 5 `effective_max` resolution (merged into that resolution rather than applied as a separate filter, so the `mandatory` protection and the reviewer floor are not re-implemented — it is applied before the final clamp, so it is not a hard "at most 3" cap), and the verification mandate is reduced to the touched tests (full-suite sandbox duplication and mutation experiments become M-and-above equipment). In `issue-implement`, XS/S suppress new test files in favor of adding asserts to existing suites, and XS additionally forbids introducing new descriptive derived prose. Adoption criteria, consequence classes, and the Cross-File Impact Check are unchanged — the latter also absorbs an under-declared Complexity.
- **`merge` halts on an unhealthy CI state instead of trusting `mergeable` alone** — `mergeStateStatus` and the actual job execution record are now inputs to the pre-merge safety judgment, unknown CI states halt first (fail-closed), and pending check schemas are normalized before evaluation. A merge gate whose PR token is an unresolvable variable form is denied rather than guessed.
- **`/rite:setup` covers environment bootstrap end-to-end** — dependency verification (`bash` ≥ 4 / `jq` / `flock`) with per-platform install guidance, a GitHub authentication flow, a repository creation flow, and automatic generation of the GitHub templates (`ISSUE_TEMPLATE`, `PULL_REQUEST_TEMPLATE`). `getting-started` now delegates prerequisites to `setup` instead of duplicating them.
- **GitHub Actions CI on an ubuntu + macos matrix** — the matrix workflow runs the hook and script test suites on both platforms, `shellcheck` is blocking with its error backlog cleared, the macOS suite is green with BSD/GNU differences absorbed, and the `rite-dev` tests are wired into it.
- **`findings[].verification` is added to the review-result schema** — additive, with read-side type guards so older result JSON stays readable, and the canonical write version moved to 1.1.0.
- **`issue-create` fact-checks the assertions in the Issue body it generates** before creating the Issue, so generated claims about existing behavior are verified against the code rather than asserted.
- **`no_speculative_structure` coding principle** — structures, reserved fields, extension points, and config keys added in anticipation of future needs are rejected without a real Issue; the Guardrail additionally requires runtime reachability and a fail-loud default.
- **Fix-side policy gates: Simplification-First in `fix`**, a **contract-literal mandate across all Complexity levels in `issue-implement`** — the Issue contract's literal text is the implementation ceiling and expansion requests are returned to the Issue — and **finding response limited to the minimal diff**.
- **Hook decisions are persisted as a machine-readable audit trail** — `pre-tool-bash-guard` deny decisions and the reviewer guardrail audit are written to durable logs instead of existing only in session output.
- **`flow-state` records phase transitions to an append log**, so per-phase wall-clock time is collected automatically across a run.
- **`wiki-ingest` routes machine-detectable heuristics to detector candidates** rather than filing them as prose-only Wiki entries, and its promotion classification is recorded in one line per entry with explicit `promote` field guidance.
- **New lint detectors** — `grep -q` pipelines under `pipefail`, symlinks crossing the plugin distribution boundary, environment-specific tokens leaking into distributed files, and descriptive Issue/PR number references, whose detection was extended to bare `PR #N` / `Issue #N` forms, to `index.md`, and to an added-line gate.
- **`scripts/rite-dev` (at the repository root, not the plugin's own `scripts/`), a development launcher shared across hosts**, with its own test suite and CONTRIBUTING documentation.
- **An in-house frame-step video rendering pipeline** — HTML → mp4 is produced by a repository-owned renderer and scene contract driven by virtual-clock seeking rather than real-time capture, so the same scene yields an md5-identical mp4. The intro video was rebuilt on it, first as v2 and then re-storyboarded around rite's differentiators — pre-implementation exploration, the measured-evidence gate, and Wiki heuristics compounding over time — with the default draft-PR stop reflected instead of the opt-in `--merge` path.

### Changed

- **The review/fix loop now has a reachable termination condition: `blocking` is redefined as measured** — a finding counts as blocking only when it is CONFIRMED, `verification.measured == true`, and `scope ∈ {current-pr, follow-up}`; non-measured findings leave `total_findings` and are recorded in the integrated report's "findings without measurement" section. The classification itself moved out of LLM discretion into `scripts/review-measured-gate.sh`, which runs deterministically before the mergeable judgment — the "demote my own finding to non-blocking and declare mergeable" decision collides head-on with the reviewers' own thoroughness instructions, so as long as it stayed discretionary it was structurally not performed (one observed PR ran all 9 cycles without a single demotion, keeping a loop alive for over 8 hours on prose-precision findings that contractually must not block a merge). `mergeable` is decided on the blocking subset of `scope ∈ {current-pr, follow-up}` being empty, not on `findings[]` being empty, since `nit-noted` entries remain in the array by design.
- **A consequence-class demotion axis terminates the churn tail without a manual freeze** — blocking findings are split into class A (leaving it unaddressed changes the deliverable's runtime behavior) and class B (consequences confined to detection coverage, readability, or document consistency). In a cycle where class A is 0, all class B findings are demoted to non-blocking and the run ends through the existing mergeable path. Classification is an LLM judgment made in the consolidation context (separate from the finding's author) while enforcement is mechanical (`scripts/review-class-demotion-gate.sh`), and anything undecidable — missing entry, invalid class, no rationale, duplicates — is treated as class A with a WARNING rather than rounded down into a demotion.
- **The circuit breaker detects divergence from the convergence trend instead of firing on a cycle count** — a cycle-count ceiling cannot distinguish effort from waste: healthily converging loops were cut off a few findings short of done, and diverging loops burned to the ceiling. The per-cycle blocking series for the current run is reconstructed from the persisted review JSON, and "the last two values both exceed the best level so far and are not still descending" is the mechanical divergence test (the formula is trajectory-backtested, not window-width based). `safety.max_review_cycles` is demoted to a distant backstop for slow non-convergence that slips past divergence detection, and its default is raised from 5 to 15. Breaker firing halts mechanically in both batch and interactive modes, a breaker-induced stop is distinguished from an ordinary interruption so a halted run is not misreported as completed, and a full review is forced after the breaker so no blind spot outside the diff scope is left behind.
- **From cycle 2 the review is scoped to the fix diff and reviewers are selected dynamically** — findings concentrate on the surface the fix touched, yet every reviewer re-examined the full diff and codebase each cycle. Termination semantics are unchanged (blocking 0 = fix surface clean + all prior findings resolved; unchanged surfaces were reviewed in cycle 1). The judgment input is the persisted review JSON only, because with the default `pr_review.post_comment: false` there are no PR comments to read. Reviewer narrowing replaces the pattern-match input file list with the fix diff rather than filtering after the cap, so the sole-reviewer guard / `min_reviewers` / `mandatory` invariants of Phase 5 stay in one place.
- **Blocking is limited to behavioral-consequence classes for prose findings**, **test-pin findings block only where they correspond to the Issue contract's MUST/AC**, and a **Likelihood-Evidence producer gate** is applied at the finding source.
- **Record comments are identified by durable comment id rather than body matching, and carry a pointer instead of the full text**, with their recording path moved into a helper whose execution guarantee is pinned to a completion sentinel.
- **Account migration asakaguchi → B16B1RD** — owner and author references in `marketplace.json` / `plugin.json`, README badge URLs and `marketplace add` examples, and example values across `docs/SPEC.md`, templates, skills, scripts, and test fixtures are updated.
- **Skill-level execution conventions are shared as one contract** — the autonomous-execution preamble and effort guidance are consolidated into a shared contract, reversible recommendation-level judgments are handled autonomously instead of escalated, and environment-induced workarounds are silent on success and a single line on failure.
- **Wiki heuristics specific to rite's own behavior are promoted into the plugin's references and gates** rather than left in the project Wiki, which can only remind and never enforce — review/fix loop knowledge, defensive-mechanism completeness, document verification, contract and history knowledge, and remaining rite-specific entries, with the inline-content delegation promotion audit recorded in `pr-create`.
- **`pr-review`'s SKILL.md rationale is moved to its bundled references**, leaving one-line pointers and restoring line-count headroom in the executable procedure.
- **Installation and template documentation** — the install procedure gains the `/reload-plugins` step, README dependency requirements are expanded in both languages alongside a `.gitattributes`, and the Issue template's Section 9 states the acceptance criteria for the Decision Log.
- **Recurring internal mechanisms are consolidated into tested helpers** — `wiki-ingest`'s index manipulation, marker emit and matching on the `iterate` path (now shared functions with fixture tests), ownership of the `[CONTEXT]` marker's value-side matching convention, and `.gitignore` bundling. The completed one-time Wiki backfill helper is removed.
- **Documentation is re-synced to the shipped implementation** — the guard decision contract in CONTRIBUTING, `max_review_cycles`'s documented default, `review.loop` comments in the config template, schema invariant 6 and its gate wiring, the test coverage gate's connection to its decision point, the active guard and sentinel file listing in SPEC, unqualified "0 findings" phrasing left over after the measured-evidence gate, parenthetical claims in `fix` / SPEC AC-3 whose premises the gate wiring changed, `wiki-ingest`'s index instructions corrected to the actual 5-column table form, and two omissions in SPEC's Plugin Structure reference listing.

### Fixed

- **The merge gate blocked exactly the PRs that had been through a full review** — it required `schema_version` / `verdict` / `reviewers` (length ≥ 2) in the review-results JSON, but the latter two had no definition in the schema Single-Source-of-Truth and neither canonical writer emitted them; merges had been passing only because hand-written JSON happened to fit. The writers are extended so the gate's requirement becomes the specification.
- **Review-result integrity gates now verify the artifact rather than its absence** — the cycle-level execution-guarantee gate on saving review-result JSON, a positive check of the current cycle's result JSON in the pre-merge gate pinned to the real HEAD, orphaned review JSON and run-start pins reclaimed mechanically, malformed Verification anchors treated as undecided (staying blocking), and `ready`'s pre-gates matched to the PR head, and a review-existence positive gate added at the merge execution point.
- **Reviewer spawn parallelism is verified mechanically** — serialization of reviewer spawns is detected from machine-recorded spawn timestamps, and the spawn-spread description and its static pin are hardened.
- **Sandbox and macOS/BSD portability in hooks** — `pr-cycle-cleanup.sh`'s `TMPDIR` decision resolves symlinks to physical paths, `mktemp` templates place the `X` run at the end so BSD/macOS regains filename uniqueness, tempfile lifecycle is extracted to a lib with lint detectors for the two remaining patterns, `pr-cycle-cleanup` reclaims detached mutation worktrees under `TMPDIR` — its skip condition moves from "the working tree is dirty", which rejected every intended target since a mutation worktree is inherently dirty, to "the commit is unreachable from every named ref", with an undecidable check skipped under a WARNING, stale issue-claim takeover works on macOS, `flow-state`'s `_atomic_write` guards the `flock` call with `command -v`, and the diagnostic-neutralization parity sweep is extended to `scripts/`.
- **Guard path handling is canonicalized** — `pre-tool-edit-guard`'s AC-2 symlink resolution no longer depends on `realpath`, edit path and audit normalization are aligned, the cwd content guard canonicalizes its input, and the live cwd is detected from `lsof` output.
- **Silent failures in hooks are made loud** — a failed pending-marker deletion is reported instead of swallowed, and working-memory checklist integration no longer returns a no-op success.
- **Working memory survives the paths that used to drop it** — `pr_number` / `loop_count` are carried forward through the ordinary update path and the canonical-copy selection is decided by content inspection rather than mere existence.
- **`cleanup` no longer stalls or over-deletes** — a session started inside a worktree splits its items and delegates main-checkout operations, branch-delete guards are hardened with the remote-branch delete guard switched to `--exit-code` so `merge`'s guarantee matches reality, a transient `ls-remote` rc=128 gets one retry, and `recovery=auto` is synchronized with the reaper's dirty gate.
- **`wiki-query` and `wiki`** — Pass 1 candidate extraction handles the table-formatted index and a zero-candidate result is fail-loud instead of silently empty, diagnostic wording uses ASCII in place of em-dashes that macOS CI corrupted into invalid UTF-8, Japanese titles no longer trigger the C1 false positive, and Wiki pushes are retried outside the sandbox.
- **`lint` and skill-definition parsing** — board-drift detection no longer restricts itself to a closure reason, and positional-parameter expansion inside fenced bash no longer makes the YAML parser return every key empty.
- **Test suites no longer pass vacuously** — TC-036a's floor condition is derived from the blocking gate rather than a Linux check, TC-124 (d) gains a positive control, comment-journal mutation survivors are killed, the bash-guard suite continues past a `jq` extraction failure instead of aborting the run, the backlink checker is excluded across worktrees, and the ERE bare-pipe hazard is documented in the grep assert helpers' docstrings.
- **Intro video assets** — v2's BGM audibility and loop phase are corrected, Pixabay BGM provenance is restored, and `assemble.sh`'s `-P` follows the current scene composition.

## [0.9.2] - 2026-07-24

### Changed

- Intro video (Japanese and English) badge/ending-meta version stamps are updated from `v0.8.0` to `v0.9.1`, and the README Demo section video links point to the new BGM-mixed final renders. Scene composition and duration (~125s) are unchanged.

## [0.9.1] - 2026-07-23

### Added

- **`cleanup`/`wiki-ingest`/`batch-run`/`recover` completion reports now aggregate an Outstanding Items section** instead of leaving non-blocking failures (a skipped wiki push, a deferred branch deletion, etc.) scattered only as inline checklist annotations that were easy to miss — the aggregated section states "none" explicitly when there are zero outstanding items. `cleanup` emits a `[cleanup:outstanding:N]` sentinel that `batch-run` rolls up across the whole batch's `--merge` completion notice, and `recover` gained an informational check (only when the resolved phase is `cleanup`/`completed`) that detects unpushed wiki commits or leftover local branches directly from git state.

### Fixed

- **`setup` Phase 4.8/4.9 guidance messages are now output verbatim instead of being paraphrased into indirect reporting language by the executing agent** — an explicit instruction to output the template text verbatim, without summarizing, was added immediately before each message block.
- **Corpse worktrees (admin-dir half-broken and unrecognized by git) are now reclaimed immediately instead of waiting on the 24h age guard** — `cleanup`'s Step 4-W now records the worktree path to the reap manifest (`.rite/tmp-artifacts.tsv`) whenever removal is skipped (busy failure or sandbox mask) and the PR is confirmed merged, since a corpse worktree can't resolve its checked-out branch for the existing branch-name-based bypass to apply.
- **`pr-review`'s worktree-axis drift detection no longer false-positives from sandbox ghost-mount entries** appearing on only one side of the snapshot/verify comparison — both `post-review-state-verify.sh`'s `current_worktree_hash` and `pr-review`'s Step 4.0.A `ORIG_WTH` now compute through the existing ghost-mount filter (`lib/git-status-filtered.sh`) instead of raw `git status --porcelain`.
...
- **`mktemp`'s hardcoded `/tmp`-root templates are eliminated project-wide for sandbox environments**, where writes are confined to `$TMPDIR` and `/tmp` itself is read-only — both production code and the test harness now use the `${TMPDIR:-/tmp}/rite-*` form, and a new `tmp-hardcode-check.sh` lint guard (check table #16) prevents regressions.
- **Sandbox write-blocking mounts (character-device `/dev/null` bind mounts) were misdetected as untracked (`??`) by `git status`**, causing `cleanup`/`recover`/`issue-update`/`pr-create`'s dirty checks to false-positive with no real changes present — a new shared filter (`lib/git-status-filtered.sh`) excludes only untracked character-device entries, and the 4 affected skills' dirty checks now go through it.
- **`issue-implement` replaces `git add .` with explicit path staging (`git add {changed_files}`)** — sandboxed sessions mask read-denied home dotfiles (`~/.ssh`, `.bashrc`, `.gitconfig`, etc.) as untracked character devices, and `git add .` hard-failed (exit 1) trying to pick them up as "not a regular file," stalling the implementation-phase commit.
...
- **`/rite:batch-run` no longer stalls on `open`'s plan approval or `pr-review`'s configuration prompt** — reconciling batch-run's declared fully-autonomous (no-confirmation) contract with actual behavior. `open` step 3.4 now auto-approves the implementation plan when a batch run is active (run-queue detection, same shape as `iterate` step 6), while standalone runs still use `AskUserQuestion`; `pr-review` step 3.3 skips the reviewer-configuration `AskUserQuestion` on the E2E (iterate-driven) path via a flow-state phase-whitelist check (same shape as `ready` Phase 2.1), keeping the launched/omitted-reviewer summary lines on both paths. Both fall back to interactive/standalone on helper failure.

## [0.8.2] - 2026-07-15

### Added

- **`setup` detects and warns on a version mismatch between `plugin-path-resolution.md`'s two resolution methods** — step 4.5.0's marketplace branch now cross-checks the direct key lookup against the canonical one-liner resolution and displays both paths, the mismatch, and remediation guidance when they disagree (non-blocking; matching resolution is unaffected). Adding a third resolution method is now explicitly disallowed in `plugin-path-resolution.md`.
...
- **`pr-review` step 4.3.1 requires an explicit `run_in_background: false` on every reviewer Task spawn** — the prior prohibition ("do NOT use `true`") left the parameter's default unstated, and the harness's default of background execution meant omitting the parameter silently spawned reviewers in the background; this is now a MUST with rationale documented.
- **`issue-create` pre-creates labels idempotently and surfaces helper failures** — label creation no longer fails when a label already exists, and helper failures are surfaced as a result instead of being swallowed.
- **`lint`'s `gitignore-health-check` false positives are fixed** — the check now performs an effective-ignore determination instead of a naive pattern match, and unreachable entries introduced by `setup` are excluded from the scan.
- **`setup` re-runs `gh project link` idempotently after Project creation** — fixes a first-Issue-create Projects registration failure that occurred when the newly created Project wasn't yet linked to the repository.

## [0.8.1] - 2026-07-11

### Fixed

- **`recover` now detects and resumes an interrupted `/rite:batch-run` queue** — a new Phase 5.5 in `recover/SKILL.md` autonomously judges whether an interruption occurred mid-active-batch (via `run-queue.json`'s `active` flag, cursor match, and a 2-hour freshness check reusing `session-ownership.sh`'s `parse_iso8601_to_epoch`) and, if so, continues processing the remaining queue by following `batch-run/SKILL.md`'s existing step 3-8 branch table rather than duplicating it; stale `run-queue.json` leftovers no longer trigger a false continuation. `run-queue.json` gains an `updated_at` timestamp field, updated on every cursor advance / active-flag write.
...
- **The reviewer registry's three-way sync (`agents/*-reviewer.md` ⇔ the `pr-review/SKILL.md` Available Reviewers table ⇔ its Reviewer Type Identifiers table) is now verified by a single machine check** instead of only the tech-writer-row equality check, catching agent-only additions, one-sided table updates, and slug mismatches that previously passed silently.
- **Per-call hooks gain an early-exit fast path and consolidated `jq` calls for non-rite projects**, reducing the per-Bash/Edit-call subprocess cost outside rite projects while leaving rite-project inputs, outputs, and side effects unchanged.
...
- **`issue-create` recognizes a `/rite:unknowns` exploration summary and lightens Assumption Surfacing accordingly** — questions and blind spots already resolved by the summary skip re-asking, and unresolved questions flow directly into the existing three-way classification.
- **`issue-create` Step 4.0 gains a "blind spot pass" (unknown unknowns) sub-step** for Complexity M+ issues, feeding discoveries into the existing derive/ask/defer classification without new code paths.
- **`open` Step 3.3's implementation-plan template adopts a volatile-first ordering rule** — items likely to change under user judgment (data model changes, type/interface definitions, user-visible behavior/UX) are presented before mechanical refactors and boilerplate, focusing plan-approval review on substantive decisions.
- **`pr-review` Step 7 is redesigned from "automatic Issue creation" to "out-of-scope finding triage"** — the AskUserQuestion recommendation moves from agent discretion to a rule-table machine decision, gains a "record to Decision Log" option, and approved records are appended to the source Issue's Section 9 Decision Log (or work memory as fallback).
- **`pr-review` gains a `max_reviewers` cap and a pre-spawn cost-estimate summary** — reviewer selection now caps the spawned reviewer count after the existing min-reviewer/mandatory-Security guards, and always surfaces which reviewers were dropped and why instead of silently capping. Default `max_reviewers: 6` preserves prior selection for matches within the cap.
- **`pr-create` PR bodies now summarize the Decision Log and plan-deviation log** — Phase 3.2.2 reads the Issue's Section 9 Decision Log and work memory's plan-deviation log, surfacing implementation-time judgment calls that a diff alone can't show; the section is omitted when both are empty.
- **`iterate`'s review⇄fix loop gains a circuit breaker** — `safety.max_review_cycles` (default 5) bounds non-convergent review/fix cycling; on `batch-run`, a maxed-out PR is recorded as failed and the cursor advances instead of stalling the whole batch.
- **`recover` detects merge-conflict and interrupted-rebase state** — unmerged markers, `MERGE_HEAD`, and `rebase-merge`/`rebase-apply` (resolved worktree-safely via `git rev-parse --git-path`) are surfaced ahead of phase inference, so conflict resolution isn't skipped in favor of a generic "implementation in progress" recovery.
- **`batch-run` shows a pre-run summary before starting** — issue count, run mode, a rough time estimate, and interrupt/resume guidance are shown once right after the queue is finalized, plus an "N/M done" progress line per completed Issue.
- **`setup` now writes a `safety` section into `rite-config.yml`** — previously commented out below the `--- Advanced ---` marker, `safety` (`max_implementation_rounds`, `max_review_cycles`, etc.) is promoted to the active block alongside `wiki`/`multi_session`/`tdd`, so a fresh config surfaces these safety limits instead of hiding them.
- **The sentinel contract (~29 `[skill:action]` strings) is now Single-Source-of-Truth documented and CI-verified** — `sentinel-contract.md` centralizes the emitter/consumer table, `sentinel-contract-check.sh` verifies it bidirectionally, and the check runs as `lint` Phase 3.20 and in a dedicated GitHub Actions workflow on every push/PR.
...
- The intro video HyperFrames sources are now tracked in-repo under
  ...
  build steps and BGM source/license.
...
- **The release skill's Phase 1 tag-sync no longer fails silently** — a failed `git fetch --tags --force` now emits a log line and continues with local tags instead of swallowing the error.

### Docs

- **Documented that the `git tag … | head -1` SIGPIPE is benign in the release skill** — the trailing `head -1` can close the pipe early, but `latest_tag` is guarded by a `[ -n "$latest_tag" ]` check so the pipeline exit status is never consulted; a note clarifies this and flags the `set -o pipefail` caveat for the future.
...
- **Parent-Issue detection via tasklist search in `projects-integration.md` and `close.md` no longer misidentifies self or unrelated Issues as the parent** — the search retrieves multiple candidates, excludes self-matches, and validates candidate bodies before adopting a parent.
- **`/rite:pr:open` now surfaces the parent-Issue GitHub Projects status update in `open.md`**, so starting work on a Sub-Issue transitions the parent Issue's status from Todo to In Progress instead of leaving it at Todo.
- **Resolved the contradictory standalone parent-not-detected handling in `projects-integration.md`** — the spec now consistently emits the `[DEBUG] parent not detected` log before skipping, matching the rule that silent skips are prohibited.
- **Unified the standalone parent-not-detected DEBUG wording to `methods tried:` across `close.md`, `projects-integration.md`, and `open.md`**, so all three sites emit verbatim-identical diagnostics.

## [0.6.6] - 2026-06-24

### Fixed

- **GitHub Projects registration no longer performs owner-type detection, so Organization-owned Projects are supported** — the previous owner-type probe failed for Projects owned by an Organization; removing it lets both user-owned and organization-owned Projects register.
- **GitHub Projects field names resolve via built-in English/Japanese aliases plus optional config overrides, supporting Japanese-named Projects** — field lookups (`Status`/`ステータス`, etc.) now succeed on Projects whose fields use Japanese names without extra configuration.
- **PR cleanup worktree detection is anchored to the physical cwd**, eliminating leftover worktrees when a worktree was not recorded in flow state.
- **Fixed the `cleanup.md` anchor link to point to `main-checkout`.**

### Changed

- **Updated the graphql-helpers owner-type documentation to a `repository()`-independent path**, aligning the docs with the registration logic that no longer depends on `repository()`.

## [0.6.5] - 2026-06-22

### Fixed

- **`/rite:pr:cleanup` and `/rite:pr:open` base-branch updates now use `git fetch` + `git merge --ff-only` instead of `git pull --ff-only`** — under a consumer environment with `pull.rebase=true` and a dirty working tree, `git pull --ff-only` aborted early with `cannot pull with rebase: You have unstaged changes` because `git pull` enters its rebase pre-check (which requires a clean working tree) even when `--ff-only` is set. Splitting the update into `git fetch origin {base}` + `git merge --ff-only origin/{base}` bypasses the rebase path entirely, so a fast-forwardable base branch updates reliably regardless of the `pull.rebase` setting or working-tree state. Applies to the `multi_session` and legacy paths of `cleanup.md`, the branch-creation chain of `open.md` (with the `git switch -c` `&&` chain preserved), and the matching descriptions in `getting-started.md` / `docs/SPEC.md`.
...
- Restructured the README into a bilingual layout: `README.md` (English, with the English intro video) and a new `README.ja.md` (Japanese, with the Japanese intro video), cross-linked by a language switcher at the top of each

## [0.6.2] - 2026-06-19

### Docs

- Added a Demo section with an introductory video (Japanese subtitles) near the top of the README

## [0.6.1] - 2026-06-18

### Fixed

- **`EnterWorktree` failure under harness git misdetection now points to a restart remedy** — when `multi_session.enabled: true` (default), `/rite:pr:open` Step 2.3-W and `/rite:resume` re-entry detect the harness git repository misdetection (`.git` present and `git` CLI healthy, but the startup check reports false) and recommend restarting Claude Code from the repository root so the already-created worktree is reused (`WT_CASE=reuse`), instead of offering only the prior "abort / `git switch -c`" fallback. Documented in `getting-started.md` and `git-worktree-patterns.md`.

## [0.6.0] - 2026-06-18

### Added

- **Canon TDD cycle for the implementation phase** — when `tdd.enabled: true` (default on), `/rite:issue:implement` drives implementation through a Canon TDD loop: pick one behavior from the test list → confirm Red → minimal Green → Refactor → repeat until the list is empty. Falls back gracefully: `commands.test: null` runs in "Degraded TDD" mode (test-list discipline kept, auto-run skipped with a warning), and `tdd.enabled: false` restores the prior non-TDD flow.
- **`tdd:` configuration section, default-on (opt-out)** — distributed `rite-config.yml` ships a `tdd:` section with `enabled: true`; `/rite:init --upgrade` back-adds it to existing projects via the same active-section mechanism as `wiki` / `multi_session`.
- **Canon TDD documentation and test-list framing** — the Issue template Section 6 "Test Specification" is now framed as the Canon TDD test list (one T-xx row = one Red→Green→Refactor cycle), reflected across `skills/rite-workflow`, `docs/SPEC.md`, getting-started, and `pr/open.md`.

## [0.5.5] - 2026-06-17

### Fixed

- **`bang-backtick-check.sh` no longer hard-blocks consumer repos** — a new `--skip-if-no-target` flag makes `--all` return a clean skip (rc=0) instead of an `rc=2` invocation error when there is no `plugins/rite/` markdown to scan (marketplace-only consumer repos), removing the forced manual gate bypass in `/rite:pr:ready` and `/rite:pr:create`. Self-host repos keep the original `rc=2` misconfiguration diagnostic.
- **Active-but-idle session worktrees protected from lazy reap** — `pr-cycle-cleanup.sh` keeps a worktree whose issue claim holder is `active=true` even when the claim heartbeat has gone stale (idle past `CLAIM_STALE_SECONDS`), preventing the `/clear` "Path does not exist" error from recurring on resume.
- **Read-only commands relay their real output again** — removed `context: fork` from `/rite:issue:list`, `/rite:investigate`, `/rite:workflow`, and `/rite:skill:suggest` so their output is shown inline instead of being replaced by the harness control wrapper.
- **`orphan-reference-check.sh --all` works inside session worktrees** — the `--all` scan now walks paths relative to `REPO_ROOT`, so running from a `.rite/worktrees/issue-N` worktree no longer excludes every file and exits `2`; orphan detection is restored under the default `multi_session.enabled: true`.
