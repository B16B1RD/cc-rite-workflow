...
- **`[fix:pushed-wm-stale]` output pattern** — `/rite:pr:fix` now emits `[fix:pushed-wm-stale]` when Phase 4.5 work memory update silently failed (e.g. `current_body` empty, issue_number not found, `git diff` failure, PATCH failure). Callers (`/rite:issue:start` review-fix loop) **must not** silently treat this as `[fix:pushed]`; instead, they must surface a warning via `AskUserQuestion` and let the user decide whether to continue with stale work memory or pause for manual recovery. See `commands/pr/fix.md` Phase 8.1 caller semantics for the full contract (#349)
