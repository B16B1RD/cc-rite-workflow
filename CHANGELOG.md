...
- **`[fix:pushed-wm-stale]` output pattern** — `/rite:pr:fix` now emits `[fix:pushed-wm-stale]` when Phase 4.5 work memory update soft-failed. Firing conditions (e.g. `current_body` empty, `issue_number` not found, PATCH 4xx/5xx, `pr_body` grep IO error, branch grep IO error, `gh api comments` fetch failure, Python script unexpected exit, work memory body corruption detected, mktemp failure). See the reason table in `commands/pr/fix.md` Phase 8.1 for the complete list of `reason` values. The `git diff` failure path is handled separately as a hard fail-fast (`exit 1`) and surfaces as `[fix:error]`, not `[fix:pushed-wm-stale]`. Callers (`/rite:issue:start` review-fix loop) **must not** silently treat `[fix:pushed-wm-stale]` as `[fix:pushed]`; instead, they must surface a warning via `AskUserQuestion` and let the user decide whether to continue with stale work memory or pause for manual recovery. See `commands/pr/fix.md` Phase 8.1 caller semantics for the full contract (#349)
