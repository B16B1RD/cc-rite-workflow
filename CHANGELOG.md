...
- **`findings[].verification` is added to the review-result schema** — additive, with read-side type guards so older result JSON stays readable (#2035). `verdict` and `reviewers` — required by the merge gate but absent from the schema Single-Source-of-Truth — became formal fields written by the canonical writers (#2229), and the canonical write version moved to 1.1.0 (#2211).
...
- **`scripts/rite-dev`, a development launcher shared across hosts**, with its own test suite and CONTRIBUTING documentation. (#2178)
...
- **Sandbox and macOS/BSD portability in hooks** — the `TMPDIR` decision in Step 4-P resolves symlinks to physical paths (#2172), `mktemp` templates place the `X` run at the end so BSD/macOS regains filename uniqueness (#2080), tempfile lifecycle is extracted to a lib with lint detectors for the two remaining patterns (#2124), `pr-cycle-cleanup` reclaims detached mutation worktrees under `TMPDIR` (#2156), stale issue-claim takeover works on macOS (#2201), `flow-state`'s `_atomic_write` guards the `flock` call with `command -v` (#2003), and the diagnostic-neutralization parity sweep is extended to `scripts/` (#2206).
...
- **Silent failures in hooks are made loud** — a failed pending-marker deletion is reported instead of swallowed (#2205), working-memory checklist integration no longer returns a no-op success (#2152), and the Step 4-P dirty protection is replaced with unreachable-commit protection that matches the actual risk (#2160).
...
- **The batch/interactive stop path distinguishes a breaker-induced failure stop from an ordinary interruption** so a stopped run is not misreported as completed. (#2220)
