...
- **tech-writer Critical Checklist concretized** — Added 5 doc-implementation consistency items (Implementation Coverage / Enumeration Completeness / UX Flow Accuracy / Order/Emphasis Consistency / Screenshot Presence). Each item carries a verification method (Grep/Read/Glob), and 3 new Finding Examples from blocks-documentation PR #1137 have been added (#349)
- **internal-consistency.md reference** — New reference file that complements `fact-check.md` (external specs) with an internal-facts verification protocol. Defines 5 Verification Protocol categories, Confidence 80+ gate, severity mapping, and Cross-Reference (#349)
- **Doc-Heavy PR Detection (Phase 1.2.7)** — Automatic detection of documentation-centric PRs (`doc_files_ratio >= 0.6` OR `doc_files_count_ratio >= 0.7` AND `total_diff_lines < 2000`). Excludes rite plugin's own `commands/skills/agents` paths (prompt-engineer territory). Added optional schema `review.doc_heavy.*` to `rite-config.yml` (#349)
- **Doc-Heavy Reviewer Override (Phase 2.2.1)** — When `{doc_heavy_pr == true}`, promote tech-writer from recommended to mandatory and add code-quality as co-reviewer. Passes `{doc_heavy_pr=true}` flag to tech-writer to activate the enhanced "Doc-Heavy PR Mode (Conditional)" protocols (Mandatory Implementation Cross-Reference, Screenshot Completeness Check) (#349)
- **`/rite:pr:fix` accepts PR URL / comment URL directly** — Extended `fix.md` Arguments table to 3 forms (pr_number / pr_url / comment_url). Phase 1.1.1 Argument Parsing uses regex to route input, and Phase 1.2 gains a target_comment_id branch that parses findings directly from an external review tool's comment URL (e.g. `/verified-review`) and feeds them into the fix loop (#349)
