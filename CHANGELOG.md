...
- **`/rite:pr:fix` accepts PR URL / comment URL directly** — Extended `fix.md` Arguments table to 3 forms (pr_number / pr_url / comment_url). Phase 1.0 Argument Parsing uses regex to route input, and Phase 1.2 gains a target_comment_id branch that parses findings directly from an external review tool's comment URL (e.g. `/verified-review`) and feeds them into the fix loop. Requires the external comment to contain a markdown table with at least 4 columns. Header names are flexibly recognized for all 4 columns: `severity` (`severity` / `重要度` / `sev` / `level` / `深刻度` / `priority`), `file:line` (`file` / `ファイル` / `path` / `location` / `場所`), `content` (`content` / `内容` / `message` / `description` / `指摘` / `issue`), `recommendation` (`recommendation` / `推奨` / `fix` / `suggestion` / `対応` / `action`). Header-less tables fall back to default column order `severity | file:line | content | recommendation`. See `plugins/rite/commands/pr/fix.md` Phase 1.2 best-effort parse for the authoritative keyword table (#349)
