...
- **Multi-host runtime contract** — `host-workflow-operations.md` defines, inside the distribution, the eight common operations (state isolation, worktree entry, sub-skill continuation, hook compatibility, review collection, explicit execution, and so on) with a version-and-evidence capability table for Claude Code, Codex CLI, and Grok Build. `session-identity.sh` resolves the current host's real session ID and stops instead of degrading to shared state when it is missing or ambiguous; `host-runtime.sh` exposes init, pre/post-operation guards, checkpoint, and next-step reads as explicit commands; `reviewer-completion-check.sh` lets result merging start only when every selected reviewer's run ID and completed output are present.
