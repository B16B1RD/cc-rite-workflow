...
domain: "heuristics"
description: "既存コンポーネントを参照する design doc は記憶ベースで書かず、現 develop HEAD の registration ファイル (hooks.json 等) や writer/reader を grep evidence として確認してから書く。"
promote: rite-plugin
created: "2026-04-26T09:20:00+00:00"
sources:
  - type: "fixes"
    resource: "raw/fixes/20260504T232902Z-pr-829.md"
  - type: "reviews"
  ...
  - type: "fixes"
    resource: "raw/fixes/20260426T082521Z-pr-677-cycle-3.md"
  - type: "fixes"
    resource: "raw/fixes/20260426T084232Z-pr-677-cycle-4.md"
  - type: "reviews"
    resource: "raw/reviews/20260519T195007Z-pr-1065.md"
  - type: "fixes"
    resource: "raw/fixes/20260519T195351Z-pr-1065.md"
  - type: "reviews"
    resource: "raw/reviews/20260523T174827Z-pr-1111.md"
  - type: "reviews"
...
### `.rite-flow-state` multi-state Decision Log での実測
...
### Documentation の code-state claim は review 時にも grep verify する
...
`wiki:* commands` を cleanup.md スタイルへ本格フラット化した PR で、本 page の grep verification 義務が **新規 SoT documentation 作成時** にも適用される事例を実測。cycle 1 で `wiki-patterns.md` に新規 SoT セクション (YAML パース helper の分散実装一覧) を documentation 化したが、3 reviewer (prompt-engineer / error-handling / code-quality) 全員が cycle 2 で同 cluster の指摘を出した:
...
- [9 findings: stale code reference cluster + schema inheritance + split-config + cross-doc](../../raw/reviews/20260426T080650Z-pr-677.md)
- [44% findings traced to single root cause](../../raw/fixes/20260426T081122Z-pr-677-cycle-1.md)
- [SoT verification gap + multi-location enumeration drift](../../raw/fixes/20260426T081939Z-pr-677-cycle-2.md)
- [SoT grep evidence pattern 確立](../../raw/fixes/20260426T082521Z-pr-677-cycle-3.md)
- [hooks.json SoT 大発見、cycle 4 spike の root cause](../../raw/fixes/20260426T084232Z-pr-677-cycle-4.md)
- [design doc plan PR の 4 修正パターン: (1) 採用案 SoT 分裂を 4 箇所同期、(2) 検証 grep の syntax 不能 = markdown table cell 内 `\|` ERE alternation 不能 + BRE `+` literal 扱いで silent-pass、表内 grep を表外 fenced code block に分離、(3) 数値主張の grep 実測値乖離を再現コマンド明記で再計算、(4) scope rationale の事実誤認を表現緩和](../../raw/fixes/20260504T232902Z-pr-829.md)
- [doc mechanism scope claim 曖昧 = `auto_demote_low` の対象を「LOW or MEDIUM」と表記したが実装は LOW のみ + 別経路を混合記述](../../raw/reviews/20260519T195007Z-pr-1065.md)
- [2 経路 (auto_demote_low / reviewer 直接 assign) の分離記述で doc-impl scope drift を解消](../../raw/fixes/20260519T195351Z-pr-1065.md)
- [doc-only PR の orphan marker code-state claim を 2 reviewer が独立 grep-confirm、0 findings / 1 cycle = review-side grep verify の positive evidence](../../raw/reviews/20260523T174827Z-pr-1111.md)
- [新規 SoT documentation 作成時の 3 種 factual error: site 一覧の grep 実在検証なし / 実装 case 文 verbatim verify なし / helper 関数名取り違え、3 reviewer 独立合意の cluster 形成](../../raw/reviews/20260526T182658Z-pr-1155-cycle2.md)
- [SoT 新設時の grep 義務 3 種 = site 列挙 / case 文 verbatim / helper 定義行 grep、最低 2 reviewer 独立検証の経験則化](../../raw/fixes/20260526T183041Z-pr-1155-cycle2-fix.md)
