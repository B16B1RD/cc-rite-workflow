...
domain: "heuristics"
description: "既存コンポーネントを参照する design doc は記憶ベースで書かず、現 develop HEAD の registration ファイル (hooks.json 等) や writer/reader を grep evidence として確認してから書く。記憶に頼った参照は stale code reference を生み、multi-cycle review-fix loop の dominant 原因となる。過去のレビュー事例 (関連する課題 multi-state Decision Log Phase 1) で 7 cycle 収束軌跡 (9→5→3→5→1→1→0) を実測。cycle 1 で 9 findings の 44% (4 HIGH) が「stale code reference」単一 root cause、cycle 2 で needs_clear field の grep verify 漏れ + hook list multi-location drift (5箇所中1箇所のみ更新)、cycle 3 で SoT grep evidence pattern 確立、cycle 4 で hooks.json registration を SoT として再評価したことで cycle 1-3 積み残しが 5 finding として再 surface (非単調 spike pattern)。canonical 対策: registration ファイルを SoT に / field inheritance claim には grep verify / single SoT + reference の構造 / library vs hook 区別の Note 化。reviewer 単独 cycle scope では surface しない deeper SoT layer は複数 cycle reviewer 累積で初めて顕現する観察も実証。過去のレビュー事例 fix cycle 2 で design doc plan PR の 4 修正パターンを追加: (1) 採用案 SoT 分裂を 4 箇所同期、(2) 検証 grep の syntax 不能 = markdown table cell 内 `|` ERE alternation 不能 + BRE `+` literal 扱いで silent-pass、表内 grep を表外 fenced code block に分離、(3) 数値主張の grep 実測値乖離を再現コマンド明記で再計算、(4) scope rationale の事実誤認を表現緩和。過去のレビューで「doc mechanism scope claim の grep verify」sub-pattern を追加: doc が記述するメカニズムの scope が実装の scope より広い drift も本 heuristic の subset (auto_demote_low の対象を「LOW or MEDIUM」と曖昧表記したが実装は LOW のみで MEDIUM × current-pr は別経路 = reviewer 直接 assign 経由)、実装側の判定式を直接 grep して inline evidence で残す対策で 2 経路の分離記述に到達。過去のレビューで **review-side grep verify** の positive evidence を追加: doc-only PR が 3 marker を orphan (live writer/reader 不在・PHASE_ENUM_V3 非在) と主張する code-state claim を 2 reviewer が独立 grep-confirm し 0 findings / 1 cycle mergeable、書き手側 grep verify が抜けても review 側が safety net として機能することを実証。過去のレビューで **SoT 新設時の verify 義務 3 種** を追加: 新規 SoT documentation 作成時の (1) site 一覧の grep 0-hit check、(2) semantic 記述の case 文 verbatim diff、(3) helper 関数名の定義行 grep、を最低 2 reviewer 独立検証する経験則として確立 (3 reviewer 独立 cluster 検出 evidence)。"
promote: rite-plugin
