...
domain: "heuristics"
description: "bash block に新規 `exit 1` fail-fast 経路 / sentinel type enum / incident type を追加する PR は、同一ファイル内の `9.3 exit code` 節例外リスト・エラーハンドリング表・SPEC.md / protocol.md の enum 列挙という canonical SoT 一覧群を必ず同時更新する義務を負う。『N site 対称化』counter 宣言は drift 検出アンカーとして grep で「counter 宣言 vs 実登録数」の gap を機械検証可能にする。過去のレビュー事例 cycle 2 で 2 件 MEDIUM sync drift を実測、過去のレビューで sentinel type enum drift (`gitignore_drift`) にスコープ拡張、過去のレビューで「scope 外 drift → 別 Issue 化 → 後続 PR で解消」フローを +4 lines の minimal cycle で実証、過去のレビューで sibling entry 間 parallelism suffix (「N 種で同型」) の micro-drift 拡張観点 (3 文字 micro-fix の cross-validation 検出) を追加。過去のレビューで「header docstring の caller list と実 caller の cross-file drift」+「TC で全 caller 6+ を category 別 (lifecycle / observability / command-level) に enforce する設計義務」を追加 (MEDIUM × 1)。test fixture が grep evidence と直接対応する構造で silent regression を CI で decisive 検出可能化。過去のレビューで **PR description ⇔ commit message ⇔ reference 内 prose の 3 site 数値 commitment drift** sub-pattern を追加: 同一概念の数値表現 (削減目標 / 件数 / 閾値) が複数 site に散在する場合、SoT 集約 or 統合表現 (`12 → 4 (上限 ≤ 5)`) で構造的に drift 経路を塞ぐ。過去のレビューで **refactor 対象外 reference 内の "site count" stale 化** sub-pattern を追加 (2 reviewer cross-validation MEDIUM): 新 SoT 宣言時は本体 + 全 references 横断で「N callsite」「N 箇所」絶対参照を検索し移管対象外 reference 内の stale claim も同 cycle で更新する。過去のレビューで **'N-site 対称化' narration claim と peer 経路 (watchdog 等) の含意整合性** sub-pattern を追加 (3 reviewer cross-validated): claim の N は「PR の修正対象 site 数」と「同型 idiom が peer 経路に existing する事実」を分けて narration する (例: `3-site 対称化 (peer 経路の watchdog は scope 外)` と footnote で peer 存在明示)。`grep -rn` で repo 全体の occurrence 数と claim N の乖離を check する canonical 検出経路を追加。過去のレビュー事例 (関連する課題、0 findings 初回 mergeable) で **hand-maintained counter 撤廃 → step enumeration 列挙統一への転換** を追加: 「5 site」「N 箇所」counter は計数規則の曖昧さ (inline のみか helper 委譲分を含むか) を内包し drift 源化するため、grep で各 site を直接実在検証できる step enumeration 列挙へ統一する。「各 site で同型」qualitative 表現は具体的 step 集合の併記があれば残置可。counter は line 番号と同じ hand-maintained literal の一種 (drift-check-anchor-semantic-name と同型の構造的閉塞)。"
