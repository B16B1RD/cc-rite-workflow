...
domain: "heuristics"
description: "同じ処理を 2 つの入力形式・2 つの経路で実装したとき、新しく足した側の fixture が「素朴な形」しか持たないと、経路の**存在**は測れても経路の**正しさ**は測れない。既存側が生パイプ入りタイトル・コードスパン入りサマリーといった「厄介な入力」で処理との相互作用を pin しているなら、新設側にも同じ厄介さを持つ行を置く。"
