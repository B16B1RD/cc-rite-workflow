...
domain: "heuristics"
description: "素朴な fixture は経路の存在は測るが、経路の正しさは測らない。片方の経路に生パイプ入りタイトルやコードスパン入りサマリーで相互作用を pin する fixture があるなら、もう片方にも同じ厄介さを持つ行を置く。PR #2070 cycle 2 では新設した箇条書き経路の fixture が干渉のない素朴な形しか持たず、形式 dispatch の行頭 anchor を外す変異も、マスク前後の文字列を取り違える変異も、172 PASS のまま生き残った。"
