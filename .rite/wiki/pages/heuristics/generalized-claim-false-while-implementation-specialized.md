...
PR #2070 cycle 5 では「除外ブロックが未閉鎖なら検出失敗」という契約が無限定に書かれていた。しかし実装は 2 種類のラッチに限定され、対象ファイルも index.md に限定されている。同一 PR 内の分岐表は正しく限定できていたので、限定的な側を正として他を揃えれば済んだ。
...
なお、レビュアーが挙げた「隣接する穴」を最終 cycle で全部塞ごうとしないこと。PR #2070 では 4 名が別々に同じ未カバー箇所を挙げたが、各人とも producer 不在・Hypothetical と結論し、拡張すると誤発火することも明示していた。**指摘の substance と、そこから連想される拡張は別物**である。
