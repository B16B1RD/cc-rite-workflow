...
domain: "heuristics"
description: "documentation の prose が言及する実装側 (commands/scripts/templates/.ja./.md ペア) の現状 (file 存在・key live・sentinel emit・heading slug) を grep / `ls` / `git show` で機械的に verify する step が review protocol に欠落すると、CHANGELOG / SPEC / CONFIGURATION / migration-guide の記述が実装の現状と乖離したまま merge される。過去のレビュー事例 (関連する課題、v0.5.0 リリース前 docs 見直し、cycle 14 / 51 findings) で 14 サイクル通じて発火し、cycle 5 で「documentation 内 prose が言及する実装側を grep で確認する」が明文化され、cycle 8 で「fix 履歴の遡及確認」、cycle 12 で「SPEC 全文 1 度通読」、cycle 13 で「prose 内 step 番号書き換えの implementation SoT grep verify」と運用層拡張された。過去のレビューで prose 散文括弧書きの正確性を helper 出力語彙 (`nothing removed`) と cross-check + 対称注記行の grep verify する 0 findings の successful application を追加。過去のレビューで Doc-Heavy mode 5 カテゴリ検証が SPEC.md の全実装主張 (per-session 構造 / merge.md Step 1 / 設計ドキュメント / work-memory パス / 継続ループ系列挙) を grep verify し全件一致 + pre-existing SPEC.ja.md i18n drift を revert-test で follow-up #1262 化する 0 findings successful application を追加。過去のレビュー事例 (SPEC.ja.md per-session 全面同期) で翻訳 PR での実装突合により EN 原本由来の事実誤り 2 件 (存在しない `state-read.sh` / `_resolve-flow-state-path.sh`+`STATE_FILE_PATH` 参照) の忠実翻訳転写を表面化 — 原本への盲目的信頼ではなく翻訳 PR でも実装 grep verify を行う価値を実証する successful application を追加。過去のレビュー事例 (コメント/docstring のみ +8/-0 の docs PR) で script docstring に追記した契約 claim (stdout contract / TC-D 観測性制約) の実装整合を 4 reviewer 全員が Grep/Read/テスト実行 (19/19 pass) で独立に全数検証し claim-実装乖離ゼロを確認 — 本 protocol が docstring contract 層でも 1 cycle / 0 findings 収束を支える successful application を追加。 過去のレビュー事例 (関連する課題、0 findings / 1 cycle) で本 protocol の変種を追加: 調査文書自身が主張する大規模定量値 (675箇所/55ファイルの [CONTEXT] marker 棚卸し) を、実装の現状記述としてではなく grep 再実行可能な形 (再現コマンド + 完全分割された内訳) で書いたことで、tech-writer/code-quality 両 reviewer が独立に全数値を grep 実測検証し捏造・不整合ゼロで 1 cycle mergeable に到達した successful application。過去のレビュー事例 cycle 3 (アンインストール手順ドキュメント) で「削除対象一覧」型ドキュメントへの variant を追加: tech-writer が Doc-Heavy Implementation Coverage 検証で削除対象ファイル `.rite-guidance-shown` が過去 refactor で生成箇所自体が既に除去されていることを grep で発見、検出後の是正判断は「削除」ではなく後方互換性を理由とした accept (意図的現状維持) が正しかった事例 — grep verify による乖離の可視化と、是正方法の判断 (削除 vs 意図的保持) の責務分離を実証。過去のレビューで in-source 設計コメントの構造断定への variant を追加: bash file-wide trap のヘッダコメントが関数 local tmpfile の除外根拠を「command substitution subshell 経由で親 trap から構造的に到達不能」と全関数に一般化したが、cache_comment_id は init mode の親シェル直接呼び出し 2 経路を持ち Comment Rot 化 — 2 reviewer 独立検出 (cross-validation で MEDIUM→HIGH boost)。教訓:「構造的に不可能」と断定する設計コメントは全呼び出し site の実行形態 (subshell か直接か) を grep 確認してから書く。"
promote: rite-plugin
