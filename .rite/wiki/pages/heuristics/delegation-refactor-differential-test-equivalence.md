...
domain: "heuristics"
description: "動作を verbatim 保持する委譲リファクタ (inline→helper/transform) は、原アルゴリズムを参照実装として再現し新実装と同一エッジケースで出力を byte 比較する differential equivalence test で等価性を機械的に立証する。期待値ハードコードの unit test と違い原実装の非自明な暗黙挙動 (silent drop / 末尾改行正規化等) まで保証。過去のレビューで 5 reviewer 中複数が独立に同手法を採用、0 finding 1 cycle 収束。過去のレビュー事例 (#1195 #10) で出力契約 (marker block / 3値 enum) の verbatim 保持も差分検証対象に含める次元を追加 (byte-level diff + test 34/34 + 実機 injection 検証、trust boundary 明示)、同 umbrella 2 例目の独立再現。過去のレビュー事例 (#1195 #2) で同 umbrella 3 例目を再現 (byte diff + test 37 + drift-check delta=0)、加えて差分検証では「意味論的差分ゼロ」でも抽出境界に取り残されるデッドコード (cleanup 変数の非空代入が別 fence/別プロセスに残り no-op 化 / 未使用 SCRIPT_DIR / 誤 namespace enumeration への reason 追加) を新 failure mode として追加、pre-existing バグは revert test FAIL として follow-up Issue 化する scope 規律も収録。過去のレビュー事例 (init.md inline Python→helper 委譲) で umbrella 外 4 例目を再現 (5 fixture 差分一致 + bash-heaviness 8→7 + lint 新規 warning ゼロ、2 cycle 収束)、加えて **caller-doc の挙動列挙が helper の全 return 経路を catch-all で網羅しない drift** facet を追加 (NO_RITE_HOOKS 注記の閉じたリスト列挙 vs helper の catch-all 実挙動、#1226/#1228 json_invalid doc 精度ポリシーと同系統、caller doc 単体の return-path 網羅性という直交軸)。過去のレビュー事例 (#1221 部分対応、review.md 6.1.c→helper 委譲) で 5 例目を再現 (byte diff + bash-heaviness 2→0 + drift-check 新規0、2 cycle 収束)、加えて **新規 helper が採用した hardening pattern (#1224 shift;shift) に対応する sibling 回帰テスト登録の対称性**が差分テストの盲点になる facet を追加 (helper 動作正しさと回帰テスト登録は独立軸、3 reviewer 独立検出の high-signal、mutation testing で TC-6/TC-7 dual-layer 実証、test coverage 対称性という第 3 の直交軸)。過去のレビュー事例 (関連する課題、#1193 follow-up) で 6 例目を再現 (3 cycle 収束)、加えて差分テストの**観測範囲そのものの双方向線引き**を確立: (a) 観測捕捉範囲の穴 — dump_state が commit subject (%s) のみ捕捉し body drift が素通り (mutation 実証) → wiki_body/main_body 捕捉 + verbatim assert pin で解消、(b) 契約対象外の確定 — helper 内部 validation 変数 (severity_map_json) の観測不能 MEDIUM を 3 reviewer 検証との討論で合意降格し「観測範囲は外部観測可能な挙動 (rc/stdout/stderr/file)、in-process 中間変数は契約対象外」doctrine を確立。委譲 refactor で validation gate 純増 + silent-failure hole 2 件解消、SoT universal MUST vs 未移行 caller 矛盾は漸進移行 note (Option B) で解消。"
...
### 観測捕捉は外部観測可能な挙動の全域、in-process 中間変数は契約対象外

その follow-up PR は `pr/fix.md` 1.2.0 severity_map build (~154 行) の `review-findings-maps.sh` 委譲を含む MEDIUM 5 + LOW 1 件の重量 inline bash を helper 委譲 / args_json 分離で解消した。faithful-port 委譲の **6 例目** の独立再現で、3 cycle (1→2→0 blocking) 収束。本 PR は差分テストの**観測範囲そのもの**について双方向の線引きを確立した:
