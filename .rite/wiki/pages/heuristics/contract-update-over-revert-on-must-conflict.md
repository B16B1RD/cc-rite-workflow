...
観測された事例では、分類 helper 失敗時の marker 値を `none` から `unknown` へ変えた逸脱がこの形で閉じた。前 cycle に CRITICAL を出した reviewer 自身が次 cycle で FIXED と判定している。往復を止めたのは revert ではなく契約の明文化だった。
