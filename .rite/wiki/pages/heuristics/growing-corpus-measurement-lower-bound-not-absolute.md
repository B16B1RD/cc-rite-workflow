...
domain: "heuristics"
description: "荷重を担っているのが「0 ではない」ことなのか「N である」ことなのかを見極める。前者なら下限表記にする。同一ファイル内で hedge の有無が混在していたら、それ自体が drift のサイン。PR #2070 では 3 種のマスクの実測行数が 15/1/1 と書かれたまま実体は 27/3/9 へ増加し、最も弱く見える項目を 3 分の 1 に過小表示していた。stale な絶対値は 3 ファイル 6 箇所にあり、1 箇所だけ直すと残りが次サイクルで再浮上する。"
