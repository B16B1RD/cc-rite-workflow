...
domain: "heuristics"
description: "ドキュメントやコメントに実測値を書くとき、その値が **サイクルごとに増え続ける corpus** から取られたものなら、絶対値のまま書くと確実に陳腐化する。書く前に「この文の荷重を担っているのは **『0 ではない』** ことなのか、**『N である』** ことなのか」を見極める。前者なら下限表記（「N 件以上」「少なくとも N 件」）+ スナップショットである旨の caveat にする。"
