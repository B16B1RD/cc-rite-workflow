...
domain: "heuristics"
description: "accept fixture は先行分岐を短絡させて受理判定を非 vacuous にするのが正しいが、同じ形状を reject fixture にも適用すると両分岐が同時に発火しうる形状が 1 つも無くなり、ガードの配置（precedence）を pin できない。accept は先行分岐を黙らせ、reject は両方を鳴らして precedence を観測する。PR #2035 で配置軸の mutation が 107 passed のまま生存して露見。"
