...
sources:
  - type: "reviews"
    resource: "raw/reviews/20260913T031734Z-pr-2763.md"
  - type: "reviews"
...
## 変種: 対象を列挙する文書を書き換えるときは、母集団を呼び出し元の grep で確定する

特定の経路群の欠陥を直し、同時に「宣言を持つ経路 / 持たない経路」を列挙する文書を書き換えた修正で、変更前の列挙に載っていなかった同種の経路が 1 つ残っていた。3 reviewer がそれぞれ独立に共通 helper の呼び出し元を洗い出し、同じ漏れを見つけた。修正側は列挙を「変更前の文書に載っていた経路」から出発して差分更新しており、変更前の列挙がそもそも網羅でなかったため、差分更新しても網羅にはならなかった。
