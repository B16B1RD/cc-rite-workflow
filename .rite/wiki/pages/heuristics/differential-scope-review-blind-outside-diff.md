...
domain: "heuristics"
promote: rite-plugin
description: "レビューループの cycle 2 以降を差分スコープ（前 cycle の fix diff のみを見る）にすると効率は上がるが、**範囲の外にある欠陥は「基準を満たすか」以前に観測されない**。「範囲は絞るが基準は緩めない」という規定は、範囲内の判定品質を約束するだけで、範囲外の被覆については何も言っていない。"
