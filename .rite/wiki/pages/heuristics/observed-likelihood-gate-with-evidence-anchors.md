...
tags: ["review", "severity", "likelihood-evidence", "cross-validation"]
confidence: medium
---
...
| anchor の有無 | severity | 扱い |
|--------------|---------|------|
| anchor 提示あり | CRITICAL/HIGH/MEDIUM/LOW | fix 対象 |
| anchor なし、推測のみ | — | 推奨事項（fix 対象外、discussion のみ） |

PR #540 では 2 件の finding が「Observed Likelihood Gate により推奨事項に降格」され、severity distribution は `HIGH: 0, MEDIUM: 0` に収束した。
