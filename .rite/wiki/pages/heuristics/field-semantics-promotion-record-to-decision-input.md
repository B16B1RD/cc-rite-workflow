...
domain: "heuristics"
description: "既存の記録専用フィールドを blocking 判定などの入力に格上げする変更は、(a) そのフィールドを評価する全コピーの grep 同期、(b) SoT 内に残る旧規則記述の棚卸し、(c) 判定が走る時点でフィールドが実在するかの確認、(d) 同一 prompt に同時注入される他文書との整合、の 4 点が揃わないと SoT 非同期が同時多発する。read 側と write 側を別 PR に分割するなら、read 側単独マージ時の挙動を明示的に文書化しないと分割の境界そのものが欠陥になる。"
...
「これまで記録・表示にしか使っていなかったフィールドを、これからは判定の入力にする」という変更は一見小さいが、そのフィールドを読む経路・語る文書・書く経路のすべてに影響が及ぶ。PR #2036 cycle 1 では 5 reviewer 中 3-4 名が独立に、この格上げ由来の SoT 非同期を検出した (CRITICAL 1 / HIGH 6)。
