...
domain: "heuristics"
description: "「fail-safe の reason が原因を誤帰属する」への是正として診断 WARNING を 1 行足したところ、次 cycle でその 1 行に 4 件（述語が両方向で誤る / 外部入力を無中和で出力 / || が SIGPIPE で捕捉済みの値を破棄 / 当該分岐に assert ゼロ）が集中した。診断の追加は最小の変更に見えて 3 領域を同時に開くため、足すなら述語の形・中和・rc の扱い・双方向 assert の 4 点を最初からセットで設計する。目的が切り分けなら外部入力を診断チャネルへ通さない形が最も安い。"
...
診断メッセージの追加は「1 行足すだけ」に見える。しかし外部入力を 1 つでも文面へ埋め込んだ瞬間、**その 1 行は入力検証・出力衛生・エラー伝播・テスト網羅のすべてのレビュー対象になる**。PR #2142 cycle 1 で足した WARNING 1 行に、cycle 2 で 4 件の指摘が集中し、うち 1 件は 6 reviewer 全員が独立に検出した。
...
- [PR #2142 review results (cycle 2)](../../raw/reviews/20260808T004750Z-pr-2142.md)
