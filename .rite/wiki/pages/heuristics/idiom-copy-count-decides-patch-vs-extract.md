...
review-fix loop で同じ箇所への指摘が cycle をまたいで繰り返すとき、追加パッチを当て続けるのが自然な反応になる。しかし原因が「その箇所の品質」ではなく「**その箇所が N 番目のコピーであること**」にある場合、パッチはコピー間の分岐をさらに広げるだけで終わる。PR #2120 cycle 4 で orchestrator が 5 reviewer 全員にこのシグナルを明示して投げたところ、**5 者中 4 者が「単純化すべきは本 PR のコードではなく、リポジトリ内に 4 コピーある idiom の側だ」という同一の結論を返した**。
