...
domain: "heuristics"
description: "charter cleanup / simplification refactor で過去の review cycle 経緯記述や finding ID 引用を機械的に削除する際、reasoning prose (なぜそう実装したかの根拠) を残し、review-history journal (`旧実装は X だった、本 cycle で Y を追加` 等) のみを除去するのが正しい削除単位。3 分類 (inline parenthetical / bash literal comment / prose blockquote) で削除単位が異なる (phrase / line / 文節) のは構造別に「reasoning vs journal」境界が出る場所が違うため。pre-commit baseline grep + Reviewer 並列起動 + 機能契約 test PASS の 3 layer で安全性を担保する canonical 手順。過去のレビュー事例 / 過去のレビュー事例 / 過去のレビュー事例 / 過去のレビュー事例 / 過去のレビュー事例 / 過去のレビュー事例 / 過去のレビュー事例 / 過去のレビューにおける連続 8 PR (合計 115 件削除、ファイル別密度にスケール) で 0 finding 1 cycle 着地が再現し high confidence が累積確認された。strict regex マッチ件数で削除対象を確定する運用が高信頼であることを副次的に再確認 (過去のレビュー事例)。極小規模 (+2/-2) でも同パターンが成立することが 過去のレビューで実証され、規模スケール下限が 2 件まで拡張された。references/ 配下の reference ファイルにも本パターンが適用可能であることが 過去のレビュー事例 / 過去のレビュー事例 / 過去のレビューで実証された。事前 Wiki query injection (Phase 3.0.W) が「reasoning prose 保持」原則の早期反映に寄与する実例として 過去のレビュー事例 / 過去のレビューで記録された。**行数不変の inline 置換 (`新セクションは` → 具体的セクション名)** でも reviewer が「specificity 向上」を改善方向の変更として認識することが 過去のレビューで実証され、本経験則の適用範囲は「-N 行の削除規模」だけでなく「行数不変の inline 置換」にも及ぶ。過去のレビュー事例 (Phase C1 cleanup.md slim、-70 行) で **初めて 1 cycle 着地から外れた 3 cycle 構成** を実測し、section heading / Item 番号体系を削除する slim refactor では同ファイル内の broken intra-file reference (削除済み識別子への dangling reference) と literal 構造ラベル変更時の verbatim 引用 grep 不一致が発生する限界を明らかにした。canonical 対策の拡張は pre-commit baseline grep に「**削除する識別子だけでなく変更する構造ラベルも grep の対象に含める**」を追加することで、本系列の信頼性を維持しつつ適用範囲の境界を実測した (累計 117 件 / 9 PR、8 PR 1 cycle + 1 PR 3 cycle)。過去のレビュー事例 (Phase C2 cleanup.md Phase 1/4.W 散文削除、-24 行) で **過去のレビューから学習した構造ラベル grep 拡張を実適用し、再び 0 finding 1 cycle 着地に回復**。canonical 対策の効果が実測され、累計 141 件 / 10 PR で 9 PR (1 cycle) + 1 PR (3 cycle) の sibling 比率に更新。過去のレビュー事例 (Phase D 第 1 弾、`pr/references/fact-check.md` slim、-16 行) で references/ 配下適用 4 例目 (過去のレビュー事例 / #883 / #889 に続く) を実証、`fact-check.md` は 過去のレビュー事例 inline 置換中心と本 PR 段落削除中心という異なる削除手法でも 0 finding 1 cycle 着地が再現。multi-PR 分割スコープ親 Issue (`Refs` 運用、Status を In Progress に戻すハンドリング) という新運用パターンも記録。累計 157 件 / 11 PR で 10 PR (1 cycle) + 1 PR (3 cycle) の sibling 比率に更新。過去のレビュー事例 (Phase D 第 2 弾、`pr/references/internal-consistency.md` slim、352→294 行で -58 行) で references/ 配下適用 5 例目を実証し AC-2 (4 ファイル合計 ≤1627 行) を達成 (1676→1618)。`internal-consistency.md` は 過去のレビュー事例 機械的削除中心と本 PR 冗長性整理 (前置き散文・出力例・段落構造再編) 中心という削除対象 class が異なるケースでも 0 finding 1 cycle 着地が再現。累計 158 件 / 12 PR で 11 PR (1 cycle) + 1 PR (3 cycle) の sibling 比率に更新、references/*.md への適用は 5 例連続成功で very high confidence で確立。過去のレビュー事例 (Phase B、関連する課題、累積 13 件目) で **test-pinned anchor string** 第三のサブクラスが除外要件として doctrine に追加 — cycle 1 で 1 CRITICAL test breakage を踏み抜いた後 2 cycle で 0 finding 着地、本体 commands/ への scope 拡張 1 例目として test pin 密度の高さが doctrine 拡張要因に。canonical 対策: pre-commit baseline grep に test ファイル経由 pin 検出 (`grep -q` / `assert_contains`) を追加。過去のレビューで **preventive direction sub-pattern** (新規 PR comment が self-applying lint に self-trap) を追加: 既存経験則 → lint rule → 新規 PR への self-apply → 違反検出 → fix で経験則準拠 wording に修正 という自己強化ループ。本ページの中核原則「reasoning は残し journal は削除する」は cleanup direction (remediation) だけでなく新規 PR comment writing (preventive) にも双方向適用される。canonical 対策: PR pre-commit gate で `comment-journal-check.sh` を必ず通す + reasoning prose のみを書く規律 (cycle/finding ID 引用を初回から書かない)。過去のレビューで **preventive direction 2 例目**を追加: 新規 PR comment ではなく **新設する canonical reference セクション本文自体**が journal narration (`過去のレビュー事例 review cycle 2 (error-handling reviewer, F-01)`) を含む variant。tech-writer の Doc-Heavy mode が cycle 1 で HIGH 検出、番号を削除し Why を散文で保持する同型の書き換えで cycle 2 に 0 findings 到達。SoT 化・規約明文化目的の PR ほど journal narration 混入リスクが高いという追加観察。過去のレビューで **preventive direction 3 例目**を追加: 機械判定を目的文へ書き換える仕様削減 refactor で「`#1880 で廃止`」の複合形 journal phrase が `comment-journal-check.sh` の既存 regex 未カバーのため LLM レビューのみで検出、仕様削減・簡素化目的の PR も journal narration 混入リスクを持つことを実証。"
promote: rite-plugin
