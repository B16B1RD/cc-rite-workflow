...
domain: "heuristics"
description: "判定形式を変えた理由を「旧形式は X を受け入れていた」と書くとコメントの主語が過去の行為になり、変更履歴をコードに残す禁止パターンに該当する。同じ情報は「素の X テストではこの契約を表現できない: … so they would all pass」という現在形の制約文で完全に保持でき、退行防止の価値も落ちない。削除したコードを定冠詞で指す表現は差分を見ない読み手に指示対象が存在しない。規約自身が「このサブ分類は regex で機械検出できないため reviewer の LLM 判定で扱う」と定めている場合、lint が 0 件であることは適合の証拠にならない。"
