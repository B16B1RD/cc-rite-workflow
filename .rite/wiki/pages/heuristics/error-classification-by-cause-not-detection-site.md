...
domain: "heuristics"
description: "「失敗したら空にする」型の慣用句（cmd 2>/dev/null || var=\"\"）は、呼び出し先の実行失敗と、実行は成功したが結果が期待と違うケースを同じ値へ畳む。両者の復旧手順が違う場合、畳んだ側は誤った復旧手順へ operator を誘導し、差し戻し機構を持つ設計では非収束ループになる。分類の境界は検出位置（exit code / コード上の場所）ではなく原因の類型で引く。逆に帰結（action）まで理由ごとに分けると、周辺状態との交差ごとにガードが要る — 分けるのは診断であって帰結ではない。"
