...
domain: "heuristics"
description: "注入・詐称対策の中和やガードを、攻撃が成立する literal（例: `](` の連なり）より広い範囲（全 `]`、brace 囲み形状）へ適用すると、正当な値を棄却・破壊する回帰が実データで出る。形状ヒューリスティックは enum やブランチ名のような閉じた値域にのみ適用し、free text には exact 突合・literal 限定の中和を使う。条件を攻撃面と同じ幅に絞ると攻撃は閉じたまま回帰が消える。"
