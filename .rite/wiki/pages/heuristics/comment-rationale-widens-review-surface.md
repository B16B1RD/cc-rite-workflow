...
コードの修正そのものより、その修正に添えたコメントのほうが次のレビューサイクルで指摘を生みやすい。ある PR のとある cycle では指摘 6 件のうち 4 件が、前 cycle の fix で自分が書いたコメント由来だった。**「なぜこの防御が必要か」を機構レベルで説明しようとすると、検証可能な主張が増えた分だけ指摘も増える。**
...
### 削減の効果は測れる（別の PR の 5 cycle 実測）

その PR で同じ現象が再現し、**削減の効果が cycle をまたいで定量観測された**。cycle 3 の指摘 7 件中 4 件が、cycle 2 で追加した約 20 行のコメント塊と 1 つの恒真 assert から出ていた。
