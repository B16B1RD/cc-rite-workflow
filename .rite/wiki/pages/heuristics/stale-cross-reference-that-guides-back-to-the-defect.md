...
domain: "heuristics"
description: "「ステップ 8.1 はステップ 1.1 と同じパーサ」という記述は、1.1 を helper 委譲へ変えた時点で偽になるだけでなく、それを読んだ次の編集者が 1.1 の旧形を 8.1 へ持ち込む経路を作る — つまり修正した欠陥へ戻す誘導になる。この種の記述は「古い」と判定して放置せず、なぜ現状がこうなっているかの理由まで書き足す。"
...
PR #2051 でステップ 1.1 を helper（`parse_wiki_scalar`）への委譲に変えた時点で、この記述は偽になった。だがそれだけではない。

- ステップ 1.1 の**旧形**は `awk -v k="$1" '$0 ~ "^[[:space:]]+" k ":"'` で、skill loader が本文の位置パラメータを起動引数へ展開するため壊れる形だった（これが PR #2051 が塞いだ欠陥そのもの）
...
PR #2051 では、逆方向の誤りも見つかった。「skills が inline パーサを持てないのは Skill loader が本文の位置パラメータを起動引数へ展開するため」という断定が、**同じファイルの 14 行上で自身が列挙している 2 つの安全な site と矛盾**していた（それらは bare regex 形で正常動作している）。正しい規則は「**位置パラメータを参照する** パーサを skill 本文に置けない」である。
