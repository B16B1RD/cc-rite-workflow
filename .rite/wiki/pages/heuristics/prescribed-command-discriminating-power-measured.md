...
domain: "heuristics"
description: "手順書が処方する検証コマンドが、その手順の防ごうとしている失敗モードを実際に判別できるとは限らない。gh issue view は PR 番号でも成功して PR を返すため、Issue/PR 混同の検出に title や state を使うと空振りする。判別子は url のパスセグメントだけ。"
