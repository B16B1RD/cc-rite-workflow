...
呼び出し側は `y` を評価するが、カーネルは `y<LF>` を辿る。両者が別のファイルを指すため、パスの所属で許可/拒否を決める処理は誤った側に倒れる。PR #2017 ではこれが「隔離ワークツリー内に着地したので許可」→ 実際の書き込みは親リポジトリの `.git` に着弾、という形で成立した（`origin/develop` では deny、修正版では allow という逆転を実測）。
