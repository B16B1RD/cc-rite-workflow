...
domain: "patterns"
description: "drift 防止用 anchor コメントに literal 行番号 (`(L1331-1332)` 等) を埋め込むと自身が drift 源になる。`# >>> DRIFT-CHECK ANCHOR: <canonical-semantic-name> <<<` 形式 (+ END marker) で semantic name 参照として記述するのが canonical。ingest.md Phase 5.0.c の 4 site が reference implementation。過去のレビューで大量行挿入時のコメント行番号 drift (fix コメント等) にも原則を拡張。過去のレビューで「Phase 番号 + 特徴的コード片」による semantic identifier の canonical 階層を実証。過去のレビューで ±3 行 drift brittleness を実証し bidirectional backlink sub-pattern を追加。過去のレビューで入れ子追加時の outer/inner END 順序 (well-formed nesting vs crossing 構造) と awk depth tracking 検証を追加 (HEREDOC / Markdown code fence など対称 delimiter 全般に適用)。過去のレビューで bidirectional backlink を team guideline として既存 5 ANCHOR (ingest.md 4 + lint.md 1) に一律拡張し、sibling sync 契約 prose と backlink 行の allowed redundancy 観察 (役割分離による意図的冗長性) を canonical 化。過去のレビュー事例 cycle 2 で「直前 merge PR 規約への後続 PR 違反」(24 時間以内の規約 velocity risk) を追加し、後続 PR reviewer が直近 merged PR 一覧 + commit 時系列を明示確認する canonical 化。過去のレビュー事例 review で 過去のレビュー事例 canonical format 統一 refactor の 9 site 1:1 対応・semantic 情報保全・scope 境界・wiki branch 別 commit 分離の 4 点が 0 findings で cross-validation された (極小対称化 PR 運用の追加実証)。過去のレビュー事例 (累積 11 回目) で「5 種表記 hardcoded line-number reference の scan coverage」(parenthesized form / short form / colon form / 散文形式 / capitalized form) を実測し、cleanup.md:1674 と create-interview.md:605 の 2 site 同時違反を cross-validation で検出 (REC-04 で lint pattern 拡張提案)。過去のレビュー事例 (累積 12 回目) で 6 種目 **approximation 接尾辞 `~`** (`(line ~N)` 形式) を追加。cycle 4 fix で 5 箇所新規導入された `~` 付き literal を cycle 5 で全削除し、`grep -E '\(line[s]?\s*[~]?[0-9]+'` を SoT lint 自動化に追加することを wiki 経験則として蓄積。過去のレビュー事例 (関連する課題、累積 13 回目) で Severity 全 0 の healthy landing でありながら、Same-file 3-site sync 経験則を強化する PR description 自身が他文書を `L131-138` の literal line-number で参照する meta-self-undercut を Confidence 90 R-04 finding として検出。`hardcoded-line-number-check.sh` の scan scope に PR body 領域が含まれない点を確認し、PR description のクロス参照ブロックを 6 種表記 lint の scope に拡張する REC を追加。過去のレビュー事例 (累積 14 回目) で partial symbolic anchor adoption の罠を実測 — cycle 1 で partial 適用、cycle 3 で新規追加サイトを line-number で書いて drift 再発、cycle 3 fix で全 line cite を symbolic name に置換し cycle 4 で 0 findings 収束 (4-cycle convergence pattern)。過去のレビューで reference 文書 NOTE 内の cross-file 行番号 citation の全件 stale 化を実測 — 旧 5 citation が全 drift、うち `(L569 で dual-site 備考)` は指す記述自体が消失しており citation 削除が net-positive (誤情報除去)。section 見出し名 + placeholder 名 anchor 採用後も見出しリネームで切れる残存 drift 経路を design_confirmation として明示。過去のレビューで #1187 が follow-up 分離した同ファイル sibling citation 3 件 (L268/L275/L276) を完遂 — 行番号だけでなく引用文言 (「3 sites still re-implement inline」→ 実コメント「Three sites still re-implement YAML parsing inline」) の drift も補正、Doc-Heavy Implementation Coverage 検証が anchor の実在性・一意性の独立検証として機能 (0 findings / 1 cycle)、pre-existing boundary は revert test で #1189 分離。過去のレビューで相対パス hop 数コメント (SCRIPT_DIR 基点の `../..` を「N 階層上」と数える) の file/directory 基点 1 ずれ drift を追加 (hop 数でなく基点+到達先を名前で書く)。過去のレビューで SoT ルールリストの再番号化により消費側の数値ルール引用 (「ルール N」) が dangling 化する事例を追加 — semantic 参照への置換で解消、圧縮宣言は保持例外を明記すべきという教訓も付随。"
promote: rite-plugin
