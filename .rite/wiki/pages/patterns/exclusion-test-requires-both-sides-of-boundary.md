...
sources:
  ...
  - type: "reviews"
    resource: "raw/reviews/20260917T115131Z-pr-2935.md"
  - type: "reviews"
    resource: "raw/reviews/20260917T122937Z-pr-2935.md"
  - type: "reviews"
    resource: "raw/reviews/20260917T124410Z-pr-2935.md"
  - type: "reviews"
    resource: "raw/reviews/20260917T125912Z-pr-2935.md"
  - type: "fixes"
    resource: "raw/fixes/20260917T120757Z-pr-2935.md"
  - type: "fixes"
...
行単位の正規化（本文から記録行を除外して仕様の同一性を比べる等）では、除外境界が複数ある（節の開始見出し、終端の `## ` / `---` / `</details>`、コードフェンスの開閉、除去後の空行詰め）。関数全体を恒等関数に差し替える mutant だけを同梱すると「正規化が呼ばれている」ことしか証明できず、個々の境界を弱めても緑のまま残る。実例では 16 本の単純 mutant のうち 9 本が生存し、仕様が名指しする境界そのもの（marker の行形、節の `---` 終端、見出しの完全一致、フェンス処理）が固定されていなかった。境界ごとに 1 本ずつ弱める mutant を当て、各 assertion がどの mutant を落とすかを対応付ける。

### 複合ケースは内側の区切りが遮蔽する

異種の区切りを組み合わせたケース（`~~~` で開いた中に backtick 3 本を置く等）は、名目上の規則を弱めても内側の区切りが代わりにフェンスを開くため通ってしまう。規則ごとに、内側に他の区切りを含まない単独ケースを置く。

### Markdown のフェンス判定そのものの落とし穴

フェンスを「区切り行でトグル」する最小実装は、開始区切りの文字種と長さを失い、異種・短い区切り行で誤って閉じる。行頭 4 空白の字下げコードもフェンスと誤認する。backtick 型の開始行は区切りの後に backtick を含まない行に限る（CommonMark）条件を落とすと、インラインの三連 backtick 行が未閉フェンスとして本文末まで保護し、後続の正当な行を処理対象から外す。フェンス判定は本文ごとに 1 回だけ行い、見出しの計数と除外の両方で同じ結果を使う（別々に判定すると、コード例の見出しが実際の節を判定不能にする）。
...
- [レビュー結果](../../raw/reviews/20260917T115131Z-pr-2935.md)
- [レビュー結果](../../raw/reviews/20260917T122937Z-pr-2935.md)
- [レビュー結果](../../raw/reviews/20260917T124410Z-pr-2935.md)
- [レビュー結果](../../raw/reviews/20260917T125912Z-pr-2935.md)
