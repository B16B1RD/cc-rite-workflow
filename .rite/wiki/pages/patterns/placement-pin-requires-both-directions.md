...
domain: "patterns"
description: "grep -c による出現数固定は marker の「追加」を捕まえるが「移設」は総数不変のため素通しする。範囲抽出による placement pin も片方向では変異が生存し、「この arm に 1 つ在る」と「別の arm に 1 つも無い」を併置して初めて『唯一の emit が意図した arm 内に在る』が確定する。冗長に見えるが統合できない。PR #2150 で mutation 実測。"
