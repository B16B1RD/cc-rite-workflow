...
domain: "patterns"
description: "「この marker を emit してよいのは 1 箇所だけ」「このガードを持つのは 3 ステップだけ」という**排他性**の主張を pin するとき、`grep -c` による出現数固定だけでは足りない。件数固定は marker の**追加**（増殖）を捕まえるが、同じ marker を別の分岐へ**移設**する変異は総数が変わらないため素通しする。"
