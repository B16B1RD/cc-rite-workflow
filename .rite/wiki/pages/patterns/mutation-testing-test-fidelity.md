...
domain: "patterns"
description: "test が「正しい input で PASS する」だけでは不十分で、「実装を mutate (sed で改変 / 削除) すると確実に FAIL する」ことを empirical に確認することで初めて regression detection power が保証される。Mutation testing は (a) test が dead code (実用上到達しない code path) を validate していないか、(b) test の assert が identification power をもつか (fixture old value と patch new value が同値で revert test 不能になっていないか) の 2 観点を decisively 検出する。過去のレビュー事例 cycles 3 → 4 → 5 → 31 で 4 連続適用実績 (TC-10 / TC-13 / TC-AC-4-WRITER-FALLBACK の dead code + identification power 0 検出)。過去のレビュー事例 cycle 42 で 5 件目「bash command substitution の trailing newline strip 仕様による false-positive (`$(cmd)` で newline 規約が consume されて assertion bypass)」を追加し、bytes-exact pin (`wc -c`) を canonical として規約化。過去のレビュー事例 review で「review test の identification power 不足 3 種 dead code 化 pattern」を実測追加: (a) Self-grep tautology (test 自身が echo した文字列を自身で grep)、(b) 件数判定の片側 mutation 隠蔽 (`grep -c >= 2` で 3 site 中 1 site mutate でも PASS)、(c) Path filter coverage gap (filter glob を空文字に置換しても全 TC PASS)。新規 lint rule / hook 追加 PR の review-fix loop で実装本体を空 / `exit 0` に置き換えた mutation を必須実行する canonical gate を確立。過去のレビュー事例 review で 6 件目「load-bearing whitespace を持つ fixture を formatter から守る pre-check」を追加: fixture 永続化 PR で fixture 内の trailing whitespace / indent 等が mutation 識別力の唯一の load-bearing 要素になっている場合、meta-test 冒頭で `grep -qE '<load_bearing_pattern>' "$FIXTURE" || fail` を mandatory 化し formatter による silent strip を防御する canonical 構造を確立。過去のレビューで 7 件目「**Regex alternation / quantifier semantic の per-branch positive coverage**」を追加 (2 種 cross-validated): (a) alternation `A|B` の `B` branch positive case 欠落で B branch が dead range、(b) quantifier `\s*` (0 回以上) の境界値 (0 回 / 1 回両方) を positive で pin しないと `\s*` → `\s+` mutation を decisive 検出できない。canonical 対策: 全 alternative 独立 positive + quantifier 境界値 positive + flag (`-i`) 必須 case + alternation 境界 negative + regex 専用 mutator (alternation 各 branch / quantifier 相互置換)。過去のレビュー事例 cycle 2 で 8 件目「新規 lint helper の regression test が implementation を mutate しても PASS する」を追加: sh-cross-ref-check.sh の strip_code_fences を cat に mutate しても test 11/11 PASS する identification power 0 を reviewer が mutation 観点で検出 (non-blocking follow-up)。新規 helper の中核ロジックを no-op に mutate して該当 TC が FAIL することを CI 確認する canonical を再確認。過去のレビューで bug fix が correctness invariant (consume の fail-closed ordering) を導入したら同 PR で mutation-fail test (TC-H6) を添える義務 + bash positional 引数誤配置の silent false-positive を mutation でのみ検出できる事例を追加。過去のレビューで 10 件目「**対称化 claim は positive / negative 両側を独立 assert で pin し双方向 mutation で検証する**」を追加 (#1169 と同じ consume-handoff 領域の follow-up): corrupt 側発火だけ pin した片側 assert は無条件 WARNING emit mutant が全 103 assert を pass する coverage gap を生むため、happy-path 非発火を pin する negative-assert を対に追加し、無条件 emit → negative-assert fail / emit 削除 → positive-assert fail の双方向 mutation で両側の identification power を実証。実装の対称化を謳ったら test も両側対称に置く (asymmetric-fix-transcription の test 側双対)。過去のレビューで 11 件目「閾値の off-by-one 境界は測定対象シグナルを単独の結果決定要因にして pin し mutation で実証する」を追加 (long-block 25 行境界を python-inline 第 2 シグナル固定で isolation、24/25 行両側 + `long-block(25)` ラベル pin、`>=25→>=26` / `25→24` 両 mutation で捕捉確認)。過去のレビューで 12 件目「非ブロッキング契約 (exit 0 always) の回帰防止に実装末尾 exit 0 削除 mutation で S-7 FAIL を立証」を追加。過去のレビューで 13 件目「reviewer 側が READ-ONLY 制約下の detached worktree mutation test で新規 self-test の behavioral 性を立証 (reason 語彙改変 → 3 fail / exit code 改変 → 2 fail)、契約照合 verbatim 照合と併用して 0 findings / 1 cycle mergeable」を successful application として追加。過去のレビューで 14 件目「mutation 検証は latent な remediation guidance 矛盾の発見にも機能する (TC-6 remediation 指示に従うと TC-1 が fail する SoT guidance 矛盾を mutation 検証の過程で発見、worktree-only mutation 6 種で TC-6 検出力を実証、0 findings / 1 cycle mergeable)」を追加。過去のレビューで 15 件目「コメント訂正のみの fix でも mutation で test の弁別力を再確認する (fix 行削除 → TC-16 の 3 assert FAIL / --c0-only → default 改変 → 日本語保持 pin FAIL の 2 種で、訂正コメントが主張する設計判断を test が pin していることを実証。前 cycle nit も同実測で自然消滅)」を追加。過去のレビューで適用 16「fix 側 mutation claim の reviewer 独立再実証 (fix 側は commit 前の核心行削除 mutation で runtime 実証、reviewer 側は独立設計の 4 種 mutation で再実証、観測数差異は非 vacuous 核心一致で合意)」を追加。過去のレビューで適用 17「vacuous 教訓 (適用 16) を設計段階から反映したテスト追加が 0 findings / 1 cycle 収束を達成 — 両方向 assert (placeholder 文言あり + 通常 reason なし) / fake binary 引数 matcher の精密スコープ (neutralize_ctrl の `\000-` レンジ引数のみ exit 1、他用途は real tr へ委譲で巻き添えゼロ) / negative assertion 単独は空出力 mutation で trivially pass するため positive ペア必須、の 3 点セットを設計時 checklist として canonical 化」を successful preventive application として追加。過去のレビューで適用 18「テストのみ変更 PR の新規 assert 4 件を reviewer 側 worktree-only mutation で非 vacuous 立証 (absent regex broaden / cleanup 除去 / sentinel swallow の各 mutation で対応 TC FAIL) + 検査方式の置換 (count delta → path 集合差分) を双方向 mutation で上位互換検証 + jq fault-injection shim の `-s` 引数完全一致 + scenario gate 二重ガード (PATH-shim 引数分岐技法の jq 版、連続 3 例目)」を追加 — 0 findings / 1 cycle mergeable。過去のレビューで適用 19「既存 TC を byte-identical に一般化した新 helper assert_single_create_caller の (a)(b)(c) 3 assertion を 3 reviewer が worktree-isolated mutation (nested cmdsub 退行 + flag-style --title 退行) で teeth 立証、加えて『既存 TC との対称性 byte 比較』を mutation と対の review 検証手段に併設」を追加 (symmetry test 保護対象拡張のテストのみ変更 PR、0 findings / 全 4 レビュアー可で初回 mergeable)。過去のレビューで適用 20「inline-gh-create-title の開始引用符 alternation `["']` の single-quote branch を pin する TC-026/027/028 を test/error-handling reviewer が独立 mutation (`["']`→`["]` で TC-026 のみ FAIL / `$` 除外撤去で TC-018+TC-027 FAIL) で非 vacuous 立証」を追加 (適用 7 regex alternation per-branch coverage と適用 13/18/19 reviewer 側 worktree mutation が合流、Asymmetric Fix Transcription の test 側監査も適用、0 findings / 1 cycle mergeable)。過去のレビューで適用 21「能動 mutation を READ-ONLY guard で回せない reviewer が negative assertion (`assert_err_lacks`) の非 vacuity を match/mismatch 差分ペア構造 + all-zeros SHA 決定性で構造的に立証、reason 文字列の実装 emit byte 一致照合を error-path behavioral test 正しさの核心と確認」を追加 (4 reviewer 可 / 0 findings / 1 cycle mergeable)。過去のレビューで適用 22「dedup assertion の非 vacuity を重複 fixture (p2 に p1 と同一の正規化済み ref) で担保し `sort -u`→`cat` mutation で count=2 FAIL を立証 + section-scoped 静的契約 test (TC-14) の helper rename を empty-section fail で検出する rename-detection 機構を io_error→true / rename 2 mutation で実証」を追加 (test/code-quality 2 reviewer 可 / 0 findings / 1 cycle mergeable)。過去のレビューで適用 23「sweep 条件の構造的盲点 (>&2 同一行条件が代入行 idiom を検出できない) を author 側 mutation で実証して fail-closed 全行 sweep + allowlist へ pivot、4 reviewer が独立 worktree-only mutation で TC-3/TC-4/TC-5/TC-025 の非 vacuity を再実証」を追加 (0 findings / 2 cycle mergeable)。過去のレビューで適用 24「回帰防止テストの grep token は修正パス固有にする — 旧コードの remediation hint 文言にもマッチする generic token は修正前でも PASS する non-discriminating 構造。修正パス固有 literal への絞り込みと修正前 base revert test での discrimination 実証を canonical 対策化 (anti-silent-failure 化修正で旧経路 hint と新経路 WARNING が同ファイル併存する場合に特に陥りやすい)」を追加。過去のレビューで適用 26「複数ガードが論理和を構成するとき『特定ガードを exercise する』テストコメントは vacuous — 『行実行順序 (unmutated でどのガードが先に発火するか)』と『mutation-isolated coverage』を混同すると over-claim になる。CAS 二段ガード (mismatch→10 / revive→10) の並行奪取テスト TC-14 は全 contender live のため 2 ガードの論理和のみ検証し単独 isolate しない (片ガード除去で緑・両除去で赤を mutation 実測)。CLI で決定的再現不能な二次防御 (revive 分岐) は『未到達』と正直に文書化し、コメント軟化 (実行コード不変) も valid な fix。併発 sub-pattern: platform skip ガードの左右対称 / 対称化 AC の test pin 強度非対称 (汎用 exit≠0 は vacuous) / SPEC の条件付き表現による over-claim 回避」を追加 (4 cycle 収束)。過去のレビューで「2 段構え fallback の fallback 側成功パスが未カバーのまま残る」を mutation (fallback を false 置換 → 全緑) で実測し tier 別 pin の教訓を追加 (適用 27)。過去のレビューで適用 31「degrade 経路テストの discriminator は『degrade が働いて初めて成立する肯定結果』で設計し、fix commit 前に mutation で識別力を検証する (同一 PR 内で識別力あり/なしのテストが分岐した実測 + strictly additive availability framing)」を追加。過去のレビューで case 分岐が先に捕捉するため allowlist 本体に assertion が届かず「N pass / 0 fail」が防御の証拠にならない実例、および保証を実測より広く書かない規範を追加。"
promote: rite-plugin
