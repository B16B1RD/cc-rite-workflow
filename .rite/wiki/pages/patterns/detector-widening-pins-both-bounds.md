...
domain: "patterns"
description: "検出器の追跡対象を広げる修正で positive fixture だけを足すと、拡張の上限が守られず次 cycle で誤検知として返ってくる。PR #2124 では第 3 のハンドル綴りを足す際に T-03h（guard 綴りを追跡する）と T-04h（guard 以外の $(bash ...) は追跡しない）を対で置き、拡張の下限と上限の両方を pin した。あわせて件数ではなく個々のハンドルを assert し、全域走査で誤検知が増えていないことを確認する 4 段の検証順序が有効だった。"
