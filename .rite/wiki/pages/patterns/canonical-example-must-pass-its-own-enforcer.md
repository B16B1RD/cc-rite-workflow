...
PR #2036 cycle 4 で **4 reviewer が独立に検出**した欠陥クラス。canonical JSON 例に追加した id `F-02b` が、同じファイルが定める書式規則 `^F-[0-9]{2,}$` に違反していた。write 側はこの例を「真実の源」として写すため、そのまま従うと永続 JSON が 1 件も保存されず、既定構成では該当ステップが `exit 2` で hard fail する。
