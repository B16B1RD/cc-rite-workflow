...
PR #2013 では本体テストを 2 段階に直した一方、CONTRIBUTING.md の「新規テストひな形」は 1 段目のみのまま残り、**コメントだけが 2 段構成を説明する**矛盾状態になった。ひな形は複製起点なので、直したばかりのバグを次の contributor が再導入する経路が開いたままになる。3 レビュアーが独立に同一箇所へ収束した。
