...
sources:
  ...
  - type: "reviews"
    resource: "raw/reviews/20260926T125425Z-pr-3147.md"
  - type: "fixes"
    resource: "raw/fixes/20260926T130048Z-pr-3147.md"
tags: []
confidence: high
...
### 構文解析の内部目印も、入力から生じ得ない値で持つ

同じ原理は、文書の同定だけでなく構文解析の内部目印にも当てはまる。シェルコマンドを語に分けて関数定義やグループの位置を判定する処理で、グループの位置を普通の語 `()` で示すと、引用した literal `'()'` やプロセス置換 `<( … )` も同じ語になる。その結果、判定が誤って発火し、commit の検出や `cd` の追跡から外れる形が生まれる。

目印を普通の文字列で表す限り、同じ文字列を作る別の構文と衝突する。形ごとに規則を絞って塞いでも、次の衝突が別の形で見つかる。原因を取り除くには、目印を入力の語に現れない値（NUL を含む値など）にする。置換のように別の意味を持つ構文は、同じ文字（`(` など）の処理でまとめて扱わず、入口で分けて別経路にする（コマンド置換と同じく、中身を入れ子として取り出し字面を語に残す）。
...
- [レビュー結果](../../raw/reviews/20260926T125425Z-pr-3147.md)
- [fix 結果](../../raw/fixes/20260926T130048Z-pr-3147.md)
