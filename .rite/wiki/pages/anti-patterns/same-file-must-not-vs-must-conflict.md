...
ある PR では「実行時観測を根拠として書ける指摘は実測アンカーも必ず併記する」という既存 Rule が無条件だった。そこへ「アンカーを付けない字面整合クラス」を新設したため、両者が正面衝突した。**両規約は同一の reviewer prompt へ同時に注入される**ので、reviewer はどちらかを破るしかない状態になる。
