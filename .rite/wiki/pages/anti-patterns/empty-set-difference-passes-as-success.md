...
description: "`comm -23 <(emit) <(表)` の出力が空なら合格、という DoD 検証は、emit 側の抽出が 0 件でも空になるため「全部文書化されている」と「1 件も抽出できなかった」を同じ成功として返す。抽出が prose markdown への正規表現マッチなら、記法が変わった時点で検証は無言で no-op 化する。入力集合の非空を独立に検査する。"
