...
description: "「本文検査 4 段」のような集合を reason=body_* という glob で指すと、命名規則に合わない既存要素を最初から取りこぼし、後から増えた別集合の要素を誤って拾う。命名規則と集合の境界は独立に変化するため、glob は集合の定義として機能しない。診断や分岐の判定述語には明示列挙を使い、対象外も併記する。"
