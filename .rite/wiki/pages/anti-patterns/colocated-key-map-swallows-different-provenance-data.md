...
PR #2036 cycle 2 で HIGH として検出。自動レビューの指摘を「実測なし = 非ブロッキング」に分類する map を `file:line` で引く設計にしたため、**同一 file:line にある人間レビュアーの未解決 thread** が同じ分類に落ち、fix も reply もされないまま「対応済み」として件数に計上されていた。
