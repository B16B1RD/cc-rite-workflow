...
description: "前提を falsify したときは、その前提を述べている箇所を言語をまたいで探す必要がある。実測必須ゲートを helper へ機械化した作業では、「JSON 配線は後続スコープ」という注記を日本語で全消ししたつもりが、同じ主張を英語で持つ docs/SPEC.md が grep から漏れた。英日ペアの文書を持つリポジトリでは、片方の言語だけで掃き出しを完了と判定すると陳腐化した主張が上位文書に残る。"
