...
description: "GNU/BSD で挙動が割れるコマンドを別コマンドに置き換えるとき、解決セマンティクスだけを比較すると、置換先が持つ別の分岐（出力の区切り文字・終了コード規約など）を持ち込む。分岐が消えたのではなく観測しにくい層へ移動しただけで、対象プラットフォームの CI が唯一の検出手段になる。"
