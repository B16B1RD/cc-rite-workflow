...
description: "判断（宣言先を A に絞る）に理由（B は届かないから）を添えると読み手には親切だが、その理由が追跡中の欠陥に依存していると、欠陥修正の瞬間に規範側が偽になる。検出する機構もない。判断だけ残して理由の断定は削るほうが壊れにくい。起点事例では断定が同一ファイル内の現行 MUST とも正面から矛盾していた。"
