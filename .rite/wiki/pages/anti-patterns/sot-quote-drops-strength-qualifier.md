...
sources:
  ...
  - type: "reviews"
    resource: "raw/reviews/20260926T145932Z-pr-3181.md"
tags: ["sot", "drift", "documentation", "dry"]
confidence: high
generated: { by: "rite-wiki-ingest/claude-opus-5-5", at: "2026-09-26T15:10:00Z" }
verified:
  - { by: "rite-wiki-ingest/claude-opus-5-5", at: "2026-09-26T15:10:00Z" }
---
...
起点事例はこれを 2 cycle 連続で踏んだ。cycle 1 は SoT の 4 経路のうち 2 経路だけを列挙し、その 2 経路がどちらも結論（「draft PR の人間レビューに委ねる」）を支えない状態だった。cycle 1 の指摘を直す過程で補った括弧書きが、今度は SoT が best-effort と明記している経路を「draft PR 上に残る」と無条件に言い切り、cycle 2 の指摘になった。
...
起点事例の該当 bullet は冒頭で「blocking の定義式は本ファイルに複製せず SoT とする」と宣言していた。式は実際に複製していないが、同じ文の後段で SoT が持つ**別の**事実（記録先チャネル一覧）を部分複製していた。宣言した drift 回避方針と非対称な複製が同居している状態で、レビュアーはこれを「自己矛盾かつ新規 drift site」として指摘した。
...
### 記述を「正確にする」修正でも条件の枝を落とす

テストのヘッダが「固定していない挙動を固定済みと読ませる」誤りを直すため、記述を assert の実範囲へ狭めた修正で同じ形が起きた。元の記述は削除条件を「fresh run **または** counter 0 の run」と正しく書いていたが、書き換えた括弧書きは固定状況を「fresh run の削除（実行済み）/ resume の保持（未固定）」の 2 分で書き、counter 0 の resume でも削除される枝が消えた。誤りを 1 つ取り除く書き換えが、元の文にあった正しい限定句を落として別の不正確さを入れている。

- 記述を狭める・言い換えるときは、狭めた後の文が実装の分岐条件（`fresh || counter == 0` のような論理和）の**全ての枝**を写しているかを実装と突き合わせる
- 「どこで固定済みか / 未固定か」を書き分けるときは、条件の枝ごとに書き分ける（枝をまとめた 2 分法にしない）
...
- [記述を狭める修正が削除条件の枝を落としたことを示したレビュー結果](../../raw/reviews/20260926T145932Z-pr-3181.md)
