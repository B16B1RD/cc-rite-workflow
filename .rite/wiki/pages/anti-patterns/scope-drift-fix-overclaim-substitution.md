...
sources:
  ...
  - type: "reviews"
    resource: "raw/reviews/20260913T081122Z-pr-2775.md"
tags: []
confidence: high
generated: { by: "rite-wiki-ingest/claude-opus-5", at: "2026-09-13T08:20:00Z" }
verified:
  - { by: "rite-wiki-ingest/claude-opus-5", at: "2026-09-13T08:20:00Z" }
---
...
## 変種: 狭すぎた条件を直す括弧書きが、括弧だけ読むと過大に読める

限定しすぎた発生条件（「最初の chunk で終了すると」）を外す修正で、本体を「writer の write がまだ残っているうちに終端行で終了すると」と正しく書き直したうえで、括弧書き「（終端行が何番目の chunk にあっても起きる）」を足した。文全体は実挙動と一致する — 再現では終端行が最初・中間の write にあれば writer は rc=141、最後の write にあれば rc=0 だった。ところが括弧だけを抜き出すと本体の条件が落ち、「最後の chunk でも必ず起きる」と読める。修正前の誤読（「後ろなら安全」）と逆向きの過大主張が、補足の側に入り込んだ形である。

> **教訓**: 本体の条件を補う括弧書きは、単独で引用されても成り立つように**条件を括弧内にも書く**（「（終端行の後に write が残っていれば、何番目の chunk にあっても起きる）」）。散文の技術的主張を直すときは、境界の両側（起きる側と起きない側）を再現で確かめてから文言を決めると、修正方向とは逆側の過大主張も検出できる。
...
- [発生条件を直した括弧書きが単独で過大に読めることを再現で確かめたレビュー結果](../../raw/reviews/20260913T081122Z-pr-2775.md)
