...
description: "「1 件も認識できない（parsed == 0 / entries == 0）」を条件にした検出失敗ガードは、1 件でも数えたあとに壊れる部分欠損では entries>=1 となり構造的に発火できない。加えて配布テンプレート自身の前文が entry 述語に一致する場合、全滅条件は恒久的に発火不能になる。ガードの条件を選ぶときは部分失敗の形を先に作ってから決め、可能なら失敗そのものを表す変数を判定に使う。PR #2070 の cycle 1→3 で同一ガードが 2 度作り直された。"
