...
sources:
  ...
  - type: "reviews"
    resource: "raw/reviews/20260914T163412Z-pr-2824.md"
tags: ["docs-drift", "precision-regression", "review-fix-loop", "regex-verification"]
confidence: high
generated: { by: "rite-wiki-ingest/claude-opus-5[1m]", at: "2026-09-14T16:45:05Z" }
verified:
  - { by: "rite-wiki-ingest/claude-opus-5[1m]", at: "2026-09-14T16:45:05Z" }
---
...
### 追記: 識別子の形を限定するときは、自分のツールが生成しうる名前の変種を列挙する

照合 key を「ファイル名 + 連番」に強め、入口でその形を正規表現で検査する修正では、同じツールの保存側が同秒衝突を避けるために付ける接尾辞（`~` と 4 桁の 16 進）を形から漏らしやすい。和集合を作る glob は接尾辞付きファイルも拾うため、そこに載った指摘だけ key が作れず、解消済みでも除外できない（過剰転記）。修正前の緩い照合では除外できていた部分集合で、新しい欠陥が生まれる。

形を限定する正規表現を書く前に、その名前を**生成する側**の実装を Read し、衝突回避・版数・一時ファイルなどで生じる全変種を列挙する。消費側の既存 fixture は正規形しか持たないことが多く、テストが緑でも変種の取りこぼしは検出されない。変種ごとの fixture を足してから形を確定する。2 人の reviewer が独立に同じ原因へ到達したことは、この取りこぼしが読み手にとって見つけやすく、書き手にとって見落としやすい種類であることを示している。
...
- [照合 key の形が同秒衝突の接尾辞付きファイル名を受け付けないことを指摘したレビュー結果](../../raw/reviews/20260914T163412Z-pr-2824.md)
