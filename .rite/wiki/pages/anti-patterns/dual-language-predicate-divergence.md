...
description: "read 側と write 側で同じ判定条件を別言語（jq と shell 等）で実装すると、正規表現エンジンの空白クラスや文字除去の範囲が違うため受理集合が locale / 実装依存で割れる。「両側は同一述語」とコメントに書いても対称性は保証されず、述語を差し替えるたびに新しい抜け道が出る。PR #2038 は 4 cycle にわたり述語を強化し続けたが収束せず、定義を 1 本に切り出して両側が同じものを評価する形にして初めてこのクラスが閉じた。"
