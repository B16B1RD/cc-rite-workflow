...
つまり単純コマンド形は「原因が最も要る側 (ENOSPC) だけを捨て、要らない側 (EACCES) を漏らす」という最悪の分配になる。**「先例と同型」を名乗る前に、先例の redirect スコープまで読む** — この事例では先例として引いた形がグループスコープで、挙動が逆だった。
