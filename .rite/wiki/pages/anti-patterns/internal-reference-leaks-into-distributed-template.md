...
起点事例（PR #2052）では `templates/wiki/*.md` が対象だった。これらは `/rite:wiki-init` が placeholder 置換または単純 `cp` でユーザープロジェクトへ展開する。開発中は同じリポジトリのファイルとして見えるため、隣接ドキュメントと同じ感覚で内部参照を書いてしまう。
