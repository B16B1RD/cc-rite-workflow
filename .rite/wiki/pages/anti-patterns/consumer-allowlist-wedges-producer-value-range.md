...
description: "入力を狭める防御を消費側にだけ追加すると、生成側が正当に作れる値まで拒否する。拒否の帰結が「後始末をしない」であり、それを別の gate が「残っている = 未実行」と読む構成だと、処理が成功しているのに毎サイクル失敗と判定される非収束ループになる。防御を足す前に (a) 生成側の値域、(b) 拒否時の帰結が既存 gate とどう合成されるか、の 2 点を確認する。"
