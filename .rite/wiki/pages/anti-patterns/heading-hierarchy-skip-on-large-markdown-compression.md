...
**canonical 防御 (累積)**:
- (a) 大規模圧縮 PR の AC に「heading hierarchy skip 0 件」を必ず含める
- (b) sibling refactor PR では、refactor scope 内で **既存の (本 PR が新規導入したのではない) skip も** 検出・修正対象に含める方針を Self-Review で明示する
