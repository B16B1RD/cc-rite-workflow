...
PR #2036 で 2 サイクル連続して踏んだ fail-unsafe。「記録の欠落を検出する gate」を追加したところ、記録が欠落しているという理由で**保存そのものが中止**され、同じファイルに載っていた blocking findings（primary データ）まで失われた。既存テスト 5 件の失敗で発覚した。
