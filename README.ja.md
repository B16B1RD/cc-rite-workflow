...
- **スマートレビュー**: 動的なマルチレビュアーコードレビュー（**Doc-Heavy PR Mode** — ドキュメント中心 PR を自動検出し、実装ソースと文書記述をクロスチェックする tech-writer 強化プロトコル `Mandatory Implementation Cross-Reference` / `Screenshot Completeness Check` / `Order / Emphasis Consistency` を起動）
