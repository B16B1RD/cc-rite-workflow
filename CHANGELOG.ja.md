...
- **internal-consistency.md reference 新設** — `fact-check.md` (外部仕様) と対の内部事実検証プロトコル。5 項目の Verification Protocol、Confidence 80+ ゲート、severity マッピング、Cross-Reference を定義 (#349)
- **Doc-Heavy PR Detection (Phase 1.2.7)** — ドキュメント中心 PR を自動判定 (判定式: `(doc_files_ratio >= 0.6)` または `(doc_files_count_ratio >= 0.7 かつ total_diff_lines < 2000)`)。rite plugin 自身の `commands/`, `skills/`, `agents/` 配下の `.md` は除外 (prompt-engineer 専管)。`rite-config.yml` に optional schema `review.doc_heavy.*` を追加 (#349)
- **Doc-Heavy Reviewer Override (Phase 2.2.1)** — `{doc_heavy_pr == true}` のとき tech-writer を recommended → mandatory に昇格、code-quality を co-reviewer 追加。tech-writer に `{doc_heavy_pr=true}` フラグを伝達し、"Doc-Heavy PR Mode (Conditional)" セクションの強化プロトコルを有効化: `Mandatory Implementation Cross-Reference`, `Screenshot Completeness Check`, `Order / Emphasis Consistency` (#349)
- **`/rite:pr:fix` に PR URL / comment URL 直渡しサポート** — `fix.md` Arguments table を 3 形式に拡張 (pr_number / pr_url / comment_url)。Phase 1.1.1 Argument Parsing で正規表現判定、Phase 1.2 に target_comment_id 分岐を追加し、`/verified-review` など外部レビューツールのコメント URL から直接 findings をパースして fix ループに投入可能に (#349)
