...
- **tech-writer Critical Checklist 具体化** — 文書-実装整合性 5 項目を追加 (Implementation Coverage / Enumeration Completeness / UX Flow Accuracy / Order/Emphasis Consistency / Screenshot Presence)。各項目に Grep/Read/Glob での検証手段を併記し、blocks-documentation PR #1137 由来の Finding Examples 3 件を追加 (#349)
...
- **Doc-Heavy PR Detection (Phase 1.2.7)** — ドキュメント中心 PR を自動判定 (`doc_files_ratio >= 0.6` または `doc_files_count_ratio >= 0.7` かつ `total_diff_lines < 2000`)。rite plugin 自身の `commands/skills/agents` 配下は除外 (prompt-engineer 専管)。`rite-config.yml` に optional schema `review.doc_heavy.*` を追加 (#349)
- **Doc-Heavy Reviewer Override (Phase 2.2.1)** — `{doc_heavy_pr == true}` のとき tech-writer を recommended → mandatory に昇格、code-quality を co-reviewer 追加。tech-writer に `{doc_heavy_pr=true}` フラグを伝達し、"Doc-Heavy PR Mode (Conditional)" セクションの強化プロトコル (Mandatory Implementation Cross-Reference, Screenshot Completeness Check) を有効化 (#349)
