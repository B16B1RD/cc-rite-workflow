...
- **`/rite:pr:fix` に PR URL / comment URL 直渡しサポート** — `fix.md` Arguments table を 3 形式に拡張 (pr_number / pr_url / comment_url)。Phase 1.0 Argument Parsing で正規表現判定、Phase 1.2 に target_comment_id 分岐を追加し、`/verified-review` など外部レビューツールのコメント URL から直接 findings をパースして fix ループに投入可能に (#349)
