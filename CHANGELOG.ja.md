...
- **実装または修正のコミットは、Wiki の検索・本文の確認・適用判断の証跡が今の作業と一致するときだけ通る** — 作業メモリには、検索したこと、ページ本文から取った抜粋、適用するか対象外かとその理由、変えたパス、検証結果を残す。証跡が無い、HEAD やファイル内容より古い、抜粋が本文に無い、検索または索引の読込に失敗したときはコミットしない。Claude、Codex、Grok は、実装と修正のコミット前に同じ確認を行う。
- **rite が作るコミットメッセージは、コミット対象の作業ツリーにある `CLAUDE.md` と `AGENTS.md` に従う** — メッセージを生成する直前に、その作業ツリーで両ファイルを探して読み、指定された言語と形式を適用する。指定の無い項目だけ既定で補う。件名や本文に収まらない必須記録は、プルリクエスト本文か共有ルートの `commit-records` に残す。共有ルートへ倒すのは、Wiki が別ブランチにあり規約エントリが無いときだけである。切れたパスは無視せず失敗する。

### 修正

- **空のコミットメッセージ指定と空のメッセージファイル指定は、解析の時点で拒否する** — 空文字は未指定として扱わない。変更が無いのに成功したことにはせず、空のまま既定の件名でコミットもしない。
- **短いオプションは、通常のコミット指定を誤って拒否しない** — メッセージを付ける普通の指定を、確認した内容と違うコミットとして止めない。
- **セッション終了は、どの hasher も digest を作れないとき失敗する** — 空の結果同士を一致とみなさない。すべて空なら失敗を出して止まる。
