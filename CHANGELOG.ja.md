...
- **`/rite:setup` が環境セットアップを一気通貫でカバーするようになった** — 依存検査（`bash` 4 以上 / `jq` / `flock`）と環境別インストール案内 (#2005)、Phase 1.3 の GitHub 認証フロー (#2236)、Phase 1.4 のリポジトリ作成フロー (#2237)、GitHub テンプレート（`ISSUE_TEMPLATE` / `PULL_REQUEST_TEMPLATE`）の自動生成 (#2238)。`getting-started` は前提条件の記述を重複させず `setup` へ委譲する。(#2239)
...
- **レビュー結果スキーマへ `findings[].verification` を追加** — additive 追加であり、read 側に型ガードを入れて旧形式の結果 JSON も読めるようにした (#2035)。merge ゲートが必須要求していながらスキーマ SoT に定義が無かった `verdict` / `reviewers` を正典 writer が書く正式フィールドとし (#2229)、canonical write version を 1.1.0 へ同期した (#2211)。
...
- **ホスト共通の開発ランチャー `scripts/rite-dev`** — 専用テストスイートと CONTRIBUTING の記載を伴う。(#2178)
...
- **繰り返し現れる内部機構をテスト付き helper へ集約** — `wiki-ingest` ステップ 6 の index 操作 (#2111)、`iterate` 経路の marker emit・照合（共有関数 + fixture テスト化）(#2137)、`[CONTEXT]` marker の値側照合規約の所有者付け (#2151)、`.gitignore` 同梱処理 (#2219)。完了済みの Wiki 一時 backfill helper は削除した (#2215)。
- **ドキュメントを実装へ再同期** — CONTRIBUTING の guard 判定契約 (#2208)、`max_review_cycles` の既定値記載 (#2209)、config テンプレートの `review.loop` コメント (#2210)、schema invariant 6 と gate 配線 (#2212)、test coverage gate と判断点の接続 (#2213)、SPEC の active guard / sentinel ファイル列挙 (#2214)、実測必須ゲート導入後に残った未修飾の「0 findings」記述 (#2043, #2106)、gate 配線で前提が変わった `fix` / SPEC AC-3 の括弧書き (#2102, #2104)、`wiki-ingest` ステップ 6 の index 更新指示を実体の 5 列テーブル形式へ是正 (#2052)、SPEC の Plugin Structure 節における reference 列挙の記載漏れ 2 件 (#2108)。
...
- **hooks の sandbox / macOS・BSD 移植性** — Step 4-P の `TMPDIR` 判定を symlink の物理パスで解決 (#2172)、`mktemp` テンプレートの `X` 連を末尾へ移して BSD/macOS の tempfile 名一意性を回復 (#2080)、tempfile ライフサイクルを lib 化し残る 2 種に lint 検出器を追加 (#2124)、`pr-cycle-cleanup` が `TMPDIR` 配下の detached mutation worktree を回収 (#2156)、stale な issue claim の奪取を macOS 対応 (#2201)、`flow-state` の `_atomic_write` の `flock` 呼び出しに `command -v` ガードを追加 (#2003)、診断中和の parity sweep を `scripts/` へ拡張 (#2206)。
...
- **hooks の silent failure を loud 化** — pending marker の削除失敗を握り潰さず報告し (#2205)、作業メモリの checklist 統合が no-op success を返さないようにし (#2152)、Step 4-P の dirty 保護を実リスクに合った到達不能 commit 保護へ置換した (#2160)。
...
- **batch / 対話の停止経路が、ブレーカー起因の失敗停止を通常の中断と区別するようになった** — 停止した run が完了として誤報告されなくなる。(#2220)
