...
- **`[fix:pushed-wm-stale]` 出力パターン** — `/rite:pr:fix` が Phase 4.5 work memory 更新で silent failure (`current_body` 空 / issue_number 抽出失敗 / `git diff` 失敗 / PATCH 失敗) を検出した場合に新規出力する。caller (`/rite:issue:start` review-fix loop) は **silent に `[fix:pushed]` 扱いしてはならず**、必ず `AskUserQuestion` で警告を提示してユーザーに「stale work memory のまま継続するか、手動修復のため中断するか」を選択させる義務を負う。詳細な caller セマンティクスは `commands/pr/fix.md` Phase 8.1 を参照 (#349)
