...
- **Ready と merge がレビュー済みの受入条件記録を強制する** — 両遷移で最新のレビュー結果を検査し、人間が記録できるのは未検証の受入条件だけとし、全条件が充足済みまたはレビュー済み HEAD で人間による確認済みの場合にだけ通す。merge にも現在の HEAD と最終レビュー済み commit の照合を追加した。受入条件記録の導入前に生成された結果 JSON は fail-closed で停止して `/rite:pr-review` の再実行を要求し、既存ファイルの移行や archive 済み結果の再利用は行わない。
