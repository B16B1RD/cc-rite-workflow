...
散文が実装本体である skill では、pin 対象は **marker 名ではなく指示語そのもの**である。`assert_grep` で `CLEANUP_DELEGATED=1` の存在だけを見るテストは、ガードの削除は検出するが「**実行しない**」→「通常どおり実行する」という**指示の反転**を検出しない（の mutation 実測: 4 サイトの指示を反転させても全 assert green）。
