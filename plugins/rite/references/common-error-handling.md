...
**適用箇所**:
- `/rite:pr-review` ステップ 6.1.a (Local JSON File Save — 14 種の `LOCAL_SAVE_FAILED` reason はすべて `exit 0` の完全な非ブロッキング。**save-pending marker は保存の成否に依らず EXIT trap で削除する** — 成功時のみ削除すると本契約が ステップ 8.0.4 経由で blocking gate に化ける。marker が残るのは (i) trap 設置**前**の `exit 1` (`--content-file` 未指定 / `unknown_option`)、(ii) `--pending-marker` が marker path guard (文字集合 allowlist / `..` 排除 / basename anchor) を通らなかった場合、(iii) `rm` 自体が失敗した場合の 3 群。(i)(ii) は caller 契約違反で再試行が 1 iteration で収束するのに対し、(iii) は環境起因で再実行では収束せず手動削除を要する。**`--pr` 欠落 / 非数値 (`pr_number_placeholder_residue`) は marker を残さない** — 同 gate は trap 設置**後**の `exit 0` のため marker は削除され、可視化は 6.1.c ケース 2 が担う。直下の 6.1.d は placeholder residue 5 種を exit-1 群に含めるが、6.1.a はこの 1 点だけ非対称)
