...
**行数を主指標にしない。** open パイロットの実測（測定時点: 本手法を確立した PR のマージ commit と
その親。squash-merge されるため PR ブランチ上の SHA は残らない。以降 open/SKILL.md を触るたびに動く
ので、引用ではなく §3.1 / §3.2 の手順で再測定して使うこと）: 総行数は 642 → 615（−4.2%）にしか
ならなかったが、
...
open パイロットの実測（測定時点は §3.1 と同じ）: **退避 12 hunk で −3,766 B（削減の 54.7%）、圧縮
