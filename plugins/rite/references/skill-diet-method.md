...
open スキルでのパイロットで確立し、実測値を添えてある。第 1 波（iterate / fix / pr-review）は
本ファイルの 3 節をそのまま適用する。
...
**rc=0 だけでは足りない。** stderr に `machine rail identical` が出ていることを確認する。出ていなければ
...
**行数を主指標にしない。** open パイロットの実測（本手法を確立した PR とその親。**概数で示す** —
open/SKILL.md を 1 行触るたびに精密値は動くので、精密値を書けば必ず陳腐化する。自分の対象では上の
テンプレートで測り直すこと）: 総行数は 642 → 615 で **−4% 程度**にしかならなかったが、散文バイト数は
19.2 KB → 12.4 KB で **約 −36%** だった。理由は総行数の内訳にある — diet 前の 642 行は機械レール
305 行 + 空行 161 行 + 見出し 40 行で 8 割を占め、そもそも散文は 136 行しかない。加えて日本語散文は
...
open パイロットの実測（§3.1 と同じく概数）: **退避 12 hunk で約 −3.8 KB（削減の約 55%）、圧縮
36 hunk で約 −3.1 KB（約 45%）**。退避は hunk あたりの効きが圧縮の約 3.6 倍で、判断をほとんど伴わない
...
4. 散文圧縮（§1「**散文層 = 削る**」チェックリスト。層 A は 1 バイトも変えない）
...
7. 対象スキルに `references/` を新設した場合、`docs/SPEC.md` の skills ツリーの当該行へ追記する
   （既存スキルと同じ慣行）。行末に説明の丸括弧が**無い**行はそのまま `(+ references/rationale.md)`
   を足し、丸括弧が**ある**行は括弧内の末尾へ `; + references/rationale.md` を併合する
