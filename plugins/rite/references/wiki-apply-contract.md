...
`git-commit-file.sh` は commit の前に gate を呼ぶ。deny、または gate スクリプトが無いときは commit しない。allow で commit できたときだけ、証跡の head を新しい HEAD に更新する。`pre-tool-bash-guard.sh` は、phase が implement または fix で作業ツリーが一致する git commit に同じ gate を使う。その経路は commit 後に head を更新してから次の commit に進む。phase がそれ以外の commit は止めない。
