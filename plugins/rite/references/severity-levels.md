...
- **アンカーは直前の境界 (行頭 / `<br>` / 空白) を要求する** — 境界を落とした転記や raw pipe を含む repro は full regex に match しない。この形は **`measured=false` へ潰さず未判定 (= blocking のまま) として扱う** (上記「適用範囲」の経路 (c))。`MEASURED_UNDETERMINED_ON_ANCHOR` marker + WARNING で可視化し、**helper は停止しない**。一方 `Verification:` marker から**同一セグメント内** (改行 / `<br>` / 句点まで) に `=>` が続かない形は「アンカーの書き損じ」ではなく散文中の言及なので、従来どおり `measured=false` で降格し `MEASURED_DEMOTED_ON_ANCHOR` marker + WARNING を出す — この絞り込みが無いと、stage 1 の意図的に緩い存在判定が拾う散文がそのまま恒久 blocking になる。**分離は完全ではない** — 同一セグメント内に `=>` が現れる散文は未判定へ倒れる (既知の残存限界。範囲と上限の担保は [`assessment-rules.md` §5.3.0.M](../skills/fix/references/assessment-rules.md#530m-実測必須ゲート-measured-confirmed-gate) の「(i) は完全な分離ではない」を SoT とする)。集約的な hard fail (「blocking 候補が全件形式崩れなら止める」等) は一度導入したが撤去しており、per-finding の 3 値化で是正した現在も導入しない。
