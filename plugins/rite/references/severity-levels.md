...
- **非実測指摘 (measured=false) の扱い**: 破棄せず **non-blocking** に分類し、**3 経路すべてに記録する** — (1) 永続 JSON (`.rite/review-results/*.json` の トップレベル `non_blocking_findings[]`)、(2) `/rite:pr-review` ステップ 5.4 統合レポートの `### 実測なし指摘 (non-blocking)` section (severity 明示、E2E でも省略禁止)、(3) E2E output line の `| non-blocking: {n}` suffix。fix サイクルは起動しない (mergeable countdown / `total_findings` から除外)。既定構成 (`pr_review.post_comment: false`) では (1) が唯一の永続チャネルであり、これによりマージ後に人間が拾い直せる状態を保つ (Issue #2024 D-01)。
