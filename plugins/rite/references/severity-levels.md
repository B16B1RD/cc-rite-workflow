...
**判定はファイル種別で行わない** (本軸の総則と同じ)。テストへの指摘でも「clean fixture のため本番バグを検出できない」類は実行時帰結を持つ class A である。同様に、手順書・仕様書の散文への指摘でも、記述に字義どおり従う実行者が誤動作に至ることを、記述された手順（仕様書が記述する実装を含む）の実行で観測した指摘 (authoring 層の挙動的帰結クラス。`Verification:` 付き) は class A であり、文書だけの PR でも降格しない。不確実を理由に class B へ倒さない。class B の「文書整合」は字面整合クラス (文書のテキスト差分だけを観測したもの) を指す。
