...
- **アンカーは直前の境界 (行頭 / `<br>` / 空白) を要求する** — 境界を落とした転記や raw pipe を含む repro は full regex に match しない。この形は **`measured=false` へ潰さず未判定 (= blocking のまま) として扱う** (上記「適用範囲」の経路 (c))。`MEASURED_UNDETERMINED_ON_ANCHOR` marker + WARNING で可視化し、**helper は停止しない**。一方 `Verification:` marker があっても `=>` を全く含まない形は「アンカーの書き損じ」ではなく散文中の言及なので、従来どおり `measured=false` で降格し `MEASURED_DEMOTED_ON_ANCHOR` marker + WARNING を出す — この絞り込みが無いと、stage 1 の意図的に緩い存在判定が拾う散文がそのまま恒久 blocking になる。集約的な hard fail (「blocking 候補が全件形式崩れなら止める」等) は一度導入したが撤去しており、per-finding の 3 値化で是正した現在も導入しない。
