...
| 帰結クラス | 定義 | `Verification:` アンカー |
|---|---|---|
| **契約対応の未 pin** | Issue 契約 (§4.4 MUST / 受入条件節の各 AC の `Then`) が規定する挙動そのものを無効化しても検出されない、または当該挙動に対応するテストが存在しない | 適格 |
...
- **実測必須ゲート**: 帰結クラスは authoring 層でアンカー添付可否に作用する。実測ゲートの 2 値 + error 判定には介入しない。実測判定不能な形式崩れアンカーは `anchor_undetermined` で失敗する。
...
**判定はファイル種別で行わない** (本軸の総則と同じ)。テストへの指摘でも「clean fixture のため本番バグを検出できない」類は実行時帰結を持つ class A である。同様に、手順書・仕様書の散文への指摘でも、記述に字義どおり従う実行者が誤動作に至ることを、記述された手順（仕様書が記述する実装を含む）の実行で観測した指摘 (authoring 層の挙動的帰結クラス。`Verification:` 付き) は class A であり、文書だけの PR でも降格しない。不確実を理由に class B へ倒さない（上の表の既定より優先する）。class B の「文書整合」は字面整合クラス (文書のテキスト差分だけを観測したもの) を指す。

**降格政策**: class A が 0 件になった cycle で、class B の blocking のうち**除外判別子に該当しないもの**を `non_blocking_findings[]` へ降格する。class A が 1 件でも残る cycle では class B も blocking のまま (磨きは実体修正と並走する)。降格後は既存の mergeable 経路で自然終了し、独立した freeze フェーズ・状態遷移は存在しない。降格分は必ず記録に残る (降格理由 = class B 認定の判定文付き。報告自体は抑制しない — 変わるのは blocking 継続条件のみ)。
...
**除外判別子 (合意済み AC の実測済み未充足)**: 関連 Issue の受入条件のうち acceptance reviewer が HEAD で未充足と実測した行 (レビュー結果 JSON の `acceptance_criteria[]` で `status == "unmet"` の行) が `finding_id` で指す finding は、class B と分類されても降格しない (class B のまま blocking を維持する)。文書同期を求める受入条件の未充足は帰結が文書整合に留まるため class B に分類されうるが、合意済み契約の未達であり磨きではない。分類 (class A/B) は変えず、除外は classification map ではなく判定表から helper が機械的に付与する。判定表と集合演算の SoT は [review-result-schema.md §acceptance_criteria](./review-result-schema.md#acceptance_criteria) と [assessment-rules.md §5.3.0.C](../skills/fix/references/assessment-rules.md#530c-帰結クラス降格政策-consequence-class-demotion-gate)。
...
blocking = CONFIRMED (全指摘事項に残存)
         AND verification.measured == true   (repro または failing_test の実測証跡あり)
                                             (※ 判定不能は error。下記「適用範囲」参照)
         AND scope in {current-pr, follow-up}  (nit-noted は従来どおり対象外)
```

- **適用範囲 (measured は 2 値 + error)**: 本式は `全指摘事項` の rite reviewer finding に適用する。成功時の gated finding は `measured=true` / `false` のいずれか。正規形に match せず marker と同一セグメント内に `=>` があり、算出時に判定不能となる finding が 1 件でもあれば `reason=anchor_undetermined` で非ゼロ終了し、元 JSON を変更しない。対象 finding IDs の reviewer 出力を同 cycle 内で再生成し、再試行上限 1 回を超えたら `[review:error]`。外部・人間レビューの扱いは既存経路を維持する。旧 JSON の gated finding で measured が欠落している場合も正常な blocking 値とは解釈せず、fix consumer は `[fix:error]` とする。
...
fatal = verification.measured == true
        ...
        AND (severity in {CRITICAL, HIGH}
             OR ((consequence_class == "A"
                  OR (consequence_class == "B" AND consequence_exclusion が空でない文字列))
                 AND pre_existing != true))
```

`/rite:fix` はこの集合のみを修正対象とする。PR 起因の class A（帰結クラス軸。放置すると今回の成果物の実行時挙動が変わる）は、重要度ラベルが MEDIUM 以下でも同じ PR で修正する。降格ゲートが除外判別子（`consequence_exclusion`）を付けて blocking に残した class B も class A と同じく扱い、MEDIUM 以下でも修正対象に残す。`consequence_exclusion` が空文字・非文字列・欠落の class B は除外判別子なしとして扱う。gated finding は revert test を通過しているため PR 起因とみなし、明示的な `pre_existing: true` だけを除外する。`consequence_class` は上記「ゲート層の class A/B 降格政策」の helper が書いた値を使う。gated な非 fatal 指摘は severity / scope / id を維持し、`non_blocking_findings[]` へ `demotion_reason: non_fatal` で移送する。nit-noted は認知のみのまま維持する。gated finding の measured 未判定・未知 severity、および実測済み MEDIUM 以下の `consequence_class` が `A` / `B` でないもの（`class_undetermined`）は `[fix:error]` とし、元 JSON を変更しない。外部・人間 thread の出自判定と解決済み判定は既存経路を維持する。
...
- **実測阻害 (`Measurement-Blocked:`)**: 検証コマンドが環境制約（ツール拒否・権限・worktree 隔離）でブロックされ回避も不能なとき、reviewer は `Verification:` ではなく `Measurement-Blocked: <cmd> => <reason>` を `内容` 列に書く。helper はこの marker を実測アンカーとして読まない（実測判定に介入しない）。統合層が件数と内訳を surface する。無言の measured=false 降格に使ってはならない。原理的に検証不能（認証付き実環境が必要等）な指摘は本 marker も付けず、従来どおり non-blocking 記録経路へ倒す。
- **非実測指摘 (measured=false) の扱い**: 破棄せず **non-blocking** に分類し、**4 経路すべてに記録する** — (1) 永続 JSON (`.rite/review-results/*.json` の トップレベル `non_blocking_findings[]`。**`description` / `suggestion` を含む全文を持つ。既定構成 `post_comment: false` では cycle 中の全文の唯一の保存先**)、(2) `/rite:pr-review` ステップ 6.1.d の関連 Issue 記録コメント (`## 📜 rite 非実測指摘の記録`、update-in-place の 1 件。**載るのは reviewer / severity / `file:line` のポインタと降格理由 (class B 降格分は `demotion.reason` の判定文、それ以外は「実測なし」) のみで全文は載らない** — 経路 (1) との対比。既定構成 `post_comment: false` でも本経路は投稿されるため、非実測 CRITICAL の詳細が修正前に public PR へ先行開示されるのを避ける。本文には全文の所在 (経路 (1) のパスと配列名) を明記する。`pr_review.post_comment` 設定に**依存しない** — 記録経路を常時維持するため opt-out 対象外。PATCH 先の同定は 2 段で、第一候補が関連 Issue body に永続化した comment id (形状の SoT は helper の `ID_MARKER_*` 定数。marker は行全体を占める)、fallback が本文照合 (author ∧ 1 行目 marker 前方一致 ∧ 最終非空行 sentinel)。id 側は author に加え **所属 Issue** と **対象が記録コメントであること** も検証する (`issues/comments/{id}` は repo スコープで issue 非依存、かつ Issue body は書き込み権限があれば編集できるため)。関連 Issue は PR body の closing keyword を第一候補、branch 命名 (`issue-{N}`) を第二候補として解決し、どちらも無いときは `related_issue_unresolved` で fail-loud する。lookup が自分の過去投稿を見つけられない場合 (**id 側と fallback 側の両方が外れたとき**に限る) は縮退する。`NONBLOCKING_ID_UNRESOLVED` marker の reason 語彙は 8 種で、いずれも帰結は fallback へ倒すこと (`id_read_failed` / `id_malformed` / `id_fetch_failed` / `id_fetch_unparseable` / `id_author_mismatch` / `id_pr_mismatch` / `id_target_not_record` / `id_comment_deleted`)。**「id 側が外れた」と「この 8 種のどれかが出る」は同値ではない** — `gh api user` が失敗した cycle は自 login が無く id 側の author 検証も本文照合の author 条件も評価できないため、段 1 自体が呼ばれず marker を 1 つも出さないまま両側が同時に外れる。**id が関連 Issue body に無い初回 cycle も marker を出さない** — 永続化前は fallback が正しい経路であり、毎 cycle 出すと本当の異常が埋もれるため。fallback 側が外れるのは (i) `gh api user` の失敗 (**この 1 種だけは段 1 も同時に外れる** — 自 login が無いと id 側の author 検証も評価できないため)、(ii) コメント一覧取得 (`gh api --paginate`) の失敗 (**段 1 には触れないので、id が解決していれば `degraded=0` のまま update-in-place が成立する**)、(iii) 別トークン identity での過去投稿、(iv) 既存記録コメントの最終非空行が機械専用 sentinel でない場合 (sentinel 導入前の記録 / marker で始まる手書きコメント。この経路は `degraded=0` のまま発生し `NONBLOCKING_LEGACY_ORPHAN` marker で可視化される)。**id で PATCH 先を特定できた cycle は本文照合の lookup が失敗しても縮退しない** (`degraded=0` のまま update-in-place が成立する) — **帰結は件数依存**で、本 cycle の非実測指摘が 1 件以上なら新規作成となり 2 件目が作られ、0 件で本文に却下台帳のエントリが無ければ投稿自体を省くため前 cycle の記録が stale で残り、台帳のエントリがあれば 0 件でも新規作成する (他 4 文書 SPEC.md / CONFIGURATION.md / rite-config.yml / assessment-rules.md と同じ分岐)、(3) `/rite:pr-review` ステップ 5.4 統合レポートの `### 実測なし指摘 (non-blocking)` section (severity 明示、E2E でも省略禁止)、(4) E2E output line の `| non-blocking: {n}` suffix (件数 > 0 のときのみ)。(1) は無条件、(2) は best-effort (「0 件 ∧ 本文に却下台帳のエントリなし ∧ helper が既存の記録コメントを**検出できない**」ときは投稿を省き、本文不備 4 種 / gh 失敗 2 種 / jq・awk 実行環境起因 1 種 (`body_check_unavailable`) では `outcome=failed` で投稿されない。lookup が degraded した cycle では既存が実在しても検出できず、台帳のエントリが無ければ skip に落ち、前 cycle の記録が stale で残る)、(3)(4) は実行モードと件数に依存する補助経路。fix サイクルは起動しない (mergeable countdown / `total_findings` から除外)。ただし同一 file:line の GitHub thread が rite finding 由来と確認できない場合は [`fix/SKILL.md`](../skills/fix/SKILL.md) ステップ 1.3 step 4 の出自確認で External review = blocking へ振り替わる — 本ゲートは finding を対象とし、thread routing は別レイヤ。既定構成 (`pr_review.post_comment: false`) では (1) がローカルの、(2) が関連 Issue 上で共有可能な永続チャネルであり、これによりマージ後に人間が拾い直せる状態を保つ (D-01。`.rite/review-results/` は gitignore 対象のためレビュアーと共有できるのは (2) のみ — ただし (2) が共有するのはポインタ + 降格理由までで、**cycle 中、全文はレビューを実行した環境のローカル JSON にしか存在せず共有経路を持たない**。checkout では取得できない。マージ後もローカル全文を残すため `/rite:cleanup` は `non_blocking_findings[]` が非空の JSON を削除せず `.rite/review-results/archive/` へ退避し、残存分の全文 (`description` / `suggestion`) を follow-up Issue 1 件へ転記する（public リポジトリではその Issue は公開される）。**転記の前にマージ後 HEAD に対する再検証を行い、後続の fix cycle で既に解消済みと判定された指摘は転記対象から除外する** — `non_blocking_findings[]` は指摘が出た cycle の観測でありその後の修正を反映しないため、無条件転記はマージ時点に存在しない drift の Issue を生む。判定不能なものは転記側へ倒し、全件が解消済みなら follow-up Issue を起票しない。iterate の sweep で起票済みの指摘も転記対象から除外し、残りが全件起票済みなら起票しない。却下台帳か最新のレビュー結果 JSON を読めなければ sweep で Issue 化済みの指摘も転記対象とし、再検証による除外は適用済みのまま WARNING を出す)。指摘への対応内容は fix コミットメッセージに書くが、`/rite:merge` は squash するため develop の git log には残らず、PR の Commits タブ経由で参照する。`pr_review.post_comment` が制御するのは統合レポートの PR 投稿のみであり、経路 (2) は本設定に依存しない。
...
- **アンカーは直前の境界 (行頭 / `<br>` / 空白) を要求する** — 正規形に match せず実測判定不能なら `[CONTEXT] MEASURED_GATE_FAILED=1; reason=anchor_undetermined; count=N; findings=F-xx,...` を emit して非ゼロ終了する。`MEASURED_UNDETERMINED_ON_ANCHOR` は出力しない。marker と矢印の間が改行 / `<br>` / 句点で切れる形や上限超過は従来の判別子どおり実測なしに分類し、成功時の `MEASURED_DEMOTED_ON_ANCHOR` で可視化する。regex と判別子の SoT は [`assessment-rules.md` §5.3.0.M](../skills/fix/references/assessment-rules.md#530m-実測必須ゲート-measured-confirmed-gate)。
...
**既存 boolean の保持**: 冪等な再実行では既存 `verification.measured` を保持する。`--reject-preset-verification` は算出分類と食い違う preset を拒否する。分類が一致する preset の証跡は helper の再抽出を経ないため、caller は生成規約どおり verification を先に書かない。成功した gated finding に measured 欠落は残らない。
