...
**なぜこの軸が要るか**: 散文では「2 文の食い違いを示す grep」が technically measured になるため、実測必須ゲートだけでは重要度を弁別できない。実測ゲート配線後、アンカー適格な指摘を生む変更 (コード・テスト) を伴う PR ではアンカーが重要度を弁別できる一方、指摘の大半が字面整合に寄った PR #2052 は 3 run・11 記録サイクルで発散した。本軸はその弁別を authoring 層で行う。
