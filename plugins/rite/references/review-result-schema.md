...
collect は `targets[]` に `verification` と `route` を返す。実測あり MEDIUM は `issued`、それ以外と nit-noted は `recorded`。`--pr` は関連 Issue コメントの台帳を読み、既存の issued/recorded/rejected を対象から除外する。`--json` 単独はオフライン収集。count は targets と未転記 already_rejected の合計で、guardrail のみでも sweep を実行する。`/rite:cleanup` ステップ 6.0 の follow-up 起票も台帳の issued 行を読み、最新 JSON に同じ `[finding_id, file:line]` がある行の `file:line` を持つ指摘を、cycle と id を問わず転記から除外する。
