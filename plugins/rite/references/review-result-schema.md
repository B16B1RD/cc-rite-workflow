...
collect は `targets[]` に `verification` と `route` を返す。実測あり MEDIUM は `issued`、それ以外と nit-noted は `recorded`。`--pr` は関連 Issue コメントの台帳を読み、既存の issued/recorded/rejected を対象から除外する。`--json` 単独はオフライン収集。`/rite:cleanup` ステップ 6.0 の follow-up 起票も台帳の issued 行（`[finding_id, file:line]`）を読み、sweep で起票済みの指摘を転記から除外する。count は targets と未転記 already_rejected の合計で、guardrail のみでも sweep を実行する。
