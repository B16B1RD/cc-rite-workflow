...
- **Commit Type** (feat/fix/refactor/chore/docs): commit message / branch 名 / PR title の語彙（作業ツリーの CLAUDE.md / AGENTS.md（対象パスに近いネストが根より優先）が件名形式を指定していればそれに従い、未指定時は Conventional Commits。`pr/open.md` の branch type 派生もこの系列）。
