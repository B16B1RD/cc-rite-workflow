...
|----------|-------------|--------|---------|
| `{owner}` | Repository owner (user or organization) | `git-remote.sh resolve-owner-repo` (fallback: `gh repo view --json owner --jq '.owner.login'`) | `{owner}` |
| `{repo}` | Repository name | `git-remote.sh resolve-owner-repo` (fallback: `gh repo view --json name --jq '.name'`) | `{repo}` |
...
| `#{number}` | Issue number with `#` prefix | Same as `{number}` with formatting | `#{n}` |
| `{issue_number}` | Issue number (alias) | Same as `{number}` | `531` |
| `#{issue_number}` | Issue number with `#` prefix | Same as `#{number}` | `#{n}` |
...
| `#{pr_number}` | PR number with `#` prefix | Same as `{pr_number}` with formatting | `#{n}` |
