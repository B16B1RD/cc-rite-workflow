...
| `description` | no | 1-2 文の要約（OKF 推奨。`{summary}` と同源）。page frontmatter に保持され、ingest が index.md `## ページ一覧` テーブルのサマリー列にも反映する。**⚠️ 読み手側は未対応**: `/rite:wiki-query` の Pass 1 は候補抽出が箇条書き行限定のため、テーブル形式 index ではサマリー列を消費しない（読み手のテーブル対応は Issue #2053） |
