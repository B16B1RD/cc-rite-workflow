...
- `{parent_issue_display}`: Read via `bash {plugin_root}/hooks/state-read.sh --field parent_issue_number --default 0` (Issue #687 AC-4 — per-session state, not legacy `.rite-flow-state` snapshot which may hold another session's residue). Display `#{N}` if non-zero, `なし` if zero or absent
...
**If flow-state does not exist**: The invoked command (e.g., `rite:issue:start`) will create it via `flow-state-update.sh create` in its own phases, so no action is needed here. The actual no-state handling lives in `hooks/resume-active-flag-restore.sh` — it reads `curr_phase` via `state-read.sh --field phase --default ""` and skips the `flow-state-update.sh patch` invocation entirely when `state-read.sh` returns an empty string, deferring to the invoked command's create-mode initialization. The canonical enumeration of paths where `state-read.sh` returns an empty string (state-read.sh fallbacks legacy when per-session is absent under schema_version=2, so single-file absence alone does not trigger an empty result): per-session **and** legacy files **both** absent (conjunctive), OR the file is present but `phase` is null/missing/false, OR `phase` is an empty string, OR the file is empty (size 0) / corrupt JSON. The `--if-exists` flag passed to `flow-state-update.sh patch` inside the helper provides a defense-in-depth no-op safety net for the case where both per-session and legacy are absent at write time. Note that PR #688 cycle 30 F-01 fix added reader-symmetric legacy fallback to `_resolve_session_state_path` (in `flow-state-update.sh`), so when `state-read.sh` falls back to legacy under schema_v=2 + valid sid + per-session absent + legacy present, the writer also routes to legacy — restoring active=true on the file `state-read.sh` actually read from, instead of silently no-opping. This eliminated the AC-4 reproduction scenario silent regression.
