...
**Context counter recovery**: `/rite:resume` は Phase 3.2 の resume execution（下記の Skill ツール呼び出し）の直前に以下の bash コマンドを実行する。`.rite-flow-state` に `context_counter_at_compact` が含まれている場合、`.rite-context-counter` をその値に復元する。これにより、コンパクション前の最後のカウント値からコンテキストプレッシャー監視を継続する（コンパクション自体がコンテキストを消費するため近似値）:
