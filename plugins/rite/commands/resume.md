...
**If flow-state does not exist**: The invoked command (e.g., `rite:issue:start`) will create it via `flow-state-update.sh create` in its own phases, so no action is needed here. The `[ -z "$curr_phase" ]` guard above is the primary handler — it skips the patch whenever `state-read.sh` returns an empty string, which happens in **all** of the following no-state paths (state-read.sh fallbacks legacy when per-session is absent under schema_version=2, so single-file absence alone does not trigger an empty result; see L343 inline comment for the canonical enumeration): per-session **and** legacy files **both** absent (conjunctive), OR the file is present but `phase` is null/missing/false, OR `phase` is an empty string, OR the file is empty (size 0) / corrupt JSON. The `--if-exists` flag inside the `else` branch is a secondary defense for the rare race where the file disappears between the read and the patch.
