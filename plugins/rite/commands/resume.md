...
**If flow-state does not exist**: The invoked command (e.g., `rite:issue:start`) will create it via `flow-state-update.sh create` in its own phases, so no action is needed here. The actual no-state handling lives in `hooks/resume-active-flag-restore.sh` — it reads `curr_phase` via `state-read.sh --field phase --default ""` and skips the `flow-state-update.sh patch` invocation entirely when the result is an empty string, deferring to the invoked command's create-mode initialization. This prose is the canonical enumeration of the four paths where `state-read.sh` returns empty (state-read.sh fallbacks legacy when per-session is absent under schema_version=2, so single-file absence alone does not trigger an empty result): per-session **and** legacy files **both** absent (conjunctive), OR the file is present but `phase` is null/missing/false, OR `phase` is an empty string, OR the file is empty (size 0) / corrupt JSON. The `--if-exists` flag passed to `flow-state-update.sh patch` inside the helper is a secondary defense for the rare race where the file disappears between the read and the patch.
