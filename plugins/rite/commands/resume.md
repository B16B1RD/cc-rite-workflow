...
**Canonical enumeration of paths where `state-read.sh` returns the DEFAULT (empty string)** (verified-review cycle 35 fix F-08 MEDIUM — extended from 4 paths to 6 paths to match implementation reality at `hooks/state-read.sh` L124-150):
...
2. file is present but `phase` is null / missing / false (jq's `// $default` operator returns DEFAULT for null AND false)
3. `phase` is an empty string
4. file is empty (size 0) or corrupt JSON
5. **schema_version=2 + valid sid + per-session absent + legacy present + legacy.session_id is a *foreign* session** (`foreign:*` classification) — reader emits `cross_session_takeover_refused` sentinel and returns DEFAULT (state-read.sh L132)
6. **schema_version=2 + valid sid + per-session absent + legacy present + legacy.session_id jq parse fails** (`corrupt:*` classification) — reader emits `legacy_state_corrupt` sentinel and returns DEFAULT (state-read.sh L142)
