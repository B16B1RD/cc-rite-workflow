...
**Context counter recovery**: If `.rite-flow-state` contains `context_counter_at_compact`, restore `.rite-context-counter` to that value after resume. This ensures context pressure monitoring continues with accurate counts after compaction:
