...
#### 3.2.1 Context Counter Recovery

**実行主体**: `/rite:resume`
**実行タイミング**: Phase 3.2 のフェーズ判定テーブル参照後、下記の Skill ツール呼び出し直前に実行する

`.rite-flow-state` に `context_counter_at_compact` が含まれている場合、`.rite-context-counter` をその値に復元する。これにより、コンパクション前の最後のカウント値からコンテキストプレッシャー監視を継続する（コンパクション自体がコンテキストを消費するため近似値）。
...
**If `context_counter_at_compact` is not present**: `.rite-context-counter` は変更されず、カウントは 0 から再開される。

#### 3.2.2 Resume Execution
