...
```bash
# Phase 1.1 で取得済みの files 配列 (gh pr view --json files の出力) を使用
...
while IFS= read -r f; do
  ...
  case "$f" in
    ...
    commands/*.md|commands/*/*.md|commands/*/*/*.md|commands/*/*/*/*.md| \
    commands/*.mdx|commands/*/*.mdx|commands/*/*/*.mdx|commands/*/*/*/*.mdx| \
    skills/*.md|skills/*/*.md|skills/*/*/*.md|skills/*/*/*/*.md| \
    skills/*.mdx|skills/*/*.mdx|skills/*/*/*.mdx|skills/*/*/*/*.mdx| \
    agents/*.md|agents/*/*.md|agents/*/*/*.md|agents/*/*/*/*.md| \
    agents/*.mdx|agents/*/*.mdx|agents/*/*/*.mdx|agents/*/*/*/*.mdx| \
    plugins/rite/i18n/*|plugins/rite/i18n/*/*|plugins/rite/i18n/*/*/*)
...
**実装上の注意**:
- `case` パターンの glob は bash の **filename expansion ではなく case statement の pattern matching** で評価される (`*` はディレクトリ階層を 1 階層しか跨がない)。深いネストに対応するため上記では 4 階層まで列挙
...
- tech-writer の出力に以下の META 行が 1 つも含まれない:
  ...
  - `META: Cross-Reference partially skipped` (外部参照スキップ、Step 4 で扱う)
...
> **Note**: `finding_count >= 1` の場合はこの Step 1 の「finding 0 件警告」をスキップするが、下記の Step 2 (META 5 カテゴリ実行確認) は**件数に関係なく必ず実施する**。Step 2 + Step 3 (Evidence field 必須化) + Step 4 (Cross-Reference partial skip 検出) の 3 つすべてを満たした場合のみ post-condition は passed とみなされる。
