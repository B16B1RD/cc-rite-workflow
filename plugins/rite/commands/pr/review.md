...
- `verification_post_condition: error` フラグを set
- overall assessment を `修正必要` に昇格（Phase 5.3 / Phase 5.4 の escalation chain と統一された label。`要修正` は reviewer 個別評価用の label で、overall 昇格には使用しない）
- 該当 reviewer 由来の指摘を **全件 blocking 扱い**
- stderr に下記 ERROR を出力し、silent pass 経路を完全に閉塞する
