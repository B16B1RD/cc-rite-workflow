...
### 推奨事項
[改善提案があれば（任意の改善、スタイル提案、別 Issue 推奨の既存問題など）]
...
Deduplicate across sources: if the same file:line appears in both, keep only the Source A entry (it has richer metadata).
...
この Issue は PR #{pr_number} のレビューで検出されたスコープ外の指摘から作成されました。
...
**Source-aware body text**: When the candidate is from Source A (findings), use "スコープ外の指摘から作成されました". When from Source B (recommendations), use "スコープ外の推奨事項から作成されました". The `{severity}` placeholder uses the actual severity for Source A, or `推奨事項（重要度なし）` for Source B. The `{file}:{line}` placeholder uses `特定ファイルなし` for Source B when no file path is mentioned.
