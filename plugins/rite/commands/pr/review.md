...
Doc-Heavy bash impl failure reasons: (`gh_files_stdout_mktemp_failure` / `gh_pr_view_files_failure` / `mapfile_io_error`)
...
- (c) `META: Cross-Reference partially skipped` (外部参照スキップ、Step 4 で扱う)
- (a + inconclusive) `META: All 5 verification categories executed, 0 inconsistencies found, but {N} categories were inconclusive. Inconclusive: [...]. Categories: [...]` ([`internal-consistency.md`](./references/internal-consistency.md#inconclusive---meta-) で定義された inconclusive 集計版、Step 4.5 で扱う)
...
BODY_EOF
then
  echo "[CONTEXT] ISSUE_CREATION_FAILED=1; reason=body_tmpfile_write_failure" >&2
  echo "ERROR: Issue 本文テンプレートの一時ファイル書き込みに失敗" >&2
  exit 1
fi

if [ ! -s "$tmpfile" ]; then
  echo "[CONTEXT] ISSUE_CREATION_FAILED=1; reason=empty_body_tmpfile" >&2
...
if [ -z "$result" ]; then
  echo "[CONTEXT] ISSUE_CREATION_FAILED=1; reason=empty_script_result" >&2
