...
| `use_context7` | boolean | `false` | context7 MCP ツール（`resolve-library-id` / `query-docs`）による検証を使用 |
...
**When `review.fact_check.use_context7: true`:**
...
**When `review.fact_check.use_context7: false` (default):**

1. **WebSearch** — ツール設定、CLI 動作、バージョン情報、CVE、ベストプラクティス → 公式ドキュメントサイトでフィルタ
2. **WebFetch** — 公式ドキュメントの URL が判明している場合 → 直接取得

> **Note**: `use_context7: false` がデフォルトの理由: サブエージェントでの MCP ツール可用性が未確認のため。メインコンテキスト実行では利用可能だが、将来のエージェント委譲時に問題となる可能性がある。
