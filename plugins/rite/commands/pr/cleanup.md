...
if [ -f .rite-flow-state ]; then
  jq '. + {active: true, phase: "cleanup", updated_at: "'"$(date -u +%Y-%m-%dT%H:%M:%S+00:00)"'"}' \
    .rite-flow-state > .rite-flow-state.tmp && mv .rite-flow-state.tmp .rite-flow-state
else
  jq -n '{active: true, issue_number: 0, phase: "cleanup", updated_at: "'"$(date -u +%Y-%m-%dT%H:%M:%S+00:00)"'"}' \
    > .rite-flow-state
fi
```
...
1. Read the contents of `$tmpfile_read` using the Claude Code Read tool
...
```bash
tmpfile_read="/tmp/tmp.XXXXXXXXXX"   # ← Step 1 output value
tmpfile_write="/tmp/tmp.XXXXXXXXXX"  # ← Step 1 output value
...
# Verify that a change was actually made
if diff -q "$tmpfile_read" "$tmpfile_write" > /dev/null 2>&1; then
  echo "INFO: チェックボックスは既に更新済みです（変更なし）"
else
  gh issue edit {parent_issue_number} --body-file "$tmpfile_write"
  echo "親 Issue #{parent_issue_number} の Tasklist チェックボックスを更新しました"
fi
