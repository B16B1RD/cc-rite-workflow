...
>
> **Issue #651 補強 (stop-guard 経由再入は正規 fallback path)**: もし本 Step 0 (および Step 1) が同 turn 内で fire せず implicit stop に至った場合でも、`.rite-flow-state.phase = create_post_interview` (Pre-flight + Return Output re-patch で記録済み) により **`stop-guard.sh` の `create_post_interview` case arm が exit 2 で stop を block**し、stderr に WORKFLOW_HINT (本 Step 0 と同一の bash literal を含む) + `[CONTEXT] WORKFLOW_INCIDENT=1; type=manual_fallback_adopted` sentinel を emit する経路が存在する (Issue #634 修正セッション 2026-04-21 の `.rite-stop-guard-diag.log` で `EXIT:2 reason=blocking phase=phase5_post_review` が複数件記録された実証根拠あり)。すなわち Step 0 → Step 1 → stop-guard exit 2 経由再入 という 3 層の defense が機能し、いずれか 1 層で sub-skill return 後の continuation が enforced される。ただし Claude Code の Stop hook の UI 上は exit 2 後でも `Churned for X` 表示が出る場合があり、ユーザーが「`continue` 手動入力が必要」と認識する余地がある (UI 限界)。本 Issue #651 では declarative 強化路線で 4-site 対称化 (本 Step 0 + Pre-flight + Return Output re-patch + stop-guard WORKFLOW_HINT) を維持・強化することで、いずれの経路でも同一 bash literal が caller に提示されるよう保証する。
>
