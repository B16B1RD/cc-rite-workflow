...
>
> **`--if-exists` は 3-site 対称から除外** (#636 cycle 9 F-01 対応): 本 Step 0 / Step 1 (create.md 側) は `--if-exists` を付与するが、create-interview.md 側の Pre-flight / Return Output re-patch は `--if-exists` を付けず `if [ -f ".rite-flow-state" ]; then ... else ... fi` の branch で file 存在の分岐を明示的に処理する (意図的非対称)。これは create-interview.md Pre-flight が「file 不在時は create mode で新規生成し、存在時は patch mode で更新する」という 2 経路を branch で区別する必要があるため、`--if-exists` (= patch mode 専用の silent skip flag) では実装できない責務の違いを反映している。3-site 対称は `--phase` / `--next` / `--preserve-error-count` のみが対象で、`--if-exists` は本 site (create.md Step 0/Step 1) のみに存在する意図的非対称性。create-interview.md L22 の pair anchor は `--if-exists` を列挙していない (両 anchor 間で整合)。
>
