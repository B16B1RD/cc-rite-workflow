...
ローカル `MEMORY.md` の Feedback / Project エントリから、本 Issue に関連する制約やルールを参照し、計画に反映すべき事項を特定する。例:

- レビュー品質ルール（指摘ゼロまでループ、確信ある指摘のみ等）
...
| R5 | レビュー品質: 確信ある指摘のみ | memory_feedback | MEMORY.md feedback_review_quality | (Phase 3.3 で埋める) |
```
...
**ギャップ検出ルール**: Issue 本文を1段落ずつ走査し、抽出漏れがないかを自己確認する。特に「変更内容」「背景」セクションの太字補足は脱落しやすいため、太字パターン（`**...**`）を Grep 的に探索すること。
...
4. **未カバー検出時のフィードバック**: 計画ドラフト生成後、`requirement_items` テーブルの `対応ステップ` 列を再確認し、空欄が残っていればステップを追加する。Phase 3.3.1 の自己レビューでも再度検証されるが、ここで先行的に解消すること。

**`requirement_items` 添付ルール**: Phase 3.3 の出力には、`対応ステップ` 列を埋めた `requirement_items` テーブルを実装計画の末尾（注意点セクションの直前）に添付すること。これにより全要件→ステップのマッピングが視覚的に確認可能になる。
...
Phase 3.3.1 のループ完了後、レビュー結果サマリーを実装計画の末尾（注意点セクションの後）に付加する。これにより、計画の信頼性と網羅性が会話コンテキストとユーザーに可視化される。
...
| `最終結果` | 必ず「全5観点 pass（指摘ゼロ）」になる（指摘が残った状態でのループ脱出は禁止） |

**Note**: このサマリーは Phase 3.5 で作業メモリにも記録され、Phase 5 以降の品質判定（メトリクス収集）にも参照される可能性がある。
