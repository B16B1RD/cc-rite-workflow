...
| Tentative complexity XL | Phase 1.1 で確定 (cancel 後も XL として記録) |
| Out-of-scope items | Implementation Contract Section 2 (Out of Scope) / Section 1 (Non-goal) |
| Spec document content (Phase 0.7.1) | Implementation Contract Section 4 (Implementation Details) で design context として参照 |
...
**Structured reuse**: Use Section IDs from Phase 0.7.1 to directly reference specification sections instead of re-analyzing the full document. See the Section ID mapping table at the top of Phase 0.7.1 — the "Phase 0.8 Usage" column indicates which sections each decomposition path consumes (`(完成度のみ)` rows are not directly referenced by Phase 0.8).
...
```bash
# Generate body content from Phase 0.7 spec and the canonical Parent Issue body structure
...
Phase 0.9.4 で参照する `{repo}` / `{parent_issue_number}` / `{plugin_root}` 等の解決規則は [`references/bulk-create-pattern.md#placeholder-descriptions`](./references/bulk-create-pattern.md#placeholder-descriptions) を参照する (Phase 0.9.2 用に定義済みの placeholder list を Phase 0.9.4 でも再利用)。
...
>
> **Design decision**: The `[create:completed:{N}]` sentinel is emitted as an HTML comment (`<!-- [create:completed:{N}] -->`) so that the user-visible final line is the `✅` completion message + next steps, not the sentinel token. The string `[create:completed:N]` inside the HTML comment is still grep-matchable, and the HTML comment form weakens the LLM's turn-boundary heuristic that would otherwise treat a bare sentinel line as a natural stopping point. Same policy as `create-register.md` Phase 4.
...
### 1.0.2 Completion Message (User-facing)
