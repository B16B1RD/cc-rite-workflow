...
**Output rules**:
0. **FIRST**: Output `[CONTEXT] INTERVIEW_DONE=1; scope={skipped|completed}; next=phase_0_6` as a **plain-text line** (not HTML-commented). Position rules (#636 cycle 8 F-06 — Rule 1 absolute 位置規定との対称化、#636 cycle 9 F-02 — sub-bullet 分解で読解負荷軽減):
   - **0a (絶対位置)**: the **4th-to-last visible line** of this sub-skill's output — i.e., the FIRST of the 4-line return block, 3 lines before the `<!-- [interview:*] -->` absolute-last sentinel defined in Rule 1
   - **0b (構造保証)**: the relative ordering of Rules 0-1 pins the full block structure: Rule 0 (FIRST) → plain-text continuation reminder → HTML-commented caller instructions → Rule 1 (absolute LAST)
   - **0c (目的)**: Issue #634 補強 — grep marker for orchestrator Pre-check Item 0 and Mandatory After Step 0 routing dispatcher; defense-in-depth against LLM turn-boundary heuristics
