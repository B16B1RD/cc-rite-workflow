...
| `{session_id}` | Auto-read from `.rite-session-id` (written by `session-start.sh`). `flow-state-update.sh create` reads this file automatically when `--session` is empty — Claude does not need to resolve this placeholder manually. Pass `--session ""` to use auto-read. |
