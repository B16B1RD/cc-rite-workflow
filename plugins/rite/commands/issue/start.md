...
| `{parent_issue_number}` | Phase 1.6 (child-issue-selection) or Phase 2.4 Step 3 (parent lookup via 2.4.7) result; persisted in flow-state via `--parent-issue` option. Read at actual call sites — see [Phase 5.1.2 (implement.md)](./implement.md) / Phase 5.7 (this file) for the canonical fail-fast capture pattern (verified-review F-03 / F11-08). Workflow Termination (this file) は Phase 5.7 の `[CONTEXT] PARENT_ISSUE=...` emit を LLM-routing signal として使い、state を re-read しない (cycle 38 F-17 で literal block 削除済み、cycle 9 F-06 regression 経路の防止のため Workflow Termination 内に capture pattern を再注入してはならない)。Issue #687 AC-4 — per-session state via `state-read.sh`, not legacy `.rite-flow-state` snapshot |
