...
| `{parent_issue_number}` | Phase 1.6 (child-issue-selection) or Phase 2.4 Step 3 (parent lookup via 2.4.7) result; persisted in flow-state via `--parent-issue` option. Read via `bash {plugin_root}/hooks/state-read.sh --field parent_issue_number --default 0` (Issue #687 AC-4 — per-session state, not legacy `.rite-flow-state` snapshot). Used in Phase 5.1.2, 5.7, and Workflow Termination routing |
