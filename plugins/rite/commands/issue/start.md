...
| `{parent_issue_number}` | **Asymmetric write/read sites**: (a) **Write side**: Phase 1.6 (child-issue-selection) or Phase 2.4 Step 3 (parent lookup via 2.4.7) detection result is substituted into `--parent-issue {parent_issue_number}` flags from LLM-context (Mandatory After ブロックの flow-state create で初期書込、後続 phase の metrics 記録時に `flow-state-update.sh patch` で更新)。(b) **Read side**: [Phase 5.1.2 (implement.md)](./implement.md) / Phase 5.7 (this file) で `state-read.sh` 経由 (canonical fail-fast capture pattern)。Workflow Termination uses the `[CONTEXT] PARENT_ISSUE=...` emit from Phase 5.7 as the LLM-routing signal and does **not** re-read state; do not reintroduce a parent-capture block inside Workflow Termination. Per-session state is read via `state-read.sh` (Issue #687 AC-4), never via the legacy `.rite-flow-state` snapshot. Historical fix-cycle context (F-03 / F11-08, cycle 9 F-06, cycle 38 F-17) is documented in [`references/state-read-evolution.md`](../../references/state-read-evolution.md) |
...
**Claude への指示 (METRICS_SKIPPED 検出時の挙動)**: 上記 bash block 実行後、stderr に `[CONTEXT] METRICS_SKIPPED=1; reason=state_read_failed` が emit された場合、Claude は **Step 2 (threshold evaluation) と Step 3 (PATCH heredoc generation) を skip** し、`Phase 5.5.2: state-read.sh 失敗のため metrics 更新を skip しました (manual intervention で次回計測してください)` を stderr に出力する。その後、**Mandatory After 5.5.2 (`flow-state-update.sh create --phase phase5_post_metrics` の marker 書き込み) を unconditional に実行してから Phase 5.6 へ進む** (AC-5 により body skip 時も marker 書き込みは必須。これを skip すると Phase 5.6 pre-condition `expected: phase5_post_metrics` で hard abort する)。partial corruption (空文字列 substitute による意味不明な metrics 行) を防ぐための明示分岐。
