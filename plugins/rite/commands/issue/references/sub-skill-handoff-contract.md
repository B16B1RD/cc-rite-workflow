...
本契約の各層は過去の incident で段階的に強化されてきた。詳細な経緯は [`regression-history.md`](./regression-history.md) を参照 (Issue #773 PR 6 で抽出完了)。短期的な参照ポインタ:

| 関連 Issue | 強化内容 |
|-----------|---------|
| #525 | AC-3 grep 検証 phrase 4 つ (`anti-pattern` / `correct-pattern` / `same response turn` / `DO NOT stop`) を `create.md` 本体に永続化 |
| #552 | Pre-check list (Item 0/1/2/3) を導入 (Issue #773 PR 2 で [`pre-check-routing.md`](./pre-check-routing.md) へ抽出完了) |
