...
> **⚠️ 注**: 以下の自動注入は **後続 Issue で実装予定** です。本 Issue (#470) 時点では未実装で、現状は `/rite:wiki:query` コマンドおよび `wiki-query-inject.sh` スクリプトを手動で呼び出すのみ対応しています。`wiki-query-inject.sh` は下位レイヤーとして他コマンドから呼び出せる設計になっているため、将来コマンド側の統合作業が入った際には、コマンド内部から本スクリプトを呼び出して結果をコンテキストに混ぜる想定です (その作業は後続 Issue が担当します)。
