...
> **Note**: 本 bash block は probe 用の簡易パーサで、`ingest.md` Phase 1.1 の cycle-6 fix 済み堅牢版（分割実行・YAML コメント除去・F-01 pipefail 回避・stderr capture）とは挙動が異なります。値の真の採用は `wiki-query-inject.sh` 内の `_extract_yaml_value` が行うため、本 block は UX 上の早期エラー表示専用です。パース仕様を変更する際は `hooks/wiki-query-inject.sh` と `hooks/wiki-ingest-trigger.sh` の実装も同時に同期更新してください (3 箇所同期)。
...
引数が未指定の場合は `AskUserQuestion` でキーワード入力を促します。`AskUserQuestion` は選択式 UI のため、フリーテキスト入力には **Other 選択肢**を使います (既存 `issue/edit.md` Phase 2.2 と同じパターン):
...
> **⚠️ 注**: 以下の自動注入は **後続 Issue で実装予定** です。本 Issue (#470) 時点では未実装で、現状は `/rite:wiki:query` コマンドおよび `wiki-query-inject.sh` スクリプトを手動で呼び出すのみ対応しています。`wiki-query-inject.sh` は rite ワークフロー各コマンドから直接呼び出せる設計です。

将来的には以下のコマンドで自動注入が行われます（`wiki.auto_query: true` 時）:
