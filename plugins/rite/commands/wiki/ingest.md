...
**Enforcement coupling**: protocol violation 時は `stop-guard.sh` が `ingest_pre_lint` / `ingest_post_lint` phase を block し、`manual_fallback_adopted` workflow_incident sentinel が stderr に echo されて Phase 5.4.4.1 (start.md 配下) で post-hoc 検出される (AC-7)。**ただし** `ingest.md` は単独実行 (`/rite:wiki:ingest`) と caller (cleanup/review/fix/close) からの sub-skill invoke の両方の経路を持ち、後者では caller 側 phase が flow-state を支配する (例: cleanup_pre_ingest)。本 Pre-check list は両経路で共通に適用する。
...
|---|-------------|------------------------|
| 0 | **Routing dispatcher** (状態質問ではない): 直前の sub-skill return tag は何か? | grep the recent output (HTML comments included) for `Lint:` 行 (lint.md Phase 9.2 emit) または stop-guard hint。`Lint:` 行 matched → **continuation trigger** — immediately run 🚨 Mandatory After Auto-Lint (writes `ingest_post_lint` if standalone, then Phase 9 Completion Report). If no recognized tag: 通常の Phase 進行中なので Item 1-2 を評価。**本 Item は YES/NO 集計から除外**。 |
...
**Caller continuation marker** (Issue #604 三点セット — sub-skill invoke 経由時のみ): When `ingest.md` is invoked from a caller (`pr/cleanup.md` Phase 4.W.2 / `pr/review.md` Phase 6.5.W / `pr/fix.md` Phase 4.6.W / `issue/close.md` Phase 4.4.W), Phase 9 also outputs a caller continuation HTML comment (`<!-- continuation: caller MUST proceed to its own Phase 5/Phase X (Completion Report). DO NOT stop. -->`) **before** the `[ingest:completed]` sentinel. This makes the caller's continuation requirement machine-readable (grep-able) while remaining invisible in rendered views. Single (`/rite:wiki:ingest`) executions skip the caller continuation marker.

**Defense-in-depth — flow-state ownership**: `ingest.md` does **NOT** directly write `cleanup_pre_ingest` / other caller phases (caller の責務)。ただし、単独実行で flow-state がそもそも `ingest_pre_lint` 等で active な場合は Phase 9 末尾で `ingest_completed` への transition を `--if-exists` patch で試みる (caller 由来の active flow-state は触らない設計のため、`--if-exists` で安全)。caller 由来の active state を上書きすると caller の terminal completion が壊れるため、本ファイル内では caller phase 名 (例: `cleanup_*`) を flow-state-update.sh の `--phase` 引数に直接書かない。
...
> **flow-state ownership** (Issue #604): `ingest.md` は **flow-state を直接書き換えない** 設計です。caller (cleanup/review/fix/close) からの sub-skill invoke 経由時、`.rite-flow-state.phase` は caller phase (例: `cleanup_pre_ingest`) に固定され、本 Phase 8.2 / Phase 9 は flow-state を触りません。`stop-guard.sh` が caller phase に対応した HINT を出すため、Lint Skill return 直後の implicit stop は caller の Mandatory After セクションが実行されることで防がれます。単独実行 (`/rite:wiki:ingest`) で flow-state がない場合は stop-guard 保護なし (既存挙動)。
...
### 🚨 Mandatory After Auto-Lint (Issue #604 — Defense-in-Depth)

> **⚠️ 同 turn 内で必ず実行すること (MUST execute in the SAME response turn)**: `rite:wiki:lint --auto` の return 直後、**応答を終了せずに** 以下の Step 1-2 を即座に実行する。Phase 9 (Completion Report) は本セクションを経由してのみ実行される唯一の経路である。
...
1. **Has `<!-- [ingest:completed] -->` been output as the absolute last line of the response?**
   - **Yes** — terminal state reached. Step 1-2 below are no-ops.
   - **No** — Phase 8.3 / 8.4 / 8.5 / 9 have NOT been completed yet. Steps 1-2 below are **critical** — execute immediately.

**Step 1**: Proceed to Phase 8.3 (Lint result parse) → 8.4 (Ingest 完了レポートへの統合) → 8.5 (n_warnings カウンタへの加算) → Phase 9 (Completion Report) **without ending the turn**. Each phase boundary is a logical step, not a turn boundary.

**Step 2**: At Phase 9, output the user-visible completion message + caller continuation HTML comment (sub-skill invoke 経由時) + `<!-- [ingest:completed] -->` HTML comment sentinel as the absolute last line.

> **Caller-side coupling**: caller (cleanup/review/fix/close) は本 sub-skill return 後に caller の Mandatory After セクション (例: cleanup.md `🚨 Mandatory After Wiki Ingest`) を実行する責務を持つ。caller 継続 HTML コメントはその責務を grep-able 形式で表現したもの (`<!-- continuation: caller MUST proceed ... -->`)。`stop-guard.sh` の HINT で caller phase 別の継続指示が出るため、本マーカーは defense-in-depth の二重防御として機能する。
...
**Step 2**: 出力する継続マーカーを判定する。

| 実行経路 | 判定方法 | 出力する HTML コメント |
|---------|---------|---------------------|
| caller (cleanup/review/fix/close) からの sub-skill invoke 経由 | `.rite-flow-state` が active (`active: true`) で、phase が `cleanup_*` / `phase5_*` / その他 caller-owned phase である | Caller 継続 HTML コメント (下記 Step 3) を `<!-- [ingest:completed] -->` の **直前** に出力 |
| 単独実行 (`/rite:wiki:ingest`) | `.rite-flow-state` が absent または `active: false`、または phase が `ingest_*` | Caller 継続 HTML コメントは出力しない (sentinel のみ) |

実装的には、bash で `[ -f .rite-flow-state ] && jq -r '.active' .rite-flow-state` で判定可能だが、実際には caller 経由 (cleanup 等から呼ばれる) が主シナリオであり、単独実行時も caller 継続 HTML コメントを出力しても無害 (該当する caller がいないため grep 結果が利用されないだけ)。**シンプルさを優先し、常に caller 継続 HTML コメントを出力する** デフォルト動作とし、単独実行時のみ省略可能とする。

**Step 3**: Output the caller continuation HTML comment (sub-skill invoke 経由時、本 step を実行):
