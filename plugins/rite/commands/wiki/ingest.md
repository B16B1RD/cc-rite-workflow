...
case "$candidate" in
  wiki:*)
    ...
    if ! file_body=$(git show "${wiki_branch}:${actual_path}" 2>&1); then
      echo "WARNING: failed to read ${actual_path} from wiki branch: $file_body" >&2
...
if [ "$branch_strategy" = "same_branch" ]; then
  ...
  #
  # 1. Raw Source の frontmatter `ingested: false` を Edit ツールで `ingested: true` に書き換え
  #    (same_branch では Raw Source は既にワークツリーに存在するため Write 上書き不要)
