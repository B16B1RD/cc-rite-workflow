...
|---|-------------|------------------------|
| 0 | **Routing dispatcher** (状態質問ではない): 直前の sub-skill return tag は何か? | grep the recent output (HTML comments included) for `Lint:` 行 (lint.md Phase 9.2 emit) または stop-guard hint。`Lint:` 行 matched → **continuation trigger** — immediately run 🚨 Mandatory After Auto-Lint (本 skill は flow-state-update.sh を呼ばないため何も書かない、そのまま Phase 9 Completion Report に進む). If no recognized tag: 通常の Phase 進行中なので Item 1-2 を評価。**本 Item は YES/NO 集計から除外**。<br><br>⚠️ **Fragility note**: `Lint:` prefix は tag 形式ではなく一般的な文字列 prefix のため、user-provided text や Phase 9 template 内の rendered 結果に混入する経路があり fragile。より堅牢にするには lint.md 側に HTML コメント sentinel (例: `<!-- [lint:completed:auto] -->`) を導入することが望ましい (別 Issue で追跡)。現状は「`Lint:` 行が会話直近に出現 **かつ** Phase 8.3 以降の文脈である」を複合条件として LLM が判断すること。unknown tag 検出時の default path は「ingest 自身の Phase 8.3 に進む」とする。 |
...
**Caller continuation marker** (Issue #604 三点セット — **Phase 9.1 設計判断により常時出力**): When `ingest.md` is invoked from a caller (現時点では `pr/cleanup.md` Phase 4.W のみ — Issue #547 以降、`pr/review.md` Phase 6.5.W / `pr/fix.md` Phase 4.6.W / `issue/close.md` Phase 4.4.W は `wiki-ingest-trigger.sh` + `wiki-ingest-commit.sh` の単一プロセス設計に移行し、Skill: `rite:wiki:ingest` を invoke しない)、Phase 9 also outputs a caller continuation HTML comment (`<!-- continuation: caller MUST proceed to its own Phase 5/Phase X (Completion Report). DO NOT stop. -->`) **before** the `[ingest:completed]` sentinel. This makes the caller's continuation requirement machine-readable (grep-able) while remaining invisible in rendered views. **シンプルさ優先の設計判断 (Phase 9.1 Step 1) により、単独 (`/rite:wiki:ingest`) 実行時も含め invoke 経路を問わず常に出力する** (単独実行時に出力しても無害 — 該当する caller がいないため grep 結果が利用されないだけ)。

**Defense-in-depth — flow-state ownership**: `ingest.md` does **NOT** call `flow-state-update.sh` at all (Phase 9.1 Step 1 設計判断)。caller 由来の active state を上書きすると caller の terminal completion が壊れるため、本ファイル内では caller phase 名 (例: `cleanup_*`) を flow-state-update.sh の `--phase` 引数に直接書かない。flow-state lifecycle は完全に caller 側 (cleanup.md Phase 4.W.2 / 🚨 Mandatory After Wiki Ingest) の責務である。
...
**Step 2**: At Phase 9, output the user-visible completion message + caller continuation HTML comment (Phase 9.1 設計判断により実行経路を問わず常に出力) + `<!-- [ingest:completed] -->` HTML comment sentinel as the absolute last line.
