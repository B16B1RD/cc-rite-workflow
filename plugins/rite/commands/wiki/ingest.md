...
4.5. **スキップ決定 Raw Source の処理** (cycle 9 HIGH fix — n_skipped カウンタ更新の契約明示): Phase 4 で「スキップ」と決定した Raw Source について、以下を実施する:
   - Edit ツールで Raw Source frontmatter の `ingested: false` を `ingested: true` に書き換える (再 Ingest 対象から除外)
   - Phase 7 の log.md 追記 step で `ingest:skip` エントリを追加する (reason も記録)
   - **`n_skipped` を +1 する** (Phase 9 完了レポート / commit message の整合性確保)
   - `processed_files[]` 配列にも append する (step 7 と同じ prefix 規約。skip 決定ファイルも wiki ブランチ側なら `wiki:` prefix を保持する)
...
if [ "$branch_strategy" = "same_branch" ]; then
  ...
  if ! git commit -m "docs(wiki): ingest {n_pages_created} new / {n_pages_updated} updated pages from {n_raw_sources} raw source(s) (skipped: {n_skipped})"; then
    ...
    if ! git reset HEAD .rite/wiki/ 2>/dev/null; then
