...
1. **エラー出力の確認** (PR #564 F-11 対応で **step 2 を先に試み、match 時は本 step 1 の異常扱いを skip する優先順位を採用**): Lint Skill の応答テキストに **step 2 の 6 フィールド正規表現がマッチしない** 前提で、以下いずれかの行が含まれる場合、Lint の主処理失敗として扱い `n_warnings += 1` と `n_lint_anomaly += 1` を加算してパースを skip します (Phase 9 完了レポート内訳の「Lint 異常経路」に計上):

   - `ERROR:` で始まる任意行 (例: `ERROR: 未知の branch_strategy 値を検出しました`)
   - 正規表現 `^WARNING: \.rite/wiki/.+ の (git show|cat) に失敗しました` にマッチする行 (Phase 6.0 の log.md 読取失敗 / Phase 6.2 の page 読取失敗 — 主処理の IO エラー)
   - 正規表現 `^WARNING: git ls-tree` にマッチする行 (Phase 2.2 の主処理失敗)
   - 正規表現 `^WARNING: .+ の sources\[\]\.ref 抽出に失敗` にマッチする行 (Phase 6.2 per-page 読取失敗)

   **重要な設計変更** (PR #564 F-11 対応): 旧実装の `WARNING: .+に失敗しました` 広域 regex は mktemp 失敗 WARNING (例: `WARNING: stderr 退避 tempfile の mktemp に失敗しました` / `WARNING: awk/sort stderr 退避 tempfile の mktemp に失敗しました`) にもマッチし、**stdout が正常な 6 フィールド 1 行を出力している状況でも二重加算が発火する silent regression** を抱えていた。本仕様では (a) step 2 (6 フィールド parse) を先に試み、match 時は step 1 を skip、(b) step 2 match しなかった場合のみ本 step 1 を narrow 版 regex で実行する優先順位に変更することで、mktemp 等の diagnostic WARNING を Lint 主処理失敗と混同しない。
   **注**: 旧来の「`WARNING: ... 実行失敗`」パターンは lint.md が実際に emit する文字列に含まれないため撤去した (PR #564 cycle 4 MEDIUM 対応、dead path 排除):
...
2. **stdout のパース**: exit 0 の場合、stdout の 1 行目を正規表現 `^Lint: contradictions=([0-9]+), stale=([0-9]+), orphans=([0-9]+), missing_concept=([0-9]+), unregistered_raw=([0-9]+), broken_refs=([0-9]+)$` で抽出し、6 つの変数を会話コンテキストに保持します:
...
4. **1 行目が正規表現にマッチしない場合**: Lint 側のフォーマット変更を検出した警告として扱い、以下を stderr に出力してから全変数を `0` に設定し、`n_warnings` と `n_lint_anomaly` に各 1 を加算します（silent に 0 件と誤認することを防ぐ。step 1 / step 3 と対称に「Lint 異常経路」として計上）。旧 5 フィールド形式（`missing=N`）の stdout が流れてきた場合もここに fallback するため、ingest.md / lint.md の同期 merge を徹底すること:
