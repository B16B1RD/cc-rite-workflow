...
`rite-config.yml` から Wiki 設定を読み取ります。`init.md` Phase 1.1 と同じ判定ロジックを使用します (判定結果は同一ですが、init.md のような INFO メッセージ「デフォルト値 'false' を使用」は省略しています):
...
```bash
# F-05 fix: trigger.sh F-23 修正済みパターンに統一 (sed 's/[[:space:]]#.*//')
wiki_section=$(sed -n '/^wiki:/,/^[a-zA-Z]/p' rite-config.yml 2>/dev/null) || wiki_section=""
wiki_branch_line=""
if [[ -n "$wiki_section" ]]; then
...
case "$candidate" in
  ...
  *)
    ...
    # F-13 fix: cat 失敗時のエラーハンドリング (silent data loss 防止)
    if ! file_body=$(cat "$actual_path" 2>/dev/null); then
      echo "WARNING: failed to read ${actual_path} (permission denied / file disappeared)" >&2
      echo "  この候補をスキップして次の Raw Source に進みます" >&2
      continue
    fi
    ;;
...
>
> Block A の最後で `trap - EXIT INT TERM HUP` により trap が解除され、Block B の冒頭で再武装する必要はありません。Block A のシェル状態は失われるため、LLM Write/Edit phase 中にエラーが発生した場合は LLM が `git checkout "$current_branch" && git stash pop` を含む cleanup Block を即座に発行する責務を負います。これは 1 回の bash invocation に収めるべき設計だが、Claude Code の Bash tool は LLM の Write/Edit を bash の中間に挟めないための制約による (将来 1 ブロック実行可能になれば統合される)。
...
if [ "$branch_strategy" = "separate_branch" ]; then
  ...
  _rite_wiki_ingest_cleanup() {
    ...
    if [ -n "$current_branch" ]; then
      cleanup_err=$(mktemp /tmp/rite-wiki-ingest-cleanup-err-XXXXXX 2>/dev/null) || cleanup_err=""
      # cycle 2 H6 fix: stderr を保持して原因を可視化 (2>/dev/null は禁止)
      if ! git checkout "$current_branch" 2>"${cleanup_err:-/dev/null}"; then
        echo "WARNING: cleanup の git checkout '$current_branch' に失敗。手動で元ブランチに戻ってください" >&2
        [ -n "$cleanup_err" ] && [ -s "$cleanup_err" ] && head -3 "$cleanup_err" | sed 's/^/  /' >&2
      fi
    fi
    if [ "${stash_needed:-false}" = true ]; then
      cleanup_err=$(mktemp /tmp/rite-wiki-ingest-cleanup-err-XXXXXX 2>/dev/null) || cleanup_err=""
      if ! git stash pop 2>"${cleanup_err:-/dev/null}"; then
        echo "WARNING: cleanup の git stash pop に失敗。手動回復が必要: git stash list" >&2
        [ -n "$cleanup_err" ] && [ -s "$cleanup_err" ] && head -3 "$cleanup_err" | sed 's/^/  /' >&2
      fi
    fi
    [ -n "${cleanup_err:-}" ] && rm -f "$cleanup_err"
  }
...
# F-05 fix: {n_pages_created} / {n_pages_updated} / {n_raw_sources} は整数値に置換すること
...
**same_branch 実行モデル** (Phase 5.1 の Block A / Write-Edit / Block B と同じ 3 段構成):
...
| `{confidence}` | F-12/F-27 fix: **page-template.md の `confidence: medium` はリテラル値**であり `{confidence}` プレースホルダーではない。書き込み時に `sed 's/confidence: medium/confidence: {Phase 4 の判定値}/'` または Edit ツールで `confidence: medium` を `confidence: high` / `confidence: medium` / `confidence: low` のいずれかに置換する。他の `{...}` プレースホルダーとは処理方式が異なるため注意 |
