...
**Defense-in-depth — flow-state ring** (Issue #618, supersedes the former #608 follow-up YAGNI stance): `ingest.md` は Phase 8.2 直前 (Pre-write) / 🚨 Mandatory After Auto-Lint Step 1 (post-lint patch) の 2 箇所で `flow-state-update.sh` を呼ぶ。caller 経由時は caller phase (例: `cleanup_pre_ingest`) を一時的に `ingest_pre_lint` / `ingest_post_lint` に上書きし、sub-skill return 後 caller の Mandatory After (例: cleanup.md `🚨 Mandatory After Wiki Ingest`) が caller phase (`cleanup_post_ingest`) に書き戻す ring 構造で完遂する。単独実行 (`/rite:wiki:ingest`) 時は flow-state が不在 (caller 未起動) のため `--if-exists` フラグで no-op となり、従来挙動と互換。本反転の目的は Issue #621 (cleanup→ingest) と同型の Mode B 症状 (ingest→lint return 後の implicit stop) を多層防御で防ぐこと (Issue #618 AC-2/AC-3/AC-4)。
...
<!-- >>> DRIFT-CHECK ANCHOR: 🚨 Mandatory After Auto-Lint ring (Issue #618) <<<
     本 section の Self-check + Step 1-3 構造は cleanup.md `🚨 Mandatory After Wiki Ingest` と
     semantic 対称 (両者とも同型 numbered list)。また stop-guard.sh `ingest_pre_lint` /
     `ingest_post_lint` case arm と phase-transition-whitelist.sh `ingest_pre_lint` entry は
     本 ring の enforcement 層を構成する。いずれかを変更する場合、以下 3 site を同時確認:
       - plugins/rite/commands/pr/cleanup.md `🚨 Mandatory After Wiki Ingest` Step 1
...
1. **Has `<!-- [ingest:completed] -->` been output as the absolute last line of the response?**
   - **Yes** — terminal state reached (caller 経由時は caller Mandatory After により `cleanup_post_ingest` などに書き戻し済み、単独実行時は flow-state 不在 or `completed`). **本 Yes 分岐は terminal 到達後の重複呼び出し防止のための例外経路**であり、non-terminal (phase=`ingest_pre_lint`) 時点の Step 1-3 正規路と矛盾しない。Step 1-3 below MUST be skipped. 理由: terminal 後に Step 1 の `patch --phase ingest_post_lint --if-exists` を再実行すると phase を巻き戻して flow-state を破壊する (cleanup.md `🚨 Mandatory After Wiki Ingest` Self-check と同じ rationale)。
   - **No** — Phase 8.3 / 8.4 / 8.5 / 9 have NOT been completed yet (phase=`ingest_pre_lint` など non-terminal 状態). Steps 1-3 below are **critical** — execute immediately.
...
**Step 3 (terminal patch — Issue #618)**: Step 2 の sentinel が応答に含まれることを保証した上で、`.rite-flow-state` を `ingest_completed` (active=false) に patch する。本 step は **Step 2 の絶対最終行性を壊さない** ため bash block は応答末尾 sentinel 行より前 (Step 2 記述直前) に配置し、実際の実行は Step 1 出力後・Step 2 出力前のタイミングで行うこと。caller 経由時は直後に caller Mandatory After が `cleanup_post_ingest` に書き戻すため、本 deactivate は caller による書き戻しと衝突しない (whitelist で `ingest_completed → cleanup_post_ingest` 遷移を許可):
