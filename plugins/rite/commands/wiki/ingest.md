...
### 1.2 Wiki 初期化判定と worktree セットアップ

Phase 1.1 で取得した `branch_strategy` と `wiki_branch` を使い、Wiki が初期化済みかを判定します。`separate_branch` 戦略では、wiki ブランチがローカルに存在することと `.rite/wiki-worktree/` worktree が有効に存在することを両方確認します（Issue #547）:
...
if [ "$branch_strategy" = "separate_branch" ]; then
  ...
  elif ! bash "$plugin_root/hooks/scripts/wiki-worktree-setup.sh" >/dev/null 2>&1; then
    setup_rc=$?
    echo "WIKI_INITIALIZED=false"
...
```bash
# Phase 1.2 の値をリテラル値として埋め込む (例: branch_strategy="separate_branch", wiki_branch="wiki")
branch_strategy="{branch_strategy}"
wiki_branch="{wiki_branch}"
...
# (PR #XXX 以前の stash + checkout 経路で書き込まれた残骸がある場合を検出)
if [ "$branch_strategy" = "separate_branch" ] && [ -d ".rite/wiki/raw" ]; then
  drift_count=$(find .rite/wiki/raw -type f -name '*.md' 2>/dev/null | wc -l | tr -d '[:space:]')
...
4.5. **スキップ決定 Raw Source の処理**: Phase 4 で「スキップ」と決定した Raw Source について、step 2 と同じ手順で `ingested: true` 化を行い、Phase 7 の log.md 追記 step で `ingest:skip` エントリを追加する (reason も記録)。`n_skipped` を +1 する
5. **index.md の更新**: Phase 6 の指示に従い Edit で `.rite/wiki-worktree/.rite/wiki/index.md` (separate_branch) または `.rite/wiki/index.md` (same_branch) を更新する
6. **log.md への追記**: Phase 7 の指示に従い Edit で `.rite/wiki-worktree/.rite/wiki/log.md` (separate_branch) または `.rite/wiki/log.md` (same_branch) に append-only でエントリを追加する
...
上記 Phase 5.0 手順 1-6 を Write/Edit ツールで実施した後、以下の単一 bash ブロックを実行して worktree 内の変更を commit + push します。commit 処理は `wiki-worktree-commit.sh` に完全委譲されており、LLM が bash 契約を書く必要はありません:

```bash
# Phase 1.2 の値をリテラルで埋め込む (例: branch_strategy="separate_branch")
branch_strategy="{branch_strategy}"

if [ "$branch_strategy" = "separate_branch" ]; then
  # plugin_root は Phase 1.3 で解決済み。LLM はリテラル値を substitute すること
  plugin_root="{plugin_root}"

  # {n_pages_created} / {n_pages_updated} / {n_raw_sources} / {n_skipped} は Phase 2.1 で
  # 初期化され Phase 4 / 5.0 step 4.5 で incrementate されたカウンター変数を整数値に substitute する
...
**実行モデル**: `same_branch` 戦略では Raw Source / ページ / index.md / log.md はすべて現在の dev ブランチのワークツリーに存在します。Phase 5.0 の手順 1-6 を Write/Edit ツールで実施した後、以下の bash ブロックで一括 commit します。ブランチ切り替えは発生しません (worktree も不要):
...
| Wiki 未初期化 / worktree セットアップ失敗 | `/rite:wiki:init` を案内、または `wiki-worktree-setup.sh` のエラー出力を確認して `git worktree prune` / `git fetch origin wiki:wiki` で復旧（Phase 1.2） |
