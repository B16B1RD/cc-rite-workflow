...
> | **(LLM Write/Edit phase)** | 下記 step 1-6 を Write/Edit ツールで実施 + `processed_files[]` を会話コンテキストに保持 | (bash 不要) |
...
>
> Block A の最後で trap が解除され、Block B の冒頭で再武装する必要はありません。Block A のシェル状態は失われるため、Block B では失敗時に手動回復する前提で動作します (致命的失敗時は cleanup 不可)。これは 1 回の bash invocation に収めるべき設計だが、Claude Code の Bash tool は LLM の Write/Edit を bash の中間に挟めないための制約による (将来 1 ブロック実行可能になれば統合される)。
...
2. **Raw Source の wiki ブランチ側への配置** (separate_branch 戦略のみ):
   - **Block A 実行後** (= `git checkout "$wiki_branch"` 完了後)、Write ツールで `.rite/wiki/raw/{type}/{filename}` に Raw Source 本文を書き出す
...
```bash
# Block A から渡された値を literal substitute する (例: current_branch="develop", stash_needed=false)
current_branch="{current_branch_from_block_a}"
stash_needed="{stash_needed_from_block_a}"
wiki_branch="{wiki_branch}"

# cycle 2 C1 fix: processed_files[] を bash 配列として明示的に宣言する。
# LLM は Phase 5.0 step 7 で会話コンテキストに保持したパス一覧を、
# `processed_files=("path1" "path2" ...)` のリテラル bash 配列宣言として substitute すること。
# 例: processed_files=(".rite/wiki/raw/reviews/foo.md" "wiki:.rite/wiki/raw/fixes/bar.md")
# この宣言を忘れると下流の cleanup ループが silent no-op になる。
processed_files=({processed_files_literal})
