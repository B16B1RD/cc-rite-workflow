...
**Defense-in-depth — flow-state ownership**: `ingest.md` does **NOT** call `flow-state-update.sh` at all (Phase 9.1 「flow-state ownership」設計判断)。caller 由来の active state を上書きすると caller の terminal completion が壊れるため、本ファイル内では caller phase 名 (例: `cleanup_*`) を flow-state-update.sh の `--phase` 引数に直接書かない。flow-state lifecycle は完全に caller 側 (cleanup.md Phase 4.W.2 / 🚨 Mandatory After Wiki Ingest) の責務である。
