...
case "$branch_strategy" in
  separate_branch)
    if log_content=$(git show "${wiki_branch}:.rite/wiki/log.md" 2>"${log_err:-/dev/null}"); then
      ...
      # - `does not exist`: blob not found
      # - `path '...' exists on disk, but not in '...'`: git show の path 対 ref 不整合
      # - `Not a valid object name`: revspec 自体不正 (legitimate 初期状態)
      # - `fatal: invalid object name '<ref>:.rite/wiki/log.md'`: blob path 指定で log.md 不在
      # 対照的に `fatal: invalid object name '<ref>'` (blob path なし) は wiki_branch 自体の
      # race 消失を示唆するため io_error 扱いとする (Phase 1.3 後の race 検出)。
      if [ -n "$log_err" ] && [ -s "$log_err" ] && \
        ...
        # stderr 退避失敗 + git show 失敗: 原因区別不能のため safe side で io_error 扱い
        log_read_ok="io_error"
      fi
      log_content=""
    fi
