...
# Downstream reference: same file Phase 8.3 {log_entry} 組み立て — 本 emit 値 (lint_action=...) を
#   single source of truth として参照する。Wiki 経験則 patterns/high「DRIFT-CHECK ANCHOR は semantic name
#   参照で記述する（line 番号禁止）」の bidirectional backlink sub-pattern に準拠 (PR #605 / Issue #607)。
