...
> ⏭ 継続中: caller (rite:wiki:ingest Phase 8.3-8.5 → Phase 9 Completion Report → 三点セット終端) を同 turn 内で完了してください。
<!-- [lint:completed:auto] -->
```
...
2. **caller 継続 blockquote** (`> ⏭ 継続中: ...`): LLM / user 両方に caller の Mandatory After 起動を促す user-visible reminder。LLM turn-boundary heuristic の fragile 誘発源 (plain-text `Lint:` 1 行で turn 終了) を抑止する
