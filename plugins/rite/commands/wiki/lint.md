...
# 本 Block は per-iteration で page_err tempfile を作成するため、SIGINT/TERM/HUP 到達時の
# 当該 iteration 分の orphan を防ぐ必要がある (bash-trap-patterns.md の Instantiation Checklist
# 「trap 設置は mktemp / 主処理の前」を遵守、Phase 2.2/6.0/8.2 と対称化)。
page_err=""
...
all_source_refs_read_ok="unknown"  # 3 値 enum: unknown / true / io_error (Phase 6.0 log_read_ok の 4 値とは異なる。partial_error は予約のみで未使用 — 全件 IO error を io_error に畳み込む設計)
...
for page in $pages_list; do
  ...
  if page_content=$(git show "${wiki_branch}:$page" 2>"${page_err:-/dev/null}"); then
    ...
    if [ -n "$page_err" ] && [ -s "$page_err" ] && grep -qE "does not exist|path '.+' exists on disk, but not in|Not a valid object name|fatal: invalid object name '[^']*:" "$page_err"; then
      ...
      # 集合には追加しないが read_ok は下げない (partial_error にしない)
      :
    else
      # 真の IO error: partial_error に降格し件数をカウント
...
if [ -n "$all_source_refs" ]; then
  ...
  normalized=$(printf '%s\n' "$all_source_refs" | LC_ALL=C sort -u 2>"${sort_err:-/dev/null}" | grep -v '^$')
  sort_rc=$?
...
| いずれかが非空 (`io_error`) | **3 行すべてそのまま展開** (空行 + 警告文 + 空行、warning が 2 つあれば 2 行連続表示し空行 1 行で区切らない) |
