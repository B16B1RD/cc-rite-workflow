...
exit 0 で終了。`--auto` モードでも stdout には必ず 6 フィールドの 1 行を出力する（0 件でも `Lint: contradictions=0, stale=0, ..., broken_refs=0`）。stdout 空は ingest 側 Phase 8.3 step 3 で「Lint 実行失敗」として扱う unreachable 経路である。
...
all_source_refs_read_ok="unknown"  # 4 値 enum: unknown / true / partial_error / io_error (Phase 6.0 と対称)
...
if [ -n "$all_source_refs" ]; then
  all_source_refs=$(printf '%s\n' "$all_source_refs" | LC_ALL=C sort -u | grep -v '^$')
fi
...
LLM は Phase 6.0 bash block の stdout から `log_read_ok={value}` を読み取り、値に応じて以下を展開する。**Phase 6.0 が実行されなかった場合** (Phase 2.2 末尾で `pages_list` と `raw_list` が両方空で Phase 3-7 が skip された経路、`log_read_ok=...` stdout 行が emit されない) は、下表の `(未 emit)` 行に従い両 placeholder を空文字列として展開する:
...
| `(未 emit: Phase 6.0 skip、処理対象 0 件)` | 空文字列 | 空文字列 |

**空行処理ルール (3 行ブロック原子的扱い)**: template は `{log_read_ok_warning}` を中心に **3 行ブロック**を持つ:

1. 直前の空行 (前段「壊れた相互参照」行との区切り)
2. `{log_read_ok_warning}` placeholder 行
3. 直後の空行 (後段「検出詳細:」との区切り)

LLM は以下のルールで **3 行すべてを原子的に展開**する (部分的な削除・保持は禁止):

| `{log_read_ok_warning}` の値 | 3 行ブロックの展開 |
|------------------------------|-------------------|
| 空文字列 (`true` / `absent`) | **3 行すべて省略** (前段「壊れた相互参照」行の直後に後段「検出詳細:」行を隣接させる) |
| 非空 (`io_error` のみ) | **3 行すべてそのまま展開** (空行 + 警告文 + 空行) |
