...
case "$branch_strategy" in
  separate_branch)
    ls_err=$(mktemp /tmp/rite-wiki-lint-ls-err-XXXXXX) || {
...
### 8.2 書き込みブランチ判定（state-gated）

`--auto` モードであっても、ユーザーが手動で `/rite:wiki:lint --auto` を実行した場合には wiki ブランチがチェックアウトされていないため、**実際のブランチ状態を `git branch --show-current` で検証**してから書き込み方式を選択します（flag-gated ではなく state-gated に変更）:
...
if [ "$branch_strategy" = "same_branch" ]; then
  # 同一ブランチ戦略: 直接 Edit で .rite/wiki/log.md に追記
  write_mode="direct_edit"
elif [ "$current_branch" = "$wiki_branch" ]; then
  # 既に wiki ブランチにいる（Ingest の trap 内から呼ばれた正常経路）
  write_mode="direct_edit"
else
  # separate_branch 戦略だが wiki ブランチではない → stash + checkout パターンが必要
  write_mode="stash_checkout"
fi
...
**`write_mode=direct_edit`** の場合: 直接 Edit ツールで `.rite/wiki/log.md` に append するか、wiki ブランチチェックアウト済みで commit + push を行います。

**`write_mode=stash_checkout`** の場合: 以下の bash block を実行します。ingest.md Phase 5 Block A/B の wiki ブランチ書き込みパターンを踏襲し、signal-specific trap で保護します:

```bash
# Phase 8.3 stash_checkout 経路: wiki ブランチへの log.md 追記
wiki_branch="{wiki_branch}"
log_entry="{log_entry}"  # Phase 8.1 で生成した log.md 追記行

# パス先行宣言 → trap 先行設定 → stash 操作の順序で orphan race window を排除
current_branch=$(git branch --show-current 2>/dev/null || echo "")
stash_applied=0

_rite_wiki_lint_log_cleanup() {
  if [ "$stash_applied" = "1" ]; then
    git checkout "$current_branch" 2>/dev/null || true
    git stash pop 2>/dev/null || {
      echo "WARNING: git stash pop に失敗しました。手動で git stash list を確認してください" >&2
    }
  fi
}
trap 'rc=$?; _rite_wiki_lint_log_cleanup; exit $rc' EXIT
trap '_rite_wiki_lint_log_cleanup; exit 130' INT
trap '_rite_wiki_lint_log_cleanup; exit 143' TERM
trap '_rite_wiki_lint_log_cleanup; exit 129' HUP

# 1. stash で work tree を退避
if ! git stash push --include-untracked -m "rite-wiki-lint log.md append" 2>&1; then
  echo "WARNING: git stash に失敗しました。log.md 追記を skip します" >&2
  exit 0  # 非ブロッキング
fi
stash_applied=1

# 2. wiki ブランチに切替
if ! git checkout "$wiki_branch" 2>&1; then
  echo "WARNING: wiki ブランチへの切替に失敗しました。log.md 追記を skip します" >&2
  exit 0  # 非ブロッキング (trap が stash pop を実行)
fi

# 3. Edit ツールで log.md に append (LLM が実行)
# 4. commit
git add .rite/wiki/log.md 2>/dev/null
git commit -m "docs(wiki): lint report — ${log_entry}" 2>/dev/null || {
  echo "WARNING: log.md のコミットに失敗しました" >&2
}

# 5. push
git push origin "$wiki_branch" 2>/dev/null || {
  echo "WARNING: wiki ブランチへの push に失敗しました。次回 Ingest 時に同期されます" >&2
}

# 6. trap が元ブランチ復帰 + stash pop を実行
```

上記 bash block 内の step 3 (Edit ツール操作) は Claude が bash block の外で実行するため、以下の手順に分解します:
...
検出件数が全て 0 の場合は stdout を空にして exit 0。
