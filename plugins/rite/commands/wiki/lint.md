...
index_read_ok="true"
if [ "$branch_strategy" = "separate_branch" ]; then
  if ! index_content=$(git show "${wiki_branch}:.rite/wiki/index.md" 2>&1); then
    echo "WARNING: index.md を wiki ブランチから読み出せません: $index_content" >&2
    echo "  影響: Phase 5（孤児ページ検出）を skip します（非ブロッキング）" >&2
    index_content=""
    index_read_ok="false"
  fi
else
  if ! index_content=$(cat .rite/wiki/index.md 2>&1); then
    echo "WARNING: .rite/wiki/index.md を読み出せません: $index_content" >&2
    echo "  影響: Phase 5（孤児ページ検出）を skip します（非ブロッキング）" >&2
    index_content=""
    index_read_ok="false"
  fi
...
各ページについて以下を実行します。`date -d` 失敗時は log ファイルに WARNING を出力し、該当ページを skip（`n_stale` には加算しない）することで実装と仕様を一致させます:

```bash
# page_content は Phase 2 で取得した各ページの本文
...
if ! updated_epoch=$(date -d "$updated_str" +%s 2>&1); then
...
```bash
# pipefail を有効化して pipeline 途中の失敗を検出
set -o pipefail

indexed_pages=$(printf '%s\n' "$index_content" \
  | { grep -E '^\|.*\]\((\.{0,2}\/?pages/[^)]+)\)' || true; } \
  | sed -E 's/.*\]\(((\.{0,2}\/?)pages\/[^)]+)\).*/\1/' \
  | sed -E 's|^\.{0,2}/?||' \
  | sort -u)
...
if [ -z "$indexed_pages" ]; then
  ...
  echo "  影響: Phase 5 を skip します（全ページを orphan と誤検出しないため）" >&2
  # skip して次の phase へ
fi
```
...
# 画像リンク（! prefix）を除外するため、行頭 / 空白 / 記号直後の `](...)` のみをマッチ
# 続く sed で `!` prefix を検出した場合はその行を除外
...
**`write_mode=stash_checkout`** の場合: ingest.md Phase 5 の wiki ブランチ書き込みパターン（references/wiki-patterns.md の該当セクション参照）を踏襲し、以下の 8 ステップを signal-specific trap で保護します:

1. `current_branch` と `stash_applied` をパス先行宣言
2. `trap` を EXIT/INT/TERM/HUP 4 種で設置（異常終了時に元ブランチ復帰と stash pop を保証）
3. `git stash push --include-untracked` で work tree を退避（`stash_applied=1`）
4. `git checkout {wiki_branch}` で wiki ブランチに切替
