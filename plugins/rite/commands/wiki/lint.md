...
for page in $pages_list; do
  ...
  case "$branch_strategy" in
    separate_branch)
      page_read_cmd_result=$(git show "${wiki_branch}:$page" 2>"${page_err:-/dev/null}")
      page_read_cmd_rc=$?
      ;;
    same_branch)
      page_read_cmd_result=$(cat "$page" 2>"${page_err:-/dev/null}")
      page_read_cmd_rc=$?
      ;;
    *)
      echo "ERROR: Phase 6.2 で未知の branch_strategy 値を検出しました: '$branch_strategy'" >&2
      echo "  対処: rite-config.yml の wiki.branch_strategy を 'separate_branch' または 'same_branch' に設定してください" >&2
      echo "  本エラーは設定ミスを silent に通過させないための fail-fast です（Phase 2.2 / 2.3 / 6.0 と同型）" >&2
      [ -n "$page_err" ] && rm -f "$page_err"
      exit 1
      ;;
  esac
