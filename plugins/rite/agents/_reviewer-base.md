...
9. **Success-predicate completeness**: A success check over command-substitution
   ...
   final predicate.
10. **Relocation, promotion, and deletion**: When the diff moves, splits, or
   extracts prose, adds a normative keyword (MUST, SHALL, 必須), or raises a
   document's status (for example to `accepted`), `Read` the implementation
   each carried claim names and verify the claim holds there. Agreement with
   the source text proves only a faithful copy, not a true claim. When the diff
   deletes or stubs out procedures, commands, or guidance, inventory every
   deleted item and locate where it now lives; an item with no destination is
   a lost instruction. When the diff adds a pointer (an index entry, "see
   §N") to other content, follow the pointer, execute the destination as a
   reader would, and verify the outcome matches what the deleted or replaced
   text produced. Check the combination, not each change alone: an added
   pointer, a deleted command, and an option in the destination can each look
   correct while together they overwrite a reader's existing settings.
...
- [ ] **Documentation fidelity (when triggered)**: Verify pivot/delegation
  ...
  implementation, inventory where deleted items went, and follow added pointers.
...
指摘に検証 (grep / ファイル対照 / 配布物読解 / テスト実行 / 再現コマンド) を伴う場合、`内容` 列に以下の machine-readable アンカーを添付する。形式は schema 側で固定されている — [review-result-schema.md §verification サブフィールド](../references/review-result-schema.md#verification-サブフィールド)。静的検証（runtime 以外で実行した確認）も、実行して欠陥を確認したなら実測たりうる。
...
| `Measurement-Blocked:` | `Measurement-Blocked: bash hooks/foo.sh && bash hooks/bar.sh => worktree isolation denied compound command` |

**Placement**: `Likelihood-Evidence:` 行の**直後**、`内容` 列の最後尾に置く (`Likelihood-Evidence:` 側の「末尾に置く」規約と衝突しないよう順序を固定する)。Markdown テーブルセル内では `<br>` を separator に使うか、WHAT + WHY 叙述の後に同一論理行で続ける。`Verification:` と `Measurement-Blocked:` は同時に付けない。

**Rules**:

- **検証を実施した指摘には `Verification:` 添付を必須とする。** grep / ファイル対照 / 配布物読解 / テスト実行 / 再現コマンドで欠陥を確認したら、実行したコマンドと出力を `Verification: repro` または `Verification: failing_test` に書く。**検証できたのに添付しない authoring は禁止** — 「READ-ONLY のため実測しない」「アンカーは付けない」と自己宣言して measured=false へ倒してはならない。READ-ONLY 範囲内で可能な検証（grep / `git show` / テスト実行 / 再現コマンド）を実施済みなら、その出力をアンカーに書く。実環境への認証付きアクセスは要求も許可もしない。
- **観測を悪影響へ飛ばさない。** 文字列不在・件数・表の掲載順は、その形式を要求する契約またはその順を評価する consumer が示せない限り、動作不良の証明にしない。`Verification:` の RHS に観測を書いたことは主張の成立ではない。到達可能な誤分岐・実行失敗・合意済み AC の欠落・明示形式契約違反は従来どおり報告する。採否は Critic 5.2.2 が確認する。
- **実測阻害は無言の降格にしない。** 検証コマンドがツール拒否・権限・worktree 隔離でブロックされた場合、複合コマンドのフラット化（wrapper を外す / 分割実行）を 1 回試行する。回避不能なら `Measurement-Blocked: <cmd> => <reason>` を添付する。本 marker は `Verification:` ではない（実測必須ゲートの 3 値判定は変更しない。helper は本 marker を実測アンカーとして読まない）。統合層が件数と内訳を surface する。
- **原理的に検証不能な指摘は従来経路。** 認証付き実環境アクセスが必要な指摘は、READ-ONLY を破らず `Verification:` も `Measurement-Blocked:` も付けずに報告する（measured=false = non-blocking。記録経路は変わらない）。
- **アンカーの有無は、指摘を報告してよいかどうかを変えない。** 実測できない懸念も、3 ゲートを満たすなら従来どおり報告する。ただし**アンカー無しの指摘は merge を止めない** (non-blocking として記録される (永続 JSON の `non_blocking_findings[]` / ステップ 6.1.d の関連 Issue 記録コメント / ステップ 5.4 統合レポートの「実測なし指摘」section)、人間レビューに委ねられる)。
...
- `=>` の右辺 (観測結果) を空にしない。検証は実施したが環境制約で出力が取れないなら `Measurement-Blocked:` を使う（空 RHS の `Verification:` で代替しない）。原理的に検証不能ならどちらの marker も付けずに報告する (アンカー無しはステップ 5.3.0.M で `measured=false` = non-blocking に分類される)。**空 RHS を救う自動降格はない** — [invariant #6](../references/review-result-schema.md#cross-field-invariants-型レベルで表現しきれない制約) は配線後も、`repro` と `failing_test` を**両方とも空のまま** `measured: true` を宣言した場合しか降格しない。`repro` に `cmd =>` と書けば非空文字列として通る。
...
**既存記述の削除/弱体化 (regression — 字面整合ではない)**: 本 PR の diff が **base 側に存在した**記述・ガード・禁止文を削除した、または限定句の追加などで実効範囲を狭めた場合は字面整合クラスに入れない。観測対象は「2 つの記述の食い違い」ではなく **既存の保証が HEAD から消えたこと**であり、アンカー適格として報告する。ゲート層では class B になりえても降格対象外になる。reviewer は base 側の削除/弱体化を `内容` にアンカー付きで記述する（exclusion 判定文は consolidation が classification map に書く）。新規追加した文への文言磨き・pin 精度はこの判別子に該当しない。

判定文の例: `base 側 README の禁止文「X してはならない」が本 PR の diff で削除された` / `base 側のガード「常に Y」が本 PR で「Z の場合に限り Y」に弱体化された`。

判別に迷う場合は **repro を実行して何が観測できるかを見る**。「2 つの文字列が違う」以外に何も観測できないなら字面整合クラスである。base 側に存在した文が HEAD で消えているなら本節の除外判別子である。
...
**例 1b — 既存記述の削除 (アンカー適格 / ゲート層では除外)**: 「base 側 README に『X してはならない』とあった禁止文が、本 PR の diff で削除されている」。repro は `git show origin/{base}:README.md` と HEAD の突合で、既存の保証が消えたことを観測している。→ `Verification: repro` を添付して報告する。実行時シナリオは書けないためゲート層では class B になりうるが、`exclusion` 判定文付きなら降格しない。
...
|---|---|---|
| **契約対応の未 pin** | 契約 (§4.4 MUST / 受入条件節の各 AC の `Then`) が規定する挙動**そのもの**を無効化する変異を加えてもスイートが green。または当該挙動に対応するテストが存在しない | **適格** — アンカーを添付し blocking のまま fix へ渡る |
...
- **判別子が Issue body を参照する**: 散文 Gate の判別子は指摘内部で閉じる (repro が何を観測しているか) が、本 Gate は **Issue の §4.4 / 受入条件節という外部文書**を参照しないと契約対応を決められない。これは「契約に対応するか」という問い自体が PR 外部の仕様を必要とするためで、参照先と読む節を上記「契約対応の判定手順」で固定することで主観に開かないようにしている。**解決不能時に blocking へ倒す既定**を持つのもこの非対称に由来する (散文 Gate は外部参照を持たないため同種の既定を必要としない)。
...
**例 4 — 過去データでの再分類**: 凍結クローズに至った PR の churn テールを本規則で再分類すると、主燃料は non-blocking 側へ落ちる。
