...
| **移送済み (非致命)** | 上記以外の gated finding（`measured == true` かつ MEDIUM / LOW-MEDIUM / LOW）。**file-based source (Priority 0/2、および P3 の Raw JSON) でのみ発生**（P3 は下行が優先） | `non_blocking_findings[]` へ移送。修正・reply・auto-select の対象外。件数のみ表示 |
