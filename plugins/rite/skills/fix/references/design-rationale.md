...
**なぜ marker 不在時に 1.4 の「移送済み」セクションだけ見出しごと省略するか**: 他 3 サイト
(4.5.3 / 4.6 / 5.1) は件数を本文中の 1 項目として持つため `不明` に置換すれば成立するが、1.4 の
当該セクションは件数だけの節であり `不明` の見出しは意味を持たない。加えて見出し自体が
「記録済み・sweep 対象」と述べており、移送も記録も起きていない経路でこれを出すと事実に反する。
