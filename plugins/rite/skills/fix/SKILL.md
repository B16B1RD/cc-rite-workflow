  ...
  case "$schema_version" in
    "1.0.0"|"1.0"|"1.1.0")
      ...
      if p3_moved=$(jq '[.non_blocking_findings[]? | select(.demotion_reason == "non_fatal")] | length' "$p3_triage_file" 2>/dev/null); then
        ...
        p3_moved_rc=$?
        rm -f "$p3_triage_file"
        echo "ERROR: Priority 3 仕分け結果から移送件数を読み取れませんでした (rc=$p3_moved_rc)" >&2
        echo "  影響: 移送が起きたかどうかを判定できないため、記録漏れを見逃さないよう停止します" >&2
        echo "[CONTEXT] FIX_FALLBACK_FAILED=1; reason=p3_triage_moved_probe_failed; rc=$p3_moved_rc" >&2
        echo "[fix:error] reason=p3_triage_moved_probe_failed"
        exit 1
      fi
      case "$p3_moved" in ''|*[!0-9]*)
        rm -f "$p3_triage_file"
        echo "ERROR: Priority 3 仕分け結果の移送件数が数値ではありません: '$p3_moved'" >&2
        echo "[CONTEXT] FIX_FALLBACK_FAILED=1; reason=p3_triage_moved_probe_failed; value=$p3_moved" >&2
        echo "[fix:error] reason=p3_triage_moved_probe_failed"
        exit 1
        ;;
      esac
...
**review-findings-maps.sh reasons** (helper が `[CONTEXT] REVIEW_SOURCE_*` / `FIX_FATAL_TRIAGE` / `FIX_FALLBACK_FAILED` を emit。**Priority 3 も同一 helper を通る**ため鏡像 emit は存在せず、下記はすべて helper 単独の語彙。SoT は helper docstring):
...
| **全 severity 帯 (仕分け未適用)** | `FIX_FATAL_TRIAGE` marker 不在 (会話 / legacy Markdown 経路) の gated finding のうち **`measured != false`** のもの。severity で絞らない (`measured == false` は真下の non-blocking 行が引き取る — 母集団を重ねると同一 finding に逆の Action が付く) | Must fix in this PR。仕分けが走っていないため移送も記録も起きておらず、絞ると指摘が無記録のまま消える (本行は ステップ 2.1 分岐 5 の第 1 disjunct と同一母集団。scope 未登録 / enum 外は分岐 5 の第 2 disjunct が受け、本表に行を持たない) |
...
<!-- ステップ 1.2.0 の致命性仕分けで non_blocking_findings[] へ移送された gated finding。
...
5. **仕分けが走らなかった経路 (会話 / legacy Markdown。marker なし)**: `scope ∈ {current-pr, follow-up}` かつ **measured != false**、**または scope 未登録 / enum 外 (legacy / fallback / 外部ツール由来)** → severity に依らず本セクション以降を通常通り実行する。consumer 式で絞ると移送も記録もされないまま指摘が消えるため、この経路では従来どおり全 severity 帯を修正対象にする (Confidence override で取り込んだ外部ツール finding は scope を持たないため後段の disjunct でここに合流し、silent skip されない)
