...
1. Resets flow state to `active: false` (prevents `post-tool-wm-sync.sh` from recreating files). Other PRs' parked review runs (`review_run_history`) and the abandonment records they are validated against (`review_cycle_abandoned`) are kept, so a deferred PR still gets its run back when the session returns to it
