...
- `CLEANUP_WT=in_main`（resume 等で既に main 復帰済み）: 所有する保存 state / claim / 対象 branch と登録 worktree の照合、対象の dirty ゲート、共通作業先契約の退出検証を通す。矛盾・他 live claim は削除前に停止する。worktree が残っていれば 3 を実行（既削除なら 3 もスキップ = 冪等）。in_main では所有セッションが別セッションの可能性があるため、3 の self-exclusion 付き live-cwd guard が特に重要（live-cwd guard による遅延は別セッション在席時。これに加え sandbox マスク検知時（sandbox マスク）も削除を試行せず遅延する）。
  **`source=git_worktree_list`（保存 state に記録が無く、Git の登録から補完した in_main）**: helper は cwd が main checkout のときだけこの分類を返すため退出は不要。保存 state の照合の代わりに次を上から順に通す。**停止** = 手順 3 を実行せず `echo "[CONTEXT] WORKTREE_REMOVE_SKIPPED_UNVERIFIED=1; path={flow_wt}; reason=<下記の reason>" >&2` を emit し、4-W の削除だけを見送って cleanup は続行する（ステップ 5 のローカルブランチ削除は worktree 使用中で見送られ、ステップ 12 が未完了として報告する）。
  1. `bash {plugin_root}/hooks/issue-claim.sh check --issue "{issue_number}"` の出力が `own` / `free` / `stale` なら通過。`other` → 停止（`reason=claim_other`）。非 0 rc・それ以外の値 → 停止（`reason=claim_check_failed`）
  2. marker の `branch=` が `{branch_name}` と完全一致すれば通過。不一致・空（detached）→ 停止（`reason=branch_mismatch`）
  3. `missing=yes`（登録だけ残り実体が無い）→ 退避すべき変更は無いので 4 を飛ばして手順 3 へ（実体の無い登録は `git worktree remove` で登録ごと消える）
  4. `dirty=yes`（取得失敗も helper が `dirty=yes` として出す）→ AskUserQuestion（「対象 worktree で stash して続行 / 削除を見送る」。説明文はデリミタ内の生パス一覧を引用する）。「見送る」→ 停止（`reason=dirty_declined`）。「続行」→ **対象 worktree を明示して** `git -C "{flow_wt}" stash push -u -m "rite-cleanup: issue-{issue_number}"` を実行し、続けて `(cd "{flow_wt}" && bash {plugin_root}/hooks/scripts/lib/git-status-filtered.sh)` が rc=0 かつ空出力であることを確認する。stash の失敗・rc≠0・非空 → 停止（`reason=stash_incomplete`）。cwd（main checkout）で素の `git stash push` を実行してはならない — 対象 worktree の変更は退避されず、手順 3 の `--force` fallback で失われる
  通過したら手順 3 を実行する。
- `CLEANUP_WT=none`（multi_session 無効、または worktree 関連なし = 物理 cwd も当該 Issue の worktree でなく、当該 Issue の worktree が Git に登録もされていない）: 4-W 全体を no-op でスキップ。**注**: flow-state 未記録でも物理 cwd が当該 Issue の worktree なら `in_worktree_unrecorded`、cwd が main checkout でも `{worktree_base}/issue-{issue_number}` が登録済みなら `in_main`（`source=git_worktree_list`）に分類されここには落ちない。**ただし関連 Issue が未識別（`{issue_number}` 空）のときは物理 cwd 導出が働かず `none` に落ちる** — 導出が issue 番号でパス末尾を照合するため。この場合の worktree は次回セッション開始時の遅延 reap に委ねられる。
- `CLEANUP_WT=unknown`（detect helper が起動できず分類を返せなかった、または補完に必要な `git worktree list` が失敗した = `reason=worktree_list_failed`）: 分類不能なので**上記の手順 1〜4 を実行しない**。helper 起動失敗では分類そのものが無く、`worktree_list_failed` では一覧を取れないため `{main_root}` も補完候補も cwd との関係も確定しない。いずれも worktree 内にいるか判定できない以上 `in_worktree_unrecorded` と同じ扱いにする — main checkout 操作を要する 4 項目（base 更新 = ステップ 4 / worktree 削除 = 本 4-W / ブランチ削除 = ステップ 5 / wiki ingest = ステップ 9）を**試行せず**、ステップ 12 が未確認として列挙する。worktree 内で完結する項目（PR-specific state 削除・Projects Status 更新・Issue クローズ・作業メモリ更新・flow state リセット）は通常どおり実行する。`CLEANUP_DELEGATED=1` は emit しない — 委譲モードの定型案内は「main checkout で再実行すれば冪等に完了する」を前提にするが、helper が起動できない原因（`{plugin_root}` の未解決置換・helper 欠落）も `git worktree list` の失敗原因（リポジトリ・git 環境の異常）も再実行だけでは解消しないため。
