...
# （ユニークな未マージ作業は無い）。
if ! LC_ALL=C git show-ref --verify --quiet "refs/heads/{branch_name}"; then
...
# awk のフィールド完全一致で比較する。
if [ "$_ls_rc" -eq 0 ] \
   && ! awk -F'\t' -v r="refs/heads/{branch_name}" '$2 == r { found = 1 } END { exit !found }' "$_ls_out"; then
  _ls_rc=2
fi
rm -f "$_ls_out"
case "$_ls_rc" in
