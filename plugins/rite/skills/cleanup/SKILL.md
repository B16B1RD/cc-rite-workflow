...
# `origin/` 前置の誤入力等）でも rc=1 になり、同じく「既削除 = 正常系」へ倒れて削除していないのに
# 完了と報告する。rc=1 の意味を確定させるため、先に refname 合法性を検査する。
