...
`mktemp-derived-path`: mktemp's safety is that it creates a random name with `O_CREAT|O_EXCL`. A name derived from that result was never created that way and becomes predictable once the original is observed, so a symlink planted at the derived path is followed and its target truncated — measured end to end by a security reviewer on `dollar-zero-check.sh` (#2051). The fix is to call mktemp a second time; the cost is negligible and the handler removes both.

`pipefail-grep-q-stream`: `grep -q` exits at the first match, so a producer still writing takes SIGPIPE and `pipefail` turns the pipeline's status into 141. It only fires when the match lands early, which is why it appeared as flakiness rather than a failure: on #2094 it dropped one file per ~200 runs from a parity sweep, and the floor guard was loose enough (`>= 24` against a real 29–35) never to notice. Pipelines headed by `printf` or `echo` are exempt — a short in-memory string fits the pipe buffer, so the producer finishes before the consumer can exit. That exemption is what makes the check usable: 15 of the 16 `grep -q` pipelines in this repo are that shape, and flagging them would bury the one real finding.
