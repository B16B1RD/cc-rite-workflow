...
Two shell defects kept returning to `hooks/` and `scripts/` because every new helper writes the surrounding boilerplate from scratch, and each writing is a fresh chance to get it wrong: a tempfile path derived from a `mktemp` result (`"$tmp.part"`), and `grep -q` consuming a pipeline under `set -o pipefail`. Both already had Wiki pages and prose rules in `fix/SKILL.md` when they recurred — prose does not reach the author at the moment they type `mktemp`.

The primary fix is not this check. `hooks/scripts/lib/tempfile.sh` takes over the whole tempfile lifecycle, so the create/register/remove sequence is no longer hand-written and the third member of the family — a silenced `mktemp` failure degrading into an empty path — has no spelling left (`rite_tempfile_new` is loud and returns non-zero). This check covers the two residues a lib cannot reach, because they are ways of writing a *path* and a *pipeline* rather than ways of calling a function.

`mktemp-derived-path`: mktemp's safety is that it creates a random name with `O_CREAT|O_EXCL`. A name derived from that result was never created that way and becomes predictable once the original is observed, so a symlink planted at the derived path is followed and its target truncated — measured end to end by a security reviewer on `dollar-zero-check.sh` (#2051). The fix is to take a second handle; the cost is negligible and the handler removes both. Handles are tracked from `x=$(mktemp ...)` and from the lib form `rite_tempfile_new x` / `rite_tempdir_new x` — tracking only the raw spelling would put the form that `coding-principles.md` now mandates into the checker's blind spot. Unbraced `$tmp_suffix` is deliberately not a derivation: bash reads the whole `[A-Za-z0-9_]` run as one name, so it is the variable `tmp_suffix`, and treating it otherwise fires on every sibling variable sharing a prefix.

`pipefail-grep-q-stream`: `grep -q` exits at the first match, so a producer still writing takes SIGPIPE and `pipefail` turns the pipeline's status into 141. It only fires when the match lands early, which is why it appeared as flakiness rather than a failure: on #2094 it dropped one file per ~200 runs from a parity sweep, and the floor guard was loose enough (`>= 24` against a real 29–35) never to notice.

The producer examined is the stage *immediately* feeding `grep -q`, not the head of the whole pipeline — in `printf | jq | grep -q` the process that dies is jq, and exempting on the printf would let it through. That immediate producer is exempt when it is `printf` or `echo`: a short in-memory string fits the pipe buffer, so the producer finishes before the consumer can exit. **The exemption is keyed on the command name, which is a proxy for payload size, not the thing that actually decides whether SIGPIPE can occur.** A `printf` whose payload scales with repository content (`wiki-lint-orphans.sh:211`, `wiki-lint-broken-refs.sh:217/220`) can exceed the 64 KiB pipe buffer, and those sites are knowingly not covered. Keeping the exemption is still right: most `grep -q` pipelines here are the short-string shape, and flagging them would bury the real findings.

Out of contract by design: `x=$(mktemp 2>/dev/null) || x=""`. It resembles the silencing defect but is the sanctioned idiom for a non-blocking stderr-capture slot, at 88 sites. A warning there would be noise at a volume that gets the whole check ignored.

A file that could not be scanned is an error, not a clean bill — an unreadable target or a failed awk run is counted and the run exits 2 (findings still win the exit code). Folding "did not look" into "found nothing" inside a checker reproduces, within the guard, the defect class the guard exists to catch; same contract as `dollar-zero-check.sh`.

Scan scope: `plugins/rite/hooks/**/*.sh` and `plugins/rite/scripts/**/*.sh`, excluding `tests/` (fixtures embed the patterns deliberately). Exclusion marker: `drift-check-ignore` on the finding line or the line directly above, as in `sh-cross-ref-check.sh` / `bash-heaviness-check.sh`. Backslash-continued lines are joined before scanning, so the backslash-continued multi-line pipeline form used in this repo cannot hide its consumer from its producer.
