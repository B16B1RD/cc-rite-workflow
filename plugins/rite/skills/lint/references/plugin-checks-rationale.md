...
Verifies `.rite/.gitignore` is the 3-line composition (`*` / `!wiki/` / `!wiki/**`) that setup and hooks generate. Lint never writes that file. `same_branch` additionally checks `git add --dry-run` on a wiki probe so ingest can stage raw sources.
