...
Handles are tracked from `x=$(mktemp ...)` and from the lib form `rite_tempfile_new x` / `rite_tempdir_new x` — tracking only the raw spelling would put the form that `coding-principles.md` mandates into the checker's blind spot. Two shapes are deliberately *not* derivations. Unbraced `$tmp_suffix` is the variable `tmp_suffix`, because bash reads the whole `[A-Za-z0-9_]` run as one name; treating it as a derivation fires on every sibling variable sharing a prefix. The dirname and basename expansions (`${tmp%/*}`, `${tmp##*/}`) extract a component of the same path rather than deriving a sibling, and both are house style here.
