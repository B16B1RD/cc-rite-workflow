...
**Why this is a good finding:** Concrete evidence (specific file/line), investigation with tool usage, comparison with existing patterns, actionable recommendation. **実測必須ゲート**: `Verification: repro` に再現コマンドと観測された誤動作を添えているため、実測必須ゲート ([severity-levels.md §実測必須ゲート](../../../references/severity-levels.md#実測必須ゲート-measured-confirmed-gate)) を通過し **blocking** になる。
...
**Why this is a good finding:** Quantified impact (up to 100 queries), existing pattern reference for the fix, clear before/after recommendation. **実測必須ゲート**: `Verification: failing_test` にテストパスと失敗出力を添えているため blocking になる。
...
| Severity | Scope | File:Line | Issue | Recommendation |
|----------|-------|-----------|-------|----------------|
| MEDIUM | current-pr | `skills/pr-review/SKILL.md:45,120` | Contradictory instructions: line 45 says "report all potential issues, even if uncertain" while line 120 says "only report findings with concrete evidence." This contradicts SKILL.md's "No Hypothetical Concerns" principle. Agents receiving these instructions will produce inconsistent output.<br>Likelihood-Evidence: new_call_site `skills/pr-review/SKILL.md:45,120` (本 PR で追加)<br>Verification: repro 同一 diff で reviewer を 3 回起動 => 2 回は uncertain 指摘を報告し 1 回は抑制 (指示の矛盾により出力が非決定) | Remove line 45 or scope it to a specific context (e.g., security-only). Align with SKILL.md's established "concrete evidence only" principle. |

**Why this is a good finding:** Identified a real contradiction by cross-referencing multiple documents, explained the downstream impact on agent behavior, provided specific resolution options. **実測必須ゲート**: 定義文書の指摘でも「実際に走らせて観測した挙動」を `Verification: repro` に書けば blocking になる (書けなければ non-blocking として PR コメントに記録される)。
