...
**Why this is a good finding:** Concrete evidence (specific file/line), investigation with tool usage, comparison with existing patterns, actionable recommendation. **実測の記録**: `Verification: repro` に再現コマンドと観測された誤動作を添えているため `measured=true` として記録される。
...
**Why this is a good finding:** Quantified impact (up to 100 queries), existing pattern reference for the fix, clear before/after recommendation. **実測の記録**: 既存のテストが実際に落ちるため `Verification: failing_test` にテストパスと失敗出力を添えている。
