...
**CRITICAL** (exploitable vulnerability with immediate impact), **HIGH** (security flaw that could lead to data breach), **MEDIUM** (security weakness requiring attention), **LOW-MEDIUM** (bounded blast radius minor concern; SoT 重要度プリセット表 `_reviewer-base.md#comment-quality-finding-gate` で `Whitelist 外造語` 等に適用される first-class severity — `severity-levels.md#severity-levels` 参照), **LOW** (minor security improvement opportunity).
