...
bash は両サイトで canonical と literal 一致させ、サイト識別子（`ステップ 5.{X}`、X は 1 または
2）だけ置換する。既定テンプレートの既定源は 5.0.c のみであり、template 変更で 5.1 / 5.2 の
quoted heredoc へ既定英文を複製すると drift する。
