...
`.rite/wiki/index.md` の `## ページ一覧` セクションにある **5 列 GFM テーブル**（列順: ページ / ドメイン / サマリー / 更新日 / 確信度）を更新する。**本ステップの分岐で選択した形式以外の行を混ぜてはならない** — 1 つの index に 2 形式が混在すると挿入行以降のテーブル構造が崩壊する。

**分岐の判定キー**: `## ページ一覧` 見出しの有無**ではなく**、既存の**登録行**（`](pages/{domain}/{slug}.md)` を含む行）の有無と形式で決める。index.md 本文の散文・HTML コメント内の形式例は判定根拠にしない（旧テンプレート由来の宣言が残っている場合があるため）。下記を上から評価し、**最初に成立した 1 つだけ**を適用する:

| # | 条件 | 適用する手順 |
|---|------|-------------|
| (a) | 登録行が 0 件 | 「テーブル節の用意」→「新規ページ行の追加」 |
| (b) | テーブル行でない登録行が 1 件以上（リンクの後に ` - ` 区切りで説明文が続く旧形式の箇条書き） | 「テーブル形式への移行」→「テーブル節の用意」→「新規ページ行の追加」。**旧形式のまま追記してはならない** — 形式は一方向にテーブルへ収束させる |
| (c) | テーブル行の登録行が 1 件以上 | 「新規ページ行の追加」/「既存ページ行の更新」 |

**セル区切り文字のエスケープ（新規追加・既存更新・移行のすべての経路に適用）**: `{title}` / `{description}` に `|` が含まれる場合、index 登録行では `\|` にエスケープして substitute する。**インラインコード `` ` `` の内側も対象** — GFM はコードスパン内の生 `|` もセル区切りとして解釈するため、エスケープしないと列がずれて 5 列構造が壊れる。値を言い換えて `|` を避けるのは禁止（ステップ 4.3 の title 規約が frontmatter `title` との literal 一致を要求するため）。**エスケープは index 登録行にのみ適用し、page frontmatter の `title` / `description` は改変しない**。残る `{path}` / `{domain}` / `{updated}` / `{confidence}` は slug・enum・ISO 8601 タイムスタンプで `|` を含み得ないため対象外。

**手順**:

- **テーブル節の用意**: `## ページ一覧` 節が無ければ見出しと下記 2 行を新設する。**挿入位置**は既存の本文・HTML コメントより後で、`## 統計` 節があればその直前、なければファイル末尾とする（位置を自分で決めない）。節が既にありヘッダ行・区切り行が欠けている場合は補う:
...
- **テーブル形式への移行**（分岐 (b) のみ）: 既存の旧形式登録行を 1 行ずつテーブル行へ移送する。リンク `[title](path)` の title / path をそれぞれ ページ列 のリンクへ、` - ` 以降の説明文を サマリー列 へ移し、ドメイン列 は path の `pages/{domain}/` 部分から、更新日列 / 確信度列 は当該 page の frontmatter `updated` / `confidence` から補う。移送後に旧形式の行を削除し、`## ページ一覧` 節に**テーブル行だけが残る**状態にする。対応する page が存在せず frontmatter を読めない登録行は移送せずそのまま残し、`## ページ一覧` 節の外（`## 統計` より前）へ退避して人間の判断に委ねる（推測値で埋めない）。
- **新規ページ行の追加**: `## ページ一覧` テーブルの末尾に次の 1 行を追加する:
...
- **既存ページ行の更新**: 対象行は列位置ではなく `](pages/{domain}/{slug}.md)` のリンク一致で同定する（既に列がずれている行での誤爆を防ぐ）。**サマリー列と更新日列**を上書きし、リンク列 `[{title}]({path})` は不変とする。更新日列は page frontmatter の `updated` と同値。
- **統計**: `## 統計` 節が存在する場合、**総ページ数 / ドメイン別内訳 / 最終更新**の 3 行を今回の ingest 結果と同期する。総ページ数 = `pages/` 配下の **`*.md` ファイル数**（`wiki-init` が置く `.gitkeep` 等の非ページファイルを除く）、ドメイン別 = `patterns` / `heuristics` / `anti-patterns` 各配下の `*.md` 件数、最終更新 = 今回の `{updated}` タイムスタンプ。**総ページ数は `## ページ一覧` テーブルのデータ行数と一致すること**（不一致は登録漏れ・重複の兆候なので、統計を書く前に登録行側を直す）。`## 統計` 節が**存在しない** index.md では統計同期をスキップする（節を新設しない）。全 Raw Source が skip されたサイクル（新規・更新ページ 0 件）も統計同期をスキップする。

> **読み手側の対応状況**: `wiki-lint-orphans.sh` は `](pages/...)` リンクの grep ベースで形式非依存に登録判定するため、いずれの分岐でも `{path}` の形式維持だけが孤児検出の必須条件になる。一方 `/rite:wiki-query` の Pass 1 はテーブル行を parse しないため、テーブル形式 index では index 経由のキーワード照合が機能しない（ステップ 5.3 の `{description}` 注記と同じ理由。読み手のテーブル対応は別 Issue）。
