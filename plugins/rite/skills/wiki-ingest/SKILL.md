...
- **`## ページ一覧` 節が存在しない index.md**（`wiki-init` 直後の初期状態。`templates/wiki/index-template.md` は同節を生成しない）: 先に `## ページ一覧` 見出しと下記 2 行を新設し、その配下に「新規ページ」の 1 行目を追加する（節を新設せずに行を書き足す / 別形式へ improvise するのは禁止）。**挿入位置**は既存の本文・HTML コメントより後で、`## 統計` 節が既に存在する場合はその直前、存在しない場合はファイル末尾とする（位置を自分で決めない）:
