...
| exit 0 + `stats_sync=skipped_unreadable` | 統計同期をスキップ（原因は stderr の WARNING に出ているのでそれを表示する。`## 統計` 節は前サイクルの内容のまま）。続行。**本行と部分未同期 WARNING はステップ 9 の未完了事項へ載せる** — 統計可視化の追加機構（専用 lint / marker attest / サイクル gating）は この仕様により「実装しない」と確定（根拠は helper ヘッダ「Observability of procedure 3b」） |
