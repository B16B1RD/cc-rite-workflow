...
2.6. **検出器化候補 routing**: この経験則は grep / lint / lib 関数で機械的に強制できるか。できるならページを作らず、ステップ 9 完了レポートに『検出器化候補』として 1 行列挙する（`promote: rite-plugin` タグ付きページ化と同様、人間が Issue 化を判断する材料にする）。既存ページ・既存検出器と同一クラスの場合は新規ページを作らない（既存ページの update に留める）
...
| 判定 | アクション |
|------|----------|
| 機械検出可能（grep / lint / lib で強制可能）で既存ページ・既存検出器なし | ページ化せずスキップし、ステップ 9 完了レポートに検出器化候補として 1 行列挙 |
| 機械検出可能かつ既存ページ・既存検出器と同一クラス | 新規ページを作らず既存ページの更新（または既存検出器管轄としてスキップ） |
