...
| `raw/**`（走査しない） | レビュー / fix の生ログ = provenance 資料。番号は出典そのものであって説明的参照ではない（実測 1,400 件超。review / fix サイクルごとに増えるため厳密値は持たない） |
...
**E5（TODO / FIXME）の適用位置はページと index で異なる**。ページは行単位で落とす（従来どおり）が、index はサマリーを
抽出したあと、コードスパンのマスク後に判定して `skipped` に載せる。フィルタ段で落とすとエントリ行が `entries` にも
`skipped` にも計上されないまま消え、欠損を露出する `descriptive_refs_skipped_rows` が構造的に観測できない唯一の穴になる。
副次的に、コードスパン内に**引用された** TODO は E4 のマスクで無効化されるため index では hit として残る（引用は主張ではない）。
