...
差分スコープでも Issue の受入条件全体と変更による回帰を確認する。修正計画の検証記録は内容・コマンド・関連環境が一致した実測結果だけを証拠にし、未確認を成功へ補完しない。時間短縮や反復回数を理由に実欠陥を対象外へ落とさない。
...
**新面のフル審査が自動的に成立する**: fix が前回レビュー範囲外の共有 helper を触った場合、そのファイルは差し替えた入力一覧に含まれるため、パターンマッチにとって単なる 1 つの変更ファイルとして扱われる。「範囲外だったファイルを昇格させる」特別扱いの分岐を書く必要がない。

**acceptance reviewer は本選抜の外にある**: 受入条件確認は差分ではなく全 AC を HEAD で確認する役割なので、パターンマッチの入力を差し替えても名簿から外れない。ステップ 3.2.2 が cap 適用後に `mandatory` で毎 cycle 追加し、差分スコープの mandate も注入しない。
