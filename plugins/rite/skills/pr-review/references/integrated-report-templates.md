...
### 受入条件確認
<!-- ゲート適用済 JSON の acceptance_criteria から描画する。skipped なら表の代わりに
     「受入条件確認: 対象外（{理由}）」の 1 行。E2E フローでも省略禁止。 -->

| AC | 判定 | 根拠 |
|----|------|------|
| {ac_id} | 充足 / 未充足 / 未検証 | {evidence}（未充足行は {finding_id} を併記） |
...
### 受入条件確認
<!-- full mode template と同一。ゲート適用済 JSON の acceptance_criteria から描画し、skipped なら 1 行。E2E でも省略禁止。 -->
