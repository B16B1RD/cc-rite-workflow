...
実測付き blocking を class A（実行時挙動が変わる）/ class B（検出網・可読性・文書整合）に分け、A=0 の cycle で exclusion なし B を non-blocking にして churn 尾部を自然終了させる。exclusion 付き B（既存記述の削除/弱体化）は blocking 維持。旧 JSON 等を直接受け取った場合の実測未判定は分類対象外で class A 固定。成功した実測ゲート出力の gated finding は boolean を持つ。不確実なら class B（攻め側既定）。ファイルパスで機械分類しない。
