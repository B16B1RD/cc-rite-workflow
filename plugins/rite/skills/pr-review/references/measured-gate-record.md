...
the contract の受入基準は `{pr_number}` / `{non_blocking_count}` / `{existing_comment_id}` / `{review_tmp_dir}` の 4 点に gate を求めていた。単一 invocation 化で substitution point 自体が変わったため、実装した gate 集合との対応を以下に明示する（要求の黙殺を防ぐため）。
