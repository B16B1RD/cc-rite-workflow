...
2. **表記 allowlist（全称、言語非依存）**: 区間内の**全データ行**（`^|` かつヘッダー/セパレータ行を除く）が、規約文言 `the next gate in the 8.0 evaluation order` / `**ERROR**` / `legitimately skipped` のいずれかを含むこと。件数一致ではなく全称で判定するため、和文で書かれた pass 行が規約文言を欠いたまま紛れ込んでもこの層で確実に検出される（層 1 だけに頼らない）。近似誤差は「規約文言 3 種のいずれも持たないが実質的に終端相当を示す言い回し」を通す余地が残る点（層 1・層 2 に限った話であり、層 3 が別軸で補う）。
3. **gate ごとの実在性と hand-off（厳密等値）**: 各 gate 区間のデータ行数と、`Gate passes` ∧ 規約文言を共起させる pass 行数を、gate ごとに厳密な等値で固定する（`_g_spec` の `見出し:データ行数:pass 行数`）。**層 1・層 2 はいずれも行の削除に無反応**であり、AC-6 の中核 failure mode（pass 行の削除で後続 gate が到達不能になる）を捕捉するのは本層のみ。実測: 8.0.2 の pass 行 1 本を削除すると本層の 2 assertion だけが落ち、層 1・層 2 は緑のまま通る。
