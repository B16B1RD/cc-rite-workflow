...
2. **表記 allowlist（全称、言語非依存）**: 区間内の**全データ行**（`^|` かつヘッダー/セパレータ行を除く）が、規約文言 `the next gate in the 8.0 evaluation order` / `**ERROR**` / `legitimately skipped` のいずれかを含むこと。件数一致ではなく全称で判定するため、和文で書かれた pass 行が規約文言を欠いたまま紛れ込んでもこの層で確実に検出される（層 1 だけに頼らない）。近似誤差は「規約文言 3 種のいずれも持たないが実質的に終端相当を示す言い回し」を通す余地が残る点のみ。
