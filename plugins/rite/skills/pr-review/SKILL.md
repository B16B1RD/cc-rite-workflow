...
- **`findings[].scope` を必ず明示する** (`current-pr` / `follow-up` / `nit-noted`)。scope は本ゲートの blocking 判定の入力そのもので、本 step は常に `--reject-preset-verification` を指定するため、**欠落は `reason=scope_missing` で hard fail し JSON も書き換えられない**（フラグなしの互換モードでのみ severity ベース default mapping + `MEASURED_GATE_SCOPE_DEFAULTED` marker で続行する）。enum 外の値は `reason=scope_enum_violation` で hard fail させる（フラグ有無に依らず発火）。いずれも未知 / 欠落 scope が blocking 集合からも移送対象からも同時に外れ、mergeable を無音で確定させるのを防ぐため。なお本節が pin する `schema_version: "1.0.0"` は review-result-schema.md 上「scope を持たない版」の定義であり、実体との不整合が残っている (canonical 記述の bump は Non-Target ファイルの変更を伴うため #2073 で追跡)
...
| 観測 | LLM action |
|---|---|
| `[CONTEXT] MEASURED_GATE_FAILED=1; reason=verification_preset_by_caller` / `scope_enum_violation` / `scope_missing` (helper 非ゼロ終了) | **caller (step 1) 契約違反で、JSON を作り直せば同 cycle 内で収束する。** helper の ERROR 行が原因と対処を示すので、それに従って step 1 の JSON を**作り直してから** step 2 を再実行する (`non_blocking_findings[]` は `[]` に戻すこと — helper は append するため、ゲート適用済みファイルを部分編集して再実行すると降格済み finding が重複する)。**再試行は reason ごとに 1 回まで** — `measured_gate_retry_count` (下記) が 1 に達している reason で再発したら `[review:error]` を stdout に出力して停止する |
...
成功時に併記されうる観測 marker (いずれも WARNING と対。**分類を変えず、続行を妨げない**。ステップ 6 Retained flag mapping 登録済)。なお `MEASURED_GATE_SCOPE_DEFAULTED` は配線済み経路では `scope_missing` の hard fail が先に立つため成功時に出ない (フラグなしの互換モード専用):

- `[CONTEXT] MEASURED_DEMOTED_ON_ANCHOR=1; count={n}; cause=anchor_unparseable` — アンカーはあるが正規形でない gate 対象 finding が降格された。**silent 降格の唯一の検出層**。次 cycle でアンカー書式を直す材料にする。**helper は本 marker では停止しない** — `blocking=0` と併記された場合は「実測済みだったはずの指摘が書式ミスだけで mergeable へ反転した」可能性があるので、ステップ 5.4 の統合レポートにその旨を記載すること
...
**`measured_gate_retry_count`** (retained flag): dict `{reason: int}`。初期値は空 dict `{}`。上記 caller 契約違反 3 種それぞれについて 0 → 1 の transition を**最大 1 回**許可する。ステップ 5.1.1.1 の `verification_post_condition_retry_count` と同型で、新しい構造を持ち込まない。回数を保持しないと「再実行でも同じ」の判定が LLM の記憶依存になり、無限 retry と 1 回目での誤 `[review:error]` の両方を許す。
...
- Required JSON fields: `schema_version: "1.0.0"`, `pr_number`, `timestamp` (literal sentinel `"__RITE_TS_PLACEHOLDER_7f3a9b2c__"` を書き、helper が ISO 8601 JST 値に注入), `commit_sha`, `overall_assessment` (`mergeable` / `fix-needed`), `findings[]`. 加えて **`non_blocking_findings[]`** (5.3.0.M で降格した非実測 finding。**0 件のときも空配列 `[]` を出力する** — キー自体を省略すると「降格ゼロ」と「本ゲート未適用の旧形式」が区別できなくなる。`review-result-save.sh` はキー欠落・非配列を `NON_BLOCKING_FINDINGS_KEY_MISSING`、和集合 id 欠陥を `NON_BLOCKING_FINDINGS_ID_UNION_VIOLATION` で報告するが**いずれも保存を続行する**ため、本配列側の欠陥で blocking findings が失われることはない). Each finding の必須フィールドは以下の通り — 完全なスキーマは [review-result-schema.md](../../references/review-result-schema.md#json-schema) を真実の源として参照すること:
 ...
 - `scope`: `current-pr` / `follow-up` / `nit-noted` のいずれか。**省略してはならない** — 本フィールドは ステップ 5.3.0.M の実測必須ゲートが blocking 集合を決める入力そのもので、欠落すると severity ベースの default mapping (`LOW` / `LOW-MEDIUM` → `nit-noted`、それ以外 → `current-pr`) に倒れ、reviewer が `current-pr` を割り当てた LOW / LOW-MEDIUM が blocking から脱落する。**配線済み経路 (step 2 が常に `--reject-preset-verification` を指定) では欠落を `reason=scope_missing` で hard fail し、JSON も書き換えない**（フラグなしの互換モードでのみ default mapping + `MEASURED_GATE_SCOPE_DEFAULTED` marker で続行）。enum 外の値は `reason=scope_enum_violation` で hard fail させる (フラグ有無に依らず)。判定規則の SoT は [severity-levels.md](../../references/severity-levels.md) と `agents/_reviewer-base.md` §Scope Assignment Flowchart。<br>**注**: 本節が pin する `schema_version: "1.0.0"` は review-result-schema.md 上「`scope` を持たない版」の定義であり、`scope` を必須とする本規約と定義上矛盾している。canonical 記述の `1.1.0` への bump は Non-Target ファイル (`references/review-result-schema.md`) の変更を伴うため #2073 で追跡する。
