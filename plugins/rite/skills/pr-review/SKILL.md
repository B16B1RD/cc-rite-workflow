...
5.3.0.M の**後**・5.3.1 の**前**。実測付き blocking を class A / class B に分け、**A=0 のとき exclusion なしの B を non-blocking へ降格**する。exclusion 付き B（既存記述の削除/弱体化）は class B のまま blocking を維持する。旧 JSON 等を直接渡した場合の実測未判定は分類対象外で class A 固定（主経路では前段が error で止める）。SoT: [assessment-rules.md §5.3.0.C](../fix/references/assessment-rules.md) / [severity-levels.md §帰結クラス軸](../../references/severity-levels.md#帰結クラス軸-consequence-class)。
...
- `[CONTEXT] CLASS_DEMOTION_UNDETERMINED_MEASURED=1; count={n}` — 実測未判定 (verification 欠落) のため**分類対象外として class A 側に固定算入**した gated finding が {n} 件ある (map のエントリは参照されない)。旧 JSON 等を直接渡した場合の防御であり、現行 5.3.0.M の成功出力からは生じない
