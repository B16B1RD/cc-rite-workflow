...
- `[CONTEXT] MEASURED_DEMOTED_ON_ANCHOR=1; count={n}; cause=anchor_unparseable` — アンカーはあるが正規形でない gate 対象 finding が降格された。**silent 降格の唯一の検出層**。次 cycle でアンカー書式を直す材料にする (blocking 候補の**全件**がこの理由で降格した場合は helper が `all_demoted_on_anchor` で停止するため、本 marker が成功時に出るのは一部のみ形式崩れだったケース)
...
- `[CONTEXT] MEASURED_GATE_SCOPE_DEFAULTED=1; count={n}` — `scope` 欠落を severity ベース default mapping で補完した。**本 step は常に `--reject-preset-verification` を指定するため、配線済み経路では `scope_missing` の hard fail が先に立ち本 marker は成功時に出ない**。フラグなしの互換モード (旧形式 JSON の手動再処理) でのみ現れる

**`measured_gate_retry_count`** (retained flag): dict `{reason: int}`。初期値は空 dict `{}`。上記 caller 契約違反 4 種それぞれについて 0 → 1 の transition を**最大 1 回**許可する。ステップ 5.1.1.1 の `verification_post_condition_retry_count` と同型で、新しい構造を持ち込まない。回数を保持しないと「再実行でも同じ」の判定が LLM の記憶依存になり、無限 retry と 1 回目での誤 `[review:error]` の両方を許す。
