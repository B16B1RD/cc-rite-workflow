...
- **`findings[].verification` は書かない** — 本フィールドは helper が `description` のアンカーから算出する唯一の書き手である。Claude が先に書くと helper は既存値を正として尊重し (§4.5)、アンカー検出を経ない値がそのまま blocking 判定に入る (= 本ゲートが閉じたはずの裁量が復活する)。**step 2 の `--reject-preset-verification` が強制するのは「description のアンカー有無と矛盾する preset」だけ**であり、アンカーと一致する preset は rc=0 で素通りし、その `repro` / `failing_test` は helper の抽出を経ず Claude が書いた文字列のまま永続 JSON に残る (= 部分的な強制)。preset の存在自体を hard fail させる形には**できない** — ゲート適用後の JSON は全件が `verification` を持つため、再実行が必ず失敗し冪等性 (AC-5) が壊れる。したがって本規約の完全な履行は依然として Claude 側の忠実性に依存する
- **アンカーの直前の境界を保つ** — reviewer の `内容` 列から `description` へ転記するとき、`Verification:` / `Likelihood-Evidence:` アンカーの**直前は行頭・`<br>`・空白のいずれか**でなければならない。helper の検出 regex は境界を要求するため、Markdown セルの `<br>` を日本語の句点 (`。`) や連結で潰すと**全 finding が `anchor_unparseable` として降格し `blocking=0` = mergeable に反転する**。単一行の JSON 文字列へ畳むときは `<br>` をそのまま残すこと。加えて repro 本文には raw pipe (`|`) と `<br>` を含めない (LHS が両者を受理しないため no-match になる) — パイプは `¦` で代替表記する
...
- **`findings[].scope` を必ず明示する** (`current-pr` / `follow-up` / `nit-noted`)。scope は本ゲートの blocking 判定の入力そのもので、欠落すると helper が severity ベースの default mapping に倒れ、reviewer が `current-pr` を割り当てた LOW / LOW-MEDIUM が blocking から脱落する。enum 外の値は helper が `reason=scope_enum_violation` で hard fail させる (未知 scope は blocking 集合からも移送対象からも同時に外れ、mergeable を無音で確定させるため)。なお本節が pin する `schema_version: "1.0.0"` は review-result-schema.md 上「scope を持たない版」の定義であり、実体との不整合が残っている (canonical 記述の bump は Non-Target ファイルの変更を伴うため #2073 で追跡)
...
| `[CONTEXT] MEASURED_DEMOTED_ON_ANCHOR=1; count={n}; cause=anchor_unparseable` (WARNING と対) かつ **`blocking >= 1`** | アンカーはあるが正規形でない finding が降格された。**silent 降格の唯一の検出層**として会話コンテキストに残す (ステップ 6 Retained flag mapping 登録済)。ゲート自体は成功しているので続行する |
| `[CONTEXT] MEASURED_DEMOTED_ON_ANCHOR=1; count={n}` かつ **`blocking=0`** (= 全 blocking 候補が形式崩れで降格し mergeable に反転した) | **続行してはならない。** step 1 の転記でアンカー直前の境界 (`行頭` / `<br>` / 空白) を落とした疑いが高い (境界が無いと全件が同じ理由で一斉に外れる)。step 1 の JSON を **転記し直してから** step 2 を再実行する。再実行後も同じ形なら reviewer 出力側のアンカー書式を疑い、その旨を ステップ 5.4 に記載した上で続行する |
| `[CONTEXT] MEASURED_RUNTIME_OBS_WITHOUT_ANCHOR=1; count={n}` (WARNING と対) | `Likelihood-Evidence: runtime_observation` を主張しながら `Verification:` アンカーを欠く reviewer 契約違反。会話コンテキストに残して続行する (分類は変えない) |
| `[CONTEXT] MEASURED_GATE_SCOPE_DEFAULTED=1; count={n}` (WARNING と対) | `scope` 欠落を severity ベース default mapping で補完した。step 1 の生成規約 (scope 明示) が守られていない。続行してよいが、次 cycle では scope を明示すること |
| `[CONTEXT] MEASURED_GATE_FAILED=1; reason=verification_preset_by_caller` | **caller (step 1) 契約違反で、作り直せば同 cycle 内で収束する。** step 1 へ戻り `verification` を除いた JSON を再 Write してから step 2 を再実行する。再実行でも同じ reason なら `[review:error]` を stdout に出力して停止する |
| `[CONTEXT] MEASURED_GATE_FAILED=1; reason=scope_enum_violation` | 同上 (caller 契約違反)。helper が enum 外 scope を持つ finding の id / file を stderr に列挙するので、step 1 へ戻り当該 finding の `scope` を 3 値 enum に直してから step 2 を再実行する。再実行でも同じなら `[review:error]` で停止 |
| `[CONTEXT] MEASURED_GATE_FAILED=1; reason=...` (上記 2 種以外。`jq_missing` / `input_missing` / `input_unreadable` / `json_invalid` / `findings_not_array` / `non_blocking_not_array` / `jq_transform_failed` / `stats_read_failed` / `mktemp_failure` / `write_failure` / `mv_failure`) | **`[review:error]` を stdout に出力して停止する。LLM 分類へ fallback してはならない** — fallback は本ゲートが閉じた不発の再生産になる。helper が ERROR 行の直後に外部コマンドの stderr 先頭 5 行を転記するので、原因はそこを見る |
...
- Required JSON fields: `schema_version: "1.0.0"`, `pr_number`, `timestamp` (literal sentinel `"__RITE_TS_PLACEHOLDER_7f3a9b2c__"` を書き、helper が ISO 8601 JST 値に注入), `commit_sha`, `overall_assessment` (`mergeable` / `fix-needed`), `findings[]`. 加えて **`non_blocking_findings[]`** (5.3.0.M で降格した非実測 finding。**0 件のときも空配列 `[]` を出力する** — キー自体を省略すると「降格ゼロ」と「本ゲート未適用の旧形式」が区別できなくなる。`review-result-save.sh` はキー欠落・非配列を `NON_BLOCKING_FINDINGS_KEY_MISSING`、和集合 id 欠陥を `NON_BLOCKING_FINDINGS_ID_UNION_VIOLATION` で報告するが**いずれも保存を続行する**ため、本配列側の欠陥で blocking findings が失われることはない). Each finding の必須フィールドは以下の通り — 完全なスキーマは [review-result-schema.md](../../references/review-result-schema.md#json-schema) を真実の源として参照すること:
 ...
 - `scope`: `current-pr` / `follow-up` / `nit-noted` のいずれか。**省略してはならない** — 本フィールドは ステップ 5.3.0.M の実測必須ゲートが blocking 集合を決める入力そのもので、欠落すると helper が severity ベースの default mapping (`LOW` / `LOW-MEDIUM` → `nit-noted`、それ以外 → `current-pr`) に無通知で倒れ、reviewer が `current-pr` を割り当てた LOW / LOW-MEDIUM が blocking から脱落する (`MEASURED_GATE_SCOPE_DEFAULTED` marker で可視化されるが分類は戻らない)。enum 外の値は helper が `reason=scope_enum_violation` で hard fail させる。判定規則の SoT は [severity-levels.md](../../references/severity-levels.md) と `agents/_reviewer-base.md` §Scope Assignment Flowchart。<br>**注**: 本節が pin する `schema_version: "1.0.0"` は review-result-schema.md 上「`scope` を持たない版」の定義であり、`scope` を必須とする本規約と定義上矛盾している。canonical 記述の `1.1.0` への bump は Non-Target ファイル (`references/review-result-schema.md`) の変更を伴うため #2073 で追跡する。
