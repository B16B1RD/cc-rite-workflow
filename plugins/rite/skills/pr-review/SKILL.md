...
| 観測 | LLM action |
|---|---|
| `[CONTEXT] MEASURED_GATE_FAILED=1; reason=verification_preset_by_caller` / `scope_enum_violation` (helper 非ゼロ終了) | **caller (step 1) 契約違反で、JSON を作り直せば同 cycle 内で収束する。** helper の ERROR 行が原因と対処を示すので、それに従って step 1 の JSON を**作り直してから** step 2 を再実行する。**再試行は reason ごとに 1 回まで** — `measured_gate_retry_count` (下記) が 1 に達している reason で再発したら `[review:error]` を stdout に出力して停止する |
