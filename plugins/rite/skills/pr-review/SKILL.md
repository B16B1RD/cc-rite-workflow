...
- **JSON 本文**: Claude が review-result-schema.md に従って JSON 本文を生成し、**Write tool で `{review_tmp_dir}/rite-review-result-{pr_number}.json` に保存**する。生成漏れ / 不正 JSON は `review-result-save.sh` 内の jq timestamp 注入が `write_failure` として非ブロッキングに fail-fast 検出する (後続の `jq empty` post-condition `json_invalid` は注入成功後に走る defense-in-depth backstop で、syntactic invalidity では実挙動上ここに到達しない)。
 - **`non_blocking_findings` の除外契約 (実測必須ゲート)**: ステップ 5.3.0.M で `non_blocking_findings` に移動した非実測 finding は、本 JSON 本文の `findings[]` 配列に **含めない** (`suppressed_findings` と同じ除外契約)。`findings[]` は 5.3.0.M 通過後の `全指摘事項` = blocking 集合と一致する。**帰結**: (a) JSON 経路 (`/rite:fix` Priority 0/2/3) には非実測 finding が渡らないため、fix 側の `measured_map` は JSON 経路では常に空になり `non_blocking_count` は 0 になる (Markdown / 会話経路の N とは一致しない)。(b) 非実測指摘の記録先は ステップ 5.4 統合レポートの `### 実測なし指摘 (non-blocking)` section のみで、`.rite/review-results/` の JSON には残らない。(c) 非実測 finding と同一 file:line に GitHub thread がある場合、その thread は severity_map 未登録となり `/rite:fix` ステップ 1.3 step 5 で External review (blocking) に分類される — 安全側 (取りこぼしではなく過剰 blocking) だが、`pr_review.post_comment: false` (既定) では rite 由来 thread 自体が存在しないため通常は発生しない。**write 側が `verification` を出力するようになった後**は本契約を見直し、非実測 finding を `measured: false` 付きで `findings[]` に含める設計に切り替えてよい (その場合は cross-field invariant #2 の 3 経路 + SoT 同期が前提)。
