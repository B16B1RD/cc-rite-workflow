...
- `schema_required_fields_missing`: JSON は parse 可能だが必須フィールド (schema_version 非空文字列 / pr_number 数値型 / findings[] 配列型 / verdict が `mergeable`・`fix-needed` の 2 値 enum / reviewers[] が重複の無い非空配列) が欠落。実際に落ちた条件は helper が `欠落/不正: <キー名>` として名指しする。本列挙と helper の WARNING 文言の drift は `hooks/tests/review-verdict-reviewers-contract.test.sh` が検出する
