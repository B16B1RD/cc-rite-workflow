...
- `[CONTEXT] MEASURED_DEMOTED_ON_ANCHOR=1; count={n}; cause=anchor_unparseable` — `Verification:` marker はあるが `=>` を欠く (= アンカーではなく散文中の言及)、または既存 `verification.measured=false` を保持したまま降格された gate 対象 finding。**silent 降格の唯一の検出層**。**helper は本 marker では停止しない**。降格された finding 自体は `non_blocking_findings[]` へ移送され、ステップ 5.4 の `### 実測なし指摘 (non-blocking)` section (E2E でも省略禁止) と ステップ 6.1.d の PR 記録コメントに載るため、人間への到達は既存経路で担保される
