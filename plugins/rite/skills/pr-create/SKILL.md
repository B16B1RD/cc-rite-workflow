...
`WM_PR_NUMBER` を渡すのは、PR 番号を知っているのがワークフロー中この時点だけだからである。以降の caller（`pr-review` / `fix` / `ready` 等）は番号を持たないため、ここで seed しないと `pr_number` はどの更新経路でも書かれないままになる（seed 後は `work-memory-update.sh` の carry-forward が値を保持する）。
