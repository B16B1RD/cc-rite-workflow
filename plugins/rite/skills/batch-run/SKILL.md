...
- **サーキットブレーカーは failed 遷移（即停止ではない）**: iterate は収束トレンドの発散検出、または `safety.max_review_cycles` 到達（backstop）で `[iterate:max-cycles-reached]` を emit する。**sentinel は発火理由に依らず同一の literal** であり、run 側は理由を区別しない — 理由別に sentinel を分けると本節の grep 契約が壊れるため意図的に据え置いている（名称の `max-cycles` は発散発火に対しては misnomer だが、契約の安定を優先）。run はこれを他の失敗 sentinel（`[fix:error]` 等 → 即停止）と区別し、当該 Issue のみ failed 記録して次へ進める。非収束 PR 1 件でバッチ全体がストールするのを防ぐのが本 sentinel の目的（Issue #1701）。failed 記録は run-queue-{session_id}.json の `failed[]` に永続化し、compact / 中断を跨いでも完了通知で報告できる。`failed[]` 欠落の旧形式キューは `[]` 互換
