...
自己矛盾候補（M 以上）は上表と**直交する軸**のため行にしない。**上表のいずれの行に該当した場合でも**、矛盾候補が 1 件以上あれば AskUserQuestion を発行する（`修正して続行` / `そのまま続行`。自動修正しない）。`CONTRADICTED` と同時発生時は同一呼び出しに載せ、`CONTRADICTED` が 0 件なら本項目だけで新規に発行する。
