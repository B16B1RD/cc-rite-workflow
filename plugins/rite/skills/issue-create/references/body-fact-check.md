...
- 「本リポジトリでは」「本 PR 時点では」「`{file}` の `{関数名}` では」等の限定句の追加を**訂正案として提示する**（自動適用しない）
- 限定を付けられない断定は、そもそも根拠が無い可能性を疑う

本クラスは `CONTRADICTED` 判定を出さない。検出した項目は `UNVERIFIED` として下記「3 値の処理」へ合流させる（本クラス独自の処理経路を持たない）。
...
自己矛盾クラスの矛盾候補も確認に載せる（`修正して続行` / `そのまま続行`）。`CONTRADICTED` と同時に検出したときは同一の AskUserQuestion に相乗りし、`CONTRADICTED` が 0 件のときは本項目だけで新規に発行する。

surface する項目は 1 項目 = 1 question とし、上限は **AskUserQuestion 1 呼び出しの質問枠（4 件）から consumer が既に使う質問数を差し引いた残枠**とする（絶対値ではなく残枠で表すのは、既存の確認へ畳み込む consumer で上限を超えないようにするため）:

| consumer | 既存質問 | surface 上限 |
|----------|---------|-------------|
| issue-create ステップ 4.2.1 / 5.1.1（専用に発行する） | 0 | 4 件 |
| issue-edit Phase 3.1.1（Phase 3.2 の確認へ畳み込む） | 1 | 3 件 |

溢れた分の扱いは consumer 側の規定に従う。**承認前の本文書き換えを禁じている consumer（issue-edit）では本文へ直接列挙してはならない** — 確認テキスト内に一覧として提示し、付記は承認後に行う。
