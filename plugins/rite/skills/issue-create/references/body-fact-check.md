...
| 条件 | 挙動 |
|------|------|
| 参照先の番号 / SHA が存在しない（`gh` の stderr が `Could not resolve`、`git` の stderr が `unknown revision` / `bad object` を含む非ゼロ終了） | `CONTRADICTED`（タイポの可能性を訂正案として提示する） |
| **上記以外**の非ゼロ終了（オフライン / `gh` 認証切れ / 浅い clone に commit が無い 等） | `UNVERIFIED` として「要確認」を付記し続行。stderr に `WARNING` を出す。Issue 作成 / 編集を**ブロックしない** |
