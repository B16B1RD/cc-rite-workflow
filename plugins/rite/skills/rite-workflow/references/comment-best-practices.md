...
PR #688 の `plugins/rite/hooks/state-read.sh` から抜粋した Bad ↔ Good 対比。
...
- 行番号参照 (`work-memory-update.sh:97-104 / 177-183 / 184-190`) → 原則 3 (no_line_reference) 違反
...
| 2. no_journal_comment | コメント内に `cycle\s*\d+`, `F-\d+`, `PR\s*#\d+`, `verified-review`, `Issue\s*#\d+` のいずれかが含まれる (whitelist 例外あり) | `# verified-review cycle 35 fix (F-04 HIGH)` |
| 3. no_line_or_cycle_reference | コメント内に `[a-z_]+\.\w+:\d+` (file:line) パターン | `# state-read.sh:93 と同じ` |
...
| ジャーゴン | 意味 | 出典 |
|-----------|------|------|
| `stop-guard` | rite workflow の Stop hook ガード機構 | `plugins/rite/hooks/stop-guard.sh` |
| `sentinel` | hook / grep 契約のためのマーカー (`[review:mergeable]` 等) | `plugins/rite/skills/rite-workflow/references/workflow-identity.md` |
| `verified-review` | ファクトチェック付き PR レビュー | `plugins/rite/skills/verified-review/` |
| `workflow-incident-emit` | workflow incident 検出 sentinel emit | `plugins/rite/hooks/workflow-incident-emit.sh` |
| `flow-state` | `.rite-flow-state` / `.rite/sessions/{id}.flow-state` の状態管理 | `plugins/rite/hooks/flow-state-update.sh` |
| `Mandatory After` | sub-skill 完了後に caller が必ず実行する遷移 step | `plugins/rite/commands/issue/start.md` |
| `Pre-write` | sub-skill 起動前に caller が必ず行う `.rite-flow-state` 更新 | `plugins/rite/commands/issue/start.md` |
...
| `oracle` | 既存の正しい実装を参照実装として使うパターン | `plugins/rite/skills/rite-workflow/references/coding-principles.md` |
