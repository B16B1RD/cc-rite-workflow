...
3. stop-guard hook が flow state の `.active=true` かつ継続必須 phase (例: `create_post_interview` / `create_delegation` / `phase5_post_review` 等) で stop を block し、`workflow_incident sentinel` + WORKFLOW_HINT を stderr 経由で feed back する。
