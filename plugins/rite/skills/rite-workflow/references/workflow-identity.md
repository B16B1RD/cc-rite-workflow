...
3. stop-guard hook が `.rite-flow-state.active=true` かつ継続必須 phase (例: `create_post_interview` / `create_delegation` / `phase5_post_review` 等) で stop を block し、`workflow_incident sentinel` + CREATE_HINT を stderr 経由で feed back する。
