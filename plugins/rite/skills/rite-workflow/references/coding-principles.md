...
> **Operating-environment declaration**: Each project declares its operating environment (single-user development machine / shared host / multi-tenant, etc.) as prose in its own `CLAUDE.md` or Wiki — not as a config key — because the reviewer-side [Finding Quality Guardrail](../../../agents/_reviewer-base.md#finding-quality-guardrail) Category #2 judges whether a defensive / hardening finding is reachable at all against that declaration.
