...
elif [ -f "$repo_root/.rite-plugin-root" ]; then
  mkdir -p "$wt_path/.rite"
  cp "$repo_root/.rite-plugin-root" "$wt_path/.rite/plugin-root" 2>/dev/null || true
fi
...
| 観測 | アクション |
|---|---|
| `## 計画レビュー結果` かつ `指摘件数:` が整数かつ各行の視点が 4 種かつ 反映先 `種別=` が 実装ステップ追記 / 変更対象ファイル追記 / 要判断ポイントへ昇格 | 指摘を計画へ反映（`種別=` どおり。新規ステップ追加はしない）。blocking にしない。1 行でも外れれば全体を未実施（部分適用しない）。`[CONTEXT] PLAN_REVIEW=done; findings=N` を出し、指摘件数と反映内容（または指摘なし）を承認材料として提示して 3.4 へ。再 spawn しない |
