...
audit="$ROOT/plugins/rite/skills/pr-review/references/promotion-audit-defense-mechanism-integrity.md"
...
assert_grep 'runtime trust covers the complete graph' "$reviewer" 'complete execution graph loads no PR-controlled'
assert_grep 'runtime trust covers config and dependencies' "$reviewer" 'code, configuration, or dependencies'
assert_grep 'runtime isolation removes secrets and network' "$reviewer" 'explicitly removes secrets, network access'
assert_grep 'runtime isolation bounds writes' "$reviewer" 'write access outside a'
