...
case "$PHASE" in
  ...
  create_post_interview)
    ...
    # bash 引数 symmetry (--phase, --active, --next, --preserve-error-count) の崩れは error_count reset loop
    # (verified-review cycle 3 F-01) または `active=false` 残存による stop-guard early return (Issue #660)
    # を再発させる。create-interview.md / create.md の同名 anchor と pair で 4 site 同期する。
