...
# in a corrupt-state-file fragment degrades to `?` runs. The default /
# --keep-newline / --c0-only call sites are diagnostic-only output for
# corrupt/unknown input (git/tool stderr, JSON fallback), where neutralizing
# on the safe side outweighs snippet readability. The fourth mode
# (`--c0-only --keep-newline`) is the exception: UTF-8 catalog text that the
# operator must be able to read, with only C0+DEL stripped.
#
...
#   - --keep-newline: \n (0x0a) のみ素通し (旧 `sed 's/[[:cntrl:]]/?/g'` の行指向挙動と同じ)。C1 も ? 化
...
#     排他契約は維持する — 単独の --keep-newline は C1 を潰したまま、単独の --c0-only は
#     改行を潰したまま。未知フラグは default 範囲へ倒す (typo を緩いモードへ落とさない)。
