...
r=$(make_repo merged)
# 2 ストリームを分けて捕捉する。docstring がストリーム分割を契約として宣言している以上、
# 「stdout の marker で報告する」assert は stdout だけを見なければ意味を持たない（結合すると
# marker が stderr へ移る回帰を素通しする）。一方 family 不在の検査は stderr 側 marker
# （BRANCH_DELETE_FAILED / BRANCH_DELETE_UNMERGED）も覆う必要があるので結合側に当てる。
_dry_err=$(mktemp "$TMP_ROOT/dryerr.XXXXXX")
out=$(cd "$r" && bash "$HELPER" --branch "$BR" --pr-merged true --branch-identity-verified true --dry-run 2>"$_dry_err"); rc=$?
both=$(printf '%s\n%s\n' "$out" "$(cat "$_dry_err")")
rm -f "$_dry_err"
...
assert_not_contains "dry-run: ローカル削除済み marker を出さない" "$both" "[CONTEXT] BRANCH_DELETED=1"
assert_not_contains "dry-run: リモート削除済み marker を出さない" "$both" "[CONTEXT] REMOTE_BRANCH_DELETED=1"
...
assert_not_contains "dry-run: ローカル family (BRANCH_DELETE_*) に入らない" "$both" "[CONTEXT] BRANCH_DELETE_"
assert_not_contains "dry-run: ローカル family (BRANCH_ALREADY_ABSENT) に入らない" "$both" "[CONTEXT] BRANCH_ALREADY_ABSENT"
assert_not_contains "dry-run: ローカル family (BRANCH_CHECK_FAILED) に入らない" "$both" "[CONTEXT] BRANCH_CHECK_FAILED"
assert_not_contains "dry-run: リモート family (REMOTE_BRANCH_*) に入らない" "$both" "[CONTEXT] REMOTE_BRANCH_"
