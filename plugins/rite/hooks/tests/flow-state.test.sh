...
rc=0; err=$( (cd "$d" && bash "$HOOK" migrate >/dev/null) 2>&1 ) || rc=$?
# Why (the governing rationale review F-04): this is the only negative assertion in TC-8b — it passes when the
...
# Why (the governing rationale review F-06): the braces were added because an unbraced variable abutting the
