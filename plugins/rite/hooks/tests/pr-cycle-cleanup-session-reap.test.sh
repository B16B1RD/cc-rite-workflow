...
cleanup_dirs=()
holder_pids=()   # background processes that hold cwd inside a worktree (the governing rationale TCs)
cleanup() {
  local p d
...
# ===========================================================================
# the governing rationale — worktree liveness guard, signal (A): flow-state.worktree scan
...
# ===========================================================================
# the governing rationale — OS-level live-cwd guard (regression of #1524). The cross-session
...
echo "=== T-08 (the governing rationale non-regression): same clean+stale worktree with NO live cwd → reaped ==="
...
# ===========================================================================
# the governing rationale — claim-join (regression of #1524/#1544). The claim's liveness is
...
echo "=== T-10 (the contract.surgical): active+stale holder whose claim records a DIFFERENT worktree → candidate still reaped ==="
...
# ===========================================================================
# the governing rationale — session-feature-branch recovery after reap (dead-letter fix).
...
# ===========================================================================
# the governing rationale — Gate 2 free-arm manifest bypass. cleanup.md defers the worktree
...
# ===========================================================================
# the governing rationale — corpse reap. A sandbox-masked `git worktree remove --force`
...
# violating the the governing rationale MUST (skips must be logged). Non-vacuous: revert the
...
# Gate 2's live-claim arm. That skip must be loud (the governing rationale MUST) — the
...
# ===========================================================================
# the governing rationale — corpse age-guard manifest bypass, keyed on PATH not branch. A
...
print_summary "$(basename "$0")" \
  "Drift hint: pr-cycle-cleanup.sh Step 5 §8 — Gate 0 self-exclusion (cwd/RITE_WORKTREE == self → never reap) + worktree liveness guard (the governing rationale: a session's active flow-state worktree ref → never reap; reap → null owner ref / the governing rationale: claim-join — issue's claim holder still active=true, even with a stale 2h heartbeat → never reap) + OS-level live-cwd guard (the governing rationale: any live process standing in the tree → never reap, via worktree-live-cwd.sh) + 3 gates (strict ^issue-[0-9]+$ / claim not-live / clean); the governing rationale corpse reap: admin-HEAD-missing AND git-unrecognized trees bypass Gate 3 and reap (rm -rf tree + admin dir) behind claim + 24h age guards — HEAD-present rc≠0 trees stay on the conservative skip; the governing rationale branch recovery: after reap, SAFE-delete the branch (merged → recovered) and FORCE-delete only manifest-recorded (merge-confirmed) branches, preserving unmerged work; the governing rationale free-arm manifest bypass: a claim-free worktree whose checked-out branch is manifest-recorded (merge-confirmed) bypasses the 24h age guard (harness mtime churn would otherwise leak it forever) and its manifest entry is consumed immediately after any successful branch recovery (-d and -D alike, best-effort with WARNING on failure); the governing rationale corpse-path manifest bypass: a corpse cannot resolve its branch (git doesn't recognize the tree) so the #1966 branch bypass never fires for one — cleanup.md Step 4-W now records the worktree's own PATH (not branch) into the manifest when removal fails/is skipped for busy/sandbox-mask reasons (merge-confirmed only), and the corpse age guard checks that PATH before falling back to the 24h wait, consuming the entry on successful reap (surgical: a mismatched path entry does not bypass); wiki-worktree excluded; session-start best-effort wiring."
