...
# Why (the governing rationale review F-05): `n_stale=0` + rc 0 is also exactly what the
...
# without exercising the empty-input path (the governing rationale review F-05).
if [ "$HELPER_RC" -eq 0 ] \
   && printf '%s\n' "$HELPER_STDOUT" | grep -qx 'n_stale=0' \
