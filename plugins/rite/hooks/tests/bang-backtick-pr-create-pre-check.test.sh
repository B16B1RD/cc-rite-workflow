...
for needle in 'ready-pr-head-gate.sh' 'ready-reviewed-head-gate.sh' '--pr "$ready_pr_number"' '|| exit' 'ready-work-memory-update.sh' '--pr {pr_number} --issue {issue_number}'; do
