...
if [ ! -f "$CLEANUP_MD" ]; then
  ...
  assert_grep "T-15 helper へ --exclude-ids を渡す" "$CLEANUP_MD" 'exclude-ids "\$_fu_exclude_ids"'
  assert "T-15 _fu_exclude_ids を既定初期化する" "1" \
    "$(grep -cF '_fu_exclude_ids="${_fu_exclude_ids:-}"' "$CLEANUP_MD" || true)"
