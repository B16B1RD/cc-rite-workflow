...
# 参照側 4 箇所 (1.4 / 4.5.3 / 4.6 / 5.1) が委譲文を保持していること。C-04b の複製禁止が
...
if [ "$_nb_ref_count" = "4" ]; then
  pass "C-04b 参照側 4 箇所が marker 不在時規則への委譲を保持している"
else
  fail "C-04b 委譲文が $_nb_ref_count 箇所 (期待 4 = 1.4 / 4.5.3 / 4.6 / 5.1) — 欠けた箇所に規則が残らない"
fi
