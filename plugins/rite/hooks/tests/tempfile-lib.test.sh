...
assert "T-01b the directory and its contents are removed on exit" "gone" \
  "$([ -n "$created_dir" ] && [ -e "$created_dir" ] && echo present || echo gone)"

# --- T-01c: release removes early and is not double-removed at exit ---------
rc=$(run_child '
rite_tempfile_init || exit 90
rite_tempfile_new a "rel-a" || exit 91
rite_tempfile_new b "rel-b" || exit 91
rite_tempfile_release "$a"
[ -e "$a" ] && exit 92
[ -f "$b" ] || exit 93
printf "%s\n%s\n" "$a" "$b" > "'"$PATHLOG"'"
' TMPDIR="$SANDBOX")
assert "T-01c release removes the file immediately and the exit path stays clean" "0" "$rc"
...
# --- T-02b: signal paths remove the file and exit 128+signum ----------------
...
sleep 5
' TMPDIR="$SANDBOX")
  assert "T-02b $signame exits $expected_rc" "$expected_rc" "$rc"
  sig_path="$(cat "$PATHLOG" 2>/dev/null || true)"
  assert "T-02b $signame removes the tempfile" "gone" \
    "$([ -n "$sig_path" ] && [ -e "$sig_path" ] && echo present || echo gone)"
done
