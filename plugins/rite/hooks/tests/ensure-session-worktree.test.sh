...
# 退出確認はホストの作業先を見るものなので、作業先を移す操作を伴わない独立したシェル呼び出しで
...
# 行位置で pin する。語句の存在だけでは順序が崩れても通るため、各手順の出現行番号が単調増加で
# あることを確認する。
...
RECOVER_SKILL="$SCRIPT_DIR/../../skills/recover/SKILL.md"
residue_anchor='git-worktree-patterns.md#native-入場が前のセッション-worktree-への残留で拒否される'
assert_grep "open 2.3-W references the residue diagnosis anchor" "$OPEN_SKILL" "$residue_anchor"
assert_grep "open 2.3-W keeps the procedure in the contract" "$OPEN_SKILL" '手順は契約側のみが持つ'
assert_grep "recover 3.1.5 references the residue diagnosis anchor" "$RECOVER_SKILL" "$residue_anchor"
assert_grep "recover 3.1.5 keeps the procedure in the contract" "$RECOVER_SKILL" '手順は契約側のみが持ち、本スキルに複製しない'
# 複製していない: 番号付き手順（残留診断 → keep 退出 → ...）は契約にのみ存在する。
assert "open does not duplicate the numbered residue procedure" "yes" "$(grep -q '^[0-9]\. \*\*残留診断\*\*' "$OPEN_SKILL" && echo no || echo yes)"
assert "recover does not duplicate the numbered residue procedure" "yes" "$(grep -q '^[0-9]\. \*\*残留診断\*\*' "$RECOVER_SKILL" && echo no || echo yes)"
assert "open still forbids git switch -c continuation" "yes" "$(grep -q '分離を捨てる `git switch -c` での続行は行わない' "$OPEN_SKILL" && echo yes || echo no)"
