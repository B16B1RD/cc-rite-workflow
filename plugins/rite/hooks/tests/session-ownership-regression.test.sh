...
# overriding the file-based per-session fixtures. `_hermetic-env.sh` clears
# them (with the rest of the runner's list), so every such invocation resolves
...
#  the working-tree git-verb denylist is gone).
#
