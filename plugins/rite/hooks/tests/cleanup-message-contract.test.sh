#!/bin/bash
# Tests for cleanup.md ステップ 4-W / 5 / 12 の message・配線 contract (the governing rationale, T-06).
#
...
echo "=== ステップ 4-W: session_worktree manifest 記録が {pr_merged}=true ガード配下にあること (the governing rationale AC-4) ==="
...
echo "=== ステップ 4-W: busy (EBUSY) 失敗時の sandbox 干渉 WARNING (the governing rationale AC-5) ==="
...
echo "=== ステップ 4-W: sandbox マスク検知による remove 抑止 (the governing rationale AC-1/AC-2) ==="
...
echo "=== ステップ 4-W: in_worktree_unrecorded の委譲 routing (the governing rationale T-01/T-03) ==="
...
# ガード迂回の禁止を明記する (the governing rationale MUST NOT — 実測で拒否済みの複合コマンドを再試行させない)。
...
echo "=== ステップ 4/5/9: 委譲モードのスキップガード (the governing rationale T-01) ==="
# main checkout 操作を持つ 3 ステップすべてに対称にガードを置く。1 箇所でも欠けると
# harness の worktree 隔離ガードに拒否され、the governing rationale が消した長文の診断報告に戻る。
...
echo "=== 委譲配線の排他性 (the governing rationale T-01/T-03 negative control) ==="
...
echo "=== ステップ 12: 委譲モードの定型報告 (the governing rationale T-01/T-02) ==="
...
# 条件も案内先も列挙せず、退路は直後の手動コマンドが与える形に留める — the governing rationale In Scope の
...
if ! print_summary "$(basename "$0")" "cleanup.md ステップ 4-W/5/12 の self-exclusion 配線・branch 回収・平易メッセージ contract (the governing rationale T-06) + in_worktree_unrecorded 委譲 routing "; then
  exit 1
fi
