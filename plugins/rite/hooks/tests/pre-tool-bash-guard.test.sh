...
#
# the governing rationale: the working-tree verb denylist (git checkout / reset / commit /
...
# (the governing rationale AC-1/AC-4). These commands were denied by the removed sub-blocks
...
# pinned in TC-127 below. They were never working-tree verbs (the governing rationale removed
...
# --------------------------------------------------------------------------
# TC-203: past false-positive commands → allow (the governing rationale AC-4)
...
# worktree-add arg with a bare glob from a CWD holding a `-b` file (the governing rationale
...
#   the governing rationale: Pattern 4 shared the fail-OPEN ERR trap with the convenience
...
# be an allow-all backdoor to the security boundary (the governing rationale review F-02). So
...
# actual restore statement — not just its comment (the governing rationale review F-04).
...
# Pin the exact value (the governing rationale review F-03): the .sh header comment documents
...
# so it fires here. Non-vacuous (the governing rationale review F-06): the prefix `git status` is
...
# --------------------------------------------------------------------------
# TC-125: reviewer WRITE into a .git directory (the governing rationale AC-1, sub-block (H))
...
# --- CRITICAL regression: repo under a `/git`-containing ancestor (the governing rationale fix) ---
...
# --- fileverb absolute-path / backslash invocation (the governing rationale fix) ---
...
# --- dd of=<gitpath> write vector (the governing rationale fix) ---
...
# --- value-quoted dd of= (the governing rationale cycle-2 fix: quote-strip-after-of= ordering) ---
...
# --- interior / nested quotes (the governing rationale cycle-3 fix: global quote removal) ---
...
# --- backslash-escaped .git path components (the governing rationale cycle-4 fix: backslash removal) ---
...
# --- obfuscated file-verb NAME (the governing rationale cycle-5 fix: dequote the verb token too) ---
...
# --- additional positional file-writers (the governing rationale cycle-5: sponge/patch, tee twins) ---
...
# a future edit that drops one literal from the case is caught (the governing rationale follow-up). ---
...
# (`*`/`?`/`[`) is NOT pathname-expanded against the hook CWD (the governing rationale follow-up). Without noglob
...
# --------------------------------------------------------------------------
# the governing rationale: variable-form PR token must not fall back to flow-state
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# the governing rationale: persist deny-only audit records
# --------------------------------------------------------------------------
