...
in_order "5.3 実行順: 5.3.0.A → 5.3.0.R → 5.3.1-5.3.7" \
  "$(line_of "$PR_REVIEW" '4. **5.3.0.A 受入条件の最終整合検査**')" \
  "$(line_of "$PR_REVIEW" '5. **5.3.0.R PR 内推奨の登録**')" \
  "$(line_of "$PR_REVIEW" '6. **5.3.1-5.3.7**')"
...
pin "5.3.0.A: reroll 5.2.2 は初回不採用を破棄しない" "$PR_REVIEW" '再実行 5.2.2 は初回の不採用行を破棄しない'
