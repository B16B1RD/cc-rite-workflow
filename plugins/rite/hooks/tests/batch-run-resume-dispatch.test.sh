...
#
# Test cases:
#   T-01 phase=ready / merge モード / PR あり → stage=ready（ready の完了確認後に merge へ進む入口）
#   T-02 phase=review / fix → stage=iterate、pr= と branch= を運ぶ
#   T-03 phase=implement、または flow-state が別 Issue → stage=open
#   T-04 PR 以降の phase で PR 番号 0 → stop(pr_number_missing)。flow-state 読出失敗 → stop(state_read_failed)
#   T-05 default モードで phase=ready / cleanup → advance（ready / merge / cleanup を実行しない）
#   T-06 phase=cleanup / merge モード / branch 空 → stop(branch_missing)。未知 phase → stop
#   T-07 静的 pin: 分岐表が各 stage 値の行を持つ / ステップ 2 冒頭の前提行 / ステップ 8 復旧行 /
#        ステップ 1.5 の bash が gh を呼ばない / recover Phase 5.3 の phase 集合を漏れなく扱う
#
...
# 読出失敗: flow-state.sh を壊れた helper に差し替える
...
echo "--- T-07: 静的 pin ---"
for s in open iterate ready cleanup advance stop; do
...
assert_grep "T-07 ステップ 8 の復旧行が止まった段階から続く" "$SKILL" '止まった段階から続ける'
...
# recover Phase 5.3 の phase 集合を ステップ 1.5 の case が漏れなく扱う
recover_phases=$(awk '/^### 5\.3 Phase enum/{f=1;next} f&&/^### /{exit} f&&/^\| `[a-z_]+` \|/{gsub(/[`| ]/,"",$1); print $1}' "$RECOVER" | sort -u)
missing=""
for p in $recover_phases; do
  grep -qE "^\s*[a-z_|]*\b$p\b[a-z_|]*\)" "$BLOCK" || missing="$missing $p"
done
