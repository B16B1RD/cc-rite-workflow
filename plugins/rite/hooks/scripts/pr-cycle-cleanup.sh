...
#     machine-gated since the governing rationale), so these names should normally never
...
# `gh pr checkout`) during the workflow (the governing rationale §3.2 Open Question resolved:
...
# (the governing rationale AC-1 / D-02).
readonly BARE_PR_PATTERN='^pr-[0-9]+$'
readonly WIKI_WORKTREE_PATH=".rite/wiki-worktree"
# Name-independent reap manifest (the governing rationale D-01/D-05). Producers append
...
# reviewer-tmp 名前空間として回収する (the governing rationale AC-2 / D-03)。prefix 自体が
...
# ための保守的マージン (the governing rationale D-04: 即時 0 残骸ではなく cross-session 安全と両立する
...
# (the governing rationale D-01/D-05, AC-4). A producer that creates a throw-away branch /
...
# (the governing rationale: they use the distinct `session_worktree` type below, which
    ...
    while IFS= read -r _m_line || [ -n "$_m_line" ]; do
      ...
      case "$_m_type" in
        ...
        worktree)
          ...
          ;;
        session_worktree)
          # the governing rationale: session worktree paths (`.rite/worktrees/issue-N`) are
    ...
    if [ "$DRY_RUN" = "0" ]; then
      if [ -s "$manifest_keep" ]; then
        if ! cp "$manifest_keep" "$manifest_path" 2>/dev/null; then
          echo "WARNING: manifest '$manifest_path' の書き戻しに失敗しました (回収済エントリが残存し age-guard バイパスを継承する可能性 — the governing rationale)" >&2
          errors=$((errors + 1))
        fi
      ...
      # parent — sandbox masks have blocked repo writes before, the governing rationale),
...
#
# Single source of truth (the governing rationale cycle 2 review finding): this regex is
...
# Worktree liveness guard (the governing rationale + #1552). The 4th protection layer:
...
#       while `updated_at` is within the liveness TTL above (the governing rationale;
...
# now-deleted directory (the governing rationale MUST: reap → null the owner's flow-state
...
if [ -d "$session_wt_root" ]; then
  while IFS= read -r _wt_line; do
    ...
    # Gate (worktree liveness, the governing rationale + #1552): never reap a worktree that a
    ...
    # Gate (OS-level live cwd, the governing rationale): never reap a worktree that ANY live
    ...
    # A corpse bypasses this gate (the governing rationale D-01): "indeterminable =
    ...
    # Corpse age guard (the governing rationale D-01): a corpse's dirty state cannot be
    ...
    # above (the governing rationale, free-claim arm) structurally never fires for it —
    ...
    # Corpse reap is loud (the governing rationale MUST): name the target before touching it.
    if [ "$_corpse" -eq 1 ]; then
    ...
    if git worktree remove --force "$wt_path" 2>/dev/null || rm -rf "$wt_path" 2>/dev/null; then
      ...
      # Manifest entry consumption (the governing rationale, symmetric with the branch
        ...
        if [ "$_branch_recovered" -eq 1 ] && [ -f "$manifest_path" ] \
          ...
          if session_branch_mf_keep=$(mktemp "${TMPDIR:-/tmp}/rite-pr-cycle-cleanup-mf-XXXXXX" 2>/dev/null); then
            ...
            if [ "$_mf_rc" -ge 2 ]; then
              ...
              # the governing rationale). `rm -f` returns 0 for a missing file, so this
      ...
      if [ "$_corpse" -eq 1 ]; then
        ...
        # failure must carry the manual recovery command (the governing rationale §4.5),
