...
#
# fail と degraded の境界 (the governing rationale §4.5 / AC-6):
...
#   the governing rationale §4.4 の契約に従った上での既知の残余で、silent ではない (pass 行の result_json=
#   にどのファイルで通ったかが出る)。
#
...
# results dir の **不在** は degraded にしない — the governing rationale §4.5 が degraded に置くのは
