...
#
# Verify that `.rite/.gitignore` is the 3-line composition generated by setup
# and hooks (`*` / `!wiki/` / `!wiki/**`). Lint never writes that file.
#
# Detection strategy:
#
...
# --- Always-on: `.rite/.gitignore` must match the generation helper (3-line SoT) ---
...
if [ ! -f "$nested_gi" ]; then
  echo "==> gitignore-health-check: DRIFT DETECTED (nested): '.rite/.gitignore' missing" >&2
  echo "==> Hint: run /rite:setup or /rite:setup --upgrade (generation only; lint does not write)." >&2
  echo "WARNING: gitignore-health-check: .rite/.gitignore missing" >&2
...
log_info "gitignore-health-check: nested .rite/.gitignore composition healthy"
