...
- mixed (#370) も 9.1% で rollback 閾値の 30% を大きく下回る。
...
- ⚠️ ~~**未検証**~~ → ✅ **検証済み (Issue #416 pre-fix replay)**: Phase C2 (#361 分散伝播漏れ検出) が pre-fix code に含まれる実バグを検出できるか (= 検出感度) — **検出率 100%、FP rate 0%** で Phase C2 の有効性が実証された。詳細: [pr334-prefix-replay-findings.md](./pr334-prefix-replay-findings.md)

~~**Phase C2 は特に問題**: PR #334 は silent abort 防止修正の **適用後コード**のため、検出対象の pattern が diff から消失しており、reviewer の検出能力を証明も反証もできない。~~

**✅ 解決済み (2026-04-10)**: Issue #416 の pre-fix replay (PR #417) により、PR #334 の inverse diff を reviewer に提示。error-handling reviewer が silent abort 経路復活を **CRITICAL** で検出し、devops も **MEDIUM** で合意。Phase C2 の検出感度は十分であることが定量的に確認された。
...
|-------|:--------------:|------|
| **Phase A (#357)** | ❌ 不要 | 全 replay で FP rate 閾値内 (最大 mixed 9.1%) |
| **Phase B (#358)** | ❌ 不要 | subagent 解決失敗は 3 replay で 0 件 |
| **Phase C (#359)** | ❌ 不要 | プロンプト品質由来の FP は 0 件 |
| **Phase C2 (#361)** | ❌ 不要 | post-fix で FP 0 件 + **pre-fix replay (Issue #416) で検出率 100%、FP rate 0%** — 検出感度と正確性の両方が確認済み |
...
- **仮説 3** (#403 由来): 「bash hook で Phase C2 の stderr 混入検出が発火する」 → ✅ **確認 (Issue #416 pre-fix replay)**: pre-fix replay (PR #417) で error-handling reviewer が silent abort 経路を CRITICAL で検出。Phase C2 機構の検出感度が実証された。
