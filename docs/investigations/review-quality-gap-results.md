...
| reviewer 構成 | (詳細レポート §2 参照、5 reviewer) | — |
...
| unique finding 数 | 23 件 (`grep -A3 parser` が 3 reviewer から重複検出) | — |
| **FP rate (strict)** | **8.0%** (2/25) | ✅ ≤30% rollback 閾値 |
...
3. **prompt-engineer が commands/**/*.md 専管で最多指摘 (11 件)**: `start.md` +313 行に対して placeholder substitution 責任 / flag check 漏れ / Step 番号 drift 懸念 / jq 引数置換規約等を検出。
...
3 子 Issue (#402 / #403 / #404) の replay 結果を doc-heavy (PR #350, Phase D baseline) と並べて比較する。
...
| **unique finding** | 19 件 | 24 件 | 0 件 | 23 件 |
| **FP rate (strict)** | 10.5% (2/19) | **0.0%** (0/24) | N/A (0/0) | 8.0% (2/25) |
...
- ❌ **未検証**: Phase C2 (#361 分散伝播漏れ検出) / Phase C (#359 プロンプト改善) が pre-fix code に含まれる実バグを検出できるか (= 検出感度)
