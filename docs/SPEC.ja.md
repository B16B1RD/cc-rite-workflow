...
**検証モード** (`review.loop.verification_mode`): サイクル 2 以降、フルレビューに加えて前回指摘の修正検証と差分に対するリグレッションチェックを補足的に実施する。未変更コードに対する新規の MEDIUM/LOW 指摘は、non-blocking の「安定性懸念」として報告される。
