...
`pr/review.md` Phase 6.5.W / 6.5.W.2、`pr/fix.md` Phase 4.6.W / 4.6.W.2、`issue/close.md` Phase 4.4.W / 4.4.W.2 は **Wiki growth path**（Wiki ブランチを成長させる経路）を構成します。Issue #524 は、これらの Phase が silent に skip されて wiki ブランチが成長しなくなる regression を 3 層防御で防止します:

| 層 | 仕組み | ファイル |
|---|--------|----------|
| **1. Mandatory execution** | 各 Phase X.X.W に「**E2E Output Minimization 下でも NEVER skip**」を明示し、完了時に `[CONTEXT] WIKI_INGEST_DONE=1` / `WIKI_INGEST_SKIPPED=1; reason=...` / `WIKI_INGEST_FAILED=1; reason=...` の status line を必ず emit する (成功 / config-skip / trigger 失敗) | `pr/review.md`, `pr/fix.md`, `issue/close.md` |
| **2. Sentinel ベースの可視化** | legitimate skip (`wiki_ingest_skipped`) と trigger 失敗 (`wiki_ingest_failed`) のどちらも `workflow-incident-emit.sh` 経由で workflow-incident sentinel を emit し、`start.md` Phase 5.4.4.1 が context grep で検出して `AskUserQuestion` でユーザーに提示する | `workflow-incident-emit.sh`, `start.md` Phase 5.4.4.1 |
| **3. Lint growth check** | `lint.md` Phase 3.8 が `wiki-growth-check.sh` を起動し、`wiki.growth_check.threshold_prs` 件以上の PR が wiki ブランチへの commit なしに merge された場合に warning を出す（非ブロッキング、`[lint:success]` 維持） | `wiki-growth-check.sh`, `lint.md` Phase 3.8 |

層 3 の閾値は `wiki.growth_check.threshold_prs`（デフォルト: 5）で設定可能。値を非常に大きくすると lint check は事実上無効化されますが、層 1-2 は維持されます。
