...
1. **意味**: 禅の精神 - シンプルで調和のとれたワークフロー
2. **実用性**: アルファベット後方（z）で始まるため、`/` 入力時の候補表示で他のコマンドより後に表示され、通常作業を邪魔しない
...
2. `git checkout -b` でブランチ作成
3. GitHub Projects Status を「In Progress」に更新
4. 作業メモリコメントを初期化
...
**レビュー・修正サイクルの継続:** `/rite:pr:review` → `/rite:pr:fix` → `/rite:pr:review` のサイクルは、総合評価が「マージ可」（blocking 指摘が 0 件）になるまで自動的に継続する。各ループ間でユーザー確認は行わず自動継続。サイクルは 3 層の収束メカニズムを使用する:

1. **検証モード** (`review.loop.verification_mode`): サイクル 2 以降、レビューは検証モードに切り替わる — 前回の修正の検証と、差分に対するリグレッションチェックのみを行う。未変更コードに対する新規の MEDIUM/LOW 指摘は、non-blocking の「安定性懸念」として報告される。
2. **段階的緩和** (`review.loop.relax_medium_after`, `review.loop.relax_high_after`): MEDIUM/LOW はサイクル 3（デフォルト）以降、HIGH はサイクル 5 以降で non-blocking になる。
3. **強制終了** (`review.loop.max_iterations`): サイクル 7（デフォルト）で、残りのすべての指摘を別 Issue に変換し、ループを終了する。

**「マージ可」の定義:** blocking 指摘が 0 件（現在のゲートモードの緩和レベルを考慮）。
