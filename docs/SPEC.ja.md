...
| `/rite:wiki:lint` | Wiki ページの矛盾・陳腐化・孤児（相互参照が無いページ）・欠落概念 (missing_concept)・未登録 raw (unregistered_raw、informational 扱いで `n_warnings` 不加算)・壊れた相互参照をチェック。CI 用の `--auto` モードをサポート |
