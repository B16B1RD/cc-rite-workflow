...
| 層 | 仕組み | ファイル |
|---|--------|----------|
| **0. 決定的な raw-commit 経路** | Phase X.X.W.2 は `wiki-ingest-commit.sh` を単一シェルプロセスとして直接呼び出す。script 内部で stash → checkout wiki → Raw Source 書き込み → commit → push → checkout back → stash pop を 1 回の bash 呼び出しで完結させる。これにより Claude の多段実行への依存が完全に除去され、リファクタ以前に層 1-3 の防御を 4 回重ねても wiki ブランチが実際には成長しなかった regression の根本原因を断ち切る。 | `hooks/scripts/wiki-ingest-commit.sh`, `pr/review.md`, `pr/fix.md`, `issue/close.md` |
