...
ただし、`.active=true` 前提条件への AND 論理依存自体は phase-transition-whitelist.sh / pre-compact.sh / post-compact.sh / pre-tool-bash-guard.sh の `.active` 参照箇所 (各 hook の `jq -r '.active'` 経路) として残存している。マルチステート対応では、これら残存 hook 群でも **同等保証を残す必要がある**。さもなくばマルチステート移行と同時に過去の N+1 patch が部分的にでも無効化されるリスクがある。経験則の射程を「`.active=true` 前提への AND 論理依存」だけに絞り、残存 hook 群への適用は S3 以降の実装フェーズで explicit に検証する。
...
- `needs_clear` (compact 復旧パス AC-2 で `pre-compact.sh` / `post-compact.sh` が書込)
...
| 6 | **Hook の自セッション state 参照最適化** (phase-transition-whitelist / pre-compact / post-compact / pre-tool-bash-guard) | ✅ session 限定で読込量削減。複数 session 起動時も自分の state のみ参照 | ❌ 全 session 読込必須（自セッション entry を `.sessions[$id]` で取得）。session 数増加で linear scan |
...
4. **残存 hook 群 (phase-transition-whitelist / pre-compact / post-compact / pre-tool-bash-guard) の自 session 参照最適化**: 各 hook が `.rite/sessions/$session_id.flow-state` のみ読込 → session 数増加に対して O(1)。Option B では全 sessions JSON の linear scan が必要
...
| 採択理由 | (1) lock 不要で並行性が**構造的に保証**される (Issue #660 で観測された race window 系列の root cause を排除)。(2) 1 session の file 破損が他 session に波及しない**障害分離**。(3) 既存 jq -n create pattern と atomic write (mktemp + mv) を**最小修正で流用可能**。(4) **残存 hook 群** (`phase-transition-whitelist.sh` / `pre-compact.sh` / `post-compact.sh` / `pre-tool-bash-guard.sh`) **が自 session の file のみを読込むため、session 数増加に対して O(1)** (stop-guard.sh は PR #675 で撤去済みのため対象外)。(5) `.rite-session-id` が既にファイルベースの SoT のため、命名規則として自然 |
...
**手順** (採択方式 Option A 採用時):
1. 旧形式 state を検出 (`.rite-flow-state` が存在し、`schema_version` キーが欠落 or `< 2`)
2. backup を `.rite-flow-state.legacy.{timestamp}` として保存 (このとき旧 source `.rite-flow-state` は別パスへの**コピーではなく rename** で確定する)
3. 旧 state から `session_id` を読み取る (なければ新規 UUID を生成)
4. 新形式 state file を `.rite/sessions/{session_id}.flow-state` に **atomic write (mktemp + mv)** で書き込み (`schema_version: 2` を必ず含める)
5. **旧 source `.rite-flow-state` を unlink (rename 済みなので物理削除は不要、step 2 で完了)**
6. stderr に明示的な migration メッセージ出力 (例: `migrated: .rite-flow-state → .rite/sessions/{id}.flow-state (backup: .rite-flow-state.legacy.{ts})`)

> **Option B 採用時の手順差分**: step 4 で同一パス `.rite-flow-state` に in-place で `{"schema_version": 2, "sessions": {"{session_id}": {<旧 state>}}}` 構造を書き込み、step 5 は不要 (path が変わらないため)。ただし両方式とも step 2 の backup 作成は必須。

**制約**:
- 旧形式に `session_id` がない場合は新規 UUID を生成して付与
- migration 自体が失敗した場合は旧形式を保持（destructive な変換を回避）— step 2 の backup を作成したまま、step 4 の atomic write で失敗した場合は新形式 file を残さず旧 source も rename しない (rollback 経路)
