...
| `loop_count` field | 現状 `post-compact.sh` / `session-start.sh` / `post-tool-wm-sync.sh` / `state-read.sh` / `work-memory-update.sh` で reader 経路が存在。writer 経路は `flow-state-update.sh` の patch mode に集約され、後続フェーズで利用される（`work-memory-update.sh` は read-only 参照のみ）。schema 維持を選択 |
