...
- `wm_comment_id` (`issue-comment-wm-sync.sh` が cache 用に書込。数値のみ — sentinel 文字列は入れない)
- `wm_replica` (`issue-comment-wm-sync.sh` が `no_comment` 時に `"absent"` のみ書込。`cache_comment_id` / `init` 成功で削除。`wm_comment_id` に sentinel は入れない)
