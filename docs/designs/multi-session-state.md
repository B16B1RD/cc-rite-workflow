...
- `error_count` (`flow-state-update.sh` patch mode が phase transition 時に `0` リセット writer として書込。`--preserve-error-count` 指定の self-patch は既存値を保持。**incrementer は PR #675 で撤去された stop-guard.sh のみで、現行 production code に incrementer 経路は存在しない** — `loop_count` と同様 reader/resetter のみ存在する半-legacy field。S3 で incrementer 不在前提のまま schema 維持するか除外するかは後追い検討事項)
