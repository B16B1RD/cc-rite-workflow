...
| F5: インデックス | `index.md` に `okf_version: "0.1"` を宣言し、ページカタログを OKF 箇条書き `* [title](path) - desc` で表現（**現在は `## ページ一覧` の 5 列テーブル形式** — #2047 で ingest 指示・テンプレート・実体を揃えた OKF からの意図的な逸脱。`docs/SPEC.md` の OKF v0.1 Conformance 節を参照） | Sub-2 (#1519) |
