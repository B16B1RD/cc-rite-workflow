...
1. **Convergence-trend divergence (primary).** `plugins/rite/hooks/scripts/review-trend-divergence.sh` reconstructs the current run's per-cycle blocking counts from the persisted review-result JSON and reports divergence when the two most recent counts both exceed the run's earlier best *and* are not still descending. This is independent of the cycle budget, so a diverging loop is cut early instead of burning it — but the rule needs at least three cycles in the run before "the run's earlier best" is defined, so cycles 1 and 2 always fall through to the backstop.
2. **`max_review_cycles` reached (backstop).** Catches the slow non-convergent runs that slip past the trend check — a count that keeps shrinking but never reaches 0, or one that plateaus at the run's best. This is a pure insurance limit and should rarely fire.

Splitting the conditions this way is deliberate: a cycle-count limit alone cannot tell effort from waste. It killed healthy converging loops that were down to a couple of findings when the budget ran out, and let diverging ones burn to the limit anyway; both were observed in practice. The project principle is to cut waste, not to cap quality with a budget. The judgment lives entirely in the helper — no conversational counting is involved — and its window and thresholds are calibrated constants rather than config keys; the calibration evidence (a backtest over the last five PRs' real trajectories) is documented in the helper's header, which is the single source of truth for the rule.
