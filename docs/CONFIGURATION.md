...
2. rite state / locks / wiki worktree still resolve to the shared main checkout root (`state-path-resolve.sh` is worktree-aware), so cross-session exclusion stays intact.
   `rite-config.yml` is read from the session worktree when it has one (a tracked config follows the branch) and otherwise from the main checkout, so an untracked config placed only in the main checkout still applies inside session worktrees. When neither exists, commands that continue with defaults print a WARNING naming the paths they tried.
