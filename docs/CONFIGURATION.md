...
**Cycle scope** (no config key): cycle 1 reviews the whole PR with the full reviewer set; from cycle 2+ the review is diff-scoped to `commit_sha..HEAD` of the previous cycle's persisted review-result JSON, plus verification that the previous blocking findings were resolved, with the reviewer set narrowed to the previous cycle's finders (rejoined as `mandatory`) union the owners of the fix diff's file patterns. Any missing input falls back to full scope with a warning. This is not configurable and not a progressive relaxation — cycle 3 and cycle 5 behave identically, and no finding criterion loosens. See `plugins/rite/skills/pr-review/references/cycle-scope.md`.
