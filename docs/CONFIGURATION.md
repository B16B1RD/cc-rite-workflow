...
3. `/rite:cleanup` exits the worktree (`ExitWorktree`), removes it, and releases the Issue claim — when the worktree was `EnterWorktree`-managed. If the session entered the worktree **by path** instead, `ExitWorktree` is a no-op, so cleanup skips the four main-checkout items and delegates them along two routes (Issue #2133): base update and wiki ingest complete on a re-run of `/rite:cleanup {pr}` from the main checkout, while worktree removal and branch deletion are handed to the lazy reap via a reap-manifest record (a re-run cannot reach them — flow-state has no worktree record there either). Abnormally-orphaned worktrees are reaped lazily by `pr-cycle-cleanup.sh`.
