...
> **The default for `max_review_cycles` is duplicated across the repository, and that duplication is itself a drift source.** It is written out in this table, in the YAML example below, in the annotated example near the top of this file, in the circuit breaker section further down (which restates the default three times while deriving loop-head counts from it), in `plugins/rite/templates/config/rite-config.yml`, in `plugins/rite/references/execution-metrics.md`, in `docs/SPEC.md`, and at three fallback sites in `plugins/rite/skills/iterate/SKILL.md` (two in step 0.6, one in step 1 — the latter is the one that actually decides whether the breaker fires). `plugins/rite/hooks/tests/max-review-cycles-default.test.sh` pins those. Three further copies are **not** pinned by that test, in two distinct states. Two are **stale**, still stating the pre-#2129 value of 5: `plugins/rite/skills/fix/references/fix-relaxation-rules.md` and the header comment of `plugins/rite/hooks/scripts/review-trend-divergence.sh` — both were declared out of scope by Issue #2129 on the grounds that they do not depend on the value, which is not true of their prose, so they stay stale until #2134 sweeps them. The third is **current but unpinned**: the comment and assertion labels in `plugins/rite/hooks/tests/review-trend-divergence.test.sh` were updated alongside this change but fall outside the T-04 loop, so they can drift silently. (The `### Added` entries in `CHANGELOG.md` / `CHANGELOG.ja.md` also name 5; those record the value as shipped in #1728 and are intentionally frozen.)
