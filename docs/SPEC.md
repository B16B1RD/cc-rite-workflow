...
| Non-measured findings still remaining at merge (full text) | one follow-up Issue (none if zero, if re-verification against the merged HEAD resolves every finding, or if every remaining finding was already filed as an Issue by the iterate sweep; public on a public repository) |
