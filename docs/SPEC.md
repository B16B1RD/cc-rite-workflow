...
| Type | Timing | Purpose |
|------|--------|---------|
| SessionStart | Session start | Load work memory, detect interrupted work; at the main checkout, report (never delete) 0-byte read-only lock files a sandboxed command left in the git dir; on `source=compact` re-inject recovery text into model context (Issue / Phase / Branch / Next action / Loop / PR, auto-continue, batch frame) |
