...
| Optional | `wm_comment_id` | `issue-comment-wm-sync.sh` (cache write) | GitHub comment ID for the work memory backup. Numeric only — never a sentinel string. Issue-scoped: `flow-state.sh set` drops it (instead of merge-preserving) when the write switches to a different Issue, and `do_fetch` discards a cached id whose fetched body does not belong to the target Issue |
