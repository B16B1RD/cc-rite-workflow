...
| `fix-report-diff-gate.sh` | `/rite:fix` ステップ 4.6 直前 — 最新 cycle の `findings_addressed[]` を `git diff -U0 {commit_sha_before}..HEAD` の + hunk（削除のみはファイル単位）と突合し、`diff_verified` を JSON に書き戻す。marker `FIX_REPORT_DIFF_GATE=passed\|unverified\|error`（`reason=map_missing\|state_unreadable\|diff_failed\|jq_missing`）。`error` は `[fix:error]`、`unverified` は停止せず「未対応」として報告する | — |
