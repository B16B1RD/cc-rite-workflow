...
/rite:iterate <pr> (Review ⇄ Fix loop)
 ...
 │ until [review:mergeable] or [fix:non-fatal-only] → 5.S
 │ or [fix:replied-only] (reply-only; moved=0)
 ▼
...
| Mechanical routing | `/rite:iterate` 5.S (`/rite:fix --nb-sweep`) and `/rite:cleanup` follow-up Issue | **Live** | Remaining non-blocking findings after `[review:mergeable]` or `[fix:non-fatal-only]` are consumed mechanically. Leftovers at merge become one follow-up Issue. This is not the removed skip-to-end-loop AskUserQuestion path. |

`[fix:non-fatal-only]` requires no push, no current-cycle accept, `fatal=0`, `non_fatal_moved_count>0`, and all required replies completed; error, WM-stale, and push/accept rows retain precedence. Pure reply-only (`non_fatal_moved_count=0`) still exits as `[fix:replied-only]`. The new sentinel sets `FINALIZE:fix:non-fatal-only:{pr}`: Stop enforces the completion notice and remaining-count field once, directing unfinished 5.S work to run first. Only successful 5.S completion returns outward `[review:mergeable]` to batch-run; sweep failure returns `[iterate:nb-sweep-error]`, never mergeable. Completion reports zero remaining plus issued/recorded counts without another full review.
