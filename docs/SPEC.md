...
| `tempfile-lifecycle-check.sh` | `/rite:lint` Phase 3.5 — detect the two tempfile/pipeline defects that `lib/tempfile.sh` cannot remove: a path derived from a tempfile handle, read or write (loses `O_CREAT|O_EXCL`), and `grep -q` consuming a pipeline under `pipefail` (the immediately preceding stage takes SIGPIPE). Scans `plugins/rite/hooks/**/*.sh` + `plugins/rite/scripts/**/*.sh` (`tests/` excluded). Exempt: an immediate producer of `printf`/`echo`, and the sanctioned `mktemp 2>/dev/null` stderr-slot idiom. A file that could not be scanned exits 2 — not a clean bill | — |
