...
| `skill-rail-diff-check.sh` | SKILL 記述ダイエットの機械レール逐語一致検証 — fenced block (fence 行込み) と markdown table row を抽出して base ref (既定 `origin/develop`) と突合し、1 バイトでも異なれば exit 1。base ref に対象が無い場合は clean skip (exit 0)。`--extract-only` で before/after 計測用のレール抽出のみ | Canonical method: `references/skill-diet-method.md` |
