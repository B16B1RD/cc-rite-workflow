...
To prevent false positives from text in heredocs (commit messages, PR descriptions, etc.), Patterns 1–5 inspect only the command portion before `<<`. Pattern 6 removes heredoc bodies while retaining executable pipeline stages and later command lines, so `cat <<EOF | gh issue create ...` is denied but text inside the body is ignored.
