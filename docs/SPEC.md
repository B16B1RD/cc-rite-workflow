...
| `pr_review.post_comment` | PR review output destination. The non-measured findings record comment (`📜 rite 非実測指摘の記録`, one comment from the Measured CONFIRMED Gate, updated in place each cycle — and, when the helper cannot identify its own previous comment, re-created (so possibly duplicated) if the cycle has findings, or not posted at all if the cycle has zero, leaving the previous record stale) is posted **independently of this setting** (not subject to the opt-out — it upholds the governing rationale D-01 "record non-measured findings as a PR comment"). That comment carries **pointers only** — reviewer, severity, and `file:line`, plus one line naming where the full text lives — never a finding's `description` or `suggestion`, so a non-measured CRITICAL is not disclosed in detail on a public PR before it is fixed; the full text lives in the local `non_blocking_findings[]`, which `/rite:cleanup` archives rather than deletes when non-empty  |
...
| `wiki-lint-skipped-refs.sh` | `/rite:wiki-lint` ステップ 6.0 — raw frontmatter (`ingest_status: skipped`) を走査して skipped_refs 集合を marker block + `log_read_ok` 4 値 enum で構築 (この仕様により skip SoT が log.md から raw frontmatter へ移行。6.2 `wiki-lint-source-refs.sh` と対称) | — |
