...
/rite:iterate <pr> (Review ⇄ Fix loop)
 ...
 │ until [review:mergeable], [fix:non-fatal-only], or [fix:replied-only] → 5.S
 │    (reply-only stays reply-only after sweep)
 │    → in-PR recommendation fix (registered pr_recommendations → /rite:fix → re-review, once per run)
 ▼
...
**Note:** The end-to-end flow is split across four single-responsibility commands. `/rite:open <issue>` handles branch creation, implementation, autonomous lint, and draft PR creation. `/rite:iterate <pr>` loops review and fix until convergence, bounded by a circuit breaker that fires on convergence-trend divergence or, as a backstop, on `safety.max_review_cycles` (default 15); on either, both modes stop mechanically without prompting — interactive runs emit a stop notice and `/rite:batch-run` batch marks the Issue failed and stops with its cursor unchanged — and a stopped `review_run` does not become a new run when `/rite:iterate` is re-run explicitly — recovery is the bounded `review-retry` (divergence only) or an explicit `review-restart` bound to that stopped run (manual abort via `Ctrl+C` + `/rite:recover` remains available; recover does not call `review-restart`). `/rite:ready <pr>` flips the PR to Ready for review. `/rite:merge <pr>` runs `gh pr merge --squash`. For the canonical live spec of each command, see [`skills/open/SKILL.md`](../plugins/rite/skills/open/SKILL.md), [`iterate.md`](../plugins/rite/skills/iterate/SKILL.md), [`ready.md`](../plugins/rite/skills/ready/SKILL.md), and [`merge.md`](../plugins/rite/skills/merge/SKILL.md). (The legacy [Phase 5: End-to-End Execution](#phase-5-end-to-end-execution) section below documents the pre-decomposition `start.md` orchestrator for archaeological / migration reference only.)
...
| Non-blocking findings full text during the review cycle | local JSON (`.rite/review-results/`; gitignored) |
| Non-blocking findings pointers during the review cycle | related Issue, one update-in-place comment |
| Non-blocking findings still remaining at merge (full text) | one follow-up Issue (none if zero, if re-verification against the merged HEAD resolves every finding, or if every remaining finding was already filed as an Issue by the iterate sweep; public on a public repository) |
...
│ │ ├── wiki-lint-log-commit.sh # skills/wiki-lint/SKILL.md 8.3 委譲 (log.md 追記の commit)
...
│ │ ├── pr-checks-classify.sh # merge / pr-review 共通の CI 分類と job 一覧
...
│ │ ├── cleanup-session-worktree-teardown.sh # cleanup.md ステップ 4-W の検出 (detect) と削除 (remove)
│ │ ├── cleanup-branch-delete.sh # cleanup.md ステップ 5 のローカル / リモートブランチ削除
│ │ ├── cleanup-pr-state-purge.sh # cleanup.md ステップ 6 の PR-specific state 削除
...
│ │ ├── fix-report-diff-gate.sh # fix 4.6 直前: findings_addressed を git diff -U0 と突合（FIX_REPORT_DIFF_GATE marker）
...
│ │ ├── distribution-docs-link-check.sh # lint Phase 3.5 配布 root 外へ出る相対 markdown リンク検出
...
│ │ ├── lib/ # 共有ライブラリ (context-marker.sh / diff-hunks.sh / git-remote.sh / git-status-filtered.sh / rite-config-path.sh / tempfile.sh / wiki-config.sh / worktree-git.sh)
...
│ ├── review-pr-recommendations.sh # pr-review 5.3.0.R PR 内推奨の登録 / iterate 5.S 後の未着手判定と記録
...
| 純 sub-skill（user は直接起動しない） | pr-review / fix / pr-create / issue-implement | `user-invocable: false`（orchestrator が Skill ツールで programmatic invoke するため `disable-model-invocation` は**付けない** — 付けると programmatic invoke まで巻き添え遮断されうる 。auto-activate 抑止は narrow description で担保する） |
...
| `acceptance-reviewer` | inherit | high | Every acceptance criterion of the related Issue, re-checked on HEAD each cycle |
...
| `pr_review.post_comment` | Scopes **only** the integrated review-report post onto the PR. The non-measured findings record comment (`📜 rite 非実測指摘の記録`, one comment from the Measured CONFIRMED Gate, updated in place each cycle on the **related Issue** — and, when the helper cannot identify its own previous comment, re-created (so possibly duplicated) if the cycle has findings, or not posted at all if the cycle has zero and the body carries no rejection-ledger entries, leaving the previous record stale) is posted **independently of this setting** (not subject to the opt-out — it upholds D-01: never discard non-measured findings; the shared durable channel is the Issue, not the PR). That comment carries **pointers plus the demotion reason** — reviewer, severity, `file:line`, and the demotion label (the class B verdict sentence for 5.3.0.C demotions, "実測なし" otherwise), plus one line naming where the full text lives — never a finding's `description` or `suggestion`, so a non-measured CRITICAL is not disclosed in detail on a public PR before it is fixed; during the review cycle the full text lives in the local `non_blocking_findings[]` (no sharing channel until merge), which `/rite:cleanup` archives rather than deletes when non-empty. Remaining non-measured findings at merge become one follow-up Issue that transcribes `description` / `suggestion` in full (public on a public repository). Before that transcription, `/rite:cleanup` re-verifies each finding against the merged HEAD and excludes the ones a later fix cycle already resolved — a finding that cannot be decided (moved `file:line`, prose intent, insufficient evidence) is transcribed rather than dropped, and when every finding is resolved no follow-up Issue is opened at all. Findings the iterate sweep already filed as Issues are also excluded: a finding is dropped when a ledger row judged `issued` in the related-Issue record comment has the same finding id and `file:line` and its source column names the review result the finding came from (the basename of the review result the sweep read), so a sweep-filed finding stays excluded after a later review cycle; a legacy row without a source column is matched only against the latest review result. A finding whose review result differs from the row's source is still transcribed even at the same id or `file:line`, because the ledger cannot tell a re-report from a different finding and losing a finding is worse than a duplicate. A warning names only earlier-cycle findings at the same `file:line` as a finding excluded from the latest review result; a re-report whose line moved may therefore duplicate the sweep's Issue without a warning. If every remaining finding is excluded this way, no follow-up Issue is opened, and if the ledger or the latest review result cannot be read, only this sweep-filed exclusion is skipped — findings already excluded by re-verification stay excluded — and a warning is surfaced. How a finding was addressed is in the fix commit message, readable from the PR Commits tab after squash merge (not in develop's git log) |
...
**Definition of "Approve":** Zero blocking findings, where **blocking = a CONFIRMED finding, in scope `current-pr` / `follow-up`, whose `measured` is `true`** (a repro command with the observed misbehavior, or a failing test with its output — Measured CONFIRMED Gate). Successful gate output has boolean measured values for all gated findings; malformed anchors are errors. `nit-noted` scope is outside the gate. `measured` is resolved **per layer, and within `/rite:fix` per read-route**: in `/rite:pr-review` step 5.3.0.M writes the review-result JSON first, then `plugins/rite/scripts/review-measured-gate.sh` reads the `Verification:` anchor out of each `findings[].description` and sets `verification` on that JSON in place. In `/rite:fix`, every input route converges on step 1.2.2: materialize durable JSON, run the common fatal triage, then reload that JSON. A fatal fix target has boolean `verification.measured == true`, scope `current-pr` or `follow-up`, and either severity `CRITICAL` or `HIGH`, or `consequence_class == "A"` without `pre_existing: true` — a PR-introduced runtime defect stays a fix target whatever severity label it carries. A class B finding whose `consequence_exclusion` is a non-empty string (the demotion gate kept it blocking) is treated the same way as class A. Every gated finding passed the revert test, so only an explicit `pre_existing: true` excludes it. Other gated findings move atomically to `non_blocking_findings[]` with `demotion_reason: non_fatal`, preserving severity and evidence; `nit-noted` entries remain unchanged. Missing/non-boolean measured values, invalid severity, or a measured MEDIUM-or-lower gated finding whose `consequence_class` is not `A` / `B` (`class_undetermined`) stop with `[fix:error]` without changing the source or falling back. `non_blocking_count` is the reloaded array length on every route. Persisted JSON, the related-Issue pointer record, the report, and the E2E notice use this same count. A rite reviewer finding whose `内容` column carries no `Verification:` anchor is demoted to non-blocking: it keeps its severity, is recorded (**unconditionally** in the persistent JSON `non_blocking_findings[]`, and — best-effort, since a gh failure or a malformed body aborts the post — as a related-Issue comment, `## 📜 rite 非実測指摘の記録`, updated in place each cycle — falling back to a new comment when the helper cannot identify its own prior one, see the `pr_review.post_comment` row — and posted independently of `pr_review.post_comment`), and does not drive the fix cycle. The gate uses two boolean outcomes plus an error: when a gated finding contains a marker and `=>` in the same segment but fails the detection grammar, it exits nonzero with `reason=anchor_undetermined`, the count, and the finding IDs. The input JSON remains unchanged, including when a boolean was already present. The caller requests only those findings again from their reviewers and reruns the gate in the same cycle, using the existing shared retry limit of one. No success verdict is emitted before the gate succeeds. This check applies only to anchors rejected by the detection regex; accepted anchors remain measured even when their left-hand side contains a `。` or newline. Inside that set, a marker whose `=>` sits past a segment boundary (a segment ends at a newline, a `<br>`, or a `。`) or further away than the scan bound is demoted instead. The test is purely lexical and does not tell a mistyped anchor apart from prose that merely mentions the marker. In `/rite:fix`, those undetermined gated findings are input errors at common triage; they are neither silently demoted nor presented as selectable fix targets. This makes "zero blocking findings" a reachable exit condition and bounds the review ⇄ fix loop. Whether the anchor may be attached at all is decided upstream of the gate by the **consequence class**, which has two application domains. For findings against prose (procedure documents, specifications, references): a repro that observes only a text difference within the reviewed document itself (asymmetric wording, missing pin, missing qualifier, unsynchronised duplicate definitions) is not anchor-eligible, so such findings reach the gate without an anchor and are demoted to non-blocking; a repro that executes the described procedure and observes a broken artifact stays blocking. For test-coverage findings (surviving mutants, weak assertions, missing pins): a surviving mutant demonstrates the absence of a guard against a defect the reviewer introduced, not a misbehaviour of HEAD, so anchor eligibility is decided by whether the mutation disables a behaviour the Issue contract states (the `## 4. Implementation Details` §4.4 MUST bullets and the `Then` clauses of the acceptance-criteria section (`Acceptance Criteria` / `受入基準` / `受入条件` / `受け入れ条件`)). A gap in contract-stated behaviour — or a test with zero verifying power for it — is anchor-eligible and stays blocking; a request to pin finer-grained mutants of implementation internals the fix itself introduced is not anchor-eligible and is demoted. When the PR carries no resolvable Issue link, the finding defaults to blocking. A test that cannot fail for any implementation of the behaviour it names (tautological assertion, fixture that never reaches the path, an assertion pinning the inverse of the spec) is a **correctness** defect, outside this class, and stays blocking. Both domains act on authoring only, preserve severity and scope, must not be routed through `scope=nit-noted`, and do not alter the gate's boolean-or-error contract. The consequence class additionally acts at the **gate layer** as a second demotion axis: after the measured gate, each remaining measured blocking finding is classified — by the consolidation context, never the finding's own reviewer, with the classification map mechanically enforced by `plugins/rite/scripts/review-class-demotion-gate.sh` — as **class A** (leaving it unfixed changes the runtime behaviour of this deliverable; a one-line runtime scenario can be written — including a finding against prose whose repro executes the described procedure, or the implementation a spec describes, and observes a literal follower misbehave) or **class B** (the consequence stays within detection-net granularity, readability, or document consistency observed only as a text difference; uncertain cases fall to B, except a prose finding whose repro executes the described procedure or the implementation a spec describes). The class helper retains its defensive class-A handling for legacy input missing `verification`; successful measured-gate output cannot contain that shape for gated findings. A class B finding carrying an exclusion stays blocking: either the diff deleted or weakened existing text, or an agreed acceptance criterion measured as unmet points to it (an `acceptance_criteria[]` row with `status: unmet`, recorded as `consequence_exclusion: ac_unmet:AC-N`). In the cycle where class A reaches zero, every class B finding without an exclusion is demoted to `non_blocking_findings[]` with a `demotion` audit record (policy + the classification sentence) and a top-level `class_demotion` audit flag, and the loop exits through the existing mergeable path — no freeze phase or new state transition exists. While any class A remains, class B findings stay blocking. A missing or malformed classification entry falls back to class A with a WARNING (never a silent demotion). Classification is never by file path: a test-directed finding whose consequence is a runtime defect escaping detection (e.g. a clean fixture that cannot catch a production bug) is class A. See `plugins/rite/references/severity-levels.md` §実測必須ゲート for the gate definition, the error contract, and the canonical formula, §帰結クラス軸 for the consequence class (both layers), and `plugins/rite/skills/fix/references/assessment-rules.md` §5.3.0.C for the set arithmetic.

For `category: "number_reference"`, the helper overrides the classification map and keeps the effective consequence class at A. If a well-formed class B entry conflicts with that category pin, it emits a warning and `CLASS_DEMOTION_CATEGORY_PINNED` while leaving the finding blocking.
...
Get changed files list
 ↓
Analyze files and select appropriate reviewers (+ acceptance-reviewer if the Issue has acceptance criteria)
 ↓
Spawn subagents in parallel (Task tool)
 ...
 └─ acceptance-reviewer: Acceptance criteria of the related Issue on HEAD (mandatory when the Issue has template acceptance criteria)
 ↓
...
Reviewers are automatically selected based on file patterns and content analysis. Not all reviewers are invoked for every PR - only relevant ones are selected. The exception is `acceptance-reviewer`, which runs as `mandatory` every cycle whenever the related Issue has template acceptance criteria.
...
> **Worktree Mode (`multi_session.enabled: true`)**: Entry and exit follow the [host worktree execution contract](../plugins/rite/references/git-worktree-patterns.md#host-worktree-execution). Cleanup verifies the saved state, claim, registered branch and dirty status, then uses the selected native or verified explicit-working-directory route to reach the main checkout. Only a successful exit check permits the existing teardown helper to remove the target; live-session and sandbox-mask protections remain in force. Unrecorded ownership, denied exit or an unverifiable destination leaves the main-checkout tasks outstanding for a cleanup re-run from the main checkout. Local branch deletion follows worktree removal, and base updates respect the main-checkout inviolability rule. Issue claims are released by cleanup. With multi-session disabled, the existing single-tree flow is unchanged.
...
To prevent false positives from text in heredocs (commit messages, PR descriptions, etc.), Patterns 1–5 and 7 inspect only the command portion before `<<`. Pattern 6 removes heredoc bodies while retaining executable pipeline stages and later command lines, so `cat <<EOF | gh issue create ...` is denied but text inside the body is ignored. Pattern 8 (the commit evidence gate) reuses Pattern 6's surface for the same reason, and scans it as a sequence of direct `git commit` / `git merge` commands — not as a general shell. Script bodies, aliases and dynamically assembled subcommands are outside its inspection. A `git commit -m "$(cat <<'EOF' … EOF)"` form is not stripped by Pattern 6 when `<<` sits inside double quotes (`_state=plain` is required for heredoc detection); the quote state carries across lines, so a `<<` inside a multi-line quoted string is not a heredoc. Quotes are tracked across a whole word, so a quote may open in the middle of a word. A command substitution (`$(…)` or backquotes, quoted or not) and a `( … )` group run in a subshell: a `cd` inside does not move the outer target. Outside a subshell, only a `cd` the shell is known to run moves the target: a plain `cd <literal>` that starts a list (at the start or after `;` or a newline), or one inside an `&&` chain, which reaches only the rest of that chain, in a command with no `( … )` group, compound command (`if`, `while`, `until`, `for`, `case`, `select`, `{ … }`, `coproc`), function definition or background `&`; such a structure is found by its word anywhere in the command, not only at its start. Any other `cd` — after `||`, in a pipeline, `cd -`, with options, redirections or no directory, or anywhere in a command with one of those structures — makes the target dynamic. The standard message form — a substitution that is exactly `cat` of one heredoc, where the first delimiter line is followed only by whitespace and `)` — is data and is not parsed; with an unquoted delimiter, bash expands the body, so it is data only when the body holds no `$(` or backquote. Any other substitution is parsed as commands. Unfinished quoting and an unfinished command substitution are denied fail-loud (`unfinished quoted command` / `unfinished command substitution`), and the denial names `git commit -F <message-file>`. A `git commit` / `git merge` inside a subshell or behind a wrapper, a dynamic `cd` / `-C` target, a target that does not resolve to a repository (missing, unenterable or not a repository) or an alternate git dir is denied only when that commit or merge would move HEAD. Because the target cannot be resolved, this denial applies before the unrelated-worktree and retained-run exemptions, while a review state exists. Pass a multi-line message with `git commit -F <message-file>`, writing that file outside the work tree so it is not seen as an unplanned changed path.
...
6. When the fetch reports `skipped; reason=no_comment` — the replica does not exist yet — no PATCH is issued (`round_trips=1`). The fetch side records the `wm_replica=absent` negative cache that step 5 short-circuits on from the next phase change onward, the hook emits a `systemMessage` pointing at `/rite:open`, and the phase counts as synced. The `systemMessage` is suppressed at `phase=init`, the replica-init transient window between `/rite:open` step 1.6 (which records the phase) and step 2.5 (which creates the replica): there the replica's absence is the expected state and the notice would tell the user to run the `/rite:open` that is already running. Only the notice is suppressed — the negative cache and the phase advancement below are unchanged, and every other phase keeps the notice
...
**`last_synced_phase` advancement:** the field advances when the single PATCH succeeds. A transform that *fails* aborts the PATCH and leaves the field un-advanced, so the next invocation re-runs the pipeline for that phase. A transform that is *skipped* rather than failed — the progress table when neither `branch.base` nor `origin/HEAD` resolves, or when `git diff` fails — still lets the PATCH through and advances the field, so that transform is not re-attempted for the phase; the skip surfaces as its own `systemMessage`. The `wm_replica=absent` short-circuit and the `no_comment` first detection above also count as success and advance the field without a PATCH, which is why their degradation notices surface once per phase change rather than on every Bash tool call. The `phase=init` suppression in step 6 drops only the notice, so the field advances there exactly as it does when the notice is emitted.
...
| `distribution-docs-link-check.sh` | `/rite:lint` Phase 3.5 — detect markdown links under `plugins/rite/**/*.md` whose destination resolves outside the plugin root (marketplace installs do not ship development-repo `docs/`) | Design and precision boundary: `skills/lint/references/plugin-checks-rationale.md` |
...
| `fix-reason-coverage-check.sh` | `/rite:fix` DoD 検証 (手動実行) — verify `WM_UPDATE_FAILED` emits from both fix/SKILL.md and scripts/fix-work-memory-update.sh against the skill reason table; rc=1 lists the undocumented ones | — |
...
| `wiki-lint-descriptive-refs.sh` | `/rite:wiki-lint` ステップ 7.5 — ページ本文 (`## ソース` 節の bullet を含む) / `index.md` のエントリサマリー / `log.md` に残った Issue/PR 番号参照を検出し marker block + `WIKI_DESCRIPTIVE_REFS` 合計 + `descriptive_refs_read_ok` enum で構築。**検出文法は `number-reference-check.sh` に委譲**し本 helper はコピーを持たない (3-4 桁の番号トークン、キーワードの有無を問わない)。`index.md` と `log.md` は helper が自力で読み出す (stdin 不要。`index.md` はエントリごとのサマリーのみ — テーブル / OKF 箇条書きを行単位で判別)。frontmatter の `sources:` ブロック / コードフェンス / コードスパン / TODO・FIXME を除外し、`index.md` に限り行頭 `<!--` の HTML コメントブロックも落とす（箇条書きテンプレートが配布されていた期間に初期化された bundle の index.md 前文に残る記法例、および手書き index のコメントを実エントリと数えないため）。サマリーを抽出できなかった行数（列位置不明 / 列数不一致）は `descriptive_refs_skipped_rows` で surface し、**`index.md` の HTML コメントブロック / コードフェンス**が未閉鎖のまま EOF に達した場合は検出失敗として `descriptive_refs_read_errors` に載せる（frontmatter の `sources:` ブロック、およびページ本文側は検査対象外）。`raw/**` / `SCHEMA.md` は意図的に走査しない (informational 指標、`n_warnings` 不加算) | — |
| `wiki-lint-log-commit.sh` | `/rite:wiki-lint` ステップ 8.3 — log.md 追記の commit。`--auto` なら `wiki-worktree-commit.sh --commit-only`（push は ingest ステップ 8.6）、standalone は commit + push、same_branch は `git-commit-file.sh`。commit メッセージは `--message-file` で受け、commit 失敗は WARNING のみ (非ブロッキング) | — |
...
| `pr-cycle-cleanup.sh` | 残留 `pr-{N}-cycle{X}` worktree / branch の冪等掃除 + `${TMPDIR:-/tmp}/rite-pr-create-*` 孤児 workdir の age ベース GC (mtime > 24h)。消費済み `.rite/release-promotions/{N}.json`（対応 PR が MERGED/CLOSED）も回収し、OPEN/状態不明は理由付きで残し `.gitignore` は削除しない。`session-start.sh` のセッション開始時 **worktree** lazy reap 起動 (`CWD == STATE_ROOT` ゲート) では、stdout/stderr を `STATE_ROOT/.rite/logs/pr-cycle-cleanup.log` へ退避する (ログディレクトリ初回作成時に自己完結的な `.gitignore` (`*`) も生成し downstream consuming repo でも自動除外、上書き方式、ログ書き込み準備失敗時（ディレクトリ作成不可または既存ディレクトリへの書き込み不可）は従来どおり破棄にフォールバック、)。同じゲート内で reap より前に、git dir 直下に残った 0 バイト・書き込みビットなしの `*.lock` を `session-start.sh` が検知して hook の stderr に WARNING を出す（ログへは退避せず、削除もしない。exit 0 の hook の stderr は Claude Code の debug ログにのみ残る）。run-queue ファイルの回収は別 helper（`run-queue-reap.sh`） | — |
| `run-queue-reap.sh` | 他セッションの stale `run-queue-{session_id}.json` と対応 `.watchdog` を、所有セッションが生きていないときだけ削除する。stale 判定は `session-ownership.sh` の `parse_iso8601_to_epoch` と 2h（`-gt 7200`）を再利用（`updated_at` 欠落 / パース失敗は stale）。キューの `updated_at` は batch 開始時と cursor 前進時しか動かないため、生存は所有セッションの `.rite/sessions/{session_id}.flow-state` の `updated_at`（phase 遷移ごとに更新）で判定する: 7200 秒以内なら残す、flow-state 不在または古ければ削除、JSON object として読めなければ flow-state のパス付き WARNING を出して残す、`updated_at` 欠落 / パース失敗なら理由を stderr に出してキュー側の判定どおり削除する。自セッションは触らない。`failed` / `outstanding` 非空は削除前に 1 行ずつ stderr。jq 不能は WARNING して残す。`session-start.sh` が SessionStart（explicit init 以外）で呼び、`CWD == STATE_ROOT` ゲートの外・stdout/stderr は hook に残す（`pr-cycle-cleanup.log` へは退避しない）。失敗は `|| true` | — |
...
| `fix-report-diff-gate.sh` | `/rite:fix` ステップ 4.6 直前 — 最新 cycle の `findings_addressed[]` を `git diff -U0 {commit_sha_before}..HEAD` の + hunk（+ 側が空の純削除 hunk のみ削除前の行番号で hunk 単位）と突合し、`diff_verified` を JSON に書き戻す。marker `FIX_REPORT_DIFF_GATE=passed\|unverified\|error`（`reason=map_missing\|state_unreadable\|diff_failed\|jq_missing`）。`error` は `[fix:error]`、`unverified` は停止せず「未対応」として報告する | — |
...
| `lib/` (`context-marker.sh` / `diff-hunks.sh` / `git-remote.sh` / `git-status-filtered.sh` / `rite-config-path.sh` / `tempfile.sh` / `wiki-config.sh` / `worktree-git.sh`) | 共有ライブラリ — `diff-hunks.sh` は `git diff -U0` を + / 純削除 hunk の行範囲に分け、範囲の重なりを判定する（`fix-report-diff-gate.sh` と `scripts/review-pr-recommendations.sh` が source する）。`rite-config-path.sh` は読むべき `rite-config.yml` を解決する（worktree 自身のもの、無ければ main checkout のもの。見つからなければ試したパスを出して rc=1、読めなければ rc=2。`--or-devnull` は不在を WARNING + `/dev/null` にする）。汎用 git helper + wiki 系 helper (owner/repo 解決 / sandbox ghost mount と終了後に残る 0 バイト・書き込みビットなしのスタブを除外する git status（review drift / fix guard は `--tracked-only` で untracked を件数・名前付き WARNING に分離） / wiki config 読取 / worktree git 操作) と tempfile ライフサイクル、`[CONTEXT]` marker の emit / 照合。`context-marker.sh` は source 型 lib で `marker_emit KEY VALUE [FIELD=VALUE ...]` / `marker_get KEY [--field NAME] [--branch BRANCH]` を提供する。emit 書式 `[CONTEXT] KEY=VALUE; FIELD=VALUE`(区切りは `"; "`) は不変で、照合側は行頭アンカー・複数行 stderr 混入への耐性・`branch=` スコープ・同一 KEY の recency・キーと field 名のトークン完全一致 (`ITERATE_CB` が `ITERATE_CB_MODE` に当たらない) を関数内で解決する。パースは BSD/GNU 差異を避けるため sed/grep ではなく bash `case` glob。契約の SoT は `hooks/tests/context-marker.test.sh`。現在の呼び出し元は `scripts/iterate-step.sh` と `references/projects-integration.md` で、他経路の直接 `echo "[CONTEXT] ..."` も同関数で読める (後方互換) ため段階移行できる。`tempfile.sh` のみ状態を持つ source 型 lib で、`rite_tempfile_init` が EXIT/INT/TERM/HUP handler を設置し、`rite_tempfile_new` / `rite_tempdir_new` が生成と登録を 1 手順で行う (out-variable 形式のため `f=$(rite_tempfile_new)` 型のコマンド置換で登録が消えない (サブシェル `( ... )` や pipeline 段での生成は親に登録されないので top-level で呼ぶ))。caller が既に handler を持つ signal があれば上書きせず rc=1 で拒否する — その場合は `--caller-traps` + 自 handler からの `rite_tempfile_cleanup` で合成する。プロセスが**無視状態で継承した** signal (nohup の HUP、非同期子プロセスの INT) は handler ではないので拒否対象にならず、その signal だけ設置を skip する (無視された signal はプロセスを殺さないので回収対象も生じない)。この免除は signal 3 種に限る — EXIT は signal ではなく継承もされないため `trap '' EXIT` は必ず caller 自身の記述で、しかも無視指定でも発火するので通常どおり rc=1 で拒否する。**tempfile / tempdir を必要とする**新規 `hooks/` / `scripts/` helper はこれを使う (coding-principles.md「Shell Helper Conventions」が SoT) | — |
| `tests/` | hooks/scripts レベルのテストスイート | — |
...
| Optional | `wm_comment_id` | `issue-comment-wm-sync.sh` (cache write) | GitHub comment ID for the work memory backup. Numeric only — never a sentinel string. Issue-scoped: `flow-state.sh set` drops it (instead of merge-preserving) when the write switches to a different Issue, and `do_fetch` discards a cached id whose fetched body cannot be confirmed to belong to the target Issue (a mismatch and an unparseable body are treated alike) |
| Optional | `wm_replica` | `issue-comment-wm-sync.sh` on `no_comment` | Negative cache. Value `"absent"` only; written when the work-memory Issue comment is not found. Deleted by `cache_comment_id` / `init` success. Not a numeric sentinel in `wm_comment_id`. Issue-scoped like `wm_comment_id` — dropped on an Issue switch so one Issue's `absent` never suppresses another Issue's sync |
...
| Optional | `cycle_count` | `flow-state.sh review-start`; `set --cycle-count 0` for run closure | The review⇄fix counter advances once when the fixed reviewer roster and HEAD are recorded. Resuming an incomplete review preserves the counter and bypasses new-cycle breaker evaluation. Direct `set` increments are rejected. Normal completion and circuit-breaker closure retain their reset behavior; an unfinished review cannot be reset into a successful transition. Switching to another Issue or PR archives the source run with its counter, then restores an eligible destination run and its counter; only a destination without an eligible archive starts from zero. Stopped runs remain eligible despite historical close/defer markers. A non-stopped run is excluded only when its close/defer marker matches its archived completed cycle; older markers do not discharge a later cycle. The trend helper continues to use the separate run-start pin rather than this counter as its run boundary. Unspecified values are merge-preserved; zero removes the key. |
| Optional | `review_cycle` | `flow-state.sh review-start / review-finish / review-abandon` | The current review contract: fixed selection and `review_context` (session, run, PR, cycle, full HEAD), collection/completion status, and persisted evidence paths. Finish verifies every selected reviewer and the actual saved result before marking completion. Ordinary state updates preserve the record and reject unverified transitions to fix/ready or another cycle. Replay reuses the saved result without double counting. `review-abandon` is the only writer that removes the record, and only for a `collecting` cycle that points at no manifest, content or result and has no saved receipt on disk; it moves `phase` back to `pr` and leaves the counter and the session/Issue/PR/branch/worktree identity untouched. Direct file edits and unverified host events are outside this CLI guarantee. |
| Optional | `review_cycle_abandoned` | `flow-state.sh review-abandon` | Append-only history of dropped review cycles. Each entry carries the abandoned `review_context`, its `selected_reviewers`, the `reason` the caller supplied, `abandoned_at`, and `head_at_abandon`. Ordinary `set` calls carry the array forward unchanged, so the record of why a cycle was dropped outlives the phase transitions that follow it. Nothing but `review-abandon` appends, and no writer removes entries. The latest entry for that run (not the latest entry across all runs) is also what makes a `review_run` without a `review_cycle` legitimate: `review-stagnation` accepts that shape only when the entry's session, PR, run and counter all match the run. This describes the record-to-run identity check; the retained-run validator also requires a matching Issue, valid run status, the clock/observation/fix/replan history arrays, and a nonnegative diagnostic clock. Then `review-start` inherits the run's `run_id` and leaves the counter alone so the retry stays in the same run. An invalid latest entry for the run is rejected without falling back to an older valid entry. Without a matching entry the same shape is corruption and every write is refused. |
...
flow-state は上書き式 (atomic write) のため工程履歴が残らず、工程別の所要時間は従来ファイル mtime やコミット時刻からの手動逆算でしか得られなかった。これを自動収集するため、[`flow-state.sh`](../plugins/rite/hooks/flow-state.sh) の `cmd_set` は **実際に書き込みが成立した** `set` ごとに 1 行を `STATE_ROOT/.rite/logs/phase-transitions.log` へ append する (`_append_phase_transition`、)。配置は `pr-cycle-cleanup.log` と同じ `.rite/logs/` 慣習に従い、自己完結的な `.gitignore` (`*`) も併せて生成する (`/rite:setup` が生成する `.gitignore` は `.rite/logs/` を含まないため)。生成の要否は存在ではなく**中身** (`-s`) で判定する — `> file` は書き込み前に truncate するため失敗すると 0 バイトの `.gitignore` が残り、存在判定ではそれを「生成済み」と読んで以後の全 `set` が書き直しを恒久的に skip する。生成に失敗した場合は WARNING を出し、原因を字下げして併記する (`hooks/review-result-save.sh` / `hooks/scripts/review-results-archive-or-rm.sh` と同形)。`STATE_ROOT` を再解決せず script 冒頭の解決結果を再利用するため、`RITE_STATE_ROOT` override が効き、linked worktree 内で走った phase も `state-path-resolve.sh` の worktree 統合により main checkout の同一ログへ着地する (セッションの時系列が checkout 単位に分断されない)。
...
|---|---|---|
| Create / enter | `/rite:open N` | `git worktree add --no-track -b {branch} {worktree_base}/issue-{N} origin/{base}` (idempotent across 5 cases — reuse / stale-residue prune / branch-only / new / other-worktree abort; `--no-track` avoids sandbox-rejected `.git/config` tracking writes), then native entry or a verified explicit-working-directory route (Step 2.2-W / 2.3-W). A pre-existing `worktree` flow-state value triggers Step 0.5 re-entry on resume |
| Work | implement / lint / push / PR create | Every shell, file edit, check and delegated operation targets the verified worktree; root, branch, claim and state are checked before edits and commits (Steps 3–6) |
| Exit / remove | `/rite:cleanup` | Verify ownership and dirty status, exit through the selected native or verified explicit-working-directory route, and check the actual main root before invoking the existing teardown helper. Unrecorded ownership or denied/unverified exit delegates the outstanding work to a main-checkout cleanup re-run |
...
When a step of the end-to-end flow (`/rite:open` → `/rite:iterate` → `/rite:ready` → `/rite:merge`) fails or is skipped (Skill load failure, hook abnormal exit, Wiki ingest skip/failure, `.gitignore` drift, etc.), the relevant script or hook emits a plain `WARNING` / `ERROR` line to **stderr**. Stderr is diagnostic output for the LLM and is not guaranteed to reach the user's terminal, so the orchestrators that carry a **`要対応:` section** — `/rite:open`, `/rite:iterate` and `/rite:batch-run` — transcribe any line that needs user action into their completion report (the transcription rule lives in `plugins/rite/skills/rite-workflow/references/autonomous-execution.md`; the section is omitted entirely when there is nothing to transcribe). `/rite:ready` and `/rite:merge` carry no section of their own, so their warnings are collected only when they land on a `/rite:batch-run` report path (including a `/rite:recover` that continues a batch). On standalone invocation, and when they are invoked from a `/rite:recover` that is not continuing a batch (`BATCH_CONTINUE=none`), there is no section to collect them and they stay on stderr (a known uncovered path). For the lines that were transcribed, the user resolves them by re-running the affected step via `/rite:recover`.
...
| Mechanical routing | `/rite:iterate` 5.S (`/rite:fix --nb-sweep`) and `/rite:cleanup` follow-up Issue | **Live** | Remaining non-blocking findings after `[review:mergeable]`, `[fix:non-fatal-only]`, or `[fix:replied-only]` are consumed mechanically. Leftovers at merge become one follow-up Issue. This is not the removed skip-to-end-loop AskUserQuestion path. |

`[fix:non-fatal-only]` requires no push, no current-cycle accept, `fatal=0`, `non_fatal_moved_count>0`, and all required replies completed; error, WM-stale, and push/accept rows retain precedence. The following reply-only row requires completed replies with no push or current-cycle accept, regardless of moved count. Mixed fatal replies and non-fatal transfers therefore return `[fix:replied-only]`. The new sentinel sets `FINALIZE:fix:non-fatal-only:{pr}`: Stop enforces the completion notice and remaining-count field once, directing unfinished 5.S work to run first. After successful 5.S completion, iterate uses the retained entry sentinel: `[fix:replied-only]` remains reply-only (merge-mode batch stops), while the mergeable and non-fatal-only entries return outward `[review:mergeable]` to batch-run. The internal `[fix:sweep-done]` never replaces the entry reason; sweep failure returns `[iterate:nb-sweep-error]`, never mergeable. Completion reports zero remaining plus issued/recorded counts without another full review.
...
| `/rite:iterate` | `[review:mergeable]`, `[fix:non-fatal-only]`, or `[fix:replied-only]` then 5.S (success preserves reply-only or returns mergeable according to the entry; failure returns `[iterate:nb-sweep-error]`; a pending in-PR recommendation fix runs before the notice and loops back to review) (whichever sub-skill returns first terminates the loop) / `[fix:cancelled-by-user]` (user-initiated cancel via fix.md AskUserQuestion) / `[iterate:max-cycles-reached]` (circuit breaker in a `/rite:batch-run` batch — the Issue is marked failed and the batch stops without advancing its cursor) / `[iterate:max-cycles-stopped]` (circuit breaker in interactive mode — the loop stops mechanically, leaving the draft PR for review). **Both sentinels cover either fire reason** — convergence-trend divergence as well as the `safety.max_review_cycles` backstop. The literals are deliberately unchanged despite the now-misleading `max-cycles` naming, because `/rite:batch-run` greps them to mark the Issue failed and stop the batch; splitting them by reason would break that contract. The reason is carried in the stop notice's prose, not the sentinel |
| `/rite:ready` | `[ready:returned-to-caller]` (E2E flow) / completion display message (standalone) |
| `/rite:merge` | `[merge:returned-to-caller]` |
| `/rite:batch-run` | `<!-- [run:all-completed] -->` (all Issues completed; default = draft PRs left for review, `--merge` = merged/cleaned up) / `<!-- [run:stopped] -->` (stopped on first failure; processed/remaining Issues reported). `run-queue-{session_id}.json` (session-scoped filename — session_id derived from `flow-state.sh path`; each concurrent session holds an independent queue so parallel batch-runs cannot clobber each other; unresolvable session_id fails loud rather than falling back to a global file) persists `{issues, cursor, mode, failed, active, updated_at}`: `mode` (`default`/`merge`; missing → `default` for backward compat), `failed` (Issues whose `/rite:iterate` tripped the review⇄fix circuit breaker — convergence-trend divergence **or** the `safety.max_review_cycles` backstop → `[iterate:max-cycles-reached]`; cleared for that Issue when it later advances after an explicit restart; missing → `[]`), `active` (true while the batch drives iterate, set false on stop; missing → `false` — consulted by `/rite:iterate` ステップ6 so a dormant queue is not misread as an active batch), and `updated_at` (ISO 8601 timestamp refreshed on cursor-advance (step 6) and active-set (steps 0/8) writes — not on step 1's coarse skip-closed cursor advance; missing → freshness unknown, treated as stale — consulted by `/rite:recover` Phase 5.5 to detect a genuine active-batch interruption vs. a stale leftover queue). Does NOT use flow-state handoff; per-Issue continuation rides each sub-skill's own mechanism. On the first `process` of each run (with or without arguments) the cursor Issue's flow-state phase is read once and the run continues from the stage where it stopped (review/fix → iterate, ready_error → ready, ready → merge, cleanup → cleanup; PR-stage-or-earlier or a mismatched state → open; in default mode ready/merge/cleanup stages advance the cursor without running them); a state it cannot classify (unreadable or corrupt state, missing PR number, ingest/completed/unknown phase) stops the run and points to `/rite:recover` instead of defaulting to open |
