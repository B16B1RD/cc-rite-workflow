...
| `index.md` | Carries `okf_version: "0.1"`; page catalog as OKF bullets `* [title](path) - desc`. **`templates/wiki/index-template.md` and `/rite:wiki-ingest` step 6 *specify* bullets, but the live `wiki` branch is maintained in the 5-column table form and ingest keeps appending table rows to it** — the bullet form is the instruction, not the observed behaviour, so the table form is not a residue that a format migration will retire on its own. Consumers that parse entries must accept both per line. **Known non-conformer: `/rite:wiki-query` Pass 1 accepts bullets only, so a table-form index yields 0 candidates** (see `skills/wiki-lint/references/descriptive-refs-rationale.md`) | `templates/wiki/index-template.md` |
