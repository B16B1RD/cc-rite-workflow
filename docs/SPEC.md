...
| `run-queue-reap.sh` | 他セッションの stale `run-queue-{session_id}.json` と対応 `.watchdog` を削除する。stale 判定は `session-ownership.sh` の `parse_iso8601_to_epoch` と 2h（`-gt 7200`）を再利用（`updated_at` 欠落 / パース失敗は stale）。自セッションは触らない。`failed` / `outstanding` 非空は削除前に 1 行ずつ stderr。jq 不能は WARNING して残す。`session-start.sh` が SessionStart（explicit init 以外）で呼び、`CWD == STATE_ROOT` ゲートの外・stdout/stderr は hook に残す（`pr-cycle-cleanup.log` へは退避しない）。失敗は `|| true` | — |
