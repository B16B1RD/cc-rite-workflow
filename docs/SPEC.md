...
| Optional | `cycle_count` | `flow-state.sh set --cycle-count <N>` | The `/rite:iterate` review⇄fix cycle counter for the `safety.max_review_cycles` circuit breaker (#1701). `/rite:iterate` increments it at each loop head, resets it to `0` on a fresh entry (phase not in `review`/`fix`) **or when it finds the tripped marker `cycle_count > safety.max_review_cycles` at startup — written by the breaker's fire branch and reset regardless of the recorded phase (#2026)**, and otherwise continues it across `/rite:recover` (AC-3). The marker is `max_review_cycles + 1` rather than an equality test on the limit itself: `cycle_count == max_review_cycles` also holds throughout the final cycle's review and fix (step 1 writes the counter before invoking review), so treating equality as "already tripped" would let an interrupt-then-recover in that window reset the budget without the breaker ever firing. **Merge-preserve** semantics like `worktree`/`branch` (NOT default-clear like `handoff`): an unspecified `--cycle-count` preserves the existing value. `--cycle-count 0` deletes the key. Written conditionally — sessions that never run the breaker never gain the key, so the state file is byte-identical and no `schema_version` bump is needed (additive, read via `.cycle_count // 0`) |
