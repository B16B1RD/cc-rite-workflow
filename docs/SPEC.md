...
| `pr_review.post_comment` | PR review output destination. The non-measured findings record comment (`📜 rite 非実測指摘の記録`, one comment from the Measured CONFIRMED Gate, updated in place each cycle — and re-created, so possibly duplicated, when the helper cannot identify its own previous comment) is posted **independently of this setting** (not subject to the opt-out — it upholds Issue #2024 D-01 "record non-measured findings as a PR comment") |
