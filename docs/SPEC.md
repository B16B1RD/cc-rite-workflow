...
| `index.md` | OKF specifies the page catalog as bullets `* [title](path) - desc`. A bundle-root `okf_version` frontmatter block is **optional** — v0.1 says index files carry no frontmatter and that bundles *MAY* declare the version there, so the live branch having none is **not** a non-conformance. `templates/wiki/index-template.md` (deployed by `/rite:wiki-init`) does carry it. **The catalog form is a real gap: `/rite:wiki-ingest` step 6 *specifies* bullets, but the live `wiki` branch is maintained in the 5-column table form and ingest keeps appending table rows to it** — the bullet form is an instruction, not observed behaviour, so the table form is not a residue that a format migration will retire on its own. Consumers that parse entries must accept both catalog forms per line, and must not assume the frontmatter is present. **Known non-conformer: `/rite:wiki-query` Pass 1 accepts bullets only, so a table-form index yields 0 candidates** (see `plugins/rite/skills/wiki-lint/references/descriptive-refs-rationale.md`) | `templates/wiki/index-template.md` |
