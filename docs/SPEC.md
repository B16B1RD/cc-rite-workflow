...
To prevent false positives from text in heredocs (commit messages, PR descriptions, etc.), Patterns 1–5 and 7 inspect only the command portion before `<<`. Pattern 6 removes heredoc bodies while retaining executable pipeline stages and later command lines, so `cat <<EOF | gh issue create ...` is denied but text inside the body is ignored. Pattern 8 (the commit evidence gate) reuses Pattern 6's surface for the same reason, and scans it as a sequence of direct commands — not as a general shell. Script bodies, aliases and dynamically assembled subcommands are outside its inspection. A `git commit -m "$(cat <<'EOF' … EOF)"` form is not stripped by Pattern 6 when `<<` sits inside double quotes (`_state=plain` is required for heredoc detection). Unfinished or ambiguous quoting is denied fail-loud (`unfinished quoted commit command` / `ambiguous shell word`). Pass a multi-line message with `git commit -F <message-file>`, writing that file outside the work tree so it is not seen as an unplanned changed path.
