...
| Committing while a review cycle is unfinished or its fix scope is unverified | The edit-time gate would otherwise fire only *after* the commit moved HEAD, leaving the frozen review context and HEAD mismatched with no way to close the cycle or open a new one | Run `review-finish` to save the review, then check the fix plan and `review-fix-scope-check.sh verify --kind all`; commit in a separate Bash call. `--dry-run`, sessions with no review cycle, and commits in an unrelated worktree are not denied |
