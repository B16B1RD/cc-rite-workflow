...
| `review-trend-divergence.sh` | 永続 review-result JSON から現 run の per-cycle blocking 列を復元し、収束トレンドの発散を決定論的に判定 (`iterate.md` ステップ 1 のサーキットブレーカー主経路)。run 境界は caller が渡す `cycle_count` で切り出す (同一 PR の複数 run が同一ディレクトリに同居するため)。判定不能時 (JSON 破損 / 未知 schema_version / pr_number 不一致 / scope が 3 値 enum に解決不能 / ファイル数不足 / helper 実行不可) は発火させず、理由を `reason=` に載せて `safety.max_review_cycles` の backstop へ委ねる (silent skip を作らない) | — (定数。config キーを持たない) |
