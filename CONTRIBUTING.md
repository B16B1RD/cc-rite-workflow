...
- For a new hook that needs temporary files, source `hooks/scripts/lib/tempfile.sh` rather than writing `mktemp` + `trap` by hand: call `rite_tempfile_init`, then `rite_tempfile_new <outvar> [tag]` (or `rite_tempdir_new`). A hook that already installs any of the EXIT / INT / TERM / HUP handlers passes `--caller-traps` to `rite_tempfile_init` and calls `rite_tempfile_cleanup` from its own handler — `rite_tempfile_init` refuses with rc=1 rather than clobber one of the four. What the lib buys over a hand-written `trap 'rm -f "$tmpfile"' EXIT`: an EXIT-only trap does fire on INT/TERM/HUP, but it cannot set the exit code, so an interrupted run can still report success; the lib returns 130/143/129 deterministically and makes a silenced `mktemp` failure unwritable. `plugins/rite/skills/rite-workflow/references/coding-principles.md` "Shell Helper Conventions" is the SoT for this rule
