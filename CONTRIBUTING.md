...
- For a new hook that needs temporary files, source `hooks/scripts/lib/tempfile.sh` rather than writing `mktemp` + `trap` by hand: call `rite_tempfile_init`, then `rite_tempfile_new <outvar> [tag]` (or `rite_tempdir_new`). A hook that already owns any of the EXIT / INT / TERM / HUP handlers passes `--caller-traps` and calls `rite_tempfile_cleanup` from its own handler. The rule, the refusal conditions, and why the lib beats a hand-written `trap 'rm -f "$tmpfile"' EXIT` are specified in `plugins/rite/skills/rite-workflow/references/coding-principles.md` "Shell Helper Conventions" — the SoT
